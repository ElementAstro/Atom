...
PYBIND11_MODULE(flood_fill, m) {
    ...
    m.def(
        "flood_fill_bfs",
        [](py::array_t<int32_t> grid, int start_x, int start_y,
           int32_t target_color, int32_t fill_color, Connectivity conn) {
            ...
            std::vector<std::vector<int32_t>> cpp_grid(
    ...
    m.def(
        ...
        [](py::array_t<int32_t> grid, Connectivity conn) {
            py::buffer_info buf = grid.request();
            ...
            for (int y = 0; y < height; ++y) {
                for (int x = 0; x < width; ++x) {
                    if (labels_ptr[y * width + x] == -1 &&
                        ptr[y * width + x] != 0) {
                        // Found unvisited non-zero cell, start new component
                        std::vector<std::vector<int32_t>> temp_grid(
                            height, std::vector<int32_t>(width));
                        std::vector<std::vector<int32_t>> temp_labels(
                            height, std::vector<int32_t>(width, -1));
                        ...
                        size_t component_size =
                            atom::algorithm::FloodFill::fillBFS(
                                temp_grid, y, x, target_value, component_id,
                                conn);
    ...
    m.def(
        "create_test_grid",
        [](int width, int height, double fill_ratio, int seed) {
        ...
        py::arg("width"), py::arg("height"), py::arg("fill_ratio") = 0.5,
        py::arg("seed") = 42,
        R"pbdoc(
