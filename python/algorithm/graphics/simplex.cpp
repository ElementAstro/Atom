...
PYBIND11_MODULE(simplex, m) {
    ...
    py::class_<atom::algorithm::SimplexNoise>(m, "SimplexNoise", R"pbdoc(
        ...
        .def(
            "noise2d",
            [](const atom::algorithm::SimplexNoise& self, double x, double y) {
        ...
        .def(
            "noise3d",
            [](const atom::algorithm::SimplexNoise& self, double x, double y,
               double z) { return self.noise3D(x, y, z); },
        ...
        .def(
            "noise4d",
            [](const atom::algorithm::SimplexNoise& self, double x, double y,
               double z, double w) { return self.noise4D(x, y, z, w); },
        ...
        .def(
            "fractal3d",
            [](const atom::algorithm::SimplexNoise& self, double x, double y,
               double z, int octaves, double persistence, double lacunarity) {
                return self.fractal3D(x, y, z, octaves, persistence,
                                      lacunarity);
            },
        ...
        .def(
            "generate_noise_map_2d",
            [](const atom::algorithm::SimplexNoise& self, int width, int height,
               double scale, int octaves, double persistence,
               double lacunarity) {
                // Generate 2D noise map
                py::array_t<double> result =
                    py::array_t<double>({height, width});
                ...
                for (int y = 0; y < height; ++y) {
                    for (int x = 0; x < width; ++x) {
                        ...
                        if (octaves > 1) {
                            noise_value =
                                self.fractal2D(sample_x, sample_y, octaves,
            ...
            py::arg("octaves") = 1, py::arg("persistence") = 0.5,
        ...
        .def(
            "generate_noise_map_3d",
            [](const atom::algorithm::SimplexNoise& self, int width, int height,
               int depth, double scale, int octaves, double persistence,
               double lacunarity) {
                ...
                for (int z = 0; z < depth; ++z) {
                    for (int y = 0; y < height; ++y) {
                        for (int x = 0; x < width; ++x) {
                            ...
                            double noise_value;
                            if (octaves > 1) {
                                noise_value = self.fractal3D(
                                    sample_x, sample_y, sample_z, octaves,
                                    persistence, lacunarity);
                            } else {
                                noise_value =
                                    self.noise3D(sample_x, sample_y, sample_z);
                            }

                            ptr[z * height * width + y * width + x] =
                                noise_value;
                        }
                    }
                }
            ...
            py::arg("scale"), py::arg("octaves") = 1,
            py::arg("persistence") = 0.5, py::arg("lacunarity") = 2.0,
            R"pbdoc(
    ...
    m.def(
        ...
        [](int width, int height, uint32_t seed, double scale, int octaves,
           double persistence) {
            atom::algorithm::SimplexNoise noise(seed);
            return noise.generate_noise_map_2d(width, height, scale, octaves,
                                               persistence, 2.0);
        },
        py::arg("width"), py::arg("height"), py::arg("seed") = 0,
        py::arg("scale") = 0.01, py::arg("octaves") = 6,
        py::arg("persistence") = 0.5,
        R"pbdoc(
    ...
    m.def(
        ...
        py::arg("width"), py::arg("height"), py::arg("seed") = 0,
        py::arg("scale") = 0.05, py::arg("octaves") = 4,
        R"pbdoc(
