...
PYBIND11_MODULE(image_ops, m) {
    ...
    py::class_<atom::algorithm::ImageOps>(m, "ImageOps", R"pbdoc(
        ...
        .def_static("convolve", [](py::array_t<uint8_t> image, py::array_t<float> kernel) {
            ...
            if (kernel_buf.ndim != 2 || kernel_buf.shape[0] != kernel_buf.shape[1]) {
                throw std::invalid_argument("Kernel must be square 2D array");
            }
            ...
            std::span<const float> kernel_span(kernel_ptr, kernel_size * kernel_size);
            
            auto result = atom::algorithm::ImageOps::convolve(img_span, width, height, kernel_span, kernel_size);
            
            py::array_t<uint8_t> output = py::array_t<uint8_t>({height, width});
        ...
        .def_static("gaussian_blur", [](py::array_t<uint8_t> image, double sigma, int kernel_size) {
            ...
            auto result = atom::algorithm::ImageOps::gaussianBlur(img_span, width, height, sigma);
            ...
            std::copy(result.begin(), result.end(), out_ptr);
            return output;
        }, py::arg("image"), py::arg("sigma") = 1.0, py::arg("kernel_size") = 0,
        R"pbdoc(
        ...
        .def_static("sobel_edge_detection", [](py::array_t<uint8_t> image) {
            py::buffer_info buf = image.request();
            ...
            std::span<const uint8_t> img_span(ptr, width * height);
            auto result = atom::algorithm::ImageOps::sobelEdgeDetection(img_span, width, height);
        ...
        .def_static("laplacian_edge_detection", [](py::array_t<uint8_t> image) {
            py::buffer_info buf = image.request();
            ...
            std::span<const uint8_t> img_span(ptr, width * height);
            auto result = atom::algorithm::ImageOps::laplacianEdgeDetection(img_span, width, height);
        ...
        .def_static("adjust_brightness_contrast", [](py::array_t<uint8_t> image, int brightness, double contrast) {
            py::buffer_info buf = image.request();
            ...
            std::span<const uint8_t> img_span(ptr, width * height);
            auto result = atom::algorithm::ImageOps::adjustBrightnessContrast(img_span, static_cast<float>(brightness), static_cast<float>(contrast));
            ...
            std::copy(result.begin(), result.end(), out_ptr);
            return output;
        }, py::arg("image"), py::arg("brightness") = 0, py::arg("contrast") = 1.0,
        R"pbdoc(
        ...
        .def_static("histogram_equalization", [](py::array_t<uint8_t> image) {
            py::buffer_info buf = image.request();
            ...
            std::span<const uint8_t> img_span(ptr, width * height);
            auto result = atom::algorithm::ImageOps::histogramEqualization(img_span);
    ...
    m.def("create_gaussian_kernel", [](int size, double sigma) {
        if (size % 2 == 0) {
            throw std::invalid_argument("Kernel size must be odd");
        }
    ...
    m.def("create_edge_detection_kernel", [](const std::string& type) {
    ...
    m.def("batch_process_images", [](py::list images, const std::string& operation, py::kwargs kwargs) {
        ...
        for (auto& img : images) {
            ...
            py::buffer_info buf = image.request();
            std::span<const uint8_t> image_span(static_cast<const uint8_t*>(buf.ptr), buf.size);

            // Extract width and height from the array shape (assuming 2D image)
            ...
            if (operation == "gaussian_blur") {
                double sigma = kwargs.contains("sigma") ? kwargs["sigma"].cast<double>() : 1.0;
                auto result_span = atom::algorithm::ImageOps::gaussianBlur(image_span, width, height, static_cast<float>(sigma));
                result = py::array_t<uint8_t>(result_span.size());
                std::memcpy(result.mutable_data(), result_span.data(), result_span.size());
            } else if (operation == "sobel_edge_detection") {
                auto result_span = atom::algorithm::ImageOps::sobelEdgeDetection(image_span, width, height);
                result = py::array_t<uint8_t>(result_span.size());
                std::memcpy(result.mutable_data(), result_span.data(), result_span.size());
            } else if (operation == "laplacian_edge_detection") {
                auto result_span = atom::algorithm::ImageOps::laplacianEdgeDetection(image_span, width, height);
                result = py::array_t<uint8_t>(result_span.size());
                std::memcpy(result.mutable_data(), result_span.data(), result_span.size());
            } else if (operation == "adjust_brightness_contrast") {
                int brightness = kwargs.contains("brightness") ? kwargs["brightness"].cast<int>() : 0;
                double contrast = kwargs.contains("contrast") ? kwargs["contrast"].cast<double>() : 1.0;
                auto result_span = atom::algorithm::ImageOps::adjustBrightnessContrast(image_span, static_cast<float>(brightness), static_cast<float>(contrast));
                result = py::array_t<uint8_t>(result_span.size());
                std::memcpy(result.mutable_data(), result_span.data(), result_span.size());
            } else if (operation == "histogram_equalization") {
                auto result_span = atom::algorithm::ImageOps::histogramEqualization(image_span);
                result = py::array_t<uint8_t>(result_span.size());
                std::memcpy(result.mutable_data(), result_span.data(), result_span.size());
            } else {
    ...
    m.def("compute_image_statistics", [](py::array_t<uint8_t> image) {
        py::buffer_info buf = image.request();
    ...
    m.def("create_test_image", [](int width, int height, const std::string& pattern, int seed) {
        ...
        if (pattern == "random") {
            ...
            for (int y = 0; y < height; ++y) {
                for (int x = 0; x < width; ++x) {
                    ptr[y * width + x] = static_cast<uint8_t>((x * 255) / (width - 1));
                }
            }
        } else if (pattern == "gradient_y") {
            for (int y = 0; y < height; ++y) {
                for (int x = 0; x < width; ++x) {
                    ptr[y * width + x] = static_cast<uint8_t>((y * 255) / (height - 1));
                }
            }
        ...
        return image;
    }, py::arg("width"), py::arg("height"), py::arg("pattern") = "random", py::arg("seed") = 42,
    R"pbdoc(
