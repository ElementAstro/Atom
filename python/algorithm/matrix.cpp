...
void bindMatrix(py::module& m, const std::string& suffix) {
    ...
    std::string class_name = "Matrix" + std::to_string(Rows) + "x" + std::to_string(Cols) + suffix;
    std::string class_doc = "Matrix class with " + std::to_string(Rows) + " rows and " + 
    ...
    py::class_<MatrixType>(m, class_name.c_str(), class_doc.c_str())
        ...
        // Element access
        .def("__call__", [](MatrixType& self, size_t row, size_t col) -> T& {
        ...
        .def("__call__", [](const MatrixType& self, size_t row, size_t col) -> T {
             if (row >= Rows || col >= Cols) {
                 throw py::index_error("Matrix index out of bounds");
             }
             return self(row, col);
        ...
        // Properties
        .def_property_readonly("rows", [](const MatrixType&) { return Rows; },
                              "Number of rows in the matrix.")
        .def_property_readonly("cols", [](const MatrixType&) { return Cols; },
                              "Number of columns in the matrix.")
        .def_property_readonly("size", [](const MatrixType&) { return Rows * Cols; },
                              "Total number of elements in the matrix.")
        
        // Data access
        .def("get_data", [](const MatrixType& self) {
             const auto& data = self.getData();
             return std::vector<T>(data.begin(), data.end());
         }, "Returns matrix data as a list in row-major order.")
        .def("set_data", [](MatrixType& self, const std::vector<T>& data) {
             if (data.size() != Rows * Cols) {
                 throw py::value_error("Data size must match matrix dimensions");
             }
        ...
        .def("trace", [](const MatrixType& self) -> T {
             static_assert(Rows == Cols, "Trace only defined for square matrices");
             return self.trace();
         }, "Computes the trace (sum of diagonal elements) - square matrices only.")
        .def("determinant", [](const MatrixType& self) -> T {
             static_assert(Rows == Cols, "Determinant only defined for square matrices");
             return self.determinant();
         }, "Computes the determinant - square matrices only.")
        .def("inverse", [](const MatrixType& self) -> MatrixType {
             static_assert(Rows == Cols, "Inverse only defined for square matrices");
             return self.inverse();
         }, "Computes the matrix inverse - square matrices only.")
        
        // Utility methods
        .def("print", &MatrixType::print, py::arg("width") = 8, py::arg("precision") = 2,
        ...
        .def("is_symmetric", [](const MatrixType& self) -> bool {
             static_assert(Rows == Cols, "Symmetry only defined for square matrices");
             return self.isSymmetric();
         }, "Checks if the matrix is symmetric - square matrices only.")
        .def("condition_number", [](const MatrixType& self) -> T {
             static_assert(Rows == Cols, "Condition number only defined for square matrices");
        ...
        .def("__repr__", [suffix](const MatrixType&) {
             return "Matrix" + std::to_string(Rows) + "x" + std::to_string(Cols) + suffix + "()";
        ...
        // NumPy integration
        .def("to_numpy", [](const MatrixType& self) {
        ...
        .def_static("from_numpy", [](py::array_t<T> input) {
             py::buffer_info buf = input.request();
             if (buf.ndim != 2 || buf.shape[0] != Rows || buf.shape[1] != Cols) {
                 throw py::value_error("Array dimensions must match matrix size");
             }
             T* ptr = static_cast<T*>(buf.ptr);
...
void bindMatrixMultiplication(py::module& m, const std::string& suffix) {
    ...
    std::string func_name = "multiply_" + std::to_string(RowsA) + "x" + 
                           std::to_string(ColsA_RowsB) + "_" + std::to_string(ColsA_RowsB) + 
                           "x" + std::to_string(ColsB) + suffix;
    
    m.def(func_name.c_str(), [](const MatrixA& a, const MatrixB& b) -> MatrixResult {
        return a * b;
    }, py::arg("a"), py::arg("b"), 
    ("Matrix multiplication: " + std::to_string(RowsA) + "x" + std::to_string(ColsA_RowsB) + 
     " * " + std::to_string(ColsA_RowsB) + "x" + std::to_string(ColsB)).c_str());
}
...
PYBIND11_MODULE(matrix, m) {
    ...
    m.def("random_matrix3x3_double", [](double min, double max) {
        return atom::algorithm::randomMatrix<double, 3, 3>(min, max);
    }, py::arg("min") = 0.0, py::arg("max") = 1.0,
    "Creates a 3x3 double matrix with random values.");
    m.def("random_matrix4x4_double", [](double min, double max) {
    ...
    m.def("transpose2x2_double", [](const atom::algorithm::Matrix<double, 2, 2>& mat) {
        return atom::algorithm::transpose(mat);
    }, py::arg("matrix"), "Transposes a 2x2 double matrix.");

    m.def("transpose3x3_double", [](const atom::algorithm::Matrix<double, 3, 3>& mat) {
        return atom::algorithm::transpose(mat);
    }, py::arg("matrix"), "Transposes a 3x3 double matrix.");

    m.def("transpose4x4_double", [](const atom::algorithm::Matrix<double, 4, 4>& mat) {
        return atom::algorithm::transpose(mat);
    }, py::arg("matrix"), "Transposes a 4x4 double matrix.");

    m.def("transpose2x3_double", [](const atom::algorithm::Matrix<double, 2, 3>& mat) {
        return atom::algorithm::transpose(mat);
    }, py::arg("matrix"), "Transposes a 2x3 double matrix to 3x2.");

    m.def("transpose3x2_double", [](const atom::algorithm::Matrix<double, 3, 2>& mat) {
        return atom::algorithm::transpose(mat);
    }, py::arg("matrix"), "Transposes a 3x2 double matrix to 2x3.");

    // LU Decomposition functions
    m.def("lu_decomposition2x2_double", [](const atom::algorithm::Matrix<double, 2, 2>& mat) {
        return atom::algorithm::luDecomposition(mat);
    }, py::arg("matrix"), "Performs LU decomposition on a 2x2 double matrix.");

    m.def("lu_decomposition3x3_double", [](const atom::algorithm::Matrix<double, 3, 3>& mat) {
        return atom::algorithm::luDecomposition(mat);
    }, py::arg("matrix"), "Performs LU decomposition on a 3x3 double matrix.");

    m.def("lu_decomposition4x4_double", [](const atom::algorithm::Matrix<double, 4, 4>& mat) {
        return atom::algorithm::luDecomposition(mat);
    }, py::arg("matrix"), "Performs LU decomposition on a 4x4 double matrix.");

    // Singular Value Decomposition functions
    m.def("svd2x2_double", [](const atom::algorithm::Matrix<double, 2, 2>& mat) {
        return atom::algorithm::singularValueDecomposition(mat);
    }, py::arg("matrix"), "Performs SVD on a 2x2 double matrix, returns singular values.");

    m.def("svd3x3_double", [](const atom::algorithm::Matrix<double, 3, 3>& mat) {
        return atom::algorithm::singularValueDecomposition(mat);
    }, py::arg("matrix"), "Performs SVD on a 3x3 double matrix, returns singular values.");

    m.def("svd4x4_double", [](const atom::algorithm::Matrix<double, 4, 4>& mat) {
        return atom::algorithm::singularValueDecomposition(mat);
    }, py::arg("matrix"), "Performs SVD on a 4x4 double matrix, returns singular values.");
