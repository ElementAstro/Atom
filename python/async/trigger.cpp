    ...
    >>> print(f"Event has {count} listeners")
        ...
        .def_static(
            ...
            [](std::size_t queue_size) {
                return atom::async::Trigger<py::object>::createLockFreeTriggerQueue(queue_size);
            },
        ...
        .def(
            ...
            [](atom::async::Trigger<py::object>& self, py::object queue, std::size_t max_events) {
                // Note: This would require proper queue binding
                // For now, we'll provide a placeholder
                py::print("Lock-free trigger processing not fully implemented in Python bindings");
                return std::size_t(0);
            },
...
Examples:
    ...
    .def(
        "minutes",
        [](long long m) { return std::chrono::milliseconds(m * 60 * 1000); },
        py::arg("m"),
        R"pbdoc(
    ...
    .def(
        ...
        [](std::size_t num_events, std::size_t num_callbacks) -> py::dict {
            ...
            for (std::size_t i = 0; i < num_callbacks; ++i) {
                ...
                auto id = trigger.registerCallback("benchmark_event", callback);
            ...
            double events_per_second = (num_events * 1000000.0) / total_time_us;
            double total_callbacks_executed = num_events * num_callbacks;
            double callbacks_per_second = (total_callbacks_executed * 1000000.0) / total_time_us;
            ...
            results[py::str("total_callbacks_executed")] = total_callbacks_executed;
    ...
    .def(
        ...
        [](py::list event_names, py::function callback) -> py::object {
            if (event_names.empty()) {
                throw std::invalid_argument("Event names list cannot be empty");
            }

            auto trigger = std::make_unique<atom::async::Trigger<py::object>>();
            std::vector<std::size_t> callback_ids;

            // Register the same callback for all events
            for (auto event_name : event_names) {
                std::string event_str = event_name.cast<std::string>();
                auto id = trigger->registerCallback(event_str, [callback](const py::object& data) {
