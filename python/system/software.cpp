...
Note:
    ...
    m.def("get_app_version", &atom::system::getAppVersion, py::arg("app_path"),
...
Note:
    ...
    m.def("get_app_path", &atom::system::getAppPath, py::arg("software_name"),
...
Note:
    ...
    m.def("monitor_software_usage", &atom::system::monitorSoftwareUsage,
          py::arg("software_name"), py::arg("callback"),
...
Note:
    ...
    // Utility functions
    m.def(
        "get_installed_software_list",
        []() -> py::list {
            py::list result;

            // Common software to check
            std::vector<std::string> common_software = {
                "python", "git",   "node",  "npm",     "docker",
                "java",   "gcc",   "clang", "cmake",   "make",
                "vim",    "emacs", "code",  "firefox", "chrome"};
...
Note:
    ...
    m.def(
        "get_software_info",
        [](const std::string& software_name) -> py::dict {
            py::dict info;
            info["name"] = software_name;
            info["installed"] =
                atom::system::checkSoftwareInstalled(software_name);
            ...
            if (info["installed"].cast<bool>()) {
                try {
                    ...
                    try {
                        auto permissions =
                            atom::system::getAppPermissions(path);
                    ...
                    try {
                        auto process_info =
                            atom::system::getProcessInfo(software_name);
                        info["running"] = true;
