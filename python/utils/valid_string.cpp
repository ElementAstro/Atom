...
PYBIND11_MODULE(valid_string, m) {
    ...
    py::enum_<atom::utils::BracketType>(
        m, "BracketType",
        ...
        .value("CUSTOM", atom::utils::BracketType::Custom,
               "Custom bracket types")
        .export_values();

    // BracketInfo structure
    py::class_<atom::utils::BracketInfo>(
        m, "BracketInfo",
        ...
        .def(py::init<char, int, atom::utils::BracketType>(),
             py::arg("character"), py::arg("position"),
             py::arg("type") = atom::utils::BracketType::Custom,
        ...
        .def_readwrite("character", &atom::utils::BracketInfo::character,
                       "The bracket character")
        .def_readwrite("position", &atom::utils::BracketInfo::position,
                       "The position in the string")
        .def_readwrite("type", &atom::utils::BracketInfo::type,
                       "The type of bracket")
        .def("get_bracket_type_name",
             &atom::utils::BracketInfo::getBracketTypeName,
             R"(Get a readable name for the bracket type.

             Returns:
                 A string describing the bracket type.
             )")
        .def("__eq__", &atom::utils::BracketInfo::operator==,
             "Equality comparison")
        .def("__repr__", [](const atom::utils::BracketInfo& info) {
            return std::format("BracketInfo('{}', {}, {})", info.character,
                               info.position, static_cast<int>(info.type));
        });
    ...
    py::class_<atom::utils::ValidationOptions>(m, "ValidationOptions",
        ...
        .def_readwrite("ignore_quotes",
                       &atom::utils::ValidationOptions::ignoreQuotes,
                       "Whether to ignore quotes during validation")
        .def_readwrite("ignore_escaped",
                       &atom::utils::ValidationOptions::ignoreEscaped,
                       "Whether to ignore escaped characters")
        .def_readwrite("strict_mode",
                       &atom::utils::ValidationOptions::strictMode,
                       "Whether to use strict validation mode")
        .def_readwrite("allow_nested",
                       &atom::utils::ValidationOptions::allowNested,
        ...
        .def("__repr__", [](const atom::utils::ValidationOptions& opts) {
            return std::format(
                "ValidationOptions(ignore_quotes={}, ignore_escaped={}, "
                "strict_mode={}, allow_nested={}, max_depth={})",
                opts.ignoreQuotes, opts.ignoreEscaped, opts.strictMode,
                opts.allowNested, opts.maxDepth);
        });
    ...
    py::class_<atom::utils::ValidationResult>(m, "ValidationResult",
        ...
        .def_readwrite("error_message",
                       &atom::utils::ValidationResult::errorMessage,
                       "Error message if validation failed")
        .def_readwrite("error_position",
                       &atom::utils::ValidationResult::errorPosition,
                       "Position of the first error")
        .def_readwrite("invalid_brackets",
                       &atom::utils::ValidationResult::invalidBrackets,
                       "List of invalid bracket information")
        .def_readwrite("nesting_depth",
                       &atom::utils::ValidationResult::nestingDepth,
                       "Maximum nesting depth found")
        .def("__repr__", [](const atom::utils::ValidationResult& result) {
            return std::format(
                "ValidationResult(is_valid={}, error_position={}, "
                "nesting_depth={})",
                result.isValid, result.errorPosition, result.nestingDepth);
        });

    // Exception classes
    py::register_exception<atom::utils::ValidationException>(
    ...
    py::register_exception<atom::utils::BracketMismatchException>(
        m, "BracketMismatchException",
    ...
    py::register_exception<atom::utils::QuoteMismatchException>(
        m, "QuoteMismatchException",
    ...
    py::enum_<atom::utils::QuoteMismatchException::QuoteType>(
        m, "QuoteType", R"(Type of quote that caused a mismatch.)")
        .value("SINGLE", atom::utils::QuoteMismatchException::QuoteType::Single,
               "Single quote '")
        .value("DOUBLE", atom::utils::QuoteMismatchException::QuoteType::Double,
               "Double quote \"")
        .export_values();
