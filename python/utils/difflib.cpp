...
PYBIND11_MODULE(difflib, m) {
    ...
    py::register_exception<atom::utils::InvalidInputException>(
        m, "InvalidInputException");
    py::register_exception<atom::utils::AlgorithmFailureException>(
        m, "AlgorithmFailureException");

    // DiffAlgorithm enumeration
    py::enum_<atom::utils::DiffAlgorithm>(
        m, "DiffAlgorithm",
        ...
        .value("DEFAULT", atom::utils::DiffAlgorithm::Default,
               "Original algorithm")
        .value("MYERS", atom::utils::DiffAlgorithm::Myers,
               "Myers diff algorithm")
        .value("PATIENCE", atom::utils::DiffAlgorithm::Patience,
               "Patience diff algorithm")
        .value("HISTOGRAM", atom::utils::DiffAlgorithm::Histogram,
               "Histogram diff algorithm")
#ifdef ATOM_HAS_BOOST_GRAPH
        .value("GRAPH", atom::utils::DiffAlgorithm::Graph,
               "Boost graph-based algorithm")
#endif
        .export_values();
    ...
    py::class_<atom::utils::DiffStats>(m, "DiffStats",
        ...
        .def_readwrite("insertions", &atom::utils::DiffStats::insertions,
                       "Number of insertions")
        .def_readwrite("deletions", &atom::utils::DiffStats::deletions,
                       "Number of deletions")
        .def_readwrite("modifications", &atom::utils::DiffStats::modifications,
                       "Number of modifications")
        .def_readwrite("similarity", &atom::utils::DiffStats::similarity,
        ...
        .def("__repr__", [](const atom::utils::DiffStats& stats) {
            return std::format(
                "DiffStats(insertions={}, deletions={}, modifications={}, "
                "similarity={:.3f})",
                stats.insertions, stats.deletions, stats.modifications,
                stats.similarity);
        });

    // DiffOptions structure
    py::class_<atom::utils::DiffOptions>(
        m, "DiffOptions",
        ...
        .def_readwrite("enable_caching",
                       &atom::utils::DiffOptions::enableCaching,
                       "Enable result caching")
        .def_readwrite("use_parallel_processing",
                       &atom::utils::DiffOptions::useParallelProcessing,
        ...
        .def_readwrite("cache_size_limit",
                       &atom::utils::DiffOptions::cacheSizeLimit,
                       "Maximum number of cached results")
        .def_readwrite("large_file_threshold",
                       &atom::utils::DiffOptions::largeFileThreshold,
                       "Threshold for large file optimization (bytes)")
        .def_readwrite("algorithm", &atom::utils::DiffOptions::algorithm,
                       "Diff algorithm to use")
        .def_readwrite("use_flat_containers",
                       &atom::utils::DiffOptions::useFlatContainers,
                       "Use flat containers to improve cache efficiency")
        .def_readwrite("use_small_buffers",
                       &atom::utils::DiffOptions::useSmallBuffers,
        ...
        .def("__repr__", [](const atom::utils::DiffOptions& opts) {
            return std::format(
                "DiffOptions(algorithm={}, caching={}, parallel={})",
                static_cast<int>(opts.algorithm), opts.enableCaching,
                opts.useParallelProcessing);
        });
    ...
    m.def(
        ...
        [](std::string_view word, const std::vector<std::string>& possibilities,
           int n, double cutoff, const atom::utils::DiffOptions& options) {
            return atom::utils::getCloseMatches(
                word, std::span<const std::string>(possibilities), n, cutoff,
                options);
        },
...
Examples:
    ...
    // FuzzyMatcher class
    py::class_<atom::utils::FuzzyMatcher>(
        m, "FuzzyMatcher",
        ...
        .def_static(
            ...
            &atom::utils::FuzzyMatcher::levenshteinDistance, py::arg("s1"),
            py::arg("s2"),
        ...
        .def_static(
            "similarity", &atom::utils::FuzzyMatcher::similarity, py::arg("s1"),
            py::arg("s2"),
        ...
        .def(
            ...
            [](atom::utils::FuzzyMatcher& self, std::string_view needle,
               const std::vector<std::string>& haystacks, double cutoff) {
                return self.findAllMatches(
                    needle, std::span<const std::string>(haystacks), cutoff);
            },
    ...
    // InlineDiff class
    py::class_<atom::utils::InlineDiff>(
        m, "InlineDiff",
        ...
        .def("to_html", &atom::utils::InlineDiff::toHtml, py::arg("str1"),
             py::arg("str2"),
    ...
    py::class_<atom::utils::HtmlDiff::HtmlDiffOptions>(
        m, "HtmlDiffOptions",
        ...
        .def_readwrite("added_class",
                       &atom::utils::HtmlDiff::HtmlDiffOptions::addedClass,
                       "CSS class for added content")
        .def_readwrite("removed_class",
                       &atom::utils::HtmlDiff::HtmlDiffOptions::removedClass,
                       "CSS class for removed content")
        .def_readwrite("changed_class",
                       &atom::utils::HtmlDiff::HtmlDiffOptions::changedClass,
                       "CSS class for changed content")
        .def_readwrite("inline_diff",
                       &atom::utils::HtmlDiff::HtmlDiffOptions::inlineDiff,
                       "Show character-level inline diffs")
        .def_readwrite("show_statistics",
                       &atom::utils::HtmlDiff::HtmlDiffOptions::showStatistics,
                       "Show diff statistics")
        .def_readwrite("show_line_numbers",
                       &atom::utils::HtmlDiff::HtmlDiffOptions::showLineNumbers,
                       "Show line numbers")
        .def_readwrite("show_side_by_side",
                       &atom::utils::HtmlDiff::HtmlDiffOptions::showSideBySide,
                       "Show side-by-side diff view")
        .def_readwrite(
            "collapsable_unchanged",
            &atom::utils::HtmlDiff::HtmlDiffOptions::collapsableUnchanged,
            "Make unchanged sections collapsible")
        .def_readwrite("context_lines",
                       &atom::utils::HtmlDiff::HtmlDiffOptions::contextLines,
                       "Number of context lines to display")
        .def("__repr__",
             [](const atom::utils::HtmlDiff::HtmlDiffOptions& opts) {
                 return std::format(
                     "HtmlDiffOptions(inline_diff={}, show_statistics={}, "
                     "context_lines={})",
                     opts.inlineDiff, opts.showStatistics, opts.contextLines);
             });

    // Enhanced HtmlDiff class
    py::class_<atom::utils::HtmlDiff>(
        m, "HtmlDiff",
        ...
        .def(
            "make_file",
            [](atom::utils::HtmlDiff& self,
               const std::vector<std::string>& fromlines,
               const std::vector<std::string>& tolines,
               std::string_view fromdesc, std::string_view todesc,
               const atom::utils::HtmlDiff::HtmlDiffOptions& htmlOptions)
                -> std::string {
                auto result =
                    self.makeFile(std::span<const std::string>(fromlines),
            ...
            py::arg("fromlines"), py::arg("tolines"), py::arg("fromdesc") = "",
            py::arg("todesc") = "",
            py::arg("html_options") = atom::utils::HtmlDiff::HtmlDiffOptions{},
            R"(Generate an HTML file showing the differences between two sequences.
        ...
        .def(
            ...
            [](atom::utils::HtmlDiff& self,
               ...
               const atom::utils::HtmlDiff::HtmlDiffOptions& htmlOptions)
                -> std::string {
                auto result =
                    self.makeTable(std::span<const std::string>(fromlines),
                                   std::span<const std::string>(tolines),
                                   fromdesc, todesc, htmlOptions);
                if (result.has_value()) {
                    return result.value();
    ...
    py::class_<atom::utils::DiffLibConfig>(
        m, "DiffLibConfig",
        ...
        .def_static("set_telemetry_enabled",
                    &atom::utils::DiffLibConfig::setTelemetryEnabled,
                    py::arg("enabled"),
        ...
        .def_static("set_default_options",
                    &atom::utils::DiffLibConfig::setDefaultOptions,
                    py::arg("options"),
        ...
        .def_static("get_default_options",
                    &atom::utils::DiffLibConfig::getDefaultOptions,
        ...
        .def_static(
            "supports_high_performance_containers",
