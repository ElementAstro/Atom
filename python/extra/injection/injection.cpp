...
Examples:
    ...
    >>> print(tag.value)
        ...
        .def(
            "__eq__",
            [](const atom::extra::Tag& self, const atom::extra::Tag& other) {
                return self.name == other.name && self.value == other.value;
            },
            "Check tag equality")
        .def(
            "__str__",
            [](const atom::extra::Tag& self) {
...
The scope determines how instances are created and managed by the container.)")
    ...
    // Since the injection system is heavily templated and designed for
    // compile-time type safety, we'll provide a simplified Python interface
    // that works with Python objects and strings as keys.
...
Examples:
    ...
    >>> url = child.resolve("database_url")
        ...
        .def(
            ...
            &atom::extra::Container<>::createChildContainer,
...
Examples:
    ...
    py::class_<std::map<std::string, py::object>>(
        m, "ServiceRegistry",
...
Examples:
    ...
    >>> cache = registry["cache"]
        ...
        .def(
            "__getitem__",
            [](std::map<std::string, py::object>& self,
...
Raises:
    KeyError: If the service is not found.
        ...
        .def(
            "__setitem__",
            [](std::map<std::string, py::object>& self, const std::string& key,
               py::object value) { self[key] = value; },
...
Args:
    ...
    value: The service instance.
        ...
        .def(
            "__contains__",
            [](const std::map<std::string, py::object>& self,
               const std::string& key) { return self.find(key) != self.end(); },
...
Returns:
    True if the service is registered.
        ...
        .def(
            "get",
            [](const std::map<std::string, py::object>& self,
               const std::string& key, py::object default_value = py::none()) {
                auto it = self.find(key);
...
Returns:
    List of service keys.
        ...
        .def(
            "clear",
            [](std::map<std::string, py::object>& self) { self.clear(); },
