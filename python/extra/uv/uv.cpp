...
Examples:
    ...
    py::register_exception<atom::extra::uv::MessageBusError>(
        m, "MessageBusError", PyExc_RuntimeError);
...
Represents the current state of a subprocess in its lifecycle.)")
        .value("NOT_STARTED", atom::extra::uv::ProcessStatus::NOT_STARTED,
               "Process has not been started")
        .value("RUNNING", atom::extra::uv::ProcessStatus::RUNNING,
               "Process is currently running")
        .value("EXITED", atom::extra::uv::ProcessStatus::EXITED,
               "Process has exited normally")
        .value("KILLED", atom::extra::uv::ProcessStatus::KILLED,
               "Process was killed by signal")
        .value("ERROR", atom::extra::uv::ProcessStatus::ERROR,
               "Process encountered an error")
        .export_values();

    // BackPressureConfig struct
    py::class_<atom::extra::uv::BackPressureConfig>(
...
Examples:
    ...
    >>> bus = uv.MessageBus(config)
        ...
        .def_readwrite(
            "max_queue_size",
            &atom::extra::uv::BackPressureConfig::max_queue_size,
        ...
        .def_readwrite("drop_oldest",
                       &atom::extra::uv::BackPressureConfig::drop_oldest,
                       "Whether to drop oldest messages when queue is full");

    // ProcessOptions struct
    py::class_<atom::extra::uv::ProcessOptions>(
        m, "ProcessOptions",
...
Examples:
    ...
    >>> options.capture_stderr = True
        ...
        .def_readwrite("timeout_ms",
                       &atom::extra::uv::ProcessOptions::timeout_ms,
                       "Timeout in milliseconds (0 = no timeout)")
        .def_readwrite("capture_stdout",
                       &atom::extra::uv::ProcessOptions::capture_stdout,
                       "Whether to capture stdout")
        .def_readwrite("capture_stderr",
                       &atom::extra::uv::ProcessOptions::capture_stderr,
    ...
    py::class_<atom::extra::uv::MessageEnvelope<std::string>>(
        m, "MessageEnvelope",
...
Examples:
    ...
    ...     print(f"Time: {envelope.timestamp}")
        ...
        .def_readwrite("topic",
                       &atom::extra::uv::MessageEnvelope<std::string>::topic,
                       "Message topic")
        .def_readwrite("payload",
                       &atom::extra::uv::MessageEnvelope<std::string>::payload,
                       "Message payload")
        .def_readwrite(
            "timestamp",
            &atom::extra::uv::MessageEnvelope<std::string>::timestamp,
            "Message timestamp")
        .def_readwrite(
            "sender_id",
            &atom::extra::uv::MessageEnvelope<std::string>::sender_id,
            "Sender identifier")
        .def_readwrite(
            "message_id",
            &atom::extra::uv::MessageEnvelope<std::string>::message_id,
            "Unique message identifier")
        .def_readwrite("metadata",
                       &atom::extra::uv::MessageEnvelope<std::string>::metadata,
                       "Additional metadata key-value pairs");

    // MessageBus::QueueStats
    py::class_<atom::extra::uv::MessageBus::QueueStats>(
        m, "QueueStats",
...
and queue status.)")
        .def_readwrite(
            "pending_messages",
            &atom::extra::uv::MessageBus::QueueStats::pending_messages,
            "Number of messages waiting to be processed")
        .def_readwrite("max_queue_size",
                       &atom::extra::uv::MessageBus::QueueStats::max_queue_size,
                       "Maximum queue size")
        .def_readwrite("total_handlers",
                       &atom::extra::uv::MessageBus::QueueStats::total_handlers,
                       "Total number of registered handlers")
        .def_readwrite(
            "avg_delivery_time",
            &atom::extra::uv::MessageBus::QueueStats::avg_delivery_time,
            "Average message delivery time");

    // MessageBus class
    py::class_<atom::extra::uv::MessageBus>(
        m, "MessageBus",
...
Examples:
    ...
    >>> del subscription  # Automatically unsubscribes
        ...
        .def(
            "subscribe",
            [](atom::extra::uv::MessageBus& self,
               const std::string& topic_pattern, py::function handler) {
                // Create a C++ handler that calls the Python function
                auto cpp_handler =
                    [handler](
                        const atom::extra::uv::MessageEnvelope<std::string>&
                            envelope) {
...
Returns:
    Subscription handle that automatically unsubscribes when destroyed
        ...
        .def(
            "publish",
            [](atom::extra::uv::MessageBus& self, const std::string& topic,
...
Returns:
    ...
    // UvProcess class
    py::class_<atom::extra::uv::UvProcess>(
        m, "UvProcess",
...
Examples:
    ...
    >>> process.spawn("ls", ["-la"], stdout_callback=on_stdout, exit_callback=on_exit)
        ...
        .def(
            "spawn",
            [](atom::extra::uv::UvProcess& self, const std::string& file,
