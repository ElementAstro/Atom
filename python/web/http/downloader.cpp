...
void init_downloader(py::module_& m) {
...
Examples:
    ...
    >>> dm.start(2, 1024 * 1024)  # 2 threads, limit to 1MB/s
        ...
        .def("stop", &atom::web::DownloadManager::stop,
             R"(Stops all download tasks gracefully.

Examples:
    >>> dm.stop()  # Stop all active downloads
)")
...
Examples:
    ...
    >>> print(f"Downloaded {bytes_downloaded} bytes")
        ...
        .def("get_total_bytes", &atom::web::DownloadManager::getTotalBytes,
             py::arg("index"),
             R"(Gets the total size of a download task.

Args:
    index: The index in the task list.

Returns:
    int: The total size in bytes, 0 if unknown.

Examples:
    >>> total_bytes = dm.get_total_bytes(0)
    >>> print(f"Total size: {total_bytes} bytes")
)")
        .def("get_progress", &atom::web::DownloadManager::getProgress,
             py::arg("index"),
             R"(Gets the download progress as a percentage.

Args:
    index: The index in the task list.

Returns:
    float: Progress percentage (0.0 to 100.0), -1.0 if unknown.

Examples:
    >>> progress = dm.get_progress(0)
    >>> if progress >= 0:
...
Examples:
    ...
    >>> dm.on_progress_update(progress_update)
        ...
        .def("on_error", &atom::web::DownloadManager::onError,
             py::arg("callback"),
             R"(Registers a callback function to be called when an error occurs.

Args:
    callback: The callback function, with the task index and error message as parameters.

Examples:
    >>> def error_handler(index, error_message):
    ...     print(f"Task {index} error: {error_message}")
    ...
    >>> dm.on_error(error_handler)
)")
        .def("get_active_task_count",
             &atom::web::DownloadManager::getActiveTaskCount,
             R"(Gets the number of active download tasks.

Returns:
    int: Number of tasks currently being downloaded.

Examples:
    >>> active = dm.get_active_task_count()
    >>> print(f"{active} tasks are currently downloading")
)")
        .def("get_total_task_count",
             &atom::web::DownloadManager::getTotalTaskCount,
             R"(Gets the total number of tasks.

Returns:
    int: Total number of tasks in the manager.

Examples:
    >>> total = dm.get_total_task_count()
    >>> print(f"Total tasks: {total}")
)")
        .def("is_running", &atom::web::DownloadManager::isRunning,
             R"(Checks if the download manager is currently running.
...
Examples:
    >>> if dm.is_running():
    ...     print("Download manager is active")
    ... else:
    ...     print("Download manager is stopped")
)");
