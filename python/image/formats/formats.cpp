...
void bind_formats(py::module& m) {
    ...
    py::class_<FITSFile>(m, "FITSFile",
        ...
        Example:
            ...
            >>> hdu_count = fits.getHDUCount()
            >>> primary_hdu = fits.getHDU(0)
            >>> data = primary_hdu.getData()
        )pbdoc")
        .def(py::init<>(), "Default constructor")
        .def(py::init<const std::string&>(), py::arg("filename"),
             "Construct and read FITS file")
        .def("readFITS", &FITSFile::readFITS, py::arg("filename"),
             "Read FITS file from disk")
        .def("readFITSAsync", &FITSFile::readFITSAsync, py::arg("filename"),
             "Read FITS file asynchronously")
        .def("writeFITS", &FITSFile::writeFITS, py::arg("filename"),
             "Write FITS file to disk")
        .def("writeFITSAsync", &FITSFile::writeFITSAsync, py::arg("filename"),
             "Write FITS file asynchronously")
        .def("getHDUCount", &FITSFile::getHDUCount, "Get number of HDUs")
        .def("isEmpty", &FITSFile::isEmpty, "Check if file is empty")
        .def("getHDU", &FITSFile::getHDU, py::arg("index"),
             py::return_value_policy::reference_internal, "Get HDU by index")
        .def("addHDU", &FITSFile::addHDU, py::arg("hdu"), "Add HDU to file")
        .def("removeHDU", &FITSFile::removeHDU, py::arg("index"),
             "Remove HDU by index")
        .def("getPrimaryHDU", &FITSFile::getPrimaryHDU,
             py::return_value_policy::reference_internal, "Get primary HDU")
        .def("createImageHDU", &FITSFile::createImageHDU, py::arg("width"),
             py::arg("height"), py::arg("bitpix") = 16, "Create new image HDU")
        .def("setProgressCallback", &FITSFile::setProgressCallback,
             py::arg("callback"), "Set progress callback for long operations")
        .def("validate", &FITSFile::validate, "Validate FITS file structure")
        .def("getInfo", &FITSFile::getInfo, "Get file information")
        .def("clear", &FITSFile::clear, "Clear all HDUs")
        .def("__len__", &FITSFile::getHDUCount, "Get number of HDUs")
        .def("__getitem__", &FITSFile::getHDU,
             py::return_value_policy::reference_internal, "Get HDU by index")
