...
class TestReorganizer:
    ...
    def analyze_current_structure(self) -> Dict[str, any]:
        ...
        analysis["missing_cpp_modules"] = (
            self.cpp_modules - analysis["cpp_modules_found"]
        )
        ...
        if self.python_test_dir.exists():
            for item in self.python_test_dir.iterdir():
                if (
                    ...
                    item.is_file()
                    and item.suffix == ".py"
                    and item.name not in ["__init__.py", "conftest.py"]
                ):
                    analysis["python_orphaned_files"].append(item)

        analysis["missing_python_modules"] = (
            self.python_modules - analysis["python_modules_found"]
        )
...
"""
                ...
                changes.append(
                    f"Created C++ test directory: {module_dir.relative_to(self.project_root)}"
                )
    ...
    def organize_orphaned_files(self) -> List[str]:
        ...
        for orphaned_file in analysis["cpp_orphaned_files"]:
            ...
            if target_module:
                ...
                changes.append(
                    f"Moved {orphaned_file.relative_to(self.project_root)} to {target_path.relative_to(self.project_root)}"
                )
            else:
                changes.append(
                    f"Could not determine module for orphaned file: {orphaned_file.relative_to(self.project_root)}"
                )
...
"""
                ...
                changes.append(
                    f"Created C++ test template: {test_file.relative_to(self.project_root)}"
                )
...
"""
                ...
                changes.append(
                    f"Created Python test template: {test_file.relative_to(self.project_root)}"
                )
...
"""
        ...
        changes.append(
            f"Created test documentation: {readme_file.relative_to(self.project_root)}"
        )
    ...
    def run_reorganization(self) -> Dict[str, List[str]]:
        ...
        print_colored(
            f"Found {len(analysis['cpp_modules_found'])} C++ modules", Colors.GREEN
        )
        print_colored(
            f"Found {len(analysis['python_modules_found'])} Python modules",
            Colors.GREEN,
        )
        print_colored(
            f"Missing {len(analysis['missing_cpp_modules'])} C++ modules", Colors.YELLOW
        )
        print_colored(
            f"Missing {len(analysis['missing_python_modules'])} Python modules",
            Colors.YELLOW,
        )
...
Examples:
  ...
  %(prog)s --apply            # Apply the changes
    ...
    parser.add_argument(
        "--apply", action="store_true", help="Apply the changes to files"
    )
    ...
    for category, changes in results.items():
        if changes:
            ...
            print_colored(
                f"\n{category.replace('_', ' ').title()}: ✅ No changes needed",
                Colors.GREEN,
            )
    ...
    if total_changes == 0:
        ...
        if args.dry_run:
            print_colored(
                f"📋 Found {total_changes} potential improvements. Use --apply to make them.",
                Colors.YELLOW,
            )
        else:
            print_colored(
                f"✅ Applied {total_changes} improvements successfully!", Colors.GREEN
            )
