...
class BuildSystemValidator:
        ...
        for module in self.modules:
            ...
            if not module_dir.exists():
                self.validation_results["errors"].append(
                    f"Module directory missing: {module}"
                )
                all_valid = False
                continue
            ...
            if not cmake_file.exists():
                self.validation_results["errors"].append(
                    f"CMakeLists.txt missing for module: {module}"
                )
                all_valid = False
                continue
    ...
    def validate_dependency_configuration(self) -> bool:
        ...
        if not module_deps_file.exists():
            self.validation_results["errors"].append(
                "module_dependencies.cmake not found"
            )
            return False

        with open(module_deps_file, encoding="utf-8") as f:
            content = f.read()
        ...
        for func in required_functions:
            if func not in content:
                self.validation_results["errors"].append(
                    f"Required function missing: {func}"
                )
                return False
    ...
    def run_validation(
        self, test_builds: bool = False, modules_to_test: List[str] = None
    ) -> bool:
    ...
    def generate_report(self):
        ...
        print("\n📋 VALIDATION SUMMARY:")
        print(
            f"   Modules validated: {len(self.validation_results['module_coverage'])}"
        )
...
def main():
    ...
    parser.add_argument(
        "--test-builds", action="store_true", help="Test selective builds (slower)"
    )
    parser.add_argument(
        "--modules", nargs="+", help="Specific modules to test builds for"
    )
    parser.add_argument(
