...
class TestMigrator:
            ...
            try:
                ...
                def fix_class_name(match):
                    ...
                    if not class_name.endswith("Test"):
                        ...
                        changes.append(
                            f"Renamed class {class_name} to {new_name} in {test_file.relative_to(self.project_root)}"
                        )
    ...
    def perform_file_renames(
        self, renames: List[Tuple[Path, Path]], file_type: str
    ) -> List[str]:
        ...
        for old_path, new_path in renames:
            try:
                ...
                changes.append(
                    f"Renamed {file_type} file: {old_path.relative_to(self.project_root)} -> {new_path.relative_to(self.project_root)}"
                )
            except Exception as e:
    ...
    def run_migration(self) -> Dict[str, List[str]]:
        ...
        results["python_file_renames"] = self.perform_file_renames(
            python_renames, "Python"
        )
...
Examples:
  ...
  %(prog)s --apply            # Apply the changes
    ...
    for category, changes in results.items():
        if changes:
            ...
            for change in changes:
                print_colored(
                    f"  📝 {change}",
                    Colors.GREEN if not change.startswith("Error") else Colors.RED,
                )
                total_changes += 1
        else:
            print_colored(
    ...
    if total_changes == 0:
        ...
        if args.dry_run:
            print_colored(
                f"📋 Found {total_changes} potential changes. Use --apply to make them.",
                Colors.YELLOW,
            )
        else:
            print_colored(
                f"✅ Applied {total_changes} changes successfully!", Colors.GREEN
            )
