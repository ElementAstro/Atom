...
def open_report() -> None:
    ...
    if report_path.exists():
        print_colored(
            f"Opening coverage report: {report_path.absolute()}", Colors.GREEN
        )
        webbrowser.open(f"file://{report_path.absolute()}")
    else:
        print_colored(
            "Coverage report not found. Run coverage analysis first.", Colors.RED
        )
...
Examples:
  ...
  %(prog)s --report-only --open     # Generate report and open
    ...
    parser.add_argument(
        "--test-path", help="Specific test path to run (default: all tests)"
    )
    ...
    if report_path.exists():
        print_colored(
            f"Coverage report available at: {report_path.absolute()}", Colors.GREEN
        )
