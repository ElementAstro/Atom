...
class AtomPortableCreator:
    ...
    def build_project(
        self, build_type: str = "Release", components: Optional[List[str]] = None
    ):
    ...
    def copy_build_artifacts(
        self, portable_dir: Path, components: Optional[List[str]] = None
    ):
        ...
        try:
            ...
            result = subprocess.run(
                install_args, cwd=self.source_dir, capture_output=True, text=True
            )
            if result.returncode != 0:
    ...
    def _copy_installed_files(
        self, install_dir: Path, portable_dir: Path, components: Optional[List[str]]
    ):
                ...
                if file_type == "lib":
                    ...
                    if "/" in pattern:
                        ...
                        if header_dir.exists():
                            ...
                            shutil.copytree(
                                header_dir, dst_component_dir, dirs_exist_ok=True
                            )
    ...
    def _copy_linux_dependencies(self, portable_dir: Path):
        ...
        for lib_file in lib_dir.glob("*.so*"):
            try:
                ...
                result = subprocess.run(
                    ["ldd", str(lib_file)], capture_output=True, text=True
                )
                if result.returncode == 0:
    ...
    def _copy_macos_dependencies(self, portable_dir: Path):
        ...
        for lib_file in lib_dir.glob("*.dylib"):
            try:
                result = subprocess.run(
                    ["otool", "-L", str(lib_file)], capture_output=True, text=True
                )
                if result.returncode == 0:
                    for line in result.stdout.split("\n")[1:]:  # Skip first line
    ...
    def _create_unix_launchers(self, tools_dir: Path, portable_dir: Path):
        ...
        with open(env_script, "w") as f:
            f.write("#!/bin/bash\n")
            f.write(
                'export ATOM_PORTABLE_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"\n'
            )
            f.write('export PATH="$ATOM_PORTABLE_ROOT/bin:$PATH"\n')
            f.write(
                'export LD_LIBRARY_PATH="$ATOM_PORTABLE_ROOT/lib:$LD_LIBRARY_PATH"\n'
            )
            f.write(
                'export CMAKE_PREFIX_PATH="$ATOM_PORTABLE_ROOT:$CMAKE_PREFIX_PATH"\n'
            )
            f.write('echo "Atom portable environment configured"\n')
            f.write('echo "ATOM_PORTABLE_ROOT=$ATOM_PORTABLE_ROOT"\n')
    ...
    def create_documentation(self, portable_dir: Path):
        ...
        with open(readme_file, "w") as f:
            f.write("# Atom Library Portable Distribution\n\n")
            ...
            f.write(
                "- `CMAKE_PREFIX_PATH`: Includes portable root for CMake discovery\n"
            )
    ...
    def create_portable_package(
        self, components: Optional[List[str]] = None, build_type: str = "Release"
    ) -> Path:
        ...
        package_name = (
            f"atom-{self._get_version()}-{self.platform}-{self.arch}-portable"
        )
    ...
    def _create_archive(self, portable_dir: Path) -> Path:
        ...
        if self.platform == "windows":
            ...
            shutil.make_archive(
                str(portable_dir), "gztar", portable_dir.parent, portable_dir.name
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create portable Atom library distribution"
    )
    parser.add_argument(
        "--source",
        type=Path,
    ...
    parser.add_argument(
        "--components", nargs="*", help="Specific components to include (default: all)"
    )
    parser.add_argument(
        "--build-type",
