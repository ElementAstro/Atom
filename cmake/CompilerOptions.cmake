# CompilerOptions.cmake - Compiler options configuration module This file
# contains compiler-specific settings and optimizations for the Atom project It
# handles compiler detection, feature testing, and warning/optimization settings
...
function(check_compiler_requirements)
  # Parse arguments
  set(options "")
  set(oneValueArgs CXX_STANDARD MIN_GCC_VERSION MIN_CLANG_VERSION
                   MIN_MSVC_VERSION)
  set(multiValueArgs "")
  ...
  set(CMAKE_C_STANDARD
      11
      PARENT_SCOPE) # C11 is the latest widely supported C standard in CMake

  # Check compiler version
  if(CMAKE_CXX_COMPILER_ID MATCHES "GNU")
    execute_process(
      COMMAND ${CMAKE_CXX_COMPILER} -dumpfullversion
              -std=c++${CMAKE_CXX_STANDARD} OUTPUT_VARIABLE GCC_VERSION)
    string(REGEX MATCH "[0-9]+\\.[0-9]+" GCC_VERSION ${GCC_VERSION})
    if(GCC_VERSION VERSION_LESS ${CHECK_MIN_GCC_VERSION})
      message(
        FATAL_ERROR
          "Minimum required version of g++ is ${CHECK_MIN_GCC_VERSION}")
    endif()
    message(STATUS "Using g++ version ${GCC_VERSION}")
  elseif(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
    execute_process(COMMAND ${CMAKE_CXX_COMPILER} --version
                    OUTPUT_VARIABLE CLANG_VERSION_OUTPUT)
    string(REGEX MATCH "clang version ([0-9]+\\.[0-9]+)" CLANG_VERSION
    ...
    if(CLANG_VERSION VERSION_LESS ${CHECK_MIN_CLANG_VERSION})
      message(
        FATAL_ERROR
          "Minimum required version of clang is ${CHECK_MIN_CLANG_VERSION}")
    endif()
  ...
  elseif(CMAKE_CXX_COMPILER_ID MATCHES "MSVC")
    if(CMAKE_CXX_COMPILER_VERSION VERSION_LESS ${CHECK_MIN_MSVC_VERSION})
      message(
        FATAL_ERROR
          "Minimum required version of MSVC is ${CHECK_MIN_MSVC_VERSION} (Visual Studio 2019 version 16.10)"
      )
    endif()
  ...
  if(NOT APPLE)
    set(CMAKE_OSX_ARCHITECTURES
        x86_64
        CACHE STRING "Build architecture for non-Apple platforms" FORCE)
  endif()
endfunction()
...
function(configure_compiler_options)
  ...
  if(MSVC)
    # Basic options
    list(APPEND compiler_options /nologo # Suppress copyright message
    )

    # UTF-8 support
    if(ARGS_ENABLE_UTF8)
      # Use the unified UTF-8 switch to avoid conflicts with /source-charset and
      # /execution-charset
      list(APPEND compiler_options /utf-8)
    endif()
    ...
    # if(ARGS_ENABLE_LTO) list(APPEND compiler_options /GL) list(APPEND
    # linker_options /LTCG) endif()
  ...
  elseif(CMAKE_COMPILER_IS_GNUCXX OR CMAKE_CXX_COMPILER_ID MATCHES "Clang")
    ...
    # if(ARGS_ENABLE_LTO) list(APPEND compiler_options -flto) list(APPEND
    # linker_options -flto) endif()
  endif()
  ...
  if(linker_options)
    ...
    set(CMAKE_EXE_LINKER_FLAGS
        "${CMAKE_EXE_LINKER_FLAGS} ${linker_flags_str}"
        PARENT_SCOPE)
    set(CMAKE_SHARED_LINKER_FLAGS
        "${CMAKE_SHARED_LINKER_FLAGS} ${linker_flags_str}"
        PARENT_SCOPE)
  endif()
...
function(apply_build_preset PRESET_NAME)
  if(PRESET_NAME STREQUAL "DEBUG")
    configure_compiler_options(
      ENABLE_UTF8 ENABLE_EXCEPTION_HANDLING ENABLE_WARNINGS WARNING_LEVEL
      "high" ENABLE_DEBUG_INFO)
...
macro(setup_project_defaults)
  ...
  set(oneValueArgs BUILD_PRESET CXX_STANDARD MIN_GCC_VERSION MIN_CLANG_VERSION
                   MIN_MSVC_VERSION)
  set(multiValueArgs PCH_HEADERS)

  cmake_parse_arguments(SETUP "${options}" "${oneValueArgs}"
                        "${multiValueArgs}" ${ARGN})
  ...
  check_compiler_requirements(
    ...
    ${SETUP_CXX_STANDARD}
    MIN_GCC_VERSION
    ${SETUP_MIN_GCC_VERSION}
    MIN_CLANG_VERSION
    ${SETUP_MIN_CLANG_VERSION}
    MIN_MSVC_VERSION
    ${SETUP_MIN_MSVC_VERSION})
  ...
  if(DEFINED SETUP_BUILD_PRESET)
    ...
    # For multi-config generators (e.g. Visual Studio), avoid injecting global
    # optimization flags that conflict with Debug (/RTC1 vs /O2).
    if(CMAKE_CONFIGURATION_TYPES)
      message(
        STATUS
          "Multi-config generator detected; applying minimal global options and deferring optimizations to configs"
      )
      configure_compiler_options(ENABLE_UTF8 ENABLE_EXCEPTION_HANDLING
                                 ENABLE_WARNINGS WARNING_LEVEL "normal")
    else()
      # Automatically select preset based on CMake build type (single-config)
      if(CMAKE_BUILD_TYPE STREQUAL "Debug")
