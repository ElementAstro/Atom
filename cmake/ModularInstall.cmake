...
# This module provides comprehensive modular installation capabilities including:
...
option(ATOM_INSTALL_COMPONENT_PACKAGES "Create separate packages for each component" OFF)
option(ATOM_INSTALL_DEVELOPMENT_FILES "Install development files (headers, CMake configs)" ON)
...
function(atom_register_component COMPONENT_NAME)
    ...
    set_property(GLOBAL PROPERTY ATOM_COMPONENT_${COMPONENT_NAME}_DEPENDS "${ARG_DEPENDS}")
    ...
    foreach(COMPONENT ${REGISTERED_COMPONENTS})
        ...
        foreach(CONFLICT ${COMPONENT_CONFLICTS})
            if(CONFLICT IN_LIST REGISTERED_COMPONENTS)
                message(WARNING "Component ${COMPONENT} conflicts with registered component: ${CONFLICT}")
            endif()
        endforeach()
    endforeach()
        ...
        foreach(TARGET ${ARG_TARGETS})
            if(TARGET ${TARGET})
                install(TARGETS ${TARGET}
                    ...
                    INCLUDES DESTINATION ${ATOM_INSTALL_INCLUDEDIR}
...
function(atom_generate_component_config COMPONENT_NAME)
    ...
    if(COMPONENT_DEPENDS)
        ...
        foreach(DEP ${COMPONENT_DEPENDS})
            file(APPEND ${CONFIG_FILE}
                "find_package(atom-${DEP} REQUIRED)\n"
            )
        endforeach()
        file(APPEND ${CONFIG_FILE} "\n")
    endif()
...
function(atom_create_component_metadata)
    ...
    file(APPEND ${ATOM_COMPONENT_METADATA_FILE} "    \"build_type\": \"${CMAKE_BUILD_TYPE}\",\n")
    file(APPEND ${ATOM_COMPONENT_METADATA_FILE} "    \"compiler\": \"${CMAKE_CXX_COMPILER_ID}\",\n")
    file(APPEND ${ATOM_COMPONENT_METADATA_FILE} "    \"platform\": \"${CMAKE_SYSTEM_NAME}\",\n")
