function(update_vcpkg_baseline)
  ...
  set(DEFAULT_VCPKG_BASELINE "YOUR_DEFAULT_BASELINE_SHA_HERE") # Replace with a
                                                               # known good
                                                               # commit SHA

  message(
    STATUS
      "Attempting to download latest vcpkg commit info from ${VCPKG_COMMIT_API_URL}"
  )

  # Download the latest commit information
  file(
    DOWNLOAD "${VCPKG_COMMIT_API_URL}" "${TEMP_FILE}"
    TIMEOUT 10 # seconds
  ...
  if(NOT DOWNLOAD_EC EQUAL 0)
    message(
      WARNING
        "Failed to download vcpkg commit info: ${DOWNLOAD_MSG}. Log: ${DOWNLOAD_LOG}"
    )
    ...
    # Extract the SHA (baseline) using regex The response is a JSON array, we're
    # interested in the "sha" of the first commit object. A more robust solution
    # might involve a JSON parsing library if available/desired. This regex
    # looks for the first "sha": "commit_hash"
    string(REGEX MATCH "\"sha\"[ \t\n\r]*:[ \t\n\r]*\"([a-f0-9]+)\""
                 LATEST_BASELINE_MATCH "${GITHUB_API_RESPONSE}")

    if(NOT LATEST_BASELINE_MATCH)
      message(
        WARNING "Could not parse latest baseline SHA from GitHub API response.")
      ...
      message(
        STATUS
          "Successfully fetched latest vcpkg baseline (commit SHA): ${LATEST_BASELINE}"
      )
    endif()
  endif()
  ...
  if(NOT EXISTS "${VCPKG_JSON_PATH}")
    message(
      WARNING "vcpkg.json not found at ${VCPKG_JSON_PATH}. Skipping update.")
    return()
  endif()
  ...
  if(VCPKG_JSON_CONTENT MATCHES "\"builtin-baseline\"")
    string(
      ...
      REPLACE "\"builtin-baseline\"[ \t\n\r]*:[ \t\n\r]*\"[^\"]*\""
              "\"builtin-baseline\": \"${LATEST_BASELINE}\"" VCPKG_JSON_UPDATED
              "${VCPKG_JSON_CONTENT}")
  else()
    # Add "builtin-baseline" after the first opening curly brace This assumes a
    # simple JSON structure; for complex JSON, a proper parser would be better.
    string(REGEX
  ...
  if(USE_DEFAULT_BASELINE)
    message(
      STATUS
        "Updated ${VCPKG_JSON_PATH} with default baseline: ${LATEST_BASELINE}")
  else()
    message(
      STATUS
        "Updated ${VCPKG_JSON_PATH} with latest baseline: ${LATEST_BASELINE}")
  endif()
endfunction()
