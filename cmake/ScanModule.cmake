...
function(atom_process_module_dependencies)
  ...
  foreach(module ${enabled_modules})
    ...
    if(MODULE_EXISTS)
      ...
      message(
        STATUS
          "Warning: Module '${module}' is enabled but its directory is missing or invalid. Disabling it."
      )
      ...
      if(DEFINED ATOM_${module_var}_DEPENDS)
        foreach(dep ${ATOM_${module_var}_DEPENDS})
          ...
          if(NOT "${dep}" IN_LIST enabled_modules AND DEP_EXISTS)
            ...
            set(process_again TRUE)
            message(
              STATUS "Adding dependency '${dep}' required by module '${module}'"
            )
          elseif(NOT DEP_EXISTS)
            string(REPLACE "atom-" "" dep_name "${dep}")
            message(
              WARNING
                "Module '${module}' depends on '${dep}', but the dependency directory is missing or invalid."
            )
            message(
              STATUS
                "Consider implementing the '${dep_name}' module or removing the dependency."
            )
          endif()
        endforeach()
      endif()
  ...
  if(iteration EQUAL max_iterations)
    message(
      WARNING
        "Reached maximum dependency resolution iterations. There may be circular dependencies."
    )
  endif()
  ...
  if(added_modules)
    message(
      STATUS "Additional modules enabled due to dependencies: ${added_modules}")
  endif()
