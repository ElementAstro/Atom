    ...
    if(GIT_REPO_CHECK EQUAL 0)
      ...
      if(GIT_TAG_RESULT EQUAL 0)
        ...
        if(VERSION_MATCH)
          ...
          set(VERSION_STRING
              "${VERSION_MAJOR}.${VERSION_MINOR}.${VERSION_PATCH}${GIT_DIRTY}")
        else()
      ...
      if(EXISTS "${VERSION_TEMPLATE_PATH}")
        ...
        message(
          STATUS
            "Generated version header file from template: ${VERSION_TEMPLATE_PATH} -> ${ARG_OUTPUT_HEADER}"
        )
      else()
        message(
          WARNING "Version template file not found: ${VERSION_TEMPLATE_PATH}")

        # Fall back to built-in template
        message(
          STATUS
            "Creating a default version header file at: ${ARG_OUTPUT_HEADER}")
        configure_file("${CMAKE_CURRENT_FUNCTION_LIST_DIR}/version.h.in"
                       "${ARG_OUTPUT_HEADER}" @ONLY)
      endif()

      message(
        STATUS
          "Git version: ${VERSION_STRING} (Major: ${VERSION_MAJOR}, Minor: ${VERSION_MINOR}, Patch: ${VERSION_PATCH}, Hash: ${GIT_HASH})"
      )
    else()
      message(
        WARNING
...
function(atom_configure_version)
  ...
  message(
    STATUS "Generated atom_version_info.h with version ${PROJECT_VERSION}")
endfunction()
