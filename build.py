...
class SystemCapabilities:
    ...
    def _detect_capabilities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            capabilities["memory_available_gb"] = memory.available / (1024**3)
        ...
        tools = ["cmake", "ninja", "xmake", "ccache", "doxygen", "git", "clang", "gcc"]
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = {executor.submit(shutil.which, tool): tool for tool in tools}
...
class BuildSystem:
    ...
    def __init__(self, console: Console):
        ...
        self.config_manager = ConfigManager(self.project_root / "build-config.yaml")
    ...
    def _optimize_parallel_jobs(self, requested_jobs: Optional[int] = None) -> int:
        ...
        logger.info(
            f"Memory per job: {base_memory_per_job}GB, Platform: {caps['platform']}"
        )
    ...
    def _cmake_build(self, args: argparse.Namespace) -> bool:
        ...
        if (
            not hasattr(args, "unity_build")
            and self.system_caps.capabilities["memory_gb"] >= 8
        ):
        ...
        logger.success(f"CMake configuration completed in {configure_time:.1f}s")
        ...
        build_args = [
            ...
            str(self.build_dir),
            "--config",
            build_type_map[args.build_type],
            "--parallel",
            str(parallel_jobs),
        ]
    ...
    def _xmake_build(self, args: argparse.Namespace) -> bool:
        ...
        configure_time = time.perf_counter() - configure_start
        logger.success(f"XMake configuration completed in {configure_time:.1f}s")
        ...
        if args.docs:
            ...
            if self.system_caps.capabilities.get("has_doxygen", False):
                ...
                if success:
                    ...
                    logger.success(f"Documentation generated in {doc_time:.1f}s")
            else:
    ...
    def _show_build_summary(self, args: argparse.Namespace, build_time: float):
        ...
        if enabled_features:
            summary_table.add_row("Enabled Features", ", ".join(enabled_features))
        else:
...
Examples:
  ...
  python build.py --cmake --lto --parallel 8     # CMake build with LTO using 8 jobs
    ...
    build_group.add_argument(
        ...
        action="store_const",
        const="minsizerel",
        help="Minimum size release",
    )
    parser.set_defaults(build_type="release")

    # Features
    parser.add_argument("--python", action="store_true", help="Build Python bindings")
    ...
    parser.add_argument("--cfitsio", action="store_true", help="Enable CFITSIO support")
    parser.add_argument("--ssh", action="store_true", help="Enable SSH support")

    # Optimization
    parser.add_argument(
        "--lto", action="store_true", help="Enable Link Time Optimization"
    )
    ...
    parser.add_argument(
        "--precompiled-headers", action="store_true", help="Enable precompiled headers"
    )
    parser.add_argument(
        "--native-optimization",
        action="store_true",
    ...
    parser.add_argument(
        "--no-test", action="store_true", help="Skip running tests after build"
    )

    # Presets
    parser.add_argument(
        "--preset",
        help="Use a build preset (debug, release, dev, python, minimal, full)",
    )
    parser.add_argument(
...
def main():
    ...
    try:
        ...
        if args.list_presets:
            ...
            if presets:
                preset_table = Table(title="Available Build Presets", box=box.ROUNDED)
