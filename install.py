...
class DependencyInstaller:
    ...
    def _install_windows_build_tools(self) -> bool:
        ...
        if not any(Path(p).exists() for p in vs_paths):
            self.log(
                "Visual Studio Build Tools not found. Please install manually.", "red"
            )
            return False
    ...
    def setup_development_environment(self) -> bool:
        ...
        if not hasattr(sys, "real_prefix") and not (
            ...
            success, _ = self.run_command(
                [sys.executable, "-m", "venv", "venv"], check=False
            )
            if success:
                self.log(
...
def main():
    ...
    parser.add_argument(
        "--build-only", action="store_true", help="Only install build dependencies"
    )
    parser.add_argument(
        "--skip-system", action="store_true", help="Skip system package installation"
    )
    ...
    if console:
        ...
        info_table.add_row(
            "Package Manager", platform_detector.package_manager or "None detected"
        )
    ...
    if success:
        ...
        installer.log(
            "Installation completed with warnings. Check the output above.", "yellow"
        )
