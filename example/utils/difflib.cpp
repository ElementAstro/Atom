...
void printSequences(const std::vector<std::string>& seq1,
...
void printOpcodes(
    const std::vector<std::tuple<std::string, int, int, int, int>>& opcodes) {
    std::cout << "Opcodes:" << std::endl;
    for (const auto& [tag, i1, i2, j1, j2] : opcodes) {
        std::cout << "  " << std::left << std::setw(8) << tag << " a[" << i1
                  << ":" << i2 << "] b[" << j1 << ":" << j2 << "]" << std::endl;
    }
    std::cout << std::endl;
}
...
int main() {
    try {
        ...
        std::vector<std::string> lines1 = {
            ...
            "Line 4: This line will be removed.", "Line 5: The end."};

        std::vector<std::string> lines2 = {
            "Line 1: This is a test.",
            "Line 2: The quick brown fox jumps over the lazy cat.",  // Changed
                                                                     // dog ->
                                                                     // cat
        ...
        if (html_table_result) {
            ...
            std::cout << "HTML table size: " << html_table_result->size()
                      << " bytes" << std::endl;

            // Show first 200 characters
            std::cout << "Preview:" << std::endl;
            std::cout << html_table_result->substr(0, 200) << "..."
                      << std::endl;
            ...
            std::cerr << "Failed to generate HTML table: "
                      << html_table_result.error() << std::endl;
        }
        ...
        if (html_file_result) {
            ...
            std::cout << "HTML file size: " << html_file_result->size()
                      << " bytes" << std::endl;
            ...
            std::cerr << "Failed to generate HTML file: "
                      << html_file_result.error() << std::endl;
        }
        ...
        std::vector<std::string> test_words = {"aple", "strberry", "lemen",
                                               "banna", "grap"};

        for (const auto& test_word : test_words) {
            std::cout << "Finding close matches for \"" << test_word
                      << "\":" << std::endl;
            ...
            auto matches2 =
                atom::utils::getCloseMatches(test_word, words, 1, 0.7);
            ...
            auto matches3 =
                atom::utils::getCloseMatches(test_word, words, 5, 0.4);
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            line1 << "Line " << i << ": This is test line number " << i
            ...
            if (i % 10 == 0) {
                ...
                line2 << "Line " << i << ": This is an EXTRA line for test "
                      << i << ".";
                large_text2.push_back(line2.str());
                line2.str("");
                line2 << "Line " << i << ": This is test line number " << i
                      << " in the second document.";
                large_text2.push_back(line2.str());
            } else if (i % 7 == 0) {
                // Modify the line (replacement)
                line2 << "Line " << i << ": This is MODIFIED test line " << i
                      << " in the second document.";
                large_text2.push_back(line2.str());
            } else {
        ...
        std::cout << "  Text 1: " << large_text1.size() << " lines"
                  << std::endl;
        std::cout << "  Text 2: " << large_text2.size() << " lines"
                  << std::endl;
        ...
        auto large_diffs =
            atom::utils::Differ::compare(large_text1, large_text2);
        ...
        std::cout << "  Generated diff with " << large_diffs.size() << " lines"
                  << std::endl;
        std::cout << "  Time taken: " << duration << " ms" << std::endl;

        // 3. HtmlDiff::makeTable
        std::cout << "\nTesting HtmlDiff::makeTable performance..."
                  << std::endl;
        start_time = std::chrono::high_resolution_clock::now();

        auto large_html_table =
            atom::utils::HtmlDiff::makeTable(large_text1, large_text2);
        ...
        if (large_html_table) {
            std::cout << "  Generated HTML table with "
                      << large_html_table->size() << " bytes" << std::endl;
        } else {
            std::cout << "  Failed to generate HTML table: "
                      << large_html_table.error() << std::endl;
        }
        std::cout << "  Time taken: " << duration << " ms" << std::endl;
        ...
        std::cout << "  Similarity ratio: " << empty_matcher.ratio()
                  << std::endl;
        ...
        std::cout << "  Found " << empty_matches.size() << " matches"
                  << std::endl;
        ...
        std::vector<std::string> dictionary = {
            "algorithm", "application", "binary",      "compiler",
            "computer",  "database",    "development", "encryption",
            "function",  "hardware",    "interface",   "iteration",
            "keyboard",  "language",    "memory",      "network",
            "operating", "processor",   "programming", "recursive",
            "software",  "storage",     "structure",   "system",
            "variable"};
        ...
        for (const auto& word : misspelled_words) {
            ...
            auto suggestions =
                atom::utils::getCloseMatches(word, dictionary, 3, 0.6);
