...
int main() {
    try {
        ...
        std::cout << "Vector contains 3: "
                  << (atom::utils::contains(vec1, 3) ? "Yes" : "No")
                  << std::endl;
        std::cout << "Vector contains 8: "
                  << (atom::utils::contains(vec1, 8) ? "Yes" : "No")
                  << std::endl;

        // Test conversion to unordered_set
        std::cout << "\nToUnorderedSet demonstration:" << std::endl;
        auto vec1AsSet = atom::utils::toUnorderedSet(vec1);
        std::cout << "Vector as unordered_set - size: " << vec1AsSet.size()
                  << std::endl;
        std::cout << "Checking membership in unordered_set:" << std::endl;
        std::cout << "Contains 3: " << (vec1AsSet.contains(3) ? "Yes" : "No")
                  << std::endl;
        std::cout << "Contains 8: " << (vec1AsSet.contains(8) ? "Yes" : "No")
                  << std::endl;
        ...
        std::cout << "Is list a subset of vector (isSubset): "
                  << (atom::utils::isSubset(list1, vec1) ? "Yes" : "No")
                  << std::endl;
        std::cout << "Is list a subset of vector (linearSearch): "
                  << (atom::utils::isSubsetLinearSearch(list1, vec1) ? "Yes"
                                                                     : "No")
                  << std::endl;
        std::cout << "Is list a subset of vector (hashSet): "
                  << (atom::utils::isSubsetWithHashSet(list1, vec1) ? "Yes"
                                                                    : "No")
                  << std::endl;
        ...
        std::cout << "Is list2 a subset of vector: "
                  << (atom::utils::isSubset(list2, vec1) ? "Yes" : "No")
                  << std::endl;
        ...
        std::set<int> setEqual1 = {3, 2,
                                   1};  // Different order but same elements
        ...
        std::cout << "Vector equals List: "
                  << (atom::utils::isEqual(vecEqual1, listEqual1) ? "Yes"
                                                                  : "No")
                  << std::endl;
        std::cout << "Vector equals Set: "
                  << (atom::utils::isEqual(vecEqual1, setEqual1) ? "Yes" : "No")
                  << std::endl;
        ...
        std::vector<Person> people = {Person("Alice", 30, "New York"),
        ...
        std::cout << "\nTransforming containers using member functions:"
                  << std::endl;
        ...
        auto namesByApply =
            atom::utils::applyAndStore(people, &Person::getName);
        ...
        std::vector<std::string> duplicateStrings = {
            "apple", "banana", "apple", "cherry", "banana", "date"};
        std::map<std::string, int> duplicateMap = {
            {"a", 1}, {"b", 2}, {"a", 3}, {"c", 4}};
        ...
        // Zip containers
        std::cout << "\nZip operation (combines corresponding elements):"
                  << std::endl;
        auto zipped = atom::utils::zip(letters, numbers);
        printPairs("Zipped pairs", zipped);
        std::cout << "Note: Zip stops at the end of the shortest container"
                  << std::endl;
        ...
        std::cout << "\nCartesian product (all possible combinations):"
                  << std::endl;
        auto product =
            atom::utils::cartesianProduct(letters, std::vector<int>{1, 2});
        ...
        auto [large, small] =
            atom::utils::partition(mixedNumbers, isGreaterThan5);
        ...
        std::vector<Person> employees = {Person("John", 42, "Seattle"),
        ...
        auto youngEmployee = atom::utils::findIf(
            employees, [](const Person& p) { return p.getAge() < 30; });
        ...
        auto seattleEmployee = atom::utils::findIf(
            employees,
            [](const Person& p) { return p.getCity() == "Seattle"; });

        if (seattleEmployee) {
            std::cout << "Found Seattle employee: "
                      << seattleEmployee->getName()
                      << ", Age: " << seattleEmployee->getAge() << std::endl;
        } else {
            std::cout << "No Seattle employee found" << std::endl;
        }

        auto oldEmployee = atom::utils::findIf(
            employees, [](const Person& p) { return p.getAge() > 50; });
