...
void demonstrateBasicErrorStack() {
    ...
    errorStack->insertError("File not found", "FileModule", "openFile", 42,
                            "file.cpp");
    errorStack->insertError("Permission denied", "FileModule", "writeFile", 78,
                            "file.cpp");
    errorStack->insertError("Connection timeout", "NetworkModule", "connect",
                            123, "network.cpp");
...
void demonstrateErrorFiltering() {
    ...
    for (int i = 0; i < 5; ++i) {
        errorStack->insertError("Error " + std::to_string(i), "ModuleA",
                                "funcA", i * 10, "a.cpp");
        errorStack->insertError("Error " + std::to_string(i), "ModuleB",
                                "funcB", i * 10, "b.cpp");
    }
    ...
    for (const auto& err : filteredA) {
        std::cout << "  " << err.moduleName << ": " << err.errorMessage
                  << std::endl;
    }

    std::cout << "\n--- Get latest error ---" << std::endl;
    if (auto latest = errorStack->getLatestError()) {
        std::cout << "  " << latest->errorMessage << " in "
                  << latest->moduleName << std::endl;
    }
}
...
void demonstrateRealWorldScenario() {
    ...
    std::cout << "Warnings: "
              << errorStack->filterByLevel(ErrorLevel::Warning).size()
              << std::endl;
    std::cout << "Errors: "
              << errorStack->filterByLevel(ErrorLevel::Error).size()
              << std::endl;
}
