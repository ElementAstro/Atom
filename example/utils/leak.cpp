...
public:
    LeakyClass(int size) {
        ...
        data = new int[size];
        std::cout << "Allocated int array with " << size << " elements at "
                  << data << std::endl;

        buffer = new char[1024];
        std::cout << "Allocated char buffer of 1024 bytes at "
                  << static_cast<void*>(buffer) << std::endl;

        vector_data = new std::vector<double>(size, 0.0);
        std::cout << "Allocated vector with " << size << " elements at "
                  << vector_data << std::endl;
    }
    ...
    void cleanupIncomplete() {
        std::cout << "Performing incomplete cleanup (will cause leaks)"
                  << std::endl;
    ...
    void noCleanup() {
        std::cout << "No cleanup performed (will cause all resources to leak)"
                  << std::endl;
    ...
    ~LeakyClass() {
        ...
        std::cout << "~LeakyClass destructor called (without proper cleanup)"
                  << std::endl;
    }
};
...
void demonstrateSimpleLeak() {
    ...
    std::cout << "Allocated array at " << leakedArray << " but didn't free it"
              << std::endl;
...
void demonstrateSmartPointers() {
    ...
    // Using unique_ptr for automatic cleanup
    {
        ...
        std::cout << "Created array with unique_ptr at " << uniqueArray.get()
                  << std::endl;
        ...
        std::cout << "unique_ptr will automatically free memory when going out "
                     "of scope"
                  << std::endl;
    ...
    // Using shared_ptr for shared ownership
    {
        ...
        std::cout << "Created vector with shared_ptr at " << sharedVector.get()
                  << std::endl;

        // Create another shared pointer to the same data
        std::shared_ptr<std::vector<double>> anotherReference = sharedVector;
        std::cout << "Created second reference, use count: "
                  << sharedVector.use_count() << std::endl;
    ...
    for (int i = 0; i < 3; i++) {
        threads.emplace_back([i]() {
            ...
            std::cout << "Thread " << i << " allocated " << 512 * (i + 1)
                      << " bytes at " << static_cast<void*>(threadBuffer)
                      << std::endl;
            ...
            if (i % 2 == 0) {
                ...
                std::cout << "Thread " << i << " is leaking its memory"
                          << std::endl;
            }
...
void demonstrateContainerLeaks() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        std::cout << "Added pointer to value " << *ptr << " at " << ptr
                  << std::endl;
    }
...
public:
    RAIIExample(int size) : resource(new int[size]) {
        std::cout << "RAII class allocated resource at " << resource
                  << std::endl;
    }

    ~RAIIExample() {
        std::cout << "RAII class automatically freeing resource at " << resource
                  << std::endl;
        delete[] resource;
    }
};
...
void demonstrateRAII() {
    std::cout << "\n=== Demonstrating RAII (Resource Acquisition Is "
                 "Initialization) ==="
              << std::endl;
    ...
    std::cout << "RAII object went out of scope, resource was freed"
              << std::endl;
}
...
int main() {
    ...
    std::cout << "===============================================" << std::endl;
    std::cout
        << "This example demonstrates how to use the leak detection utility"
        << std::endl;
    std::cout << "Note: Visual Leak Detector will report leaks at program exit"
              << std::endl;
    std::cout << "===============================================\n"
              << std::endl;
    ...
    {
        std::cout << "\n=== Demonstrating Different Cleanup Strategies ==="
                  << std::endl;
    ...
    std::cout << "1. Always use smart pointers (std::unique_ptr, "
                 "std::shared_ptr) when possible"
              << std::endl;
    std::cout << "2. Implement RAII pattern in your classes" << std::endl;
    std::cout << "3. Avoid manual memory management with new/delete"
              << std::endl;
    std::cout << "4. Use containers and algorithms from the standard library"
              << std::endl;
    std::cout << "5. Set clear ownership rules for resources" << std::endl;
    std::cout << "6. Run with memory leak detection tools regularly"
              << std::endl;
