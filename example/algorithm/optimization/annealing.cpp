...
// Define progress callback functionvoid progressCallback(int iteration, double
// energy,
                      ...
                      // Define stop condition functionbool stopCondition(int
                      // iteration, double energy,
                   const std::vector<int>& solution [[maybe_unused]]) {
                       // Stop when energy is below threshold or iterations
                       // exceed 5000
                       return energy < 10.0 || iteration > 5000;
                   }

                   // Helper function to display the solutionvoid
                   // printSolution(const std::vector<int>& solution,
                   ...
                   int main() {
                       ...
                       std::vector<std::pair<double, double>> cities = {
                           ...
                           {2.0, 2.0}, {2.5, 1.0}, {3.0, 3.0}, {0.5, 2.5}};

                       // Create TSP problem instance
                       TSP tspProblem(cities);

                       // Use Builder pattern to create SimulatedAnnealing
                       // object
                       SimulatedAnnealing<TSP, std::vector<int>>::Builder
                           builder(tspProblem);
                       builder
                       ...
                       SimulatedAnnealing<TSP, std::vector<int>> sa =
                           builder.build();
                       ...
                       std::cout << "=== Simulated Annealing with Exponential "
                                    "Cooling ==="
                                 << std::endl;
                       ...
                       std::cout << "Best path length: " << std::fixed
                                 << std::setprecision(6) << bestEnergy
                                 << std::endl;
                       std::cout << "Optimization time: " << elapsed.count()
                                 << " seconds" << std::endl;

                       // Demonstrate different cooling strategies
                       std::cout << "\n=== Simulated Annealing with Linear "
                                    "Cooling ==="
                                 << std::endl;
                       ...
                       std::cout
                           << "Linear cooling best path length: " << bestEnergy
                           << std::endl;

                       std::cout << "\n=== Simulated Annealing with Adaptive "
                                    "Cooling ==="
                                 << std::endl;
                       sa.setCoolingSchedule(AnnealingStrategy::ADAPTIVE);
                       bestSolution = sa.optimize(2);
                       bestEnergy = sa.getBestEnergy();
                       std::cout << "Adaptive cooling best path length: "
                                 << bestEnergy << std::endl;

                       // Show how to modify cooling rate
                       std::cout << "\n=== Simulated Annealing with Custom "
                                    "Parameters ==="
                                 << std::endl;
                       sa.setCoolingRate(0.99);  // Slower cooling
                       sa.setInitialTemperature(
                           500.0);  // Lower initial temperature
                       bestSolution = sa.optimize(1);
                       bestEnergy = sa.getBestEnergy();
                       std::cout << "Custom parameters best path length: "
                                 << bestEnergy << std::endl;
