...
void demonstrateHashConsistency() {
    ...
    try {
        ...
        // Test hash collision resistance (different inputs should produce
        // different hashes)
...
void demonstrateBinaryDataHashing() {
    ...
    try {
        ...
        std::string hash1 =
            MD5::encrypt(std::string(pattern1.begin(), pattern1.end()));
        std::string hash2 =
            MD5::encrypt(std::string(pattern2.begin(), pattern2.end()));
        std::string hash3 =
            MD5::encrypt(std::string(pattern3.begin(), pattern3.end()));
...
void demonstratePracticalUseCases() {
    ...
    try {
        // Use case 1: Password verification (demonstration only - MD5 is not
        ...
        std::cout << "   Match: "
                  << (storedHash == inputHash ? "✓ YES" : "✗ NO") << "\n";
        std::cout << "   ⚠️  Note: MD5 is NOT secure for password hashing in "
                     "production!\n\n";
        ...
        std::cout << "   Data integrity: "
                  << (originalHash == receivedHash ? "✓ INTACT" : "✗ CORRUPTED")
                  << "\n\n";

        // Use case 3: Simple checksums for non-cryptographic purposes
        std::cout << "3. Simple Checksum Generation:\n";
        std::vector<std::string> dataItems = {"Item 1: Configuration data",
...
void demonstratePerformanceAndLimitations() {
    ...
    try {
        ...
        // Security warnings
        std::cout << "\n⚠️  SECURITY WARNINGS:\n";
        std::cout << "   • MD5 is cryptographically broken and unsuitable for "
                     "security purposes\n";
        std::cout << "   • MD5 is vulnerable to collision attacks\n";
        std::cout
            << "   • Use SHA-256 or better for cryptographic applications\n";
        std::cout
            << "   • MD5 is acceptable for non-cryptographic checksums only\n";
        ...
        std::cout << "   • Avalanche effect: small input changes cause large "
                     "output changes\n";
