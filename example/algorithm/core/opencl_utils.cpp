...
std::cout << "Creating context with " << devices.size() << " device(s)...\n";
...
// Demonstrate buffer operationsvoid demonstrateBufferOperations() {
...
std::cout << "Creating write-only buffer (" << BUFFER_SIZE << " bytes)...\n";
auto write_buffer =
    Platform::createBuffer(context, MemoryFlags::WRITE_ONLY, BUFFER_SIZE);

std::cout << "Creating read-write buffer (" << BUFFER_SIZE << " bytes)...\n";
auto rw_buffer =
...
// Demonstrate kernel compilation and executionvoid demonstrateKernelExecution()
// {
...
// Demonstrate ComputeManager singletonvoid demonstrateComputeManager() {
...
if (initialized) {
    ...
    std::cout << "OpenCL available: " << (manager.isAvailable() ? "Yes" : "No")
              << "\n";
