...
// Helper function to map noise value to ASCII characterchar noiseToChar(f64
// value) { Normalize from [-1, 1] to [0, 1]
...
// Demonstrate basic 2D simplex noisevoid demonstrateSimplex2D() {
std::cout << "\n=== 2D Simplex Noise ===\n";
...
// Demonstrate 3D simplex noisevoid demonstrateSimplex3D() {
std::cout << "\n=== 3D Simplex Noise ===\n";
...
// Demonstrate different seedsvoid demonstrateDifferentSeeds() {
...
// Benchmark simplex noise performancevoid benchmarkSimplexNoise() {
...
std::cout << "2D noise: " << ITERATIONS << " calls in " << duration2d.count()
          << " us (" << (ITERATIONS * 1000000.0 / duration2d.count())
          << " calls/sec)\n";
...
std::cout << "3D noise: " << ITERATIONS << " calls in " << duration3d.count()
          << " us (" << (ITERATIONS * 1000000.0 / duration3d.count())
          << " calls/sec)\n";
...
std::cout << "Octave noise (4 octaves): " << (ITERATIONS / 10) << " calls in "
          << duration_octave.count() << " us ("
...
// Generate terrain-like noise mapvoid demonstrateTerrainGeneration() {
