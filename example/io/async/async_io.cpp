...
void demonstrateBasicAsyncOperations() {
    ...
    AsyncFile fileManager(ioContext, context);
#else
    AsyncFile fileManager(context);
#endif

    const bool operationsSucceeded = [&]() {
        try {
            // Asynchronous write operation
            std::cout << "1. Writing data to file asynchronously..."
                      << std::endl;
            std::promise<AsyncResult<void>> writePromise;
            fileManager.asyncWrite(
                filename,
                std::span<const char>(data_to_write.data(),
                                      data_to_write.size()),
            ...
            fileManager.asyncRead(filename,
