...
// cache,
                    ...
                    int main() {
                        ...
                        atom::search::ThreadSafeLRUCache<int, UserProfile>
                            batchCache(20);
                        std::cout << "Created a cache for batch operations "
                                     "with size: 20"
                                  << std::endl;

                        // Prepare batch of users
                        std::cout << "\nPreparing a batch of users..."
                                  << std::endl;
                        ...
                        // Insert batch
                        std::cout << "Inserting batch of " << userBatch.size()
                                  << " users..." << std::endl;
                        batchCache.putBatch(userBatch,
                                            std::chrono::minutes(10));
                        std::cout << "Batch inserted, cache size: "
                                  << batchCache.size() << std::endl;

                        // Prepare batch of keys to retrieve
                        std::cout
                            << "\nPreparing a batch of keys to retrieve..."
                            << std::endl;
                        std::vector<int> keyBatch = {
                            101, 103, 105, 107, 109, 200};  // 200 doesn't exist

                        // Get batch
                        std::cout << "Retrieving batch of " << keyBatch.size()
                                  << " users..." << std::endl;
                        ...
                        for (size_t i = 0; i < keyBatch.size(); ++i) {
                            if (resultBatch[i]) {
                                std::cout
                                    << "  Key " << keyBatch[i] << ": Found - "
                                    << resultBatch[i]->toString() << std::endl;
                            } else {
                                std::cout << "  Key " << keyBatch[i]
                                          << ": Not found" << std::endl;
                            }
                        }
                        ...
                        std::cout << "Total keys: " << allKeys.size()
                                  << std::endl;
                        std::cout << "First few keys: ";
                        for (size_t i = 0;
                             i < std::min(allKeys.size(), size_t(5)); ++i) {
                            std::cout << allKeys[i] << " ";
                        }
                        std::cout << std::endl;
                        ...
                        atom::search::ThreadSafeLRUCache<int, UserProfile>
                            concurrentCache(50);
                        std::cout << "Created a cache for concurrent access "
                                     "with size: 50"
                                  << std::endl;

                        // Set up multiple threads to operate on the cache
                        // concurrently
                        std::cout << "\nStarting multiple threads to access "
                        ...
                        std::cout
                            << "Final cache size: " << concurrentCache.size()
                            << std::endl;
                        std::cout << "Cache statistics: " << std::endl;
                        auto stats = concurrentCache.getStatistics();
                        std::cout << "  Hit count: " << stats.hitCount
                                  << std::endl;
                        std::cout << "  Miss count: " << stats.missCount
                                  << std::endl;
                        std::cout << "  Hit rate: " << (stats.hitRate * 100)
                                  << "%" << std::endl;
                        std::cout << "  Current size: " << stats.size
                                  << std::endl;
                        std::cout << "  Max size: " << stats.maxSize
                                  << std::endl;
                        ...
                        atom::search::ThreadSafeLRUCache<int, UserProfile>
                            callbackCache(10);
                        std::cout << "Created a cache with callbacks, size: 10"
                                  << std::endl;

                        // Set insert callback
                        callbackCache.setInsertCallback(
                            [](const int& key, const UserProfile& value) {
                                std::cout << "Insert callback: User " << key
                        ...
                        callbackCache.setEraseCallback([](const int& key) {
                            std::cout << "Erase callback: User " << key
                                      << " removed" << std::endl;
                        });
                        ...
                        callbackCache.setClearCallback([]() {
                            std::cout
                                << "Clear callback: Cache has been cleared"
                                << std::endl;
                        });
                        ...
                        atom::search::ThreadSafeLRUCache<int, UserProfile>
                            resizeCache(5);
                        std::cout << "Created a cache with initial size: 5"
                                  << std::endl;
                        ...
                        std::cout << "Cache size after filling: "
                                  << resizeCache.size() << std::endl;

                        // Resize the cache to a smaller size
                        std::cout << "\nResizing cache to a smaller size (3)..."
                                  << std::endl;
                        resizeCache.resize(3);
                        std::cout
                            << "Cache size after resize: " << resizeCache.size()
                            << std::endl;
                        std::cout << "Cache max size after resize: "
                                  << resizeCache.maxSize() << std::endl;

                        // Check which users remain
                        std::cout << "Checking which users remain:"
                                  << std::endl;
                        for (int i = 301; i <= 305; ++i) {
                            std::cout
                                << "  Contains user " << i << ": "
                                << (resizeCache.contains(i) ? "Yes" : "No")
                                << std::endl;
                        }

                        // Resize to a larger size
                        std::cout << "\nResizing cache to a larger size (10)..."
                                  << std::endl;
                        ...
                        std::cout
                            << "\nAdding more users to fill expanded space..."
                            << std::endl;
                        for (int i = 306; i <= 310; ++i) {
                            resizeCache.put(i, createSampleUser(i));
                        }
                        std::cout << "Cache size after adding more users: "
                                  << resizeCache.size() << std::endl;
                        ...
                        atom::search::ThreadSafeLRUCache<int, UserProfile>
                            persistentCache(10);
                        std::cout << "Created a cache for persistence "
                                     "demonstration, size: 10"
                                  << std::endl;

                        // Fill with some data
                        std::cout << "\nFilling cache with data to be saved..."
                                  << std::endl;
                        for (int i = 401; i <= 405; ++i) {
                            ...
                            if (i % 2 == 0) {
                                ...
                                std::cout << "  Added user " << i
                                          << " with 30-minute TTL" << std::endl;
                            } else {
                                persistentCache.put(i, createSampleUser(i));
                                std::cout << "  Added user " << i
                                          << " with no TTL" << std::endl;
                            }
                        }

                        // Save the cache to a file
                        const std::string cacheFile = "lru_cache_data.bin";
                        std::cout << "\nSaving cache to file: " << cacheFile
                                  << std::endl;
                        try {
                            persistentCache.saveToFile(cacheFile);
                            std::cout << "Cache successfully saved"
                                      << std::endl;
                        } catch (const atom::search::LRUCacheException& e) {
                            std::cout << "Failed to save cache: " << e.what()
                                      << std::endl;
                        }
                        ...
                        atom::search::ThreadSafeLRUCache<int, UserProfile>
                            loadedCache(20);
                        std::cout << "\nCreated a new cache with size 20"
                                  << std::endl;
                        std::cout << "Loading cache from file: " << cacheFile
                                  << std::endl;

                        try {
                            loadedCache.loadFromFile(cacheFile);
                            std::cout << "Cache successfully loaded"
                                      << std::endl;
                        } catch (const atom::search::LRUCacheException& e) {
                            std::cout << "Failed to load cache: " << e.what()
                                      << std::endl;
                        }

                        // Verify the loaded data
                        std::cout << "\nVerifying loaded data..." << std::endl;
                        std::cout << "Loaded cache size: " << loadedCache.size()
                                  << std::endl;
                        for (int i = 401; i <= 405; ++i) {
                            ...
                            if (loadedUser) {
                                std::cout
                                    << "  User " << i
                                    << " found: " << loadedUser->toString()
                                    << std::endl;
                            } else {
                                std::cout << "  User " << i << " not found"
                                          << std::endl;
                            }
                        }
                        ...
                        atom::search::ThreadSafeLRUCache<int, UserProfile>
                            prefetchCache(20);
                        std::cout << "Created a cache for prefetch "
                                     "demonstration, size: 20"
                                  << std::endl;

                        // Initialize with a few items
                        std::cout << "\nInitializing with a few items..."
                                  << std::endl;
                        ...
                        std::cout << "\nPrefetching a batch of users..."
                                  << std::endl;
                        std::vector<int> prefetchKeys = {
                            501, 504, 505, 506,
                        ...
                        size_t prefetchedCount = prefetchCache.prefetch(
                            prefetchKeys,
                            simulateDatabaseLookup,  // Function to load missing
                                                     // items
                            std::chrono::minutes(5)  // TTL for prefetched items
                        );

                        std::cout << "Successfully prefetched "
                                  << prefetchedCount << " users" << std::endl;

                        // Verify the prefetched data
                        std::cout << "\nVerifying prefetched data..."
                                  << std::endl;
                        ...
                        std::cout << "\nCache statistics after prefetching:"
                                  << std::endl;
                        std::cout
                            << "  Hit rate: " << (prefetchStats.hitRate * 100)
                            << "%" << std::endl;
                        std::cout << "  Cache size: " << prefetchStats.size
                                  << std::endl;
                        ...
                        std::cout
                            << "Attempting to create a cache with size 0..."
                            << std::endl;
                        try {
                            atom::search::ThreadSafeLRUCache<int, UserProfile>
                                invalidCache(0);
                            std::cout << "Created cache (unexpected)"
                                      << std::endl;
                        } catch (const std::exception& e) {
                            std::cout
                                << "Exception caught (expected): " << e.what()
                                << std::endl;
                        }
                        ...
                        std::cout << "This example demonstrated the following "
                                     "ThreadSafeLRUCache "
                                     "features:"
                                  << std::endl;
                        std::cout << "  1. Basic cache operations (put, get, "
                                     "erase, contains)"
                                  << std::endl;
                        std::cout << "  2. TTL and expiration of cache entries"
                                  << std::endl;
                        std::cout << "  3. LRU eviction policy when the cache "
                                     "reaches capacity"
                                  << std::endl;
                        std::cout << "  4. Batch operations for efficient "
                                     "handling of multiple items"
                                  << std::endl;
                        std::cout << "  5. Thread safety and concurrent access "
                                     "from multiple threads"
                                  << std::endl;
                        std::cout
                            << "  6. Callbacks for monitoring cache events"
                            << std::endl;
                        std::cout << "  7. Dynamic resizing of the cache"
                                  << std::endl;
                        std::cout
                            << "  8. Persistence with save and load operations"
                            << std::endl;
                        std::cout << "  9. Prefetching to proactively populate "
                                     "the cache"
                                  << std::endl;
                        std::cout << "  10. Error handling and edge cases"
                                  << std::endl;
