...
void errorCallback(const std::string& errorMsg, unsigned int errorCode) {
...
int main() {
    try {
        ...
        std::cout << "Creating MySQL connection with parameters:" << std::endl;
        ...
        if (db.connect()) {
            ...
            std::cout << "Failed to connect to database. Using mock operations "
                         "for demonstration."
                      << std::endl;
            std::cout << "Note: Ensure MySQL server is running and credentials "
                         "are correct."
                      << std::endl;
        }
        ...
        try {
            ...
            std::cout << "Last insert ID: " << db.getLastInsertId()
                      << std::endl;
            std::cout << "Affected rows: " << db.getAffectedRows() << std::endl;

        } catch (const atom::search::database::MySQLException& e) {
            std::cout << "Prepared statement error (expected if table exists): "
                      << e.what() << std::endl;
        }
        ...
        try {
            ...
            db.executeQuery(
                "INSERT INTO search_index (document_id, content, tags) VALUES "
                "('doc1', 'Machine learning algorithms', 'ai,ml,algorithms')");
            db.executeQuery(
                "INSERT INTO search_index (document_id, content, tags) VALUES "
        ...
        auto users_result = db.executeQueryWithResults("SELECT * FROM users");
        ...
        std::cout << "Performing pattern search for posts containing 'Alice':"
                  << std::endl;
        ...
        if (db.ping()) {
            ...
            std::cout << "Database connection is not responding" << std::endl;
        }

        // Set connection timeout
        std::cout << "Setting connection timeout to 5 seconds..." << std::endl;
