...
// Helper function to print search resultsvoid printSearchResults(
    ...
    int main() {
        ...
        std::cout << "Adding documents with similar content but different "
                     "click counts..."
                  << std::endl;
        ...
        tryOperation("Add ranking test documents", [&searchEngine, &rankDoc1,
            ...
            std::cout << "Ranking test documents added successfully"
                      << std::endl;
        });
        ...
        std::cout << "\nSearching for 'information retrieval' to see ranking "
                     "changes..."
                  << std::endl;
        tryOperation("Search after clicks", [&searchEngine]() {
            auto results =
                searchEngine.searchByContent("information retrieval");
            printSearchResults(results);
            ...
            if (!results.empty()) {
                std::cout
                    ...
                    << std::endl;
                std::cout
                    << "Document 'rank2' (5 clicks) should be ranked higher "
                       "than 'rank3' (2 clicks),"
                    << std::endl;
                std::cout
                    << "which should be ranked higher than 'rank1' (0 clicks)"
                    << std::endl;
            }
        });
        ...
        tryOperation("Empty prefix autocomplete", [&searchEngine]() {
            ...
            std::cout << "Empty prefix autocomplete returned "
        ...
        tryOperation("Parallel content search", [&mtSearchEngine]() {
            ...
            if (!results.empty()) {
                std::cout << "\nShowing first " << limitedResults.size()
                          << " of " << results.size()
                          << " results:" << std::endl;
                ...
                if (results.size() > maxResultsToPrint) {
                    std::cout << "... and "
                              << (results.size() - maxResultsToPrint)
