...
public:
    explicit PositionComponent(const std::string& name) : Component(name) {
        ...
        def("setPosition", [this](double newX, double newY, double newZ) {
            ...
            setValue("y", newY);
            setValue("z", newZ);
            std::cout << "  Position set to (" << newX << ", " << newY << ", "
                      << newZ << ")" << std::endl;
        });

        def("getDistance",
            [this](double targetX, double targetY, double targetZ) -> double {
                auto x = getVariable<double>("x");
                auto y = getVariable<double>("y");
                auto z = getVariable<double>("z");
...
public:
    explicit HealthComponent(const std::string& name) : Component(name) {
        ...
        def("heal", [this](int amount) {
            ...
            if (health && maxHealth) {
                int newHealth =
                    std::min(maxHealth->get(), health->get() + amount);
                setValue("health", newHealth);
                std::cout << "  Healed " << amount
...
public:
    explicit AIComponent(const std::string& name) : Component(name) {
        ...
        def("update", [this]() {
            ...
            if (behavior) {
                std::cout << "  AI updating with behavior: " << behavior->get()
                          << std::endl;
...
public:
    ...
    void addComponent(const std::string& componentName) {
        ...
        auto component =
            registry.createComponent<T>(entityId_ + "_" + componentName);
        components_.push_back(component);
        std::cout << "  Added " << componentName << " to entity " << entityId_
                  << std::endl;
    }
    ...
    void update() {
        ...
        for (auto& comp : components_) {
            // Note: hasCommand and executeCommand are not available in
            // Component base class if (comp->hasCommand("update")) {
            //     comp->executeCommand("update", {});
            // }
            std::cout << "  [ENTITY] Updating component: " << comp->getName()
                      << std::endl;
        }
    }
        ...
        switch (event) {
            case 0:  // Random movement
                if (auto pos = entity->getComponent<PositionComponent>()) {
                    ...
                    // class pos->executeCommand("move",
                    // {std::to_string(moveDist(rng_)),
                    //                               std::to_string(moveDist(rng_)),
                    //                               "0"});
                    std::cout << "  [WORLD] Moving entity randomly"
                              << std::endl;
                }
                break;
            case 1:  // Random damage
                if (auto health = entity->getComponent<HealthComponent>()) {
                    ...
                    // Note: executeCommand is not available in Component base
                    // class health->executeCommand("takeDamage",
                    // {std::to_string(damageDist(rng_))});
            ...
            case 2:  // Random healing
                if (auto health = entity->getComponent<HealthComponent>()) {
                    ...
                    // Note: executeCommand is not available in Component base
                    // class health->executeCommand("heal",
                    // {std::to_string(healDist(rng_))});
                    std::cout << "  [WORLD] Entity heals" << std::endl;
                }
                break;
            case 3:  // Change AI behavior
                if (auto ai = entity->getComponent<AIComponent>()) {
                    std::vector<std::string> behaviors = {"idle", "patrol",
                                                          "chase", "attack"};
                    std::uniform_int_distribution<> behaviorDist(
                        0, behaviors.size() - 1);
                    // Note: executeCommand is not available in Component base
                    // class ai->executeCommand("setBehavior",
                    // {behaviors[behaviorDist(rng_)]});
...
int main() {
    std::cout << "=== Atom Component Game Entity System Example ==="
              << std::endl;
    ...
    try {
        ...
        if (auto pos = player->getComponent<PositionComponent>()) {
            ...
            std::cout << "  [DEMO] Player position set to (0, 0, 0)"
                      << std::endl;
        }
        ...
        if (auto pos = npc1->getComponent<PositionComponent>()) {
            ...
            std::cout << "  [DEMO] NPC1 position set to (10, 5, 0)"
                      << std::endl;
        }
        ...
        if (auto pos = npc2->getComponent<PositionComponent>()) {
            ...
            std::cout << "  [DEMO] NPC2 position set to (-5, 8, 0)"
                      << std::endl;
        }
        ...
        std::cout << "\n=== Game Entity System Example Complete ==="
                  << std::endl;
