...
public:
    explicit LifecycleComponent(const std::string& name) : Component(name) {
        std::cout << "  [CONSTRUCTOR] Component '" << name << "' created" << std::endl;

        // Set component documentation
        doc("Comprehensive lifecycle demonstration component with full API coverage");

        // Initialize component variables with different types
        addVariable<int>("counter", 0, "Counter for operations", "cnt", "metrics");
        addVariable<std::string>("status", "created", "Current component status", "state", "info");
        addVariable<bool>("initialized", false, "Initialization flag", "init", "flags");
        addVariable<double>("performance_score", 0.0, "Performance metric", "perf", "metrics");
        addVariable<std::vector<std::string>>("operation_history", std::vector<std::string>{},
                   "History of operations", "history", "logs");

        // Set variable constraints
        setRange<int>("counter", 0, 1000);
        std::vector<std::string> statusOptions = {"created", "initializing", "initialized",
                                                  "active", "inactive", "error", "destroyed"};
...
private:
    void registerCommands() {
        ...
        def("increment", [this]() -> int {
            ...
            if (counter) {
                ...
                std::cout << "    Counter incremented to: " << newValue << std::endl;
                return newValue;
            }
            return -1;
        }, "operations", "Increment the counter by 1");

        def("decrement", [this]() -> int {
            auto counter = getVariable<int>("counter");
            if (counter) {
                int newValue = std::max(0, counter->get() - 1);
                setValue("counter", newValue);
                addToHistory("decrement");
                std::cout << "    Counter decremented to: " << newValue << std::endl;
                return newValue;
            }
            return -1;
        ...
        def("calculatePerformance", [this]() -> double {
            auto counter = getVariable<int>("counter");
            auto history = getVariable<std::vector<std::string>>("operation_history");

            if (counter && history) {
                double score = static_cast<double>(counter->get()) /
                              std::max(1.0, static_cast<double>(history->get().size()));
                setValue("performance_score", score);
                std::cout << "    Performance score calculated: " << score << std::endl;
                return score;
            }
            return 0.0;
        }, "diagnostics", "Calculate performance score");

        def("getStats", [this]() -> std::string {
            const auto& stats = getPerformanceStats();
            std::string result = "Performance Statistics:\n";
            result += "  Command calls: " + std::to_string(stats.commandCallCount.load()) + "\n";
            result += "  Command errors: " + std::to_string(stats.commandErrorCount.load()) + "\n";
            result += "  Avg execution time: " + std::to_string(stats.getAvgExecutionTime().count()) + "μs";
            return result;
    ...
    void registerLifecycleHooks() {
        ...
        lifecycle.registerHook(std::string(getName()), atom::components::LifecyclePhase::PreInitialization,
            [this](Component& comp, atom::components::LifecyclePhase phase) {
                (void)phase; // Suppress unused parameter warning
                std::cout << "  [HOOK] Pre-initialization for " << comp.getName() << std::endl;
                setValue("status", std::string("initializing"));
            });

        lifecycle.registerHook(std::string(getName()), atom::components::LifecyclePhase::PostInitialization,
            [this](Component& comp, atom::components::LifecyclePhase phase) {
                (void)phase; // Suppress unused parameter warning
                std::cout << "  [HOOK] Post-initialization for " << comp.getName() << std::endl;
                setValue("status", std::string("initialized"));
                setValue("initialized", true);
                addToHistory("initialized");
            });

        lifecycle.registerHook(std::string(getName()), atom::components::LifecyclePhase::PostActivation,
            [this](Component& comp, atom::components::LifecyclePhase phase) {
                (void)phase; // Suppress unused parameter warning
                std::cout << "  [HOOK] Post-activation for " << comp.getName() << std::endl;
                setValue("status", std::string("active"));
                addToHistory("activated");
            });

        lifecycle.registerHook(std::string(getName()), atom::components::LifecyclePhase::PreDeactivation,
            [this](Component& comp, atom::components::LifecyclePhase phase) {
                (void)phase; // Suppress unused parameter warning
                std::cout << "  [HOOK] Pre-deactivation for " << comp.getName() << std::endl;
...
public:
    ~LifecycleComponent() override {
        std::cout << "  [DESTRUCTOR] Component '" << getName() << "' destroyed" << std::endl;

        // Execute destruction lifecycle hooks
        auto& lifecycle = atom::components::LifecycleManager::instance();
        lifecycle.executePhase(*this, atom::components::LifecyclePhase::PreDestruction);

        addToHistory("destroyed");
        setValue("status", std::string("destroyed"));

        lifecycle.executePhase(*this, atom::components::LifecyclePhase::PostDestruction);
    }

    // Override lifecycle methods with comprehensive functionality
    bool initialize() override {
        std::cout << "  [INITIALIZE] Initializing component '" << getName() << "'" << std::endl;

        // Execute pre-initialization hooks
        auto& lifecycle = atom::components::LifecycleManager::instance();
        lifecycle.executePhase(*this, atom::components::LifecyclePhase::PreInitialization);
        ...
        if (success) {
            ...
            lifecycle.executePhase(*this, atom::components::LifecyclePhase::PostInitialization);
            std::cout << "  [INITIALIZE] Component '" << getName() << "' initialized successfully" << std::endl;
        } else {
            setValue("status", std::string("error"));
            std::cout << "  [ERROR] Component '" << getName() << "' initialization failed" << std::endl;
        }
    ...
    bool activate() {
        std::cout << "  [ACTIVATE] Activating component '" << getName() << "'" << std::endl;

        auto initialized = getVariable<bool>("initialized");
        if (!initialized || !initialized->get()) {
            std::cout << "  [ERROR] Cannot activate uninitialized component" << std::endl;
            return false;
        }
        ...
        lifecycle.executePhase(*this, atom::components::LifecyclePhase::PreActivation);

        setState(ComponentState::Active);

        // Execute post-activation hooks
        lifecycle.executePhase(*this, atom::components::LifecyclePhase::PostActivation);

        std::cout << "  [ACTIVATE] Component '" << getName() << "' activated successfully" << std::endl;
        return true;
    }

    bool deactivate() {
        std::cout << "  [DEACTIVATE] Deactivating component '" << getName() << "'" << std::endl;

        auto& lifecycle = atom::components::LifecycleManager::instance();

        // Execute pre-deactivation hooks
        lifecycle.executePhase(*this, atom::components::LifecyclePhase::PreDeactivation);

        setState(ComponentState::Disabled);

        // Execute post-deactivation hooks
        lifecycle.executePhase(*this, atom::components::LifecyclePhase::PostDeactivation);

        std::cout << "  [DEACTIVATE] Component '" << getName() << "' deactivated successfully" << std::endl;
        return true;
    }

    void cleanup() {
        std::cout << "  [CLEANUP] Cleaning up component '" << getName() << "'" << std::endl;
        ...
        std::cout << "  [CLEANUP] Component '" << getName() << "' cleaned up" << std::endl;
    }
    ...
    void printComponentInfo() {
        ...
        if (status) std::cout << "Status: " << status->get() << std::endl;
        if (counter) std::cout << "Counter: " << counter->get() << std::endl;
        if (score) std::cout << "Performance Score: " << score->get() << std::endl;
...
void demonstrateBasicLifecycle() {
    std::cout << "\n=== Comprehensive Component Lifecycle Demo ===" << std::endl;
    ...
    std::cout << "   Counter after setting to 10: " << std::any_cast<int>(counterResult) << std::endl;

    // Calculate and display performance
    auto perfResult = component->runCommand("calculatePerformance", {});
    std::cout << "   Performance score: " << std::any_cast<double>(perfResult) << std::endl;
    ...
    std::cout << "\n   Final status: " << std::any_cast<std::string>(finalStatus) << std::endl;
}
...
void demonstrateErrorHandling() {
    ...
    try {
        [[maybe_unused]] auto failResult = component->runCommand("nonexistent", {});
