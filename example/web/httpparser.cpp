...
int main() {
    ...
    std::optional<std::vector<std::string>> acceptValues =
        parser.getHeaderValues("Accept");
    if (acceptValues) {
    ...
    std::map<std::string, std::vector<std::string>> allHeaders =
        parser.getAllHeaders();
    ...
    std::cout << "Has Content-Type header: " << std::boolalpha << hasContentType
              << std::endl;
