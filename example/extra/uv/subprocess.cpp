...
// Helper function to print a header for each testvoid printTestHeader(const
// std::string& title) {
std::cout << "\n\n" << std::string(80, '=') << "\n";
std::cout << "  " << title << "\n";
...
// Helper to get a timestamp stringstd::string getTimestamp() {
...
// Example 1: Basic command executionbool runBasicCommand() {
...
std::cout << "[" << getTimestamp()
          << "] Process started with PID: " << process.getPid() << std::endl;

// Process events while we wait
while (process.isRunning()) {
    std::cout << "[" << getTimestamp() << "] Waiting for process to complete..."
              << std::endl;
...
// Example 2: Process with timeoutbool runProcessWithTimeout() {
...
switch (process.getStatus()) {
    case UvProcess::ProcessStatus::EXITED:
        std::cout << "EXITED with code " << process.getExitCode() << std::endl;
        break;
...
// Example 3: Interactive processbool runInteractiveProcess() {
...
// Example 4: Error handlingbool runErrorHandlingTest() {
...
std::cout << "[" << getTimestamp()
          << "] As expected, failed to start non-existent command" << std::endl;
...
if (!success) {
    std::cerr << "Failed to start process with invalid arguments" << std::endl;
    return false;
}
...
// Example 5: Process with custom environmentbool runProcessWithEnvironment() {
...
options.file = "cmd.exe";
options.args = {
    "/c", "echo Custom environment variable: %CUSTOM_VAR% & echo PATH: %PATH%"};
#else
options.file = "/bin/sh";
options.args = {
    "-c", "echo Custom environment variable: $CUSTOM_VAR; echo PATH: $PATH"};
...
if (!success) {
    std::cerr << "Failed to start process with custom environment" << std::endl;
    return false;
}
