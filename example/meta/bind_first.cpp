...
// Forward declarations for our example classesclass Person;
class Vehicle;
class DataProcessor;

// Section 1: Simple class with properties and methods for reflection
// examplesclass Person {
private:
std::string name_;
int age_;
std::string address_;
...
// Section 2: Vehicle class for further reflection examplesclass Vehicle {
private:
std::string make_;
std::string model_;
int year_;
...
// Section 3: DataProcessor class for testing bind_first functionalityclass
// DataProcessor {
private:
std::string name_;
...
double processWithFactor(const std::vector<int>& data, double factor) const {
    double result = 0.0;
    for (const auto& item : data) {
        result += item * factor;
        // 模拟处理时间
...
static void printProgress(int current, int total) {
    ...
    std::cout << "Progress: " << current << "/" << total << " (" << percentage
              << "%)" << std::endl;
}
...
// Section 4: Register Person class with TypeRegistryvoid registerPersonType() {
...
// Register methods
personMetadata.addMethod("greet",
...
personMetadata.addMethod("getDescription",
                         [](std::vector<BoxedValue> args) -> BoxedValue {
                             if (args.size() >= 1) {
                                 auto personPtr = args[0].tryCast<Person>();
                                 if (personPtr) {
                                     return BoxedValue(
                                         personPtr->getDescription());
                                 }
                             }
                             return BoxedValue{};
                         });
    ...
    "updateInfo", [](std::vector<BoxedValue> args) -> BoxedValue {
        if (args.size() >= 3) {
            ...
            if (personPtr && namePtr && agePtr) {
                if (args.size() == 3) {
                    return BoxedValue(personPtr->updateInfo(*namePtr, *agePtr));
                } else if (args.size() == 4) {
...
// Section 5: Register Vehicle class with TypeRegistryvoid registerVehicleType()
// {
...
// Register methods
vehicleMetadata.addMethod("getDescription",
                          [](std::vector<BoxedValue> args) -> BoxedValue {
                              if (args.size() >= 1) {
                                  auto vehiclePtr = args[0].tryCast<Vehicle>();
                                  if (vehiclePtr) {
                                      return BoxedValue(
                                          vehiclePtr->getDescription());
                                  }
                              }
                              return BoxedValue{};
                          });
...
vehicleMetadata.addMethod(
    "calculateDepreciation", [](std::vector<BoxedValue> args) -> BoxedValue {
        if (args.size() >= 2) {
            auto vehiclePtr = args[0].tryCast<Vehicle>();
            auto yearPtr = args[1].tryCast<int>();

            if (vehiclePtr && yearPtr) {
                return BoxedValue(vehiclePtr->calculateDepreciation(*yearPtr));
            }
        }
        return BoxedValue{};
    });

vehicleMetadata.addMethod("performMaintenance",
                          [](std::vector<BoxedValue> args) -> BoxedValue {
                              if (args.size() >= 1) {
                                  auto vehiclePtr = args[0].tryCast<Vehicle>();
                                  if (vehiclePtr) {
...
vehicleMetadata.addEventListener(
    "onPriceChanged", [](BoxedValue& obj, const std::vector<BoxedValue>& args) {
        if (auto vehiclePtr = obj.tryCast<Vehicle>()) {
            if (!args.empty()) {
                if (auto oldPricePtr = args[0].tryCast<double>()) {
                    double newPrice = vehiclePtr->getPrice();
                    std::cout << "Event: Price changed from $" << *oldPricePtr
                              << " to $" << newPrice << std::endl;
                }
            }
        }
    });
...
// Section 6: Main function with comprehensive examplesint main() {
std::cout << "===================================================="
          << std::endl;
...
auto johnBoxed = createInstance("Person", {var(std::string("John")), var(30)});

// Call methods on the boxed person
callMethod(johnBoxed, "greet", {johnBoxed});
callMethod(johnBoxed, "greet", {johnBoxed, var(std::string("Good morning"))});
...
std::cout << "Vehicle: " << makeValue.debugString() << " "
          << modelValue.debugString() << ", Year: " << yearValue.debugString()
          << ", Price: $" << priceValue.debugString() << std::endl;
...
auto updateResult1 = callMethod(
    johnBoxed, "updateInfo", {johnBoxed, var(std::string("Johnny")), var(32)});
std::cout << "Update result 1: " << updateResult1.debugString() << std::endl;

auto updateResult2 = callMethod(johnBoxed, "updateInfo",
                                {johnBoxed, var(std::string("Johnny B.")),
                                 var(33), var(std::string("456 Oak Dr"))});
std::cout << "Update result 2: " << updateResult2.debugString() << std::endl;
...
std::cout << "boundGlobalFunc(5, 10) = " << boundGlobalFunc(5, 10) << std::endl;
...
auto sharedProcessor = std::make_shared<DataProcessor>("ThreadSafeProcessor");
...
for (int i = 0; i < 3; i++) {
    threads.emplace_back([&threadSafeProcess, i]() {
        ...
        std::cout << "Thread " << i << " result: " << threadSafeProcess(data)
                  << std::endl;
    });
}
...
std::cout << "Total processed items: " << sharedProcessor->getProcessedItems()
          << std::endl;

std::cout << "\n2.7: Asynchronous execution with asyncBindFirst" << std::endl;
std::cout << "--------------------------------------------" << std::endl;
...
struct Multiplier {
    ...
    // 绑定到成员函数指针
    static void printMemberFunctionPointerMessage(Multiplier* obj, int value) {
        std::cout << "Member function pointer result: "
                  << obj->operator()(value) << std::endl;
    }

    // 绑定到 const 成员函数
    static void printConstMemberFunctionPointerMessage(const Multiplier* obj,
                                                       int value) {
    ...
    static void printReferenceMessage(const Multiplier& obj, int value) {
        std::cout << "Reference result: " << obj.operator()(value) << std::endl;
    }

    // 绑定到 const 引用
    static void printConstReferenceMessage(const Multiplier& obj, int value) {
    ...
    static void printPointerMessage(Multiplier* obj, int value) {
        std::cout << "Pointer result: " << obj->operator()(value) << std::endl;
    }
    ...
    // 绑定到 std::shared_ptr
    static void printSharedPtrMessage(const std::shared_ptr<Multiplier>& obj,
                                      int value) {
    ...
    static void printWeakPtrMessage(const std::weak_ptr<Multiplier>& obj,
        ...
        if (auto sharedObj = obj.lock()) {
            std::cout << "Weak pointer result: " << sharedObj->operator()(value)
                      << std::endl;
        } else {
    ...
    // 绑定到 std::unique_ptr
    static void printUniquePtrMessage(const std::unique_ptr<Multiplier>& obj,
                                      int value) {
