...
// 假设的atom::memory命名空间namespace atom::memory {
...
// Example constants for component IDsnamespace Constants {
...
// Example classes for demonstrationclass Logger {
public:
Logger(const std::string& name = "default") : name_(name) {
    std::cout << "Logger '" << name_ << "' created" << std::endl;
}

~Logger() { std::cout << "Logger '" << name_ << "' destroyed" << std::endl; }
...
// Helper function to print separator linesvoid printSeparator(const
// std::string& title) {
std::cout << "\n=================================================="
          << std::endl;
...
// Helper function to print pointer statustemplate <typename T>
...
// Example 1: Basic shared pointer managementvoid
...
if (logger_info) {
    ...
    std::cout << "  Access count: " << logger_info->access_count << std::endl;
    std::cout << "  Reference count: " << logger_info->ref_count << std::endl;
}
...
std::cout << "Logger exists: " << (missing_logger.has_value() ? "Yes" : "No")
          << std::endl;
}

// Example 2: Weak pointer handlingvoid demonstrateWeakPointerHandling() {
...
std::cout << "Locked expired pointer valid: " << (locked_expired ? "Yes" : "No")
          << std::endl;

// Clean up expired weak pointers
auto removed = GlobalSharedPtrManager::getInstance().removeExpiredWeakPtrs();
std::cout << "Removed " << removed << " expired weak pointer(s)" << std::endl;
}

// Example 3: Custom deleter usagevoid demonstrateCustomDeleterUsage() {
...
// Example 4: Macro usagevoid demonstrateMacroUsage() {
...
// Example 5: Metadata and diagnosticsvoid demonstrateMetadataAndDiagnostics() {
...
auto cleaned = GlobalSharedPtrManager::getInstance().cleanOldPointers(3600s);
std::cout << "Cleaned " << cleaned << " old pointer(s)" << std::endl;
}

// Example 6: Concurrency aspectsvoid demonstrateConcurrency() {
    ...
    threads.emplace_back([i]() {
        ...
        for (int j = 0; j < 10; j++) {
            ...
            std::cout << "Thread " << i << " incremented counter from "
                      << old_value << " to " << (old_value + 1) << std::endl;
...
// Example 7: Automatic cleanupvoid demonstrateAutomaticCleanup() {
...
auto removed = GlobalSharedPtrManager::getInstance().removeExpiredWeakPtrs();
std::cout << "Removed " << removed << " expired weak reference(s)" << std::endl;
...
// Example 8: Error handlingvoid demonstrateErrorHandling() {
...
try {
    ...
    if (!nonexistent) {
        throw atom::AtomException("Component: nonexistent.logger not exist");
    }
...
try {
    ...
    std::cout << "Exception during type mismatch: " << ex.what() << std::endl;
}
...
if (new_value) {
    std::cout << "Successfully replaced int with string: " << *new_value.value()
              << std::endl;
}
}
