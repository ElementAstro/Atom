...
// Function that might fail randomlyint unstableFunction(int value) {
...
// Function with expensive computationdouble expensiveCalculation(double x,
// double y) { Simulate an expensive calculation
std::this_thread::sleep_for(100ms);
return std::pow(x, 2) + std::pow(y, 2);
}

// Simple string processing functionstd::string processText(const std::string&
// text, bool uppercase) {
...
// Function that divides two numbersdouble safeDivide(double a, double b) {
if (b == 0.0) {
...
// Database query simulationstd::vector<std::string> queryDatabase(const
// std::string& query, int limit) { Simulate database query delay
...
// User authentication functionbool authenticateUser(const std::string&
// username,
                      ...
                      // Function that creates recordsvoid createRecord(const
                      // std::string& name, int age, const std::string& email) {
                      std::cout << "Creating record: name=" << name
                                << ", age=" << age << ", email=" << email
                                << std::endl;
                      ...
                      int main() {
                          std::cout << "======================================="
                                       "=================="
                                    << std::endl;
                          std::cout << "   Comprehensive Decorator Pattern "
                                       "Examples               "
                                    << std::endl;
                          std::cout << "======================================="
                                       "=================="
                          ...
                          auto decoratedProcessText =
                              decorator<std::function<std::string(
                                  const std::string&, bool)>>(processText);

                          // Use the decorated function
                          std::string basicResult =
                              decoratedProcessText("Hello, World!", true);
                          std::cout << "Basic decorated result: " << basicResult
                                    << std::endl;
                          ...
                          auto switchableCalc =
                              Switchable<double(double, double)>(
                                  expensiveCalculation);

                          // Test the original function
                          std::cout << "Original calculation: "
                                    << switchableCalc(3.0, 4.0) << std::endl;

                          // Switch to a new implementation
                          switchableCalc.switchTo([](double x, double y) {
                              std::cout << "Using alternative calculation"
                                        << std::endl;
                          ...
                          std::cout << "Alternative calculation: "
                                    << switchableCalc(3.0, 4.0) << std::endl;
                          ...
                          auto counterFunc = [count =
                                                  0](int /* unused */) mutable {
                              return ++count;
                          };
                          ...
                          int finalCount =
                              loopedCounter(5, progressCallback, 10);
                          std::cout
                              << "Final count after 5 loops: " << finalCount
                              << std::endl;
                          ...
                          auto retryUnstable =
                              std::make_shared<RetryDecorator<int, int>>(
                                  unstableFunction, 5);

                          // Stepper to combine the retry logic with the
                          // unstable function
                          auto retryStep =
                              makeDecorateStepper(unstableFunction);
                          ...
                          try {
                              ...
                              std::cout
                                  << "Retry succeeded, result: " << retryResult
                                  << std::endl;
                          } catch (const std::exception& e) {
                              std::cout << "Retry failed after 5 attempts: "
                                        << e.what() << std::endl;
                          }
                          ...
                          auto customRetryStep =
                              makeDecorateStepper(unstableFunction);
                          // 修复: 正确的参数传递给addDecorator
                          customRetryStep.addDecorator(
                              std::make_shared<RetryDecorator<int, int>>(
                                  unstableFunction, 10,
                                  std::chrono::milliseconds(50), 2.0, true));

                          try {
                              int result = customRetryStep(21);
                              std::cout << "Custom retry succeeded, result: "
                                        << result << std::endl;
                          } catch (const std::exception& e) {
                              std::cout << "Custom retry failed: " << e.what()
                                        << std::endl;
                          }
                          ...
                          auto conditionalText =
                              makeConditionCheckDecorator(processText);

                          // Define conditions
                          bool shouldProcess = true;
                          auto checkCondition = [&shouldProcess]() {
                              return shouldProcess;
                          };

                          // With condition true
                          std::string conditionalResult = conditionalText(
                              checkCondition,
                              "Default text",  // Fallback value if condition is
                                               // false
                              "Hello from conditional decorator", true);
                          std::cout << "Conditional result (true): "
                          ...
                          std::cout << "Conditional result (false): "
                                    << conditionalResult << std::endl;
                          ...
                          std::cout
                              << "Conditional result with fallback function: "
                              << conditionalResult << std::endl;
                          ...
                          // Create a cache decorator for the expensive
                          // calculation
                          auto cacheStep =
                              makeDecorateStepper(expensiveCalculation);
                          // 修复: 正确使用addDecorator方法
                          auto cacheDecorator = std::make_shared<
                              CacheDecorator<double, double, double>>(
                              expensiveCalculation, std::chrono::seconds(60),
                              100);
                          ...
                          std::cout << "First call result: " << cacheResult1
                                    << " (took " << duration1.count() << "ms)"
                                    << std::endl;
                          ...
                          std::cout << "Second call result: " << cacheResult2
                                    << " (took " << duration2.count() << "ms)"
                                    << std::endl;
                          ...
                          auto duration3 = std::chrono::duration_cast<
                              std::chrono::milliseconds>(end - start);

                          std::cout
                              << "Different parameters result: " << cacheResult3
                              << " (took " << duration3.count() << "ms)"
                              << std::endl;
                          ...
                          // Create a timing decorator for the database query
                          // function
                          auto timingDecorator = makeTimingDecorator(
                              queryDatabase, "Database Query", timingCallback);
                          ...
                          auto timingDec = std::make_shared<
                              TimingDecorator<std::vector<std::string>,
                          ...
                          auto throttleStep =
                              makeDecorateStepper(rapidFunction);
                          // 修复: 正确使用addDecorator
                          throttleStep.addDecorator(
                              std::make_shared<ThrottlingDecorator<int, int>>(
                                  std::chrono::milliseconds(
                                      500)  // Minimum 500ms between calls
                                  ));

                          // Execute several calls in rapid succession
                          std::cout << "Starting throttled calls (should be "
                                       "spaced out by at least 500ms):"
                                    << std::endl;
                          ...
                          auto divideErrorMsg = [](double a, double b) {
                              return std::string("Cannot divide ") +
                                     std::to_string(a) + " by " +
                                     std::to_string(b) + " (division by zero)";
                          };

                          // Create a validation decorator
                          auto validatedDivide =
                              ValidationDecorator<double, double, double>(
                                  divideValidator, divideErrorMsg);

                          // Create a stepper for the divide function with
                          // validation
                          ...
                          try {
                              ...
                              std::cout
                                  << "Valid division result: " << validResult
                                  << std::endl;
                          ...
                          try {
                              ...
                              std::cout
                                  << "Expected validation error: " << e.what()
                                  << std::endl;
                          }

                          // Create validators for the createRecord function
                          auto recordValidator = [](const std::string& name,
                                                    int age,
                          ...
                          auto recordErrorMsg = [](const std::string& name,
                                                   int age,
                                                   const std::string& email) {
                          ...
                          auto validateRecordStep =
                              makeDecorateStepper(createRecord);
                          // 修复: 正确传递参数给addDecorator
                          validateRecordStep.addDecorator(
                              std::make_shared<
                                  ValidationDecorator<void, const std::string&,
                                                      int, const std::string&>>(
                                  recordValidator, recordErrorMsg));

                          // Test with valid record
                          try {
                              validateRecordStep("John Doe", 35,
                          ...
                          // Create a complex decorating chain for
                          // queryDatabase:
                          // 1. Add validation
                          // 2. Add caching
                          // 3. Add timing
                          ...
                          auto queryValidator = [](const std::string& query,
                                                   int limit) {
                              return !query.empty() && limit > 0 &&
                                     limit <= 100;
                          };

                          auto queryErrorMsg = [](const std::string& query,
                                                  int limit) {
                          ...
                          auto complexQueryStep =
                              makeDecorateStepper(queryDatabase);

                          // Add validation (first to run)
                          complexQueryStep.addDecorator(
                              std::make_shared<
                                  ValidationDecorator<std::vector<std::string>,
                                                      const std::string&, int>>(
                          ...
                          complexQueryStep.addDecorator(
                              std::make_shared<
                                  CacheDecorator<std::vector<std::string>,
                                                 const std::string&, int>>(
                                  queryDatabase,
                          ...
                          complexQueryStep.addDecorator(
                              std::make_shared<
                                  RetryDecorator<std::vector<std::string>,
                                                 const std::string&, int>>(
                                  queryDatabase, 3,
                                  std::chrono::milliseconds(100), 2.0, true));

                          complexQueryStep.addDecorator(
                              std::make_shared<
                                  ThrottlingDecorator<std::vector<std::string>,
                                                      const std::string&, int>>(
                                  std::chrono::milliseconds(
                                      300)  // Minimum 300ms between calls
                                  ));

                          // Add timing (sixth to run)
                          // Execute the complex decorated function
                          std::cout << "First complex query execution:"
                                    << std::endl;
                          auto complexResult1 =
                              complexQueryStep("SELECT * FROM products", 5);
                          std::cout << "Returned " << complexResult1.size()
                                    << " results" << std::endl;

                          std::cout << "\nSecond complex query execution "
                                       "(should use cache):"
                                    << std::endl;
                          auto complexResult2 =
                              complexQueryStep("SELECT * FROM products", 5);
                          std::cout << "Returned " << complexResult2.size()
                                    << " results" << std::endl;

                          std::cout << "\nThird complex query execution "
                                       "(different parameters):"
                                    << std::endl;
                          auto complexResult3 =
                              complexQueryStep("SELECT * FROM users", 3);
                          std::cout << "Returned " << complexResult3.size()
                                    << " results" << std::endl;

                          // Try invalid parameters to trigger validation
                          try {
                              std::cout
                                  << "\nAttempting invalid query parameters:"
                                  << std::endl;
                              // 修复: 去除未使用变量
