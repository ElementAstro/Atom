...
// Define a class with reflection metadatastruct MyClass {
int x;
double y;
std::string z;
...
// Define reflection metadata for MyClassATOM_META_TYPEINFO(MyClass,
// ATOM_META_FIELD("x", &MyClass::x),
                   ATOM_META_FIELD("y", &MyClass::y),
                   ATOM_META_FIELD("z", &MyClass::z))

// Define a class with a base classstruct Base {
    int baseField;
                   }
                   ;
                   ...
                   // Define reflection metadata for
                   // BaseATOM_META_TYPEINFO(Base, ATOM_META_FIELD("baseField",
                   // &Base::baseField))

                   // Define reflection metadata for
                   // DerivedATOM_META_TYPEINFO(Derived,
                   ATOM_META_FIELD("derivedField", &Derived::derivedField))

// Function to print the field names and valuestemplate <typename T>
void printFields(const T& obj) {
                       using TypeInfo = atom::meta::TypeInfo<T>;
                       TypeInfo::ForEachVarOf(
                           obj, [](const auto& field, const auto& value) {
                               std::cout << "Field name: " << field.name
                                         << ", Value: " << value << '\n';
                           });
                   }

                   // Function to find and print the field value by nametemplate
                   // <typename T>
                   void printFieldByName(const T& obj,
                                         const std::string& name) {
                       using TypeInfo = atom::meta::TypeInfo<T>;
                       const auto& field = TypeInfo::fields.Find(TSTR(name));
                       if constexpr (std::is_same_v<decltype(field.value),
                                                    void>) {
