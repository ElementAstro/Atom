...
// Example 1: Basic SocketHub usagevoid basicSocketHubExample() {
...
// Example 3: Client connect/disconnect handlersvoid
...
// Example 4: Broadcasting messagesvoid broadcastExample() {
...
// Example 5: Sending to specific clientsvoid sendToClientExample() {
...
// Example 6: Client information trackingvoid clientInfoExample() {
...
// Example 7: Client timeout configurationvoid clientTimeoutExample() {
    ...
    hub.addConnectHandler(
        [&totalConnections](int clientId, std::string_view addr) {
            ...
            Logger::log(Logger::LOG_SUCCESS, "Workflow",
                        "New connection #" +
                            std::to_string(totalConnections.load()) + " from " +
                            std::string(addr));
        });
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        Logger::log(Logger::LOG_INFO, "Example9",
                    "Status: " + std::to_string(hub.getClientCount()) +
                        " clients, " + std::to_string(totalMessages.load()) +
                        " messages");
