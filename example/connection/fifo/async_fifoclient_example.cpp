...
void basicAsyncClientExample() {
    Logger::log(Logger::INFO, "Example1", "=== Basic Async FifoClient Usage ===");
    ...
    Logger::log(Logger::INFO, "Example1", "Basic async client example completed\n");
}
...
void timeoutHandlingExample() {
    ...
    try {
        ...
        for (const auto& timeout : timeouts) {
            ...
            Logger::log(success ? Logger::SUCCESS : Logger::WARNING, "Example2",
                        "Result: " + std::string(success ? "success" : "timeout/fail") +
                        " (elapsed: " + std::to_string(elapsed.count()) + "ms)");
        }

        // Test read with no timeout (nullopt)
        Logger::log(Logger::INFO, "Example2", "Testing read with no timeout...");
        auto readResult = client.read(std::nullopt);
        Logger::log(Logger::INFO, "Example2",
                    "Read result: " + std::string(readResult.has_value() ? "data received" : "no data"));
    ...
    Logger::log(Logger::INFO, "Example2", "Timeout handling example completed\n");
}
...
void multipleOperationsExample() {
    ...
    try {
        ...
        Logger::log(Logger::INFO, "Example3",
                    "Performing " + std::to_string(totalOperations) + " write operations...");
    ...
    Logger::log(Logger::INFO, "Example3", "Multiple operations example completed\n");
}

// Example 4: Connection state management
void connectionStateExample() {
    Logger::log(Logger::INFO, "Example4", "=== Connection State Management ===");
    ...
    try {
        ...
        bool writeAfterClose = client.write("After close", std::chrono::milliseconds(500));
        Logger::log(Logger::INFO, "Example4",
                    "Write after close: " +
    ...
    Logger::log(Logger::INFO, "Example4", "Connection state example completed\n");
}

// Example 5: Concurrent client simulation
void concurrentClientsExample() {
    Logger::log(Logger::INFO, "Example5", "=== Concurrent Clients Simulation ===");
    ...
    Logger::log(Logger::INFO, "Example5", "Concurrent clients example completed\n");
}

int main() {
    Logger::log(Logger::INFO, "Main", "==========================================");
    Logger::log(Logger::INFO, "Main", "  Async FifoClient Comprehensive Examples");
    Logger::log(Logger::INFO, "Main", "==========================================\n");
    ...
    Logger::log(Logger::SUCCESS, "Main", "  All async FifoClient examples completed!");
    Logger::log(Logger::SUCCESS, "Main", "==========================================");
