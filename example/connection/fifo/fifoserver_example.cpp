...
// Example 1: Basic FIFOServer usagevoid basicServerExample() {
...
try {
    ...
    for (int i = 1; i <= 5; ++i) {
        ...
        if (server.sendMessage(message)) {
            Logger::log(Logger::LOG_SUCCESS, "Example1", "Sent: " + message);
        } else {
...
// Example 2: Custom server configurationvoid customConfigExample() {
...
// Example 4: Async message sendingvoid asyncMessageExample() {
...
Logger::log(Logger::LOG_INFO, "Example4", "Async message example completed\n");
}

// Example 5: Batch message sending with rangesvoid batchMessageExample() {
...
try {
    ...
    std::vector<std::string> messages = {"Batch message 1", "Batch message 2",
                                         "Batch message 3", "Batch message 4",
                                         "Batch message 5"};
...
Logger::log(Logger::LOG_INFO, "Example5", "Batch message example completed\n");
}

// Example 6: Messageable concept with different typesvoid
...
// Example 7: Message and status callbacksvoid callbacksExample() {
    ...
    int msgCallbackId = server.registerMessageCallback(
        [](const std::string& message, bool success) {
            if (success) {
                ...
                Logger::log(Logger::LOG_WARNING, "MsgCallback",
...
// Example 8: Statistics trackingvoid statisticsExample() {
...
try {
    ...
    Logger::log(Logger::LOG_INFO, "Example8",
                "Avg message size: " + std::to_string(stats.avg_message_size) +
                    " bytes");
    Logger::log(Logger::LOG_INFO, "Example8",
...
// Example 9: Queue managementvoid queueManagementExample() {
...
try {
    ...
    // Fill the queue
    Logger::log(Logger::LOG_INFO, "Example9", "Filling queue with messages...");
    for (int i = 0; i < 20; ++i) {
...
// Example 10: Log level configurationvoid logLevelExample() {
Logger::log(Logger::LOG_INFO, "Example10", "=== Log Level Configuration ===");
...
try {
    ...
    std::vector<atom::connection::LogLevel> levels = {
        ...
        atom::connection::LogLevel::Warning, atom::connection::LogLevel::Error,
