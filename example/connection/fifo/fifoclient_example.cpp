...
void basicUsageExample() {
    ...
    try {
        ...
        if (readResult) {
            Logger::log(Logger::SUCCESS, "Example1", "Read: " + *readResult);
        } else {
...
void lifecycleExample() {
    ...
    try {
        ...
        if (client.isOpen()) {
            ...
            if (result) {
                Logger::log(Logger::SUCCESS, "Example8",
                            "Write after reopen: " + std::to_string(*result) +
                                " bytes");
            }
        }
