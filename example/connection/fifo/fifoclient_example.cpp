...
// Helper to create FIFO on POSIX systemsvoid ensureFifoExists(const
// std::string& path) {
#ifndef _WIN32
...
// Example 1: Basic FifoClient usage with default configurationvoid
// basicUsageExample() {
...
// Example 2: Custom configurationvoid customConfigExample() {
...
// Example 3: Message prioritiesvoid messagePriorityExample() {
...
// Example 4: Async operations with callbacksvoid asyncCallbackExample() {
...
try {
        ...
        [&writeCompleted](bool success, std::error_code ec, size_t bytes) {
            if (success) {
                Logger::log(Logger::LOG_SUCCESS, "Example4",
                            "Async write completed: " + std::to_string(bytes) +
                                " bytes");
            } else {
    ...
    int readOpId = client.readAsync(
        [&readCompleted](bool success, std::error_code ec, size_t bytes) {
            if (success) {
                Logger::log(Logger::LOG_SUCCESS, "Example4",
                            "Async read completed: " + std::to_string(bytes) +
                                " bytes");
            } else {
                Logger::log(
                    Logger::LOG_WARNING, "Example4",
    ...
    if (!writeCompleted) {
        ...
        Logger::log(Logger::LOG_INFO, "Example4", "Cancelled write operation");
    }
    if (!readCompleted) {
        client.cancelOperation(readOpId);
        Logger::log(Logger::LOG_INFO, "Example4", "Cancelled read operation");
    }
...
Logger::log(Logger::LOG_INFO, "Example4", "Async callback example completed\n");
}

// Example 5: Async operations with futuresvoid asyncFutureExample() {
...
try {
    ...
    auto writeFuture =
        client.writeAsyncWithFuture(message, std::chrono::milliseconds(2000));
    ...
    if (writeFuture.wait_for(std::chrono::seconds(3)) ==
        ...
        Logger::log(Logger::LOG_WARNING, "Example5", "Future write timed out");
    }
    ...
    Logger::log(Logger::LOG_INFO, "Example5", "Started async read with future");
    ...
    if (readFuture.wait_for(std::chrono::seconds(2)) ==
        ...
        Logger::log(Logger::LOG_WARNING, "Example5", "Future read timed out");
    }
...
Logger::log(Logger::LOG_INFO, "Example5", "Async future example completed\n");
}

// Example 6: Multiple message writingvoid multipleMessagesExample() {
Logger::log(Logger::LOG_INFO, "Example6", "=== Multiple Messages Writing ===");
...
// Example 7: Connection callbacks and statisticsvoid
...
// Example 8: Open/Close lifecycle managementvoid lifecycleExample() {
...
// Example 9: Template write with custom data typesvoid templateWriteExample() {
...
try {
    ...
    if (result1.has_value()) {
        Logger::log(
            ...
            "Wrote std::string: " + std::to_string(result1.value()) + " bytes");
    }
    ...
    if (result2.has_value()) {
        Logger::log(Logger::LOG_SUCCESS, "Example9",
                    "Wrote string literal: " + std::to_string(result2.value()) +
                        " bytes");
    }
    ...
    if (result3.has_value()) {
        Logger::log(
            Logger::LOG_SUCCESS, "Example9",
            "Wrote string_view: " + std::to_string(result3.value()) + " bytes");
    }
...
Logger::log(Logger::LOG_INFO, "Example9", "Template write example completed\n");
}
