...
class ExampleTester:
    ...
    def build_examples(self, specific_example: Optional[str] = None) -> bool:
        ...
        if exit_code != 0:
            ...
            if specific_example:
                self.results.append(TestResult(
                    f"build_{specific_example}", False, error_msg))
            return False
        ...
        if specific_example:
            self.results.append(TestResult(
                f"build_{specific_example}", True, success_msg))
        return True
    ...
    def test_standalone_example(self, name: str, executable: Path, timeout: int) -> TestResult:
        """Test a standalone example that doesn't require a server"""
        start_time = time.time()

        exit_code, stdout, stderr = self.run_command(
            [str(executable)], timeout=timeout)

        duration = time.time() - start_time

        if exit_code == 0:
            return TestResult(f"run_{name}", True, f"Completed successfully in {duration:.2f}s", duration)
        else:
            return TestResult(f"run_{name}", False, f"Failed with exit code {exit_code}: {stderr}", duration)

    def test_server_example(self, name: str, executable: Path, timeout: int) -> TestResult:
        ...
        try:
            ...
            if process.returncode in [0, -15, -2]:
                return TestResult(f"run_{name}", True, f"Server ran successfully for {duration:.2f}s", duration)
            else:
                return TestResult(f"run_{name}", False, f"Server failed with code {process.returncode}: {stderr}", duration)

        except Exception as e:
            duration = time.time() - start_time
            return TestResult(f"run_{name}", False, f"Exception: {str(e)}", duration)

    def test_client_example(self, name: str, executable: Path, timeout: int) -> TestResult:
        ...
        if exit_code == 0:
            ...
            if "connection" in stderr.lower() or "connect" in stderr.lower():
                return TestResult(f"run_{name}", True, f"Expected connection failure (no server running)", duration)
            else:
                return TestResult(f"run_{name}", False, f"Unexpected failure: {stderr}", duration)
    ...
    def run_tests(self, specific_example: Optional[str] = None, build_only: bool = False) -> bool:
        """Run all tests or tests for a specific example"""
        self.log("Starting Atom Connection Examples Test Suite",
        ...
        # Test execution
        examples_to_test = [specific_example] if specific_example else list(
            self.examples.keys())
...
def main():
    parser = argparse.ArgumentParser(
        description="Test Atom Connection Examples")
    parser.add_argument("--build-only", action="store_true",
                        help="Only test building, skip execution")
