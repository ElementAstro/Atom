...
// Helper to convert TTYResponse to stringstd::string
// responseToString(TTYBase::TTYResponse response) {
switch (response) {
...
// Get platform-specific default devicestd::string getDefaultDevice() {
#ifdef _WIN32
return "COM1";
...
// Derived class for demonstrationclass SerialDevice : public TTYBase {
public:
...
bool connectToDevice(const std::string& device, uint32_t baudRate = 9600) {
    Logger::log(Logger::LOG_INFO, "SerialDevice",
                "Connecting to " + device + " at " + std::to_string(baudRate) +
                    " baud");
...
// Example 1: Basic TTYBase usagevoid basicUsageExample() {
...
// Example 2: Connection parametersvoid connectionParametersExample() {
...
// Example 3: Synchronous read/write operationsvoid syncReadWriteExample() {
...
// Example 4: Asynchronous operationsvoid asyncOperationsExample() {
...
// Example 5: Read until stop bytevoid readSectionExample() {
...
try {
    ...
    auto response =
        device.readSection(std::span<uint8_t>(buffer), stopByte, 5, bytesRead);
...
// Example 6: Error handlingvoid errorHandlingExample() {
...
// Example 7: Debug modevoid debugModeExample() {
...
// Example 8: Using makeByteSpan helpervoid byteSpanHelperExample() {
...
// Example 9: Move semanticsvoid moveSemanticsExample() {
...
// Example 10: Complete serial communication workflowvoid
