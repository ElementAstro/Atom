...
void basicConnectionExample(const std::string& host,
    ...
    Logger::log(Logger::LOG_INFO, "Example1", "=== Basic SSH Connection ===");
    ...
    try {
        ...
        if (client.isConnected()) {
            ...
            for (const auto& path : paths) {
                ...
                Logger::log(exists ? Logger::LOG_SUCCESS : Logger::LOG_WARNING,
                            "Example4",
...
void directoryOperationsExample(const std::string& host,
                                const std::string& username,
                                const std::string& password) {
    Logger::log(Logger::LOG_INFO, "Example5", "=== Directory Operations ===");
    ...
    try {
        ...
        if (client.isConnected()) {
            ...
            Logger::log(Logger::LOG_SUCCESS, "Example5", "Directory created");

            // List parent directory
            Logger::log(Logger::LOG_INFO, "Example5", "Listing /tmp:");
            auto contents = client.listDirectory("/tmp");
            int count = 0;
            for (const auto& item : contents) {
                if (item.find("ssh_example") != std::string::npos) {
                    std::cout << "  " << item << std::endl;
                    count++;
                }
            }
            Logger::log(Logger::LOG_INFO, "Example5",
                        "Found " + std::to_string(count) + " matching items");

            // Remove directory
            Logger::log(Logger::LOG_INFO, "Example5",
                        "Removing directory: " + testDir);
            client.removeDirectory(testDir);
            Logger::log(Logger::LOG_SUCCESS, "Example5", "Directory removed");
...
void fileTransferExample(const std::string& host,
    ...
    try {
        ...
        if (client.isConnected()) {
            ...
            std::string localFile = "ssh_test_upload.txt";
            std::string remoteFile = "/tmp/ssh_test_upload.txt";
            std::string downloadFile = "ssh_test_download.txt";
            ...
            // Upload file
            Logger::log(Logger::LOG_INFO, "Example6",
                        "Uploading: " + localFile + " -> " + remoteFile);
            client.uploadFile(localFile, remoteFile);
            Logger::log(Logger::LOG_SUCCESS, "Example6", "File uploaded");

            // Check if file exists on remote
            if (client.fileExists(remoteFile)) {
                Logger::log(Logger::LOG_SUCCESS, "Example6",
                            "Remote file verified: " + remoteFile);
            ...
            Logger::log(Logger::LOG_INFO, "Example6", "Local files cleaned up");
...
void fileRenameExample(const std::string& host,
    ...
    try {
        ...
        if (client.isConnected()) {
            Logger::log(Logger::LOG_SUCCESS, "Example7", "Connected");

            std::string originalFile = "/tmp/ssh_rename_test.txt";
            std::string renamedFile = "/tmp/ssh_renamed_file.txt";
            ...
            // Rename file
            Logger::log(Logger::LOG_INFO, "Example7",
                        "Renaming: " + originalFile + " -> " + renamedFile);
            client.rename(originalFile, renamedFile);
            Logger::log(Logger::LOG_SUCCESS, "Example7", "File renamed");

            // Verify
            if (client.fileExists(renamedFile)) {
                Logger::log(Logger::LOG_SUCCESS, "Example7",
                            "Renamed file exists: " + renamedFile);
            }
            if (!client.fileExists(originalFile)) {
                Logger::log(Logger::LOG_SUCCESS, "Example7",
                            "Original file removed: " + originalFile);
            }

            // Clean up
            client.removeFile(renamedFile);
            Logger::log(Logger::LOG_INFO, "Example7", "Cleaned up");
...
void directoryUploadExample(const std::string& host,
                            const std::string& username,
                            const std::string& password) {
    Logger::log(Logger::LOG_INFO, "Example8", "=== Directory Upload ===");
    ...
    try {
        ...
        if (client.isConnected()) {
            ...
            std::string localDir = "ssh_upload_test_dir";
            std::string remoteDir = "/tmp/ssh_upload_test_dir";

            std::filesystem::create_directories(localDir + "/subdir1");
            std::filesystem::create_directories(localDir + "/subdir2");
            ...
            // Clean up
            std::vector<std::string> output;
            client.executeCommand("rm -rf " + remoteDir, output);
            std::filesystem::remove_all(localDir);
            Logger::log(Logger::LOG_INFO, "Example8", "Cleaned up");
...
void moveSemanticsExample(const std::string& host,
    ...
    try {
        ...
        if (client2.isConnected()) {
            std::vector<std::string> output;
            client2.executeCommand("echo 'Hello from moved client'", output);
            if (!output.empty()) {
...
void errorHandlingExample(const std::string& host,
    ...
    // Test invalid host
    try {
        Logger::log(Logger::LOG_INFO, "Example10", "Testing invalid host...");
        atom::connection::SSHClient client("invalid.host.example", 22);
        client.connect("user", "pass", 5);
    } catch (const std::exception& e) {
        Logger::log(Logger::LOG_WARNING, "Example10",
                    "Expected error (invalid host): " + std::string(e.what()));
    }
    ...
    try {
        ...
        Logger::log(Logger::LOG_WARNING, "Example10",
                    "Expected error (invalid creds): " + std::string(e.what()));
    }
