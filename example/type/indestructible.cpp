...
// A simple class to demonstrate Indestructible functionalityclass Resource {
private:
std::string name_;
int* data_;
size_t size_;
...
Resource(std::string name) : name_(std::move(name)), data_(nullptr), size_(0) {
    std::cout << "Resource constructor called for '" << name_ << "'\n";
}

// Constructor with name and size
Resource(std::string name, size_t size) : name_(std::move(name)), size_(size) {
    data_ = new int[size_];
...
// A class with trivial destruction for testingstruct TrivialType {
int value;
...
// Example 1: Basic Usagevoid basicUsage() {
...
// Example 2: Construction with Different Argumentsvoid constructionExamples() {
std::cout << "\n=== Example 2: Construction with Different Arguments ===\n";

// Default construction
Indestructible<Resource> res1(std::in_place);
std::cout << "Default constructed resource: " << res1->getName() << std::endl;
...
// Example 3: Copy and Move Semanticsvoid copyAndMoveExamples() {
...
moved->printData();
std::cout << "Original after move, name: " << original->getName() << std::endl;
original->printData();
...
// Copy assignment
res1 = copy;
std::cout << "After copy assignment, name: " << res1->getName() << std::endl;

// Move assignment
res2 = std::move(moved);
std::cout << "After move assignment, name: " << res2->getName() << std::endl;
std::cout << "Source after move assignment, name: " << moved->getName()
          << std::endl;
}

// Example 4: Reset and Emplacevoid resetAndEmplaceExamples() {
...
// Example 5: Implicit Conversionvoid conversionExamples() {
...
std::cout << "Reference from conversion, name: " << ref.getName() << std::endl;
...
// Example 6: Working with Trivial Typesvoid trivialTypeExamples() {
...
std::cout << "Copied trivial value: " << trivialCopy->getValue() << std::endl;
...
// Example 7: Using Indestructible with STL Containersvoid containerExamples() {
...
// Example 8: Using the destruction_guardvoid destructionGuardExample() {
...
// Use a destruction guard to ensure destruction
{
    ...
    std::cout << "Resource is guarded, name: " << res->getName() << std::endl;
...
// Example 9: Advanced Usage - Creating a Singletontemplate <typename T>
class Singleton {
private:
...
// Initialize the static membertemplate <typename T>
Indestructible<T> Singleton<T>::instance_(std::in_place);

// A singleton class exampleclass Logger {
private:
std::string prefix_;
int logCount_;
...
void log(const std::string& message) {
    ++logCount_;
    std::cout << prefix_ << " [" << logCount_ << "]: " << message << std::endl;
}
