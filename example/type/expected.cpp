...
// Example class to demonstrate expected with custom typesclass User {
private:
int id;
std::string name;
std::string email;
...
friend std::ostream& operator<<(std::ostream& os, const User& user) {
    os << "User{id=" << user.id << ", name=\"" << user.name << "\", email=\""
       << user.email << "\"}";
    return os;
}
}
...
// Custom error type for demonstrationstruct DatabaseError {
int code;
std::string message;
...
friend std::ostream& operator<<(std::ostream& os, const DatabaseError& error) {
    os << "DatabaseError{code=" << error.code << ", message=\"" << error.message
       << "\"}";
    return os;
}
}
...
// Simulated database functions that return expectedatom::type::expected<User>
// findUserById(int id) { Simulate database lookup
if (id <= 0) {
...
// Function returning expected<void> for operations that don't return a
// valueatom::type::expected<void, DatabaseError> updateUserEmail(
    ...
    // Function to demonstrate file operations with
    // expectedatom::type::expected<std::string> readFileContents(
    ...
    // Function to demonstrate math operations with
    // expectedatom::type::expected<double> divideNumbers(double a, double b) {
    if (b == 0.0) {
    ...
    // Function to demonstrate complex processing with
    // expectedatom::type::expected<std::vector<int>> parseNumberList(
    const std::string& input) {
        ...
        while (std::getline(ss, item, ',')) {
            try {
                ...
                return atom::type::Error<std::string>("Failed to parse '" +
                                                      item + "' as an integer");
            }
        }
    ...
    int main() {
        ...
        if (invalid_update.has_value()) {
            ...
            std::cout << "  Update failed: ["
                      << invalid_update.error().error().code << "] "
                      << invalid_update.error().error().message << std::endl;
        }
        ...
        if (user_result.has_value()) {
            ...
            std::cout << "  Error: " << user_result.error().error()
                      << std::endl;
        }
        ...
        if (invalid_user.has_value()) {
            ...
            std::cout << "  Error: " << invalid_user.error().error()
                      << std::endl;
        }
        ...
        auto after_and_then = error_int.and_then(
            [](int value) -> atom::type::expected<std::string> {
                return "Processed: " + std::to_string(value);
            });

        std::cout << "\nand_then with error propagation:" << std::endl;
        std::cout << "  Has error: " << (!after_and_then.has_value())
                  << std::endl;
        ...
        auto final_result =
            chain_start
                ...
                .and_then(
                    [](double value) -> atom::type::expected<std::string> {
        ...
        std::cout << "  Original value (int): " << map_start.value()
                  << std::endl;
        std::cout << "  Mapped value (double): " << map_result.value()
                  << std::endl;
        ...
        auto basic_error =
            atom::type::make_unexpected<std::string>("Basic error");
        ...
        std::cout << "  Transformed error: "
                  << transformed_error.error().error() << std::endl;

        // Transforming to a different error type
        auto string_error =
            atom::type::make_unexpected<std::string>("Code 404");
        ...
        auto simplified_error =
            atom::type::expected<User, DatabaseError>(db_error_exp)
                .transform_error([](const DatabaseError& err) {
                    return "DB-" + std::to_string(err.code) + ": " +
                           err.message;
                });
        ...
        std::cout << "  Value after transform_error: "
                  << after_transform.value() << std::endl;
        ...
        std::cout << "  Final error: " << combined_ops.error().error()
                  << std::endl;
            ...
            findUserById(3)
                ...
                .and_then(
                    [](const std::string& userInfo)
                        ...
                        return std::vector<std::string>{userInfo,
                                                        "Additional info"};
                    })
        ...
        std::cout << "  expect_err1 == expect_err2: "
                  << ((!expect_err1.has_value() && !expect_err2.has_value() &&
                       expect_err1.error().error() ==
                           expect_err2.error().error())
                          ? "true"
                          : "false")
                  << std::endl;
        ...
        std::cout << "  user1 == user2: " << (users_equal ? "true" : "false")
                  << std::endl;
        std::cout << "  user1 != user3: "
                  << (users_different ? "true" : "false") << std::endl;
