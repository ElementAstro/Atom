...
// Custom type for demonstrating complex key handlingclass ComplexKey {
private:
int id;
std::string name;
...
// Implement hash function for ComplexKeynamespace std {
template <>
...
// Implement deserialization for ComplexKeytemplate <>
...
// Helper function to print a section headervoid print_header(const std::string&
// title) {
std::cout << "\n==================================================="
          << std::endl;
...
// Helper function to print a subsection headervoid print_subheader(const
// std::string& title) {
std::cout << "\n--- " << title << " ---" << std::endl;
}

// Helper function to print timing informationvoid print_timing(const
// std::string& operation,
                  std::chrono::milliseconds duration) {
                      std::cout << "  Time for " << std::left << std::setw(25)
                                << operation << ": " << duration.count()
                                << " ms" << std::endl;
                  }

                  // Helper function to measure execution timetemplate <typename
                  // Func>
                  ...
                  int main() {
                      std::cout
                          ...
                          << std::endl;
                      std::cout
                          << "     CONCURRENT SET COMPREHENSIVE EXAMPLE     "
                          << std::endl;
                      std::cout
                          << "=============================================="
                          << std::endl;
                      ...
                      std::cout
                          << "Set size after insertion: " << int_set.size()
                          << std::endl;
                      ...
                      std::cout
                          << "Erase 30: " << (erased ? "Success" : "Failure")
                          << std::endl;
                      erased = int_set.erase(110);
                      std::cout
                          << "Erase 110: " << (erased ? "Success" : "Failure")
                          << std::endl;

                      std::cout << "Set size after erasure: " << int_set.size()
                                << std::endl;
                      ...
                      std::cout << "All tasks completed: "
                                << (completed ? "Yes" : "No") << std::endl;
                      std::cout << "Set size after async insertion: "
                                << async_set.size() << std::endl;
                      ...
                      for (int i = 1; i <= 10; i++) {
                          ...
                          async_set.async_find(
                              i * 5, [i, promise = std::move(promise)](
                                         auto result) mutable {
                      ...
                      std::cout << "Set size after async erasure: "
                                << async_set.size() << std::endl;
                      ...
                      // Insert in batch
                      auto batch_insert_time = time_execution(
                          [&]() { batch_set.batch_insert(batch_values); });

                      std::cout << "Set size after batch insertion: "
                                << batch_set.size() << std::endl;
                      print_timing("batch insert (1000 items)",
                                   batch_insert_time);
                      ...
                      auto async_batch_start =
                          std::chrono::high_resolution_clock::now();

                      batch_set.async_batch_insert(
                          async_batch_values, [&](bool success) {
                              auto end =
                                  std::chrono::high_resolution_clock::now();
                              ...
                              std::cout << "  Async batch insert completed: "
                                        << (success ? "Success" : "Failure")
                                        << std::endl;
                              std::cout << "  Time for async batch insert: "
                                        << duration.count() << " ms"
                                        << std::endl;
                      ...
                      batch_future.wait();
                      std::cout << "Set size after async batch insertion: "
                                << batch_set.size() << std::endl;
                      ...
                      std::cout << "Items erased in batch: " << erased_count
                                << " out of " << erase_values.size()
                                << std::endl;
                      std::cout << "Set size after batch erasure: "
                                << batch_set.size() << std::endl;
                      ...
                      atom::type::concurrent_set<int> set_no_cache(
                          4, 0);  // No cache
                      atom::type::concurrent_set<int> set_small_cache(
                          4, 100);  // Small cache
                      atom::type::concurrent_set<int> set_large_cache(
                          4, 10000);  // Large cache
                      ...
                      for (int i = 0; i < 100; i++) {
                          ...
                          access_pattern.push_back(
                              cache_test_data[int_dist(gen) %
                                              cache_test_data.size()]);
                      }
                      ...
                      auto small_cache_stats =
                          set_small_cache.get_cache_stats();
                      auto large_cache_stats =
                          set_large_cache.get_cache_stats();

                      std::cout
                          << "No cache stats: size="
                          << std::get<0>(no_cache_stats)
                      ...
                      std::cout
                          << "Small cache stats: size="
                          << std::get<0>(small_cache_stats)
                      ...
                      std::cout
                          << "Large cache stats: size="
                          << std::get<0>(large_cache_stats)
                      ...
                      print_timing("find after cache resize",
                                   time_after_resize);

                      auto resized_cache_stats =
                          set_small_cache.get_cache_stats();
                      std::cout
                          << "Resized cache stats: size="
                          << std::get<0>(resized_cache_stats)
                          << ", hits=" << std::get<1>(resized_cache_stats)
                          << ", misses=" << std::get<2>(resized_cache_stats)
                          << ", hit rate=" << std::fixed << std::setprecision(2)
                          << std::get<3>(resized_cache_stats) << "%"
                          << std::endl;
                      ...
                      std::cout << "Initial thread count: "
                                << pool_set.get_thread_count() << std::endl;
                      ...
                      pool_set.async_batch_insert(
                          large_batch,
                          [&](bool success) { promise_2threads.set_value(); });
                      ...
                      auto duration_2threads =
                          std::chrono::duration_cast<std::chrono::milliseconds>(
                              end_2threads - start_2threads);

                      std::cout << "Time with 2 threads: "
                                << duration_2threads.count() << " ms"
                                << std::endl;

                      // Adjust thread pool size
                      pool_set.adjust_thread_pool_size(8);  // 8 threads
                      std::cout << "Thread pool adjusted to: "
                                << pool_set.get_thread_count() << " threads"
                                << std::endl;
                      ...
                      pool_set.async_batch_insert(
                          large_batch,
                          [&](bool success) { promise_8threads.set_value(); });
                      ...
                      auto duration_8threads =
                          std::chrono::duration_cast<std::chrono::milliseconds>(
                              end_8threads - start_8threads);

                      std::cout << "Time with 8 threads: "
                                << duration_8threads.count() << " ms"
                                << std::endl;
                      std::cout
                          << "Speedup factor: " << std::fixed
                          << std::setprecision(2)
                      ...
                      error_set.set_error_callback([](std::string_view message,
                          ...
                          std::cout
                              << "Custom error handler called: " << message
                              << std::endl;
                          if (eptr) {
                              try {
                      ...
                      try {
                          ...
                          std::cout << "Caught cache exception: " << e.what()
                                    << std::endl;
                      }

                      // Get error count
                      std::cout
                          << "Error count: " << error_set.get_error_count()
                          << std::endl;
                      ...
                      std::cout << "Complex set size: " << complex_set.size()
                                << std::endl;

                      // Find complex keys
                      auto find_alice =
                          complex_set.find(ComplexKey(1, "Alice"));
                      ...
                      std::cout
                          << "Find Eve: "
                          << (find_eve && *find_eve ? "Found" : "Not found")
                          << std::endl;

                      // Erase complex key
                      bool erased_bob = complex_set.erase(ComplexKey(2, "Bob"));
                      std::cout << "Erase Bob: "
                                << (erased_bob ? "Success" : "Failure")
                                << std::endl;
                      std::cout << "Complex set size after erase: "
                                << complex_set.size() << std::endl;
                      ...
                      try {
                          ...
                          std::cout << "Save to file: "
                                    << (save_success ? "Success" : "Failure")
                                    << std::endl;
                      } catch (const atom::type::io_exception& e) {
                          std::cout << "I/O exception during save: " << e.what()
                                    << std::endl;
                      }
                      ...
                      try {
                          ...
                          std::cout << "Load from file: "
                                    ...
                                    << std::endl;
                          std::cout << "Loaded set size: " << loaded_set.size()
                                    << std::endl;
                      } catch (const atom::type::io_exception& e) {
                          std::cout << "I/O exception during load: " << e.what()
                                    << std::endl;
                      }
                      ...
                      file_set.async_save_to_file(
                          filename + ".async", [&](bool success) {
                      ...
                      auto even_numbers =
                          search_set.conditional_find([](int value) {
                      ...
                      for (size_t i = 0;
                           i < std::min(size_t(5), even_numbers.size()); i++) {
                      ...
                      search_set.async_conditional_find(
                          [](int value) {
                              return value >= 9900 && value <= 10000;
                          },
                          [&](std::vector<int> results) {
                              std::cout
                                  << "Async conditional find complete: found "
                      ...
                      auto foreach_time = time_execution([&]() {
                          search_set.parallel_for_each(
                              [&sum](int value) { sum += value; });
                      });

                      std::cout << "Parallel sum of all elements: " << sum
                                << std::endl;
                      print_timing("parallel_for_each over 10000 items",
                                   foreach_time);
                      ...
                      std::cout << "Initial set size: " << tx_set.size()
                                << std::endl;
                      ...
                      std::cout << "Transaction success: "
                                << (tx_success ? "Yes" : "No") << std::endl;
                      std::cout
                          << "Set size after transaction: " << tx_set.size()
                          << std::endl;
                      ...
                      std::cout
                          << "Find 100: "
                          << (find_100 && *find_100 ? "Found" : "Not found")
                          << std::endl;
                      std::cout << "Find 5: "
                                << (find_5 && *find_5 ? "Found" : "Not found")
                                << std::endl;

                      print_subheader("Failed Transaction");

                      // Create a transaction that will fail (throws an
                      // exception)
                      ...
                      try {
                          ...
                          std::cout << "Transaction exception: " << e.what()
                                    << std::endl;
                          tx_failed = true;
                      }

                      std::cout << "Transaction failed: "
                                << (tx_failed ? "Yes" : "No") << std::endl;
                      std::cout << "Set size after failed transaction: "
                                << tx_set.size() << std::endl;
                      ...
                      std::cout
                          << "Find 300: "
                          << (find_300 && *find_300 ? "Found" : "Not found")
                          << std::endl;
                      std::cout
                          << "Find 400: "
                          << (find_400 && *find_400 ? "Found" : "Not found")
                          << std::endl;
                      std::cout
                          << "Find 500: "
                          << (find_500 && *find_500 ? "Found" : "Not found")
                          << std::endl;
                      ...
                      std::cout << "  Insertions: " << insertion_count
                                << std::endl;
                      std::cout << "  Deletions: " << deletion_count
                                << std::endl;
                      ...
                      size_t pending_tasks =
                          metric_set.get_pending_task_count();
                      std::cout << "Pending tasks: " << pending_tasks
                                << std::endl;
                      ...
                      std::cout << "  async_set: " << async_set.size()
                                << std::endl;
                      std::cout << "  batch_set: " << batch_set.size()
                                << std::endl;
                      std::cout << "  complex_set: " << complex_set.size()
                                << std::endl;
                      std::cout << "  search_set: " << search_set.size()
                                << std::endl;
