...
// A test class to demonstrate Optional with complex typesclass Person {
private:
std::string name_;
int age_;
std::string address_;
...
// Print function for Optional valuestemplate <typename T>
...
// Example 2: Accessing Valuesvoid accessingValuesExample() {
...
std::cout << "Value using value_or(99): " << intOpt.value_or(99) << std::endl;
...
// Example 3: Modifying Valuesvoid modifyingValuesExample() {
...
// Example 4: Copy and Move Semanticsvoid copyMoveExample() {
...
// Example 5: Comparison Operationsvoid comparisonExample() {
...
std::cout << "empty == alsoEmpty: " << (empty == alsoEmpty ? "true" : "false")
          << std::endl;
...
std::cout << "a <=> empty is greater: "
          << ((a <=> empty) == std::strong_ordering::greater ? "true" : "false")
          << std::endl;
std::cout << "empty <=> a is less: "
          << ((empty <=> a) == std::strong_ordering::less ? "true" : "false")
          << std::endl;
std::cout << "empty <=> alsoEmpty is equal: "
          << ((empty <=> alsoEmpty) == std::strong_ordering::equal ? "true"
                                                                   : "false")
          << std::endl;

// Three-way comparison with nullopt
std::cout << "a <=> std::nullopt is greater: "
          << ((a <=> std::nullopt) == std::strong_ordering::greater ? "true"
                                                                    : "false")
          << std::endl;
std::cout << "empty <=> std::nullopt is equal: "
          << ((empty <=> std::nullopt) == std::strong_ordering::equal ? "true"
                                                                      : "false")
          << std::endl;
}

// Example 6: Functional Operationsvoid functionalOperationsExample() {
...
auto transformOrResult = emptyOpt.transform_or([](int x) { return x * 3; },
                                               999);  // Default value if empty
printOptional(transformOrResult, "transform_or on empty Optional");

auto transformOrResult2 = intOpt.transform_or([](int x) { return x * 3; },
                                              999);  // Default value not used
...
intOpt.if_has_value([](int x) { std::cout << "Value is: " << x << std::endl; });
...
// Example 7: Thread Safetyvoid threadSafetyExample() {
...
// Example 8: SIMD Operationsvoid simdOperationsExample() {
...
// Calculate durations
auto stdDuration =
    std::chrono::duration_cast<std::chrono::microseconds>(endStd - startStd);
auto simdDuration =
    std::chrono::duration_cast<std::chrono::microseconds>(endSimd - startSimd);
...
atom::type::Optional<std::string> configServerName;  // No server name specified
