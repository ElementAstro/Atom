...
// Simple test classesclass TestObject {
private:
int id_;
std::string name_;
...
void performOperation() const {
    access_count_++;
    std::cout << "Operation performed on TestObject #" << id_ << " (" << name_
              << ")" << std::endl;
}
}
;

// Derived class for testing cast functionalityclass DerivedObject : public
// TestObject {
private:
double extra_data_;
...
~DerivedObject() {
    std::cout << "DerivedObject with extra_data=" << extra_data_ << " destroyed"
              << std::endl;
}
...
// Helper function to print headersvoid printSection(const std::string& title) {
std::cout << "\n===== " << title << " =====\n";
}
...
if (auto locked = weak.lock()) {
    ...
    std::cout << "Object data: " << locked->getId() << ", " << locked->getName()
              << std::endl;
} else {
...
// Example 2: Advanced Locking Techniquesvoid advancedLockingExample() {
...
std::string name = weak.withLock([](TestObject& obj) { return obj.getName(); })
                       .value_or("Unknown");
...
// Example 3: Asynchronous Operationsvoid asynchronousOperationsExample() {
...
if (asyncLocked) {
    std::cout << "Async lock successful for object: " << asyncLocked->getName()
              << std::endl;
} else {
...
// Example 4: Type Casting and Special Operationsvoid typeCastingExample() {
...
if (createdShared) {
    ...
    std::cout << "Created shared_ptr use count: " << createdShared.use_count()
              << std::endl;
} else {
...
// Example 5: Void Specializationvoid voidSpecializationExample() {
...
auto name = castBack.withLock([](TestObject& obj) { return obj.getName(); });
...
std::cout << "Original weak ptr expired: "
          << (voidWeak.expired() ? "Yes" : "No") << std::endl;
std::cout << "Cast weak ptr expired: " << (castBack.expired() ? "Yes" : "No")
          << std::endl;
}

// Example 6: Group Operationsvoid groupOperationsExample() {
printSection("Group Operations");

// Create a vector of shared pointers
std::vector<std::shared_ptr<TestObject>> sharedPtrs;
for (int i = 0; i < 5; ++i) {
    sharedPtrs.push_back(
        std::make_shared<TestObject>(100 + i, "Group-" + std::to_string(i)));
}
...
// Example 7: Multi-threading Scenariosvoid multiThreadingExample() {
...
for (int i = 0; i < 5; ++i) {
    threads.emplace_back([&weak, &shouldContinue, &totalOperations, i]() {
        ...
        std::cout << "Thread " << i
                  << " finished, local operations: " << localCount << std::endl;
    });
}
...
// Example 8: Error Handling and Edge Casesvoid errorHandlingExample() {
...
bool success = tempWeak.withLock(
    [](TestObject& obj) { std::cout << "This should not print" << std::endl; });
...
for (int i = 0; i < 10; ++i) {
    racingThreads.emplace_back(
        [&contestedWeak, &successfulAccesses, &failedAccesses, &hasReset, i]() {
