...
int main() {
    ...
    auto personReflection = Reflectable<Person>(
        ...
        make_field("address", "Address of the person", &Person::address,
                   addressReflection));

    // Create a JSON object representing a person
    JsonObject personJson = {{"name", "John Doe"},
                             {"age", 30},
                             {"isEmployed", true},
                             {"skills", JsonArray{"C++", "Python", "Java"}},
                             {"address", JsonObject{{"street", "123 Main St"},
                                                    {"city", "Anytown"},
                                                    {"zip", 12345}}}};
    ...
    std::cout << "Person isEmployed: " << std::boolalpha << person.isEmployed
              << std::endl;
    ...
    std::cout << std::endl;
    std::cout << "Person address: " << person.address.street << ", "
              << person.address.city << ", " << person.address.zip << std::endl;
    ...
    YamlObject personYaml = {{"name", "Jane Doe"},
                             {"age", 25},
                             {"isEmployed", false},
                             {"skills", YamlArray{"JavaScript", "HTML", "CSS"}},
                             {"address", YamlObject{{"street", "456 Elm St"},
                                                    {"city", "Othertown"},
                                                    {"zip", 67890}}}};
    ...
    std::cout << "Person from YAML isEmployed: " << std::boolalpha
              << personFromYaml.isEmployed << std::endl;
    std::cout << "Person from YAML skills: ";
    for (const auto& skill : personFromYaml.skills) {
        std::cout << skill << " ";
    }
    std::cout << std::endl;
    std::cout << "Person from YAML address: " << personFromYaml.address.street
              << ", " << personFromYaml.address.city << ", "
              << personFromYaml.address.zip << std::endl;
