...
// Custom struct for testingstruct MyData {
int id;
std::string name;
...
// Helper function to print a headervoid printHeader(const std::string& title) {
std::cout << "\n=== " << title << " ===\n";
}

// Helper function to print a sectionvoid printSection(const std::string&
// section) {
std::cout << "\n--- " << section << " ---\n";
}

// 1. Basic Usagevoid basicUsageExample() {
...
VariantWrapper<int, double, std::string, MyData> stringVar("Hello, world!"s);
VariantWrapper<int, double, std::string, MyData> customVar(MyData(1, "test"));
...
std::cout << "intVar holds int: " << (intVar.is<int>() ? "true" : "false")
          << std::endl;
std::cout << "intVar holds double: " << (intVar.is<double>() ? "true" : "false")
          << std::endl;
...
// 2. Accessing and Modifying Valuesvoid accessingValuesExample() {
...
// 3. Type Conversionvoid typeConversionExample() {
...
if (auto val = intZero.toBool()) {
    std::cout << "int(0) -> bool: " << (*val ? "true" : "false") << std::endl;
}

if (auto val = intOne.toBool()) {
    std::cout << "int(1) -> bool: " << (*val ? "true" : "false") << std::endl;
}
...
// 4. Visiting Patternvoid visitingPatternExample() {
...
VariantWrapper<int, double, std::string, MyData> customVar(MyData(3, "custom"));
...
std::cout << "customVar description: " << customVar.visit(describer)
          << std::endl;
std::cout << "emptyVar description: " << emptyVar.visit(describer) << std::endl;
...
auto doubler = [](auto& value) -> std::string {
    ...
    if constexpr (std::is_same_v<T, std::monostate>) {
        ...
        return "Doubled the string from \"" + original + "\" to \"" + value +
               "\"";
    } else if constexpr (std::is_same_v<T, MyData>) {
        value.id *= 2;
...
// 5. Comparison and Stream Outputvoid comparisonAndOutputExample() {
...
std::cout << "var1 == var2: " << (var1 == var2 ? "true" : "false") << std::endl;
std::cout << "var1 == var3: " << (var1 == var3 ? "true" : "false") << std::endl;
std::cout << "var1 == var4: " << (var1 == var4 ? "true" : "false") << std::endl;
...
// 6. Thread Safetyvoid threadSafetyExample() {
...
std::cout << "Starting concurrent operations on shared variant..." << std::endl;
            ...
            try {
                ...
                sharedVar.withThreadSafety([&]() {
                    if (sharedVar.is<int>()) {
                        ...
                        std::cout << "Reader " << i << ": Read string value \""
                                  << val << "\"" << std::endl;
                    } else {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        try {
            if (i % 2 == 0) {
                int newVal = i * 10;
                sharedVar = newVal;
                std::cout << "Writer: Set int value to " << newVal << std::endl;
            } else {
...
// 7. Error Handlingvoid errorHandlingExample() {
...
// 8. Performance Comparisonvoid performanceExample() {
...
std::cout << "Time to construct " << iterations << " variants:" << std::endl;
std::cout << "  VariantWrapper: " << duration1 << " microseconds" << std::endl;
std::cout << "  std::variant:   " << duration2 << " microseconds" << std::endl;
...
std::cout << "  VariantWrapper: " << duration1 << " microseconds (sum: " << sum1
          << ")" << std::endl;
std::cout << "  std::variant:   " << duration2 << " microseconds (sum: " << sum2
          << ")" << std::endl;
}

// 9. Advanced Use Casesvoid advancedUseCasesExample() {
...
std::map<std::string, VariantWrapper<int, double, bool, std::string>> settings;
...
if (auto timeout = settings["timeout"].toDouble()) {
    std::cout << "Timeout (converted): " << *timeout << " seconds" << std::endl;
}
...
auto processCommand =
    [](const VariantWrapper<int, std::string, std::vector<double>>& command) {
...
VariantWrapper<int, std::string, std::vector<double>> cmd3(std::vector<double>{
    1.1, 2.2, 3.3});
...
// 10. Compatibility and Conversionsvoid compatibilityExample() {
...
std::cout << "Extended variant type: " << extendedVar.typeName() << std::endl;
...
VariantWrapper<int, double, std::string, MyData> indexVar4(MyData(5, "test"));
...
std::cout << "  monostate variant index: " << indexVar5.index() << std::endl;
