...
// Simple POD type for testingstruct Point {
float x;
float y;
...
// Example 1: Basic Usagevoid basicUsageExample() {
...
std::cout << "Vector with specified size - size: " << floatVec.size()
          << std::endl;
std::cout << "Vector with specified size - capacity: " << floatVec.capacity()
          << std::endl;
...
// Example 2: Adding Elementsvoid addingElementsExample() {
...
// Example 3: Removing Elementsvoid removingElementsExample() {
...
// Example 4: Memory Managementvoid memoryManagementExample() {
...
// Example 5: Iterationvoid iterationExample() {
std::cout << "\n=== Example 5: Iteration ===\n";
...
// Example 6: Algorithms and Operationsvoid algorithmsExample() {
...
// Example 7: Move Semanticsvoid moveAndCopyExample() {
...
vec4 = std::move(vec3);
std::cout << "vec3 size after move assignment: " << vec3.size() << std::endl;
printVector(vec4, "Target of move assignment (vec4)");
}

// Example 8: Performance Comparisonvoid performanceExample() {
...
auto duration_pod =
    std::chrono::duration_cast<std::chrono::milliseconds>(end_pod - start_pod);
...
std::cout << "  std::vector: " << duration_std.count() << " ms" << std::endl;
...
auto duration_pod_iter = std::chrono::duration_cast<std::chrono::milliseconds>(
    end_pod_iter - start_pod_iter);
...
auto duration_std_iter = std::chrono::duration_cast<std::chrono::milliseconds>(
    end_std_iter - start_std_iter);

std::cout << "\nTime to iterate through " << NUM_ELEMENTS
          << " elements:" << std::endl;
std::cout << "  PodVector: " << duration_pod_iter.count() << " ms" << std::endl;
std::cout << "  std::vector: " << duration_std_iter.count() << " ms"
          << std::endl;
std::cout << "  Sums: " << sum_pod << " vs " << sum_std << std::endl;
}

// Example 9: Working with Complex POD Typesvoid complexPodExample() {
...
// Add some particles
particles.pushBack(Particle{1.0f, 2.0f, 3.0f, 0.1f, 0.2f, 0.3f, 5.0f, 1, true});
particles.pushBack(Particle{4.0f, 5.0f, 6.0f, 0.4f, 0.5f, 0.6f, 10.0f, 2,
                            true});
particles.pushBack(Particle{7.0f, 8.0f, 9.0f, 0.7f, 0.8f, 0.9f, 15.0f, 1,
                            false});
...
// Example 10: Advanced Usage Patternsvoid advancedUsageExample() {
...
auto freeBlock = [&memoryPool](int index) {
    if (index >= 0 && index < memoryPool.size()) {
        ...
        std::memset(memoryPool[index].data, 0, sizeof(memoryPool[index].data));
    }
};
...
std::cout << "Allocated blocks " << block2 << " and " << block3 << std::endl;
...
std::cout << "Used blocks: " << usedBlocks << " out of " << memoryPool.size()
          << std::endl;
}
