...
// Sample class to demonstrate PointerSentinel functionalityclass Person {
private:
std::string name_;
int age_;
bool active_;
...
// Base class for inheritance demonstrationclass Entity {
protected:
int id_;
std::string type_;
...
virtual ~Entity() { std::cout << "Entity destroyed: ID=" << id_ << std::endl; }
...
// Derived class for inheritance demonstrationclass Player : public Entity {
private:
std::string name_;
int score_;
...
~Player() override { std::cout << "Player destroyed: " << name_ << std::endl; }
...
// Function to perform SIMD-like operations on an arrayvoid
// processArraySIMD(int* data, size_t size) { Simulate SIMD processing by
// operating on chunks of data
...
for (size_t i = 0; i < size; i += 4) {
    ...
    std::cout << "  Processing elements " << i << " to " << (i + chunk_size - 1)
              << ": ";
...
// Example 1: Basic construction and accessvoid basicConstructionExample() {
std::cout << "\n=== Example 1: Basic Construction and Access ===" << std::endl;
...
// Access the pointers
std::cout << "sentinel1 points to: " << sentinel1.get()->getName() << std::endl;
std::cout << "sentinel2 points to: " << sentinel2.get()->getName() << std::endl;
std::cout << "sentinel3 points to: " << sentinel3.get()->getName() << std::endl;
std::cout << "sentinel4 points to: " << sentinel4.get()->getName() << std::endl;
...
// Example 2: Copy and Move Semanticsvoid copyMoveExample() {
...
std::cout << "Original1 points to: " << original1.get()->getName() << std::endl;
std::cout << "Copy1 points to: " << copy1.get()->getName() << std::endl;

std::cout << "Original3 points to: " << original3.get()->getName() << std::endl;
...
std::cout << "Assigned is valid: " << (assigned.is_valid() ? "yes" : "no")
          << std::endl;
std::cout << "Assigned points to: " << assigned.get()->getName() << std::endl;
...
// Example 3: Invoking Methodsvoid invokingMethodsExample() {
...
std::string info = sentinel.apply([](Person* p) {
    return p->getName() + " is " + std::to_string(p->getAge()) + " years old";
});
...
std::cout << "Active: " << (sentinel.invoke(&Person::isActive) ? "yes" : "no")
          << std::endl;
...
// Example 4: Type Conversionvoid typeConversionExample() {
...
PointerSentinel<Entity> entity_sentinel = player_sentinel.convert_to<Entity>();

std::cout << "Entity: " << entity_sentinel.invoke(&Entity::describe)
          << std::endl;
std::cout << "ID: " << entity_sentinel.invoke(&Entity::getId) << std::endl;
std::cout << "Type: " << entity_sentinel.invoke(&Entity::getType) << std::endl;
...
try {
    ...
    std::cout << "Conversion not attempted due to static_assert" << std::endl;
} catch (...) {
    std::cout << "Exception thrown during invalid conversion" << std::endl;
}
}

// Example 5: Asynchronous Operationsvoid asyncOperationsExample() {
...
auto future = sentinel.apply_async([](Person* p) {
    ...
    // Simulate work
    for (int i = 0; i < 3; i++) {
        std::cout << "Async task working... (" << (i + 1) << "/3)" << std::endl;
...
// Example 6: SIMD-Like Operationsvoid simdOperationsExample() {
...
// Example 7: Error Handling and Safetyvoid errorHandlingExample() {
...
try {
    ...
    std::cout << "Expected exception (shared_ptr): " << e.what() << std::endl;
}
...
try {
    ...
    std::cout << "Expected exception (unique_ptr): " << e.what() << std::endl;
}
...
try {
    ...
    std::cout << "Expected exception (raw pointer): " << e.what() << std::endl;
}
...
for (int i = 0; i < 5; i++) {
    threads.push_back(std::thread([&shared_sentinel, i]() {
        try {
            ...
            std::cout << "Thread " << i << " read name: " << name << std::endl;
...
// Example 8: Working with Raw Pointersvoid rawPointerExample() {
...
// Example 9: Complex Scenariosvoid complexScenariosExample() {
...
// Example 10: Performance and Memory Managementvoid performanceExample() {
