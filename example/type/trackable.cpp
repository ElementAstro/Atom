...
// Example 2: Observer Patternvoid observerPatternExample() {
...
trackableString.subscribe([](const std::string& oldVal,
...
trackableString.subscribe([](const std::string& oldVal,
                             const std::string& newVal) {
    std::cout << "Character count changed from " << oldVal.length() << " to "
              << newVal.length() << std::endl;
});
...
// Example 3: Custom Typesstruct Point {
int x;
int y;
...
// Formatter for Point to use with iostreamstd::ostream&
...
// Example 4: Deferred Notificationsvoid deferredNotificationsExample() {
...
trackableDouble.subscribe([&notificationCount](const double& oldVal,
                                               const double& newVal) {
    notificationCount++;
...
// Use scoped deferral
{
    ...
    std::cout << "Notifications during scoped deferral: " << notificationCount
              << std::endl;
...
std::cout << "Final notifications count: " << notificationCount << std::endl;
std::cout << "Final value: " << trackableDouble.get() << std::endl;
}

// Example 5: Thread Safetyvoid threadSafetyExample() {
...
counter.subscribe([&notificationCount](const int&, const int&) {
    notificationCount++;
});
...
// Example 6: Exception Handling in Observersvoid exceptionHandlingExample() {
...
trackableInt.setOnChangeCallback([](const int& newVal) {
    std::cout << "OnChange callback (before throw): " << newVal << std::endl;
    throw std::runtime_error("Intentional exception in onChange callback");
});

// Trigger the callback
try {
    std::cout << "Changing value to trigger onChange callback (that throws):"
              << std::endl;
    trackableInt = 2;
} catch (const std::exception& e) {
    std::cout << "Exception caught: " << e.what() << std::endl;
...
// Example 7: Complex Data Structuresvoid complexDataStructuresExample() {
printSection("Complex Data Structures");

// Create a trackable vector of strings
Trackable<std::vector<std::string>> trackableVector(std::vector<std::string>{
    "apple", "banana", "cherry"});
...
// Example 8: Practical Use Casesvoid practicalUseCasesExample() {
...
// Example 9: Template Specialization Examplesvoid
...
// Complex number
Trackable<std::complex<double>> complexValue(std::complex<double>(1.0, 2.0));
...
// Example 10: Performance Considerationsvoid performanceConsiderationsExample()
// {
