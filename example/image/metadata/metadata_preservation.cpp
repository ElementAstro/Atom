...
void addProcessingStep(ImageMetadata& metadata, const std::string& operation,
    ...
    metadata.current_state["processing_steps"] =
        std::to_string(metadata.processing_history.size());
}
...
int main() {
    ...
    // Original acquisition metadata
    metadata.acquisition = {{"filename", "M31_L_001.fits"},
    ...
    // Initial state
    metadata.current_state = {{"format", "FITS"},
                              {"bit_depth", "16"},
                              {"width", "6248"},
                              {"height", "4176"},
                              {"channels", "1"},
    ...
    metadata.quality_metrics = {{"snr_estimate", 45.2},
                                {"fwhm_pixels", 3.8},
                                {"eccentricity", 0.15},
    ...
    std::cout << "   Object: " << metadata.acquisition["object_name"]
              << std::endl;
    std::cout << "   Exposure: " << metadata.acquisition["exposure_time"]
              << "s\n";
    std::cout << "   Filter: " << metadata.acquisition["filter"] << std::endl;
    std::cout << "   Date: " << metadata.acquisition["date_obs"] << std::endl;
    std::cout << "   Initial SNR: " << metadata.quality_metrics["snr_estimate"]
              << std::endl;
    ...
    // Step 1: Dark frame subtraction
    {
        ...
        metadata.quality_metrics["background_level"] =
            125.0;  // Reduced after dark subtraction

        std::cout << "   ✓ Dark subtraction applied\n";
        std::cout << "     - Dark frame: " << params["dark_frame"] << std::endl;
        std::cout << "     - Background reduced to: "
                  << metadata.quality_metrics["background_level"] << std::endl;
    }
    ...
    // Step 2: Flat field correction
    {
        ...
        metadata.quality_metrics["snr_estimate"] =
            52.1;  // Improved after flat correction

        std::cout << "   ✓ Flat field correction applied\n";
        std::cout << "     - Flat frame: " << params["flat_frame"] << std::endl;
        std::cout << "     - SNR improved to: "
                  << metadata.quality_metrics["snr_estimate"] << std::endl;
    }
    ...
    // Step 3: Bias correction
    {
        ...
        std::cout << "     - Image trimmed to: "
                  << metadata.current_state["width"] << "x"
                  << metadata.current_state["height"] << std::endl;
    }
    ...
    // Step 4: Cosmic ray removal
    {
        ...
        std::cout << "     - Cosmic rays removed: "
                  << params["cosmic_rays_found"] << std::endl;
    }
    ...
    // Step 5: Registration and alignment
    {
        ...
        std::cout << "     - Stars matched: " << params["matched_stars"]
                  << std::endl;
    }
    ...
    // Step 6: Gradient removal
    {
        ...
        metadata.quality_metrics["background_uniformity"] =
            0.95;  // Improved uniformity

        std::cout << "   ✓ Background gradient removal applied\n";
        std::cout << "     - Background uniformity: "
                  << metadata.quality_metrics["background_uniformity"]
                  << std::endl;
    }

    std::cout << "\n3. Processing history summary:\n";
    std::cout << "   Total processing steps: "
              << metadata.processing_history.size() << std::endl;
    ...
    std::cout << "   Processing level: "
              << metadata.current_state["processing_level"] << std::endl;
    std::cout << "   Calibration state: "
              << metadata.current_state["calibration_state"] << std::endl;
    std::cout << "   Dimensions: " << metadata.current_state["width"] << "x"
              << metadata.current_state["height"] << std::endl;
    std::cout << "   Last operation: "
              << metadata.current_state["last_operation"] << std::endl;

    std::cout << "\n5. Quality metrics evolution:\n";
    std::cout << "   Final SNR: " << metadata.quality_metrics["snr_estimate"]
              << std::endl;
    std::cout << "   Background level: "
              << metadata.quality_metrics["background_level"] << std::endl;
    std::cout << "   Cosmic rays removed: "
              << metadata.quality_metrics["cosmic_ray_count"] << std::endl;
    std::cout << "   Registration RMS: "
              << metadata.quality_metrics["registration_rms"] << " pixels\n";
