...
public:
    ...
    Progress getProgress() const {
        ...
        progress.percentage = progress.total > 0 ? (progress.completed * 100.0 / progress.total) : 0.0;
        
        auto elapsed = steady_clock::now() - startTime_;
        progress.elapsedSeconds = duration_cast<milliseconds>(elapsed).count() / 1000.0;
        
        if (progress.completed > 0) {
            double avgTimePerItem = progress.elapsedSeconds / progress.completed;
...
void createTestImages(const std::string& directory, size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        std::string filename = directory + "/test_image_" + std::to_string(i) + ".raw";
 ...
 */
void saveRawImage(const std::string& filename, const blob& image, uint32_t width, uint32_t height, uint32_t channels) {
    std::ofstream file(filename, std::ios::binary);
    if (!file) {
...
void demonstrateSequentialBatch() {
    ...
    try {
        ...
        std::cout << "Processing " << inputFiles.size() << " images sequentially\n";
        ...
        for (const auto& inputFile : inputFiles) {
            try {
                ...
                auto processed = processor.applyFilter(image, FilterType::GAUSSIAN_BLUR, 
                                                     {{"sigma", 1.0}, {"kernel_size", 3}});
                
                // Save processed image
                fs::path inputPath(inputFile);
                std::string outputFile = outputDir + "/" + inputPath.stem().string() + "_processed.raw";
                
                // For simplicity, assume same dimensions (would need to parse from actual processing)
        ...
        auto totalTime = duration_cast<milliseconds>(steady_clock::now() - start);
        ...
        std::cout << "  Average time per image: " << (totalTime.count() / progress.completed) << " ms\n";
        std::cout << "  Throughput: " << (progress.completed * 1000.0 / totalTime.count()) << " images/sec\n";
...
void demonstrateParallelBatch() {
    ...
    try {
        ...
        std::cout << "Processing " << inputFiles.size() << " images in parallel\n";
        ...
        std::thread progressThread([&tracker, &inputFiles]() {
            while (true) {
                ...
                std::cout << "\rProgress: " << std::fixed << std::setprecision(1) 
                         ...
                         << "/" << progress.total << ") - " 
                         << std::setprecision(2) << progress.elapsedSeconds << "s elapsed";
                std::cout.flush();
        ...
        std::cout << "  Average time per image: " << (totalTime.count() / progress.completed) << " ms\n";
        std::cout << "  Throughput: " << (progress.completed * 1000.0 / totalTime.count()) << " images/sec\n";
        std::cout << "  Parallel efficiency: " << (numThreads * 100.0 / (totalTime.count() / (progress.completed * 10.0))) << "%\n";
        
        if (!progress.errors.empty()) {
            std::cout << "  Errors encountered:\n";
            for (size_t i = 0; i < std::min(size_t(5), progress.errors.size()); ++i) {
                std::cout << "    " << progress.errors[i] << "\n";
            }
            if (progress.errors.size() > 5) {
                std::cout << "    ... and " << (progress.errors.size() - 5) << " more errors\n";
            }
        }
...
void demonstrateMemoryEfficientBatch() {
    ...
    try {
        ...
        std::cout << "Processing " << inputFiles.size() << " images with memory constraints\n";
        
        // Simulate memory constraint (process in smaller batches)
        const size_t maxBatchSize = 5;
        const size_t numBatches = (inputFiles.size() + maxBatchSize - 1) / maxBatchSize;
        
        std::cout << "Processing in " << numBatches << " batches of max " << maxBatchSize << " images\n";
        ...
        for (size_t batchIdx = 0; batchIdx < numBatches; ++batchIdx) {
            ...
            size_t endIdx = std::min(startIdx + maxBatchSize, inputFiles.size());
            
            std::cout << "Processing batch " << (batchIdx + 1) << "/" << numBatches 
                     << " (images " << startIdx << "-" << (endIdx - 1) << ")\n";
            ...
            auto progress = tracker.getProgress();
            std::cout << "  Batch completed. Overall progress: " << std::fixed << std::setprecision(1) 
                     << progress.percentage << "%\n";
        }
        ...
        std::cout << "  Average time per image: " << (totalTime.count() / progress.completed) << " ms\n";
        std::cout << "  Throughput: " << (progress.completed * 1000.0 / totalTime.count()) << " images/sec\n";
        std::cout << "  Memory efficiency: Processed in " << numBatches << " batches\n";
...
void demonstrateBatchOperations() {
    ...
    try {
        ...
        std::vector<std::pair<std::string, std::function<blob(const blob&)>>> operations = {
            {"blur", [](const blob& img) {
                ImageProcessor processor;
                return processor.applyFilter(img, FilterType::GAUSSIAN_BLUR, {{"sigma", 1.0}, {"kernel_size", 3}});
            }},
            {"sharpen", [](const blob& img) {
                ImageProcessor processor;
                return processor.applyFilter(img, FilterType::SHARPEN, {{"strength", 0.5}});
            }},
            {"edge_detection", [](const blob& img) {
                ImageProcessor processor;
                return processor.applyFilter(img, FilterType::EDGE_DETECTION, {{"threshold", 100}});
            }}
        };
        ...
        for (const auto& [opName, operation] : operations) {
            ...
            for (size_t i = 0; i < numToProcess; ++i) {
                ...
                auto future = std::async(std::launch::async, [&, inputFile, operation]() {
                    try {
                        // Load image
                        auto image = loadRawImage(inputFile);
                        ...
                        // Save processed image
                        fs::path inputPath(inputFile);
                        std::string outputFile = outputDir + "/" + inputPath.stem().string() + "_" + opName + ".raw";
            ...
            std::cout << "  " << opName << " processing: " << totalTime.count() << "ms, " 
                     << progress.completed << " images, " 
                     << std::fixed << std::setprecision(1) << (progress.completed * 1000.0 / totalTime.count()) 
                     << " images/sec\n";
        }
...
void demonstrateErrorHandling() {
    ...
    try {
        ...
        std::cout << "Testing error handling with " << inputFiles.size() << " files (including invalid ones)\n";
            ...
            auto future = std::async(std::launch::async, [&, inputFile]() {
                try {
                    ...
                    if (inputFile.find(".txt") != std::string::npos) {
                        throw std::runtime_error("Invalid file format: " + inputFile);
                    }
                    ...
                    tracker.incrementFailed("Error processing " + inputFile + ": " + e.what());
                }
            });
        ...
        std::cout << "  Total time: " << totalTime.count() << " ms\n";
        std::cout << "  Successfully processed: " << progress.completed << " images\n";
        std::cout << "  Failed: " << progress.failed << " images\n";
...
int main() {
    ...
    std::cout << "This example demonstrates batch image processing operations\n";
