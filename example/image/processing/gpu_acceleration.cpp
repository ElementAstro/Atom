...
int main() {
    ...
    try {
        ...
        if (cuda_devices > 0) {
            ...
            cv::randu(cpu_image, cv::Scalar(0, 0, 0),
            ...
            std::cout << "GPU processing time: " << gpu_duration.count()
            ...
            auto cpu_duration =
                std::chrono::duration_cast<std::chrono::microseconds>(end -
                                                                      start);
            std::cout << "CPU processing time: " << cpu_duration.count()
                      << " microseconds" << std::endl;

            double speedup = static_cast<double>(cpu_duration.count()) /
                             gpu_duration.count();
            std::cout << "GPU speedup: " << speedup << "x" << std::endl;

        } else {
            std::cout
                << "No CUDA devices found. GPU acceleration not available."
                << std::endl;
...
#else
    std::cout << "OpenCV not available. GPU acceleration example cannot run."
              << std::endl;
