...
public:
    ...
    bool pop(blob& frame, std::chrono::milliseconds timeout =
...
public:
    explicit PerformanceTracker(size_t maxSamples = 100)
    ...
    void recordFrame() {
        ...
        auto frameTime =
            duration_cast<microseconds>(now - lastFrameTime_).count() / 1000.0;
        ...
        if (!processingTimes_.empty()) {
            ...
            if (stats.avgFrameTime > 0) {
                stats.processingLoad =
                    stats.avgProcessingTime / stats.avgFrameTime;
            }
        }
...
public:
    ...
    void start(FrameBuffer& buffer) {
        ...
        captureThread_ = std::thread([this]() {
            auto frameInterval =
                microseconds(static_cast<long>(1000000.0 / targetFPS_));
...
void demonstrateBasicRealtimeProcessing() {
    ...
    try {
        ...
        std::cout << "Starting " << durationSeconds
                  << "s real-time processing demo\n";
        std::cout << "Target: " << targetFPS << " FPS, " << width << "x"
                  << height << "\n";
            ...
            while (processingActive) {
                if (inputBuffer.pop(frame, std::chrono::milliseconds(50))) {
                    ...
                    auto processed = processor.applyFilter(
                        frame, FilterType::GAUSSIAN_BLUR,
        ...
        std::cout << "  Average processing time: " << std::setprecision(2)
                  << finalStats.avgProcessingTime << "ms\n";
        std::cout << "  Max processing time: " << finalStats.maxProcessingTime
                  << "ms\n";
...
void demonstrateAdaptiveQuality() {
    ...
    try {
        ...
        std::cout << "Testing adaptive quality control for " << durationSeconds
                  << "s\n";
        ...
        std::thread processingThread([&]() {
            blob frame;
            std::vector<std::string> qualitySettings = {"low", "medium",
                                                        "high"};

            while (active) {
                if (inputBuffer.pop(frame, std::chrono::milliseconds(
                                               33))) {  // ~30 FPS timeout
                    auto start = steady_clock::now();
                    ...
                    switch (currentQuality) {
                        case 1:  // Low quality - fast processing
                            processed = processor.applyFilter(
                                frame, FilterType::GAUSSIAN_BLUR,
        ...
        std::thread adaptationThread([&]() {
            ...
            const double highThreshold =
                targetFrameTime * 1.2;  // 120% of target
            ...
            while (active) {
                ...
                std::cout << "Quality: " << currentQuality << " ("
                          << (currentQuality == 1   ? "Low"
                              : currentQuality == 2 ? "Medium"
                                                    : "High")
                ...
                if (stats.avgProcessingTime > highThreshold &&
                    currentQuality > 1) {
                    // Reduce quality to maintain frame rate
                    qualityLevel = currentQuality - 1;
                    qualityChanges++;
                    std::cout
                        << "  -> Reducing quality to maintain frame rate\n";
                } else if (stats.avgProcessingTime < lowThreshold &&
                           currentQuality < 3) {
                    // Increase quality if we have headroom
                    qualityLevel = currentQuality + 1;
                    qualityChanges++;
                    std::cout << "  -> Increasing quality (performance "
                                 "headroom available)\n";
                }
            }
        });
        ...
        std::cout << "  Processing efficiency: "
                  << (finalStats.processingLoad < 1.0 ? "Good" : "Overloaded")
                  << "\n";
...
void demonstrateMultiThreadedPipeline() {
    ...
    try {
        ...
        std::cout << "Testing " << numProcessingThreads
                  << "-thread pipeline for " << durationSeconds << "s\n";
            ...
            processingThreads.emplace_back([&, i]() {
                ...
                while (active) {
                    if (inputBuffer.pop(frame, std::chrono::milliseconds(50))) {
                        ...
                        switch (i % 3) {
                            case 0:
                                processed = processors[i]->applyFilter(
                                    frame, FilterType::GAUSSIAN_BLUR,
                                    {{"sigma", 1.0}, {"kernel_size", 5}});
                                break;
                            case 1:
                                processed = processors[i]->applyFilter(
                                    frame, FilterType::SHARPEN,
                                    {{"strength", 0.3}});
                                break;
                            case 2:
                                processed = processors[i]->applyFilter(
                                    frame, FilterType::EDGE_DETECTION,
                                    {{"threshold", 100}});
                                break;
                        }
                ...
                std::cout << "Thread " << i << " processed " << threadProcessed
                          << " frames\n";
            });
        }
        ...
        std::thread statsThread([&]() {
            while (active) {
                ...
                auto stats = tracker.getStats();
                std::cout << "Pipeline: " << std::fixed << std::setprecision(1)
                          << stats.avgFPS << " FPS"
                          << ", Avg processing: " << std::setprecision(2)
                          << stats.avgProcessingTime << "ms"
        ...
        std::cout << "  Average throughput: " << std::fixed
                  << std::setprecision(1) << finalStats.avgFPS << " FPS\n";
        std::cout << "  Average processing time: " << std::setprecision(2)
                  << finalStats.avgProcessingTime << "ms\n";
        std::cout << "  Theoretical speedup: "
                  << (finalStats.avgFPS / (targetFPS / numProcessingThreads))
                  << "x\n";
...
void demonstratePerformanceOptimization() {
    ...
    try {
        ...
        std::cout << "Testing optimization techniques with " << testFrames
        ...
        for (int i = 0; i < testFrames; ++i) {
            auto result =
                processor.applyFilter(testFrame, FilterType::GAUSSIAN_BLUR,
                                      {{"sigma", 1.0}, {"kernel_size", 5}});
        }
        auto withoutPrealloc =
        ...
        std::cout << "  Without pre-allocation: " << withoutPrealloc.count()
                  << "ms\n";
        std::cout << "  With pre-allocation: " << withPrealloc.count()
                  << "ms\n";
        std::cout << "  Speedup: "
                  << (static_cast<double>(withoutPrealloc.count()) /
                      withPrealloc.count())
                  << "x\n";
        ...
        std::cout << "  SIMD speedup: "
                  << (static_cast<double>(withoutSIMD.count()) /
                      withSIMD.count())
                  << "x\n";
        ...
        auto tiledProcessing =
            duration_cast<milliseconds>(steady_clock::now() - start);

        std::cout << "  Tiled processing: " << tiledProcessing.count()
                  << "ms\n";
        std::cout << "  Cache optimization benefit: "
                  << (static_cast<double>(withSIMD.count()) /
                      tiledProcessing.count())
                  << "x\n";
...
int main() {
    ...
    std::cout
        << "This example demonstrates real-time image processing techniques\n";
    ...
    std::cout << "- Performance optimization techniques (SIMD, caching, "
                 "pre-allocation)\n";
