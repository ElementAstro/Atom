...
struct BenchmarkResult {
    ...
    BenchmarkResult(const std::string& n, std::chrono::nanoseconds d, size_t ops, size_t mem = 0)
...
public:
    ...
    BenchmarkResult benchmark(const std::string& name, Func&& func, size_t operations = 1) {
        // Warm up
        for (int i = 0; i < 3; ++i) {
            func();
        }
    ...
    void printResults() const {
        ...
        for (const auto& result : results_) {
            std::cout << std::left << std::setw(30) << result.name
                     ...
                     << result.throughput
                     << std::setw(10) << result.operations << "\n";
        }
    }
    
    void compareResults(const std::string& baseline, const std::string& optimized) const {
        ...
        auto optimizedIt = std::find_if(results_.begin(), results_.end(),
            [&optimized](const BenchmarkResult& r) { return r.name == optimized; });
        
        if (baselineIt != results_.end() && optimizedIt != results_.end()) {
            double speedup = static_cast<double>(baselineIt->duration.count()) / optimizedIt->duration.count();
            std::cout << "\nSpeedup: " << optimized << " is " << std::fixed << std::setprecision(2) 
                     << speedup << "x faster than " << baseline << "\n";
        }
    }
};
...
void demonstrateMemoryOptimization() {
    ...
    profiler.benchmark("Sequential Access", [&]() {
        uint64_t sum = 0;
    ...
    profiler.benchmark("Random Access", [&]() {
        uint64_t sum = 0;
    ...
    profiler.benchmark("Unaligned Memory", [&]() {
        uint64_t sum = 0;
    ...
    profiler.benchmark("Aligned Memory", [&]() {
        uint64_t sum = 0;
        for (size_t i = 0; i < dataSize; ++i) {
            sum += alignedData[i];
        }
    ...
    profiler.benchmark("Without Prefetch", [&]() {
        uint64_t sum = 0;
        for (size_t i = 0; i < testData.size(); i += 64) { // Process every 64th byte
            sum += testData[i];
        }
        volatile uint64_t result = sum;
    }, 1000);
    
    profiler.benchmark("With Prefetch", [&]() {
        uint64_t sum = 0;
...
void demonstrateSIMDOptimizations() {
    ...
    profiler.benchmark("Scalar Addition", [&]() {
        for (size_t i = 0; i < dataSize; ++i) {
            result[i] = static_cast<uint8_t>(std::min(255, static_cast<int>(testData1[i]) + testData2[i]));
        }
    }, 100);
    
#ifdef __AVX2__
    // AVX2 SIMD implementation
    profiler.benchmark("AVX2 Addition", [&]() {
        ...
        for (size_t i = 0; i < simdIterations; ++i) {
            ...
            __m256i a = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(&testData1[offset]));
            __m256i b = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(&testData2[offset]));
            __m256i sum = _mm256_adds_epu8(a, b); // Saturated addition
            
            _mm256_storeu_si256(reinterpret_cast<__m256i*>(&result[offset]), sum);
        }
    ...
    profiler.benchmark("SSE2 Addition", [&]() {
        ...
        for (size_t i = 0; i < simdIterations; ++i) {
            ...
            __m128i a = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&testData1[offset]));
            __m128i b = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&testData2[offset]));
            __m128i sum = _mm_adds_epu8(a, b); // Saturated addition
            
            _mm_storeu_si128(reinterpret_cast<__m128i*>(&result[offset]), sum);
        }
    ...
    profiler.benchmark("Scalar Brightness", [&]() {
        for (size_t i = 0; i < dataSize; ++i) {
            result[i] = static_cast<uint8_t>(std::min(255, std::max(0, static_cast<int>(testData1[i]) + brightness)));
        }
    }, 100);
    
#ifdef __AVX2__
    // AVX2 brightness adjustment
    profiler.benchmark("AVX2 Brightness", [&]() {
        const size_t simdSize = 32;
        size_t simdIterations = dataSize / simdSize;
        
        __m256i brightnessVec = _mm256_set1_epi8(static_cast<char>(brightness));
        
        for (size_t i = 0; i < simdIterations; ++i) {
            size_t offset = i * simdSize;
            
            __m256i data = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(&testData1[offset]));
            __m256i adjusted = _mm256_adds_epu8(data, brightnessVec);
            
            _mm256_storeu_si256(reinterpret_cast<__m256i*>(&result[offset]), adjusted);
        }
...
void demonstrateMultiThreadingOptimizations() {
    ...
    profiler.benchmark("Single-threaded", [&]() {
        for (size_t i = 0; i < dataSize; ++i) {
            // Simulate some processing (square root approximation)
            uint8_t value = testData[i];
            result[i] = static_cast<uint8_t>(std::sqrt(value * value * 0.5));
        }
    }, 10);
    
    // Multi-threaded processing
    profiler.benchmark("Multi-threaded", [&]() {
        std::vector<std::future<void>> futures;
        size_t chunkSize = dataSize / numThreads;
        
        for (size_t t = 0; t < numThreads; ++t) {
            size_t start = t * chunkSize;
            size_t end = (t == numThreads - 1) ? dataSize : start + chunkSize;
    ...
    profiler.benchmark("Work Stealing", [&]() {
    ...
    profiler.benchmark("Thread Creation", [&]() {
...
void demonstrateAlgorithmOptimization() {
    ...
    // Loop unrolling
    profiler.benchmark("Unrolled Loop", [&]() {
        uint64_t sum = 0;
        size_t i = 0;
    ...
    profiler.benchmark("Predictable Branches", [&]() {
        uint64_t sum = 0;
        for (size_t i = 0; i < dataSize; ++i) {
    ...
    profiler.benchmark("Unpredictable Branches", [&]() {
        uint64_t sum = 0;
        for (size_t i = 0; i < dataSize; ++i) {
    ...
    // Branch-free version
    profiler.benchmark("Branch-free", [&]() {
        uint64_t sum = 0;
        for (size_t i = 0; i < dataSize; ++i) {
    ...
    profiler.benchmark("Array of Structures", [&]() {
        float sum = 0.0f;
    ...
    profiler.benchmark("Structure of Arrays", [&]() {
        float sum = 0.0f;
        for (float x : soa.x) {
...
void demonstrateProfilingTechniques() {
    ...
    std::cout << "  Memory allocated: ~" << (memoryTest.size() * memoryTest[0].size()) << " bytes\n";
    ...
    std::cout << "  L1 cache friendly (1KB): " << duration_l1.count() << " μs\n";
    std::cout << "  L3 cache unfriendly (64KB): " << duration_l3.count() << " μs\n";
    std::cout << "  Cache miss penalty: " << std::fixed << std::setprecision(2) 
             << (static_cast<double>(duration_l3.count()) / duration_l1.count()) << "x\n";
...
int main() {
    ...
    std::cout << "This example demonstrates performance optimization techniques and benchmarking\n";
    ...
    std::cout << "- Memory optimization (alignment, prefetching, access patterns)\n";
    std::cout << "- SIMD vectorization (SSE2, AVX2)\n";
    std::cout << "- Multi-threading optimization (work stealing, thread pools)\n";
    std::cout << "- Algorithm optimization (loop unrolling, branch prediction)\n";
