...
public:
     ...
     */
    static vector<float> extractHistogram(int width, int height,
                                          const string& image_type) {
        cout << "Extracting histogram features from " << image_type
             << " image (" << width << "x" << height << ")..." << endl;
     ...
     */
    static vector<float> extractTextureFeatures(int width, int height,
                                                const string& texture_type) {
        cout << "Extracting texture features (" << texture_type << ")..."
             << endl;
     ...
     */
    static vector<float> extractShapeFeatures(int width, int height,
...
public:
    ...
    void train() {
        ...
        for (const auto& config : training_configs) {
            for (int i = 0; i < 10; ++i) {  // 10 samples per configuration
                ...
                features.histogram = FeatureExtractor::extractHistogram(
                    640, 480, get<0>(config));
                features.texture_features =
                    FeatureExtractor::extractTextureFeatures(640, 480,
                                                             get<1>(config));
                features.shape_features =
                    FeatureExtractor::extractShapeFeatures(640, 480,
                                                           get<2>(config));

                // Assign label based on brightness and texture
                if (get<0>(config) == "bright" && get<1>(config) == "smooth") {
                    features.label = "bright_smooth";
                } else if (get<0>(config) == "dark" &&
                           get<1>(config) == "rough") {
        ...
        cout << "  Trained with " << training_data_.size() << " samples"
             << endl;
        cout << "  Classes: ";
     ...
     */
    float getConfidence(const ImageFeatures& features,
                        const string& predicted_class) {
        vector<float> distances;

        for (const auto& training_sample : training_data_) {
            if (training_sample.label == predicted_class) {
                distances.push_back(
                    calculateDistance(features, training_sample));
            }
        }
        ...
        float avg_distance =
            accumulate(distances.begin(), distances.end(), 0.0f) /
            distances.size();
    ...
    static void evaluateClassifier(ImageClassifier& classifier) {
        ...
        for (const auto& test_case : test_cases) {
            for (int i = 0; i < 5; ++i) {  // 5 samples per test case
                ...
                test_features.histogram = FeatureExtractor::extractHistogram(
                    640, 480, get<0>(test_case));
                test_features.texture_features =
                    FeatureExtractor::extractTextureFeatures(640, 480,
                                                             get<1>(test_case));
                test_features.shape_features =
                    FeatureExtractor::extractShapeFeatures(640, 480,
                                                           get<2>(test_case));
                ...
                float confidence =
                    classifier.getConfidence(test_features, predicted);
        ...
        cout << "  Correct predictions: " << correct_predictions << "/"
             << total_predictions << endl;
...
void demonstrateMLProcessing() {
    ...
    sample_features.histogram =
        FeatureExtractor::extractHistogram(640, 480, "bright");
    sample_features.texture_features =
        FeatureExtractor::extractTextureFeatures(640, 480, "smooth");
    sample_features.shape_features =
        FeatureExtractor::extractShapeFeatures(640, 480, "circular");

    cout << "  Total feature vector size: "
         << (sample_features.histogram.size() +
             sample_features.texture_features.size() +
             sample_features.shape_features.size())
         << endl;
    ...
    float confidence =
        classifier.getConfidence(sample_features, predicted_class);
