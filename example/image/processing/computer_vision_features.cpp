...
std::vector<cv::Mat> createTestImages() {
    ...
    // Add rectangles
    cv::rectangle(shapes, cv::Point(50, 50), cv::Point(150, 150),
                  cv::Scalar(0, 255, 0), -1);
    cv::rectangle(shapes, cv::Point(200, 100), cv::Point(300, 200),
                  cv::Scalar(255, 0, 0), -1);
    ...
    std::vector<cv::Point> triangle1 = {
        cv::Point(350, 250), cv::Point(400, 350), cv::Point(300, 350)};
    cv::fillPoly(shapes, std::vector<std::vector<cv::Point>>{triangle1},
                 cv::Scalar(255, 0, 255));
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        cv::line(texture, cv::Point(center.x - 15, center.y),
                 cv::Point(center.x + 15, center.y), cv::Scalar(0, 0, 0), 2);
        cv::line(texture, cv::Point(center.x, center.y - 15),
                 cv::Point(center.x, center.y + 15), cv::Scalar(0, 0, 0), 2);
    }
    ...
    // Face outline
    cv::ellipse(face, cv::Point(100, 100), cv::Size(80, 100), 0, 0, 360,
                cv::Scalar(200, 160, 130), -1);
    ...
    // Nose
    cv::line(face, cv::Point(100, 90), cv::Point(100, 110),
             cv::Scalar(180, 140, 110), 2);

    // Mouth
    cv::ellipse(face, cv::Point(100, 130), cv::Size(20, 10), 0, 0, 180,
                cv::Scalar(150, 50, 50), 2);
...
void demonstrateFeatureDetection() {
    ...
    try {
        ...
        cv::Mat image2;
        cv::Mat transform =
            cv::getRotationMatrix2D(cv::Point2f(200, 150), 15, 0.9);
        ...
        for (const auto& [name, detector] : detectors) {
            ...
            try {
                ...
                std::cout << "    Image 2 keypoints: " << keypoints2.size()
                          << "\n";
                std::cout << "    Detection time: " << detectionTime.count()
                          << " μs\n";
                ...
                std::cout << "    Descriptor computation time: "
                          << descriptorTime.count() << " μs\n";
                ...
                if (!descriptors1.empty() && !descriptors2.empty()) {
                    ...
                    auto matchingTime = duration_cast<microseconds>(
                        steady_clock::now() - start);

                    // Filter good matches
                    std::sort(matches.begin(), matches.end());
                    const int numGoodMatches =
                        std::min(50, static_cast<int>(matches.size() * 0.3));
                    std::vector<cv::DMatch> goodMatches(
                        matches.begin(), matches.begin() + numGoodMatches);

                    std::cout << "    Total matches: " << matches.size()
                              << "\n";
                    std::cout << "    Good matches: " << goodMatches.size()
                              << "\n";
                    std::cout << "    Matching time: " << matchingTime.count()
                              << " μs\n";
                    ...
                    if (!goodMatches.empty()) {
                        ...
                        std::cout
                            << "    Average match distance: " << std::fixed
                            << std::setprecision(2) << avgDistance << "\n";
                    }
                }
...
void demonstrateObjectDetection() {
    ...
    try {
        ...
        cv::HoughCircles(gray, circles, cv::HOUGH_GRADIENT, 1, 50, 100, 30, 10,
                         100);
        ...
        for (size_t i = 0; i < circles.size(); ++i) {
            ...
            std::cout << "    Circle " << i << ": center(" << center.x << ","
        ...
        std::cout << "    Best match location: (" << maxLoc.x << "," << maxLoc.y
                  << ")\n";
        std::cout << "    Match confidence: " << std::fixed
                  << std::setprecision(3) << maxVal << "\n";
        std::cout << "    Template matching time: " << templateTime.count()
                  << " μs\n";
        ...
        for (size_t i = 0; i < keypoints.size(); ++i) {
            ...
            std::cout << "    Blob " << i << ": center(" << std::fixed
                      << std::setprecision(1) << kp.pt.x << "," << kp.pt.y
                      << "), size=" << kp.size << "\n";
        }
    ...
    try {
        ...
        try {
            ...
            if (cascadeLoaded) {
                ...
                std::cout << "    Haar cascade file not found (this is "
        ...
        // Add mouth pattern
        cv::ellipse(faceTemplate, cv::Point(30, 45), cv::Size(8, 4), 0, 0, 180,
                    cv::Scalar(100), 1);
        ...
        cv::Mat result;
        cv::matchTemplate(grayImage, faceTemplate, result,
                          cv::TM_CCOEFF_NORMED);
        ...
        try {
            ...
            if (eyeCascadeLoaded) {
                ...
                eyeCascade.detectMultiScale(grayImage, eyes, 1.1, 3, 0,
                                            cv::Size(10, 10));
                ...
                std::cout << "    Detection time: " << eyeTime.count()
                          << " μs\n";
                ...
                std::cout << "    Eye cascade file not found (this is expected "
                             "in test environment)\n";
            }
...
void demonstrateOpticalFlow() {
    ...
    try {
        ...
        // Add moving objects
        cv::circle(frame1, cv::Point(100, 100), 20, cv::Scalar(255, 255, 255),
                   -1);
        cv::circle(frame2, cv::Point(120, 110), 20, cv::Scalar(255, 255, 255),
                   -1);  // Moved

        cv::rectangle(frame1, cv::Point(200, 150), cv::Point(250, 200),
                      cv::Scalar(0, 255, 0), -1);
        cv::rectangle(frame2, cv::Point(210, 140), cv::Point(260, 190),
                      cv::Scalar(0, 255, 0), -1);  // Moved
        ...
        if (!points1.empty()) {
            ...
            cv::calcOpticalFlowPyrLK(gray1, gray2, points1, points2, status,
                                     errors);
            ...
            std::cout << "    Successfully tracked: " << trackedPoints
                      << " points\n";
            std::cout << "    Average motion: " << std::fixed
                      << std::setprecision(2)
                      << (trackedPoints > 0 ? totalMotion / trackedPoints : 0.0)
                      << " pixels\n";
        ...
        cv::Mat flow;
        cv::calcOpticalFlowFarneback(gray1, gray2, flow, 0.5, 3, 15, 3, 5, 1.2,
                                     0);
        ...
        double avgMagnitude =
            sqrt(meanFlow[0] * meanFlow[0] + meanFlow[1] * meanFlow[1]);

        // Find maximum flow
        cv::Mat magnitude, angle;
        cv::cartToPolar(flowComponents[0], flowComponents[1], magnitude, angle,
                        true);
        ...
        std::cout << "    Average flow magnitude: " << std::fixed
                  << std::setprecision(2) << avgMagnitude << " pixels\n";
        std::cout << "    Maximum flow magnitude: " << maxMag << " pixels\n";
        std::cout << "    Dense flow time: " << denseFlowTime.count()
                  << " ms\n";
        ...
        std::cout << "    Background subtraction time: " << bgSubTime.count()
                  << " μs\n";
    ...
    try {
        ...
        for (size_t i = 0; i < std::min(size_t(5), contours.size()); ++i) {
            if (cv::contourArea(contours[i]) > 100) {
                ...
                std::cout << "    Contour " << i << ":\n";
                std::cout << "      Centroid: (" << std::fixed
                          << std::setprecision(1) << cx << "," << cy << ")\n";
                std::cout << "      Area: " << area << "\n";
                std::cout << "      Perimeter: " << perimeter << "\n";
                std::cout << "      Circularity: " << std::setprecision(3)
                          << circularity << "\n";
            }
        }
        ...
        for (int y = 1; y < gray.rows - 1; ++y) {
            for (int x = 1; x < gray.cols - 1; ++x) {
                ...
                // Check 8 neighbors
                if (gray.at<uint8_t>(y - 1, x - 1) >= center)
                    code |= 1;
                if (gray.at<uint8_t>(y - 1, x) >= center)
                    code |= 2;
                if (gray.at<uint8_t>(y - 1, x + 1) >= center)
                    code |= 4;
                if (gray.at<uint8_t>(y, x + 1) >= center)
                    code |= 8;
                if (gray.at<uint8_t>(y + 1, x + 1) >= center)
                    code |= 16;
                if (gray.at<uint8_t>(y + 1, x) >= center)
        ...
        std::cout << "    Most frequent pattern: " << maxLoc.y
                  << " (count: " << maxVal << ")\n";
        std::cout << "    Texture uniformity: " << std::setprecision(3)
        ...
        cv::calcHist(&bgr_planes[0], 1, 0, cv::Mat(), hist_b, 1, &histSize,
                     &histRange);
        cv::calcHist(&bgr_planes[1], 1, 0, cv::Mat(), hist_g, 1, &histSize,
                     &histRange);
        cv::calcHist(&bgr_planes[2], 1, 0, cv::Mat(), hist_r, 1, &histSize,
                     &histRange);
        cv::calcHist(&hsv_planes[0], 1, 0, cv::Mat(), hist_h, 1, &histSize,
                     &histRange);
        ...
        std::cout << "    Color analysis time: " << colorTime.count()
                  << " μs\n";
        std::cout << "    Mean BGR: (" << std::fixed << std::setprecision(1)
                  << meanBGR[0] << "," << meanBGR[1] << "," << meanBGR[2]
                  << ")\n";
        std::cout << "    Mean HSV: (" << meanHSV[0] << "," << meanHSV[1] << ","
                  << meanHSV[2] << ")\n";
...
int main() {
    ...
    std::cout
        << "This example demonstrates advanced computer vision operations\n";
