...
std::vector<cv::Mat> createMLTestImages() {
    ...
    cv::rectangle(shapes, cv::Point(80, 80), cv::Point(144, 144),
                  cv::Scalar(0, 255, 0), 3);
    ...
    for (int y = 0; y < texture.rows; y += 10) {
        cv::line(texture, cv::Point(0, y), cv::Point(texture.cols, y),
                 cv::Scalar(0, 0, 0), 1);
    }
    ...
    for (int y = 0; y < gradient.rows; ++y) {
        for (int x = 0; x < gradient.cols; ++x) {
            uint8_t value = static_cast<uint8_t>(
                (x + y) * 255 / (gradient.rows + gradient.cols));
            gradient.at<cv::Vec3b>(y, x) =
                cv::Vec3b(value, value / 2, 255 - value);
        }
    }
    ...
    try {
        ...
        for (const auto& [format, extension] : modelFormats) {
            ...
            try {
                ...
                std::this_thread::sleep_for(
                    std::chrono::milliseconds(10));  // Simulate loading
                auto loadTime =
                    duration_cast<milliseconds>(steady_clock::now() - start);

                std::cout << "Supported (simulated load time: "
                          << loadTime.count() << "ms)\n";
        ...
        for (size_t i = 0; i < testImages.size(); ++i) {
            ...
            cv::Mat blob = cv::dnn::blobFromImage(
                processed, 1.0, cv::Size(224, 224), cv::Scalar(), true, false);
            ...
            std::cout << "      Preprocessing time: " << preprocessTime.count()
                      << " μs\n";
            ...
            std::cout << "      Inference time: " << inferenceTime.count()
                      << " μs\n";
            std::cout << "      Top prediction: class " << maxLoc.x
        ...
        cv::Mat batchBlob = cv::dnn::blobFromImages(
            batchImages, 1.0 / 255.0, cv::Size(224, 224),
        ...
        std::cout << "    Batch preprocessing time: " << batchTime.count()
                  << " μs\n";
        std::cout << "    Time per image: "
                  << (batchTime.count() / testImages.size()) << " μs\n";
...
void demonstrateImageClassification() {
    ...
    try {
        ...
        std::vector<std::string> classLabels = {
            "geometric_shape", "texture_pattern", "gradient", "noise",
            "object",          "animal",          "vehicle",  "building",
            "nature",          "abstract"};
        ...
        for (size_t i = 0; i < testImages.size(); ++i) {
            ...
            cv::calcHist(&gray, 1, 0, cv::Mat(), lbp_hist, 1, &histSize,
                         &histRange);
            ...
            double edgeDensity =
                cv::sum(edges)[0] / (edges.rows * edges.cols * 255.0);
            ...
            std::cout << "    Feature extraction time: " << featureTime.count()
                      << " μs\n";
            std::cout << "    Classification time: "
                      << classificationTime.count() << " μs\n";
            std::cout << "    Edge density: " << std::fixed
                      << std::setprecision(4) << edgeDensity << "\n";

            std::cout << "    Top 3 predictions:\n";
            for (int k = 0; k < 3 && k < predictions.size(); ++k) {
                std::cout << "      " << (k + 1) << ". "
                          << classLabels[predictions[k].second] << " ("
        ...
        for (size_t i = 0; i < testImages.size(); ++i) {
            ...
            for (const auto& scale : scales) {
                ...
                for (int y = 0; y <= scale.height - windowSize; y += stride) {
                    for (int x = 0; x <= scale.width - windowSize;
                         x += stride) {
                        ...
                        // Simple heuristic: higher variance = more likely to
                        // contain object
                        if (variance > 1000) {
                ...
                std::cout << "    Scale " << scale << ": " << proposals.size()
                          << " proposals\n";
            }
            ...
            for (size_t j = 0; j < indices.size(); ++j) {
                ...
                std::cout << "      " << objectClasses[classIds[idx]] << " at "
                          << box << " (conf: " << std::fixed
                          << std::setprecision(3) << confidences[idx] << ")\n";
            }
        }
...
void demonstrateImageSegmentation() {
    ...
    try {
        ...
        std::vector<std::string> segClasses = {"background", "object", "edge",
                                               "texture", "noise"};
        ...
        for (size_t i = 0; i < testImages.size(); ++i) {
            ...
            std::cout << "    Segmentation time: " << segmentationTime.count()
                      << " ms\n";
            std::cout << "    Average confidence: " << std::fixed
                      << std::setprecision(3) << avgConfidence << "\n";
            ...
            std::cout << "    Segmentation mask created: "
                      << visualization.size() << "\n";
        }
...
void demonstrateMLFrameworks() {
    ...
    std::vector<std::pair<std::string, int>> frameworks = {{"TensorFlow", 150},
                                                           {"PyTorch", 120},
                                                           {"ONNX Runtime", 80},
                                                           {"OpenCV DNN", 200}};
    ...
    for (const auto& [name, baseTime] : frameworks) {
        ...
        for (int batchSize : batchSizes) {
            ...
            double efficiency =
                1.0 + (batchSize - 1) * 0.7;  // Diminishing returns
            int totalTime = static_cast<int>(baseTime * batchSize / efficiency);
            double timePerImage = static_cast<double>(totalTime) / batchSize;

            std::cout << "    Batch " << batchSize << ": " << totalTime
                      << "ms total, " << std::fixed << std::setprecision(1)
                      << timePerImage << "ms per image\n";
        }
    }
}
...
void demonstrateMLOptimization() {
    ...
    std::vector<std::pair<std::string, double>> precisions = {
        {"FP32", 1.0}, {"FP16", 0.6}, {"INT8", 0.3}, {"INT4", 0.15}};

    for (const auto& [precision, relativeTime] : precisions) {
        int inferenceTime = static_cast<int>(100 * relativeTime);
        double accuracy =
            0.95 - (1.0 - relativeTime) * 0.1;  // Simulate accuracy loss

        std::cout << "    " << precision << ": " << inferenceTime
                  << "ms, accuracy: " << std::fixed << std::setprecision(3)
                  << accuracy << "\n";
    }
    ...
    for (int batchSize : batchSizes) {
        ...
        double efficiency = std::min(1.0 + (batchSize - 1) * 0.8,
                                     static_cast<double>(batchSize));
        int totalTime =
            static_cast<int>(baseLatency * batchSize / efficiency * batchSize);
        double throughput =
            (batchSize * 1000.0) / totalTime;  // images per second

        std::cout << "    Batch " << batchSize << ": " << std::fixed
                  << std::setprecision(1) << throughput << " images/sec\n";
    }
...
int main() {
    ...
    std::cout << "This example demonstrates machine learning integration for "
                 "image processing\n";
