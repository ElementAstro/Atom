...
void demonstrateGPUDetection() {
    ...
    try {
        ...
        std::vector<std::string> backendNames = {
            "CUDA", "OpenCL", "Vulkan", "Metal", "Auto-select"
        };
            ...
            if (isAvailable) {
                try {
                    ...
                    for (size_t j = 0; j < devices.size(); ++j) {
                        ...
                        std::cout << "      Memory: " << (device.memorySize / (1024 * 1024)) << " MB\n";
                        std::cout << "      Compute Units: " << device.computeUnits << "\n";
                        std::cout << "      Max Work Group Size: " << device.maxWorkGroupSize << "\n";
                        std::cout << "      Double Precision: " << (device.supportsDouble ? "YES" : "NO") << "\n";
                        std::cout << "      Half Precision: " << (device.supportsHalf ? "YES" : "NO") << "\n";
                    }
        ...
        std::cout << "\nOptimal backend: ";
        switch (optimal) {
            case GPUBackend::CUDA: std::cout << "CUDA"; break;
            case GPUBackend::OPENCL: std::cout << "OpenCL"; break;
            case GPUBackend::VULKAN: std::cout << "Vulkan"; break;
            case GPUBackend::METAL: std::cout << "Metal"; break;
            case GPUBackend::AUTO: std::cout << "Auto-select"; break;
...
void demonstrateGPUProcessing() {
    ...
    try {
        ...
        std::cout << "Created test image: " << testImage.getWidth() << "x" << testImage.getHeight() << "\n";
        
        // Upload image to GPU
        auto start = high_resolution_clock::now();
        auto gpuBuffer = gpuProcessor.uploadImage(testImage);
        auto uploadTime = duration_cast<microseconds>(high_resolution_clock::now() - start);
        ...
        std::cout << "Image uploaded to GPU in " << uploadTime.count() << " μs\n";
        ...
        for (const auto& operation : operations) {
            ...
            try {
                ...
                if (operation == "gaussian_blur") {
                    result = gpuProcessor.gaussianBlur(*gpuBuffer, 2.0f, 7, 
                                                     testImage.getWidth(), testImage.getHeight(), 3);
                } else if (operation == "resize") {
                    result = gpuProcessor.resize(*gpuBuffer, testImage.getWidth(), testImage.getHeight(),
                                               512, 384, 3);
                } else if (operation == "histogram_equalization") {
                    result = gpuProcessor.equalizeHistogram(*gpuBuffer, testImage.getWidth(), 
                ...
                auto processingTime = duration_cast<microseconds>(high_resolution_clock::now() - start);
                
                if (result) {
                    std::cout << "  Processing time: " << processingTime.count() << " μs\n";
                    std::cout << "  Result buffer size: " << result->getSize() << " bytes\n";
                    
                    // Download result for verification
                    start = high_resolution_clock::now();
                    std::vector<uint8_t> resultData(result->getSize());
                    bool downloaded = result->download(resultData.data(), resultData.size());
                    auto downloadTime = duration_cast<microseconds>(high_resolution_clock::now() - start);
                    
                    if (downloaded) {
                        std::cout << "  Download time: " << downloadTime.count() << " μs\n";
                        std::cout << "  Total GPU time: " << (processingTime.count() + downloadTime.count()) << " μs\n";
                    } else {
    ...
    try {
        ...
        for (const auto& [width, height] : imageSizes) {
            ...
            for (int i = 0; i < iterations; ++i) {
                auto result = cpuProcessor.applyFilter(testImage, FilterType::GAUSSIAN_BLUR, 
                                                     {{"sigma", 2.0}, {"kernel_size", 7}});
            }
            auto cpuTime = duration_cast<milliseconds>(high_resolution_clock::now() - start);
            double avgCpuTime = static_cast<double>(cpuTime.count()) / iterations;
            
            std::cout << "  CPU average time: " << std::fixed << std::setprecision(2) << avgCpuTime << " ms\n";
            ...
            if (gpuAvailable) {
                try {
                    ...
                    if (gpuBuffer) {
                        start = high_resolution_clock::now();
                        for (int i = 0; i < iterations; ++i) {
                            auto result = gpuProcessor.gaussianBlur(*gpuBuffer, 2.0f, 7, width, height, 3);
                        }
                        auto gpuTime = duration_cast<milliseconds>(high_resolution_clock::now() - start);
                        double avgGpuTime = static_cast<double>(gpuTime.count()) / iterations;
                        
                        std::cout << "  GPU average time: " << avgGpuTime << " ms\n";
            ...
            double cpuThroughput = (pixels * iterations) / (cpuTime.count() / 1000.0); // pixels/second
            std::cout << "  CPU throughput: " << (cpuThroughput / 1000000.0) << " Mpixels/s\n";
        }
...
void demonstrateFallbackMechanisms() {
    ...
    try {
        ...
        // Create a processing function that tries GPU first, then falls back to CPU
        auto processImageWithFallback = [](const blob& input, const std::string& operation) -> blob {
            // Try GPU first
            GPUImageProcessor gpuProcessor;
            if (gpuProcessor.initialize()) {
                ...
                try {
                    ...
                    if (gpuBuffer) {
                        ...
                        if (result) {
                            std::vector<uint8_t> resultData(result->getSize());
                            if (result->download(resultData.data(), resultData.size())) {
                                std::cout << "  GPU processing successful\n";
                                return blob(resultData.data(), resultData.size());
                            }
                        }
                    }
                } catch (const std::exception& e) {
            ...
            if (operation == "blur") {
                return cpuProcessor.applyFilter(input, FilterType::GAUSSIAN_BLUR, 
                                               {{"sigma", 2.0}, {"kernel_size", 7}});
            }
        ...
        auto totalTime = duration_cast<milliseconds>(high_resolution_clock::now() - start);
        ...
        try {
            ...
            if (errorProcessor.initialize()) {
                ...
                if (buffer) {
                    ...
                    auto badResult = errorProcessor.gaussianBlur(*buffer, -1.0f, 0, 0, 0, 0);
                    if (!badResult) {
...
void demonstrateBatchProcessing() {
    ...
    try {
        ...
        auto uploadTime = duration_cast<milliseconds>(high_resolution_clock::now() - start);
        
        std::cout << "Uploaded " << gpuBuffers.size() << " images in " << uploadTime.count() << " ms\n";
        
        // Process batch
        start = high_resolution_clock::now();
        
        std::vector<std::unique_ptr<GPUBuffer>> results = gpuProcessor.batchProcess(
            gpuBuffers, "gaussian_blur", {{"sigma", 2.0f}, {"kernel_size", 7}});
        
        auto processingTime = duration_cast<milliseconds>(high_resolution_clock::now() - start);
        
        std::cout << "Processed " << results.size() << " images in " << processingTime.count() << " ms\n";
        std::cout << "Average processing time per image: " << 
                     (static_cast<double>(processingTime.count()) / results.size()) << " ms\n";
        ...
        auto downloadTime = duration_cast<milliseconds>(high_resolution_clock::now() - start);
        
        std::cout << "Downloaded " << outputImages.size() << " results in " << downloadTime.count() << " ms\n";
        std::cout << "Total batch processing time: " << 
                     (uploadTime.count() + processingTime.count() + downloadTime.count()) << " ms\n";
...
int main() {
    ...
    std::cout << "This example demonstrates GPU-accelerated image processing with fallbacks\n";
