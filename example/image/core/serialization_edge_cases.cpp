...
void testBasicSerialization() {
    ...
    for (size_t size : testSizes) {
        ...
        try {
            ...
            auto serializeTime = duration_cast<microseconds>(
                high_resolution_clock::now() - start);

            std::cout << "  Serialized size: " << serializedData.size()
                      << " bytes\n";
            std::cout << "  Serialize time: " << serializeTime.count()
                      << " μs\n";

            // Deserialize
            start = high_resolution_clock::now();
            auto deserializedBlob = blob::deserialize(serializedData);
            auto deserializeTime = duration_cast<microseconds>(
                high_resolution_clock::now() - start);

            std::cout << "  Deserialize time: " << deserializeTime.count()
                      << " μs\n";
            ...
            if (isIdentical && originalBlob.size() > 0) {
                isIdentical =
                    std::memcmp(originalBlob.data(), deserializedBlob.data(),
                                originalBlob.size()) == 0;
            }
            ...
            if (size > 0) {
                double compressionRatio =
                    static_cast<double>(serializedData.size()) / size;
...
void testCorruptionHandling() {
    ...
    std::cout << "Original serialized size: " << serializedData.size()
              << " bytes\n";

    // Test various corruption scenarios
    std::vector<std::string> corruptionTypes = {
        "Truncated data", "Modified header", "Modified payload", "Extra data",
    ...
    for (size_t i = 0; i < corruptionTypes.size(); ++i) {
        ...
        switch (i) {
            ...
            case 2:  // Modified payload
                if (corruptedData.size() > 20) {
                    for (size_t j = 10;
                         j < std::min(corruptedData.size(), size_t(20)); ++j) {
        ...
        try {
            ...
            std::cout << "  Deserialized size: " << corruptedBlob.size()
                      << " bytes\n";
        } catch (const std::exception& e) {
    ...
    for (size_t size : sizes) {
        ...
        try {
            ...
            double avgSerializeTime =
                static_cast<double>(serializeTime.count()) / iterations;
            ...
            auto deserializeTime = duration_cast<microseconds>(
                high_resolution_clock::now() - start);
            double avgDeserializeTime =
                static_cast<double>(deserializeTime.count()) / iterations;

            // Calculate throughput
            double serializeThroughput =
                (size / 1024.0 / 1024.0) / (avgSerializeTime / 1000000.0);
            double deserializeThroughput =
                (size / 1024.0 / 1024.0) / (avgDeserializeTime / 1000000.0);

            std::cout << "  Serialize time: " << std::fixed
                      << std::setprecision(2) << avgSerializeTime << " μs\n";
            std::cout << "  Deserialize time: " << avgDeserializeTime
                      << " μs\n";
            std::cout << "  Serialize throughput: " << serializeThroughput
                      << " MB/s\n";
            std::cout << "  Deserialize throughput: " << deserializeThroughput
                      << " MB/s\n";
            std::cout << "  Serialized size: " << lastSerialized.size()
                      << " bytes\n";
...
void testCrossPlatformCompatibility() {
    ...
    for (const auto& [name, data] : testPatterns) {
        ...
        try {
            ...
            bool isIdentical =
                (originalBlob.size() == deserialized.size()) &&
                (std::memcmp(originalBlob.data(), deserialized.data(),
                             originalBlob.size()) == 0);

            std::cout << "  Round-trip test: "
                      << (isIdentical ? "PASS" : "FAIL") << "\n";
            ...
            if (outFile) {
                ...
                if (inFile) {
                    ...
                    bool fileIdentical =
                        (originalBlob.size() == fileDeserialized.size()) &&
                        (std::memcmp(originalBlob.data(),
                                     fileDeserialized.data(),
                                     originalBlob.size()) == 0);

                    std::cout << "  File I/O test: "
                              << (fileIdentical ? "PASS" : "FAIL") << "\n";
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                std::cout
                    << "  Recovery test: Successfully recovered after error\n";
            } catch (const std::exception& e2) {
                std::cout << "  Recovery test: Failed to recover - "
                          << e2.what() << "\n";
            }
        }
...
int main() {
    ...
    std::cout << "This example demonstrates edge case handling for blob "
                 "serialization\n";
