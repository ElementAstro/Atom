...
public:
    ...
    void put(const Key& key, const Value& value, size_t valueSize = 0) {
        ...
        // Evict if necessary
        while ((cacheList_.size() > maxSize_ || currentMemory_ > maxMemory_) &&
               !cacheList_.empty()) {
            evictLRU();
        }
    }
        ...
        for (const auto& key : keys) {
            ...
            if (!get(key, dummy)) {
                ...
                try {
                    ...
                    put(key, value,
                        sizeof(Value));  // Simplified size calculation
                } catch (const std::exception& e) {
                    std::cerr << "Prefetch failed for key: " << key << " - "
                              << e.what() << "\n";
                }
            }
        }
...
public:
    ...
    Statistics getStatistics() const {
        ...
        stats.avgFrequency =
            cache_.empty() ? 0.0
                           : (totalFreq / static_cast<double>(cache_.size()));
        ...
        auto minIt = std::min_element(
            cache_.begin(), cache_.end(), [](const auto& a, const auto& b) {
                ...
                return a.second.lastAccess <
                       b.second.lastAccess;  // Tie-breaker: older access
            });
    ...
    Statistics getStatistics() const {
        ...
        if (totalRequests > 0) {
            ...
            stats.overallHitRate =
                ((stats.l1Hits + stats.l2Hits) * 100.0) / totalRequests;
        } else {
...
public:
    void recordAccess(const std::string& key) {
        ...
        if (!lastKey.empty() && lastKey != key) {
            ...
            if (accessPatterns_[lastKey].size() > 10) {
                accessPatterns_[lastKey].erase(
                    accessPatterns_[lastKey].begin());
            }
        }
    ...
    std::vector<std::string> predictNext(const std::string& key,
                                         size_t maxPredictions = 3) const {
        std::lock_guard<std::mutex> lock(mutex_);
...
void demonstrateLRUCache() {
    ...
    try {
        ...
        std::cout << "Testing LRU cache with " << testData.size()
                  << " test items\n";
        ...
        std::cout << "  After random access (" << accessCount
                  << " requests):\n";
        ...
        std::cout << "    Avg time per access: "
                  << (phase2Time.count() / accessCount) << " μs\n";
...
void demonstrateLFUCache() {
    ...
    try {
        ...
        std::cout << "Testing LFU cache with " << testData.size()
                  << " test items\n";
        ...
        std::cout << "  Hits: " << stats.hits << ", Misses: " << stats.misses
                  << "\n";
        std::cout << "  Evictions: " << stats.evictions << "\n";
        std::cout << "  Average frequency: " << std::setprecision(2)
                  << stats.avgFrequency << "\n";
...
void demonstrateMultiLevelCache() {
    ...
    try {
        ...
        std::cout << "Testing multi-level cache with " << testData.size()
                  << " test items\n";
        ...
        std::cout << "  Access time: " << accessTime.count() << " μs\n";
        std::cout << "  Avg time per access: "
                  << (accessTime.count() / totalAccesses) << " μs\n";
...
void demonstratePredictivePrefetching() {
    ...
    try {
        ...
        std::cout << "  Training completed in " << trainingTime.count()
                  << " μs\n";
        std::cout << "  Training hit rate: " << std::fixed
                  << std::setprecision(1) << trainingStats.hitRate << "%\n";
            ...
            for (size_t i = 0; i < sequence.size(); ++i) {
                ...
                for (const auto& predictedKey : predictions) {
                    ...
                    if (!cache.get(predictedKey, prefetchValue)) {
                        ...
                        if (it != testData.end()) {
                            cache.put(predictedKey, it->second,
                                      it->second.size());
                            totalPrefetches++;
                        }
                    } else {
        ...
        std::cout << "  Prefetch accuracy: "
                  << (totalPrefetches > 0
                          ? (prefetchHits * 100.0 / totalPrefetches)
                          : 0.0)
                  << "%\n";
        ...
        std::cerr << "Error in predictive prefetching demonstration: "
                  << e.what() << "\n";
    }
}
...
void compareCachingStrategies() {
    ...
    try {
        ...
        std::cout << "Comparing caching strategies with " << cacheSize
                  << " cache slots\n";
        ...
        // Test LFU
        {
            ...
            std::cout << "  Avg frequency: " << std::setprecision(2)
                      << lfuStats.avgFrequency << "\n";
            std::cout << "  Time: " << lfuTime.count() << " μs\n";
        }

        std::cout << "\nConclusion: LFU typically performs better with "
                     "frequency-based access patterns,\n";
        std::cout
            << "while LRU performs better with temporal locality patterns.\n";
...
int main() {
    ...
    std::cout << "This example demonstrates advanced caching strategies and "
                 "optimizations\n";
    ...
    std::cout
        << "- LFU (Least Frequently Used) caching with frequency tracking\n";
