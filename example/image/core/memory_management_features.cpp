...
public:
    BlobMemoryPool(size_t poolSize = 100 * 1024 * 1024) : poolSize_(poolSize) {
        ...
        std::cout << "Memory pool created: " << (poolSize_ / (1024 * 1024))
                  << "MB\n";
    }
    ...
    void printStats() const {
        ...
        std::cout << "  Allocated: " << (getTotalAllocated() / (1024 * 1024))
                  << "MB\n";
...
private:
    ...
    void mergeAdjacentBlocks() {
        ...
        for (size_t i = 0; i < freeBlocks_.size() - 1;) {
            char* currentEnd =
                static_cast<char*>(freeBlocks_[i].ptr) + freeBlocks_[i].size;
...
void demonstrateMemoryPooling() {
    ...
    for (size_t size : sizes) {
        ...
        if (ptr) {
            ...
            std::cout << "  Allocated " << (size / (1024 * 1024)) << "MB at "
                      << ptr << "\n";
        } else {
            std::cout << "  Failed to allocate " << (size / (1024 * 1024))
                      << "MB\n";
        }
    }
...
void demonstrateCacheFriendlyLayouts() {
    ...
    // Row-major layout (cache-friendly for row access)
    {
        ...
        std::cout << "  Row-major sequential access: " << duration.count()
                  << " μs\n";
    }
    ...
    // Column access on row-major data (cache-unfriendly)
    {
        ...
        std::cout << "  Row-major column access: " << duration.count()
                  << " μs\n";
    }
        ...
        for (size_t by = 0; by < height; by += blockSize) {
            for (size_t bx = 0; bx < width; bx += blockSize) {
                for (size_t y = by; y < std::min(by + blockSize, height); ++y) {
                    for (size_t x = bx; x < std::min(bx + blockSize, width);
                         ++x) {
                        for (size_t c = 0; c < channels; ++c) {
                            size_t index = (y * width + x) * channels + c;
                            tiledData[index] =
...
void demonstrateLargeImageHandling() {
    ...
    std::cout << "Simulating large image: " << largeWidth << "x" << largeHeight
              << "x" << channels << "\n";
    ...
    std::cout << "\n1. Streaming Processing:\n";
    {
        ...
        for (size_t y = 0; y < largeHeight; y += stripHeight) {
            ...
            size_t currentStripSize =
                largeWidth * currentStripHeight * channels;
        ...
        std::cout << "  Streaming processing time: " << duration.count()
                  << "ms\n";
        std::cout << "  Peak memory usage: ~" << (stripSize / (1024 * 1024))
                  << "MB\n";
    }
    ...
    std::cout << "\n2. Tiled Processing:\n";
    {
        ...
        std::cout << "  Processing in " << tileSize << "x" << tileSize
                  << " tiles\n";
        ...
        for (size_t ty = 0; ty < largeHeight; ty += tileSize) {
            for (size_t tx = 0; tx < largeWidth; tx += tileSize) {
                ...
                size_t currentTileSize =
                    currentTileWidth * currentTileHeight * channels;
        ...
        std::cout << "  Peak memory usage: ~" << (tileSizeBytes / 1024)
                  << "KB\n";
    }
}
