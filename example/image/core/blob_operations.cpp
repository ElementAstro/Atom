...
void demonstrateBasicOperations() {
    ...
    try {
        ...
        std::cout << "Created blob from data with " << dataImg.size()
                  << " bytes\n";

        // Basic blob operations
        std::cout << "Empty check: "
                  << (dataImg.isEmpty() ? "empty" : "not empty") << "\n";
...
void demonstrateSerialization() {
    ...
    try {
        ...
        std::cout << "Original image blob: " << originalImg.getCols() << "x"
                  << originalImg.getRows() << " with "
                  << originalImg.getChannels() << " channels\n";
        ...
        std::cout << "Restored image blob: " << restoredImg.getCols() << "x"
                  << restoredImg.getRows() << " with "
                  << restoredImg.getChannels() << " channels\n";
        ...
        if (dataMatches && originalImg.size() > 0) {
            ...
            size_t checkPoints[] = {0, originalImg.size() / 2,
                                    originalImg.size() - 1};
...
void demonstrateDataTypes() {
    ...
    try {
        ...
        // Note: All blobs store std::byte, but can conceptually represent
        // different data types
        ...
        blob img16u(reinterpret_cast<uint8_t*>(data16u.data()),
                    data16u.size() * sizeof(uint16_t));
        ...
        blob imgFloat(reinterpret_cast<uint8_t*>(dataFloat.data()),
                      dataFloat.size() * sizeof(float));
        ...
        std::cout
            << "Conceptual data types demonstrated with different blob sizes\n";
        std::cout << "Each blob can represent different data types through "
                     "reinterpretation\n";
