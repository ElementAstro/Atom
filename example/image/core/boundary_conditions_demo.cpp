...
void testZeroSizeConditions() {
    ...
    tester.runTest("Operations on empty blob", []() {
        ...
        try {
            auto result =
                processor.applyFilter(emptyBlob, FilterType::GAUSSIAN_BLUR, {});
            if (!result.isEmpty()) {
                throw std::runtime_error(
                    "Processing empty blob should return empty result");
            }
        } catch (const std::exception& e) {
    ...
    tester.runTest("Single pixel image", []() {
        ...
        try {
            auto result = processor.applyFilter(
                singlePixel, FilterType::GAUSSIAN_BLUR, {{"kernel_size", 1}});
...
void testMaximumSizeLimits() {
    ...
    tester.runTest("Very large blob creation (100MB)", []() {
        ...
        try {
            ...
            throw std::runtime_error(
                "Insufficient memory for very large blob (expected)");
        }
    });
    ...
    tester.runTest("Maximum size_t value", []() {
        ...
        try {
            ...
            throw std::runtime_error(
                "Should not be able to allocate max size_t bytes");
    ...
    tester.runTest("Large image dimensions", []() {
        ...
        size_t requiredSize =
            static_cast<size_t>(largeWidth) * largeHeight * channels;
    ...
    tester.runTest("Integer overflow in size calculation", []() {
        ...
        throw std::runtime_error(
            "Size calculation should have overflowed or been too large");
    });
...
void testExtremeParameters() {
    ...
    tester.runTest("Extreme Gaussian blur sigma", [&]() {
        ...
        try {
            auto result = processor.applyFilter(
                testImage, FilterType::GAUSSIAN_BLUR, {{"sigma", 1000.0}});
        ...
        try {
            auto result = processor.applyFilter(
                testImage, FilterType::GAUSSIAN_BLUR, {{"sigma", 0.0}});
        } catch (const std::exception& e) {
        ...
        try {
            auto result = processor.applyFilter(
                testImage, FilterType::GAUSSIAN_BLUR, {{"sigma", -1.0}});
        } catch (const std::exception& e) {
    ...
    tester.runTest("Extreme kernel sizes", [&]() {
        ...
        try {
            auto result = processor.applyFilter(
                testImage, FilterType::GAUSSIAN_BLUR, {{"kernel_size", 999}});
        } catch (const std::exception& e) {
        ...
        try {
            auto result = processor.applyFilter(
                testImage, FilterType::GAUSSIAN_BLUR, {{"kernel_size", 0}});
        } catch (const std::exception& e) {
        ...
        try {
            auto result = processor.applyFilter(
                testImage, FilterType::GAUSSIAN_BLUR, {{"kernel_size", 4}});
        } catch (const std::exception& e) {
    ...
    tester.runTest("Extreme brightness/contrast", [&]() {
        ...
        try {
            auto result =
                processor.adjustBrightnessContrast(testImage, 255.0, 0.0);
        ...
        try {
            auto result =
                processor.adjustBrightnessContrast(testImage, 0.0, 100.0);
        } catch (const std::exception& e) {
            // May reject extreme values
        }
    });
    ...
    tester.runTest("Extreme rotation angles", [&]() {
        ...
        try {
            auto result =
                processor.rotate(testImage, 3600.0);  // 10 full rotations
        ...
        try {
            auto result =
                processor.rotate(testImage, -720.0);  // -2 full rotations
        } catch (const std::exception& e) {
            // May reject or normalize
        }
    });
...
void testNumericalPrecision() {
    ...
    tester.runTest("Division by zero handling", []() {
        ...
        if (!std::isfinite(result2) || result2 < 1e100) {
            throw std::runtime_error(
                "Division by near-zero should produce large finite number");
        }
    });
    ...
    tester.runTest("Trigonometric edge cases", []() {
        ...
        if (!std::isfinite(sinResult) || std::abs(sinResult) > 1.0) {
            throw std::runtime_error(
                "Sin of large angle should be finite and in [-1,1]");
        }
    ...
    tester.runTest("Accumulation errors", []() {
        ...
        if (error > 1e-6) {  // Allow some accumulation error
            throw std::runtime_error("Accumulation error too large: " +
                                     std::to_string(error));
        }
    });
...
void testInvalidInputValidation() {
    ...
    tester.runTest("Null pointer handling", []() {
        try {
            ...
            throw std::runtime_error(
                "Should reject null pointer with non-zero size");
        } catch (const std::exception& e) {
            // Expected to fail
            std::string msg = e.what();
            if (msg.find("null") == std::string::npos &&
                msg.find("invalid") == std::string::npos) {
                throw std::runtime_error(
                    "Should provide meaningful error for null pointer");
            }
        }
    });
    ...
    tester.runTest("Mismatched size and data", []() {
        ...
        try {
            blob mismatchedBlob(data.data(),
                                200);  // Size larger than actual data
    ...
    tester.runTest("Corrupted data patterns", []() {
        ...
        try {
            auto result1 = processor.applyFilter(
                pattern1, FilterType::GAUSSIAN_BLUR, {{"sigma", 1.0}});
            auto result2 = processor.applyFilter(
                pattern2, FilterType::GAUSSIAN_BLUR, {{"sigma", 1.0}});
            auto result3 = processor.applyFilter(
                pattern3, FilterType::GAUSSIAN_BLUR, {{"sigma", 1.0}});
...
void testExtremePerformance() {
    ...
    tester.runTest("Performance with large data", []() {
        ...
        try {
            auto result = processor.applyFilter(
                largeBlob, FilterType::GAUSSIAN_BLUR, {{"sigma", 2.0}});
            ...
            if (duration.count() > 10000) {  // > 10 seconds
                throw std::runtime_error(
                    "Processing too slow: " + std::to_string(duration.count()) +
                    "ms");
            }

        } catch (const std::bad_alloc& e) {
            throw std::runtime_error(
                "Insufficient memory for large data processing");
        }
    });
    ...
    tester.runTest("Memory usage patterns", []() {
        ...
        try {
            ...
            for (const auto& blob : blobs) {
                try {
                    auto result = processor.applyFilter(
                        blob, FilterType::GAUSSIAN_BLUR, {{"sigma", 1.0}});
                } catch (const std::exception& e) {
    ...
    tester.runTest("Rapid successive operations", []() {
        ...
        for (int i = 0; i < 1000; ++i) {
            try {
                auto result = processor.applyFilter(
                    testBlob, FilterType::GAUSSIAN_BLUR, {{"sigma", 0.5}});
            } catch (const std::exception& e) {
        ...
        if (duration.count() > 5000) {  // > 5 seconds for 1000 operations
            throw std::runtime_error("Rapid operations too slow: " +
...
int main() {
    ...
    std::cout << "This example demonstrates handling of boundary conditions "
                 "and edge cases\n";
