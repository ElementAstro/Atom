...
void compareCreationPerformance() {
    ...
    for (size_t size : sizes) {
        ...
        std::cout << "  Normal blob (100 iterations): " << normalTime
                  << " μs\n";
        std::cout << "  Fast blob (100 iterations): " << fastTime << " μs\n";
        std::cout << "  Speed improvement: " << (normalTime / fastTime)
                  << "x\n";
        std::cout << "  Memory saved: " << (size / (1024 * 1024))
                  << "MB per fast_blob\n";
    }
}
...
void demonstrateReadOnlyOperations() {
    ...
    std::cout << "Dimensions: " << fastBlob.getWidth() << "x"
              << fastBlob.getHeight() << "\n";
    ...
    std::cout << "Sequential access (10K elements): " << sequentialTime
              << " μs\n";
...
void demonstrateMemoryUsage() {
    ...
    // Normal blob - copies data
    {
        ...
        std::cout << "  Memory usage: ~" << (dataSize * 2 / (1024 * 1024))
                  << "MB (original + copy)\n";
    ...
    // Fast blob - references data
    {
        ...
        std::cout << "  Memory usage: ~" << (dataSize / (1024 * 1024))
                  << "MB (original only)\n";
    ...
    std::cout << "  Total memory usage: ~" << (dataSize / (1024 * 1024))
              << "MB (shared)\n";
    std::cout << "  Equivalent normal blobs would use: ~"
              << (dataSize * fastBlobs.size() / (1024 * 1024)) << "MB\n";
}
...
void demonstrateBestPractices() {
    ...
    std::cout << "1. Use fast_blob for read-only operations:\n";
    {
        ...
        std::cout << "   Dimensions: " << readOnlyBlob.getWidth() << "x"
                  << readOnlyBlob.getHeight() << "\n";
    ...
    std::cout << "\n2. Ensure data lifetime exceeds fast_blob lifetime:\n";
    {
        ...
        auto localData = createTestData(1024);
        {
            ...
            std::cout
                << "   Safe usage: data available during fast_blob lifetime\n";
        }
    ...
    std::cout << "\n3. Use for large data processing pipelines:\n";
    {
        ...
        std::cout << "   Created " << views.size()
                  << " views with zero memory copy\n";
...
int main() {
    ...
    std::cout << "This example demonstrates fast_blob usage patterns and "
    ...
    std::cout << "- Significant performance and memory benefits for read-only "
                 "operations\n";
    std::cout
        << "- Ideal for data analysis, inspection, and temporary processing\n";
