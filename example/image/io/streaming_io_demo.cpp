...
void createLargeTestImage(const std::string& filename, int width, int height) {
    std::cout << "Creating large test image: " << filename << " (" << width << "x" << height << ")\n";
    ...
    try {
        ...
        for (int y = 0; y < height; y += tileHeight) {
            ...
            // Write tile
            writer.writeTile(tileData.data(), width * currentTileHeight * 3, 0, y, width, currentTileHeight);
        ...
        std::cout << "  Write speed: " << (fileSize / (1024.0 * 1024.0)) / (duration.count() / 1000.0) << " MB/s\n";
...
void demonstrateStreamingRead() {
    ...
    try {
        ...
        std::cout << "  Dimensions: " << properties.width << "x" << properties.height << "\n";
        std::cout << "  Channels: " << properties.channels << "\n";
        std::cout << "  Format: " << static_cast<int>(properties.format) << "\n";
        std::cout << "  Compressed: " << (properties.isCompressed ? "Yes" : "No") << "\n";
        std::cout << "  Tile size: " << properties.tileWidth << "x" << properties.tileHeight << "\n";
        
        // Test 1: Sequential tile reading
        std::cout << "\nTest 1: Sequential tile reading\n";
        
        const int tilesX = (properties.width + properties.tileWidth - 1) / properties.tileWidth;
        const int tilesY = (properties.height + properties.tileHeight - 1) / properties.tileHeight;
        ...
        std::cout << "  Read " << (tilesX * tilesY) << " tiles in " << duration.count() << "ms\n";
        std::cout << "  Total data: " << (totalBytesRead / (1024 * 1024)) << " MB\n";
        std::cout << "  Read speed: " << (totalBytesRead / (1024.0 * 1024.0)) / (duration.count() / 1000.0) << " MB/s\n";
        ...
        for (const auto& [tx, ty] : randomTiles) {
            ...
            if (tileData) {
                ...
                uint8_t checksum = 0;
                for (size_t i = 0; i < std::min(size_t(1000), tileData->size()); ++i) {
                    checksum ^= (*tileData)[i];
                }
            }
        }
        ...
        std::cout << "  Random access to " << randomTiles.size() << " tiles in " << duration.count() << "ms\n";
        std::cout << "  Average time per tile: " << (duration.count() / randomTiles.size()) << "ms\n";
        ...
        if (roiData) {
            std::cout << "  ROI (" << roiX << "," << roiY << " " << roiWidth << "x" << roiHeight << ") read in " << duration.count() << "ms\n";
            std::cout << "  ROI data size: " << (roiData->size() / (1024 * 1024)) << " MB\n";
        }
...
void demonstrateProgressiveLoading() {
    ...
    try {
        ...
        std::cout << "Progressive loading of " << properties.width << "x" << properties.height << " image\n";
        ...
        std::vector<std::string> qualityNames = {"Thumbnail", "Low", "Medium", "High"};
        
        for (int level = 0; level < 4; ++level) {
            std::cout << "\nLoading " << qualityNames[level] << " quality (level " << level << "):\n";
            ...
            if (progressiveImage) {
                ...
                std::cout << "  Dimensions: " << levelProps.width << "x" << levelProps.height << "\n";
                std::cout << "  Data size: " << (progressiveImage->size() / 1024) << " KB\n";
                std::cout << "  Load time: " << duration.count() << "ms\n";
                
                // Calculate compression ratio
                size_t fullSize = properties.width * properties.height * properties.channels;
                double compressionRatio = static_cast<double>(fullSize) / progressiveImage->size();
                std::cout << "  Compression ratio: " << std::fixed << std::setprecision(1) << compressionRatio << ":1\n";
            }
        }
        ...
        loader.setProgressCallback([](int level, double progress) {
            std::cout << "  Level " << level << ": " << std::fixed << std::setprecision(1) << (progress * 100) << "%\n";
        });
        ...
        if (finalImage) {
            std::cout << "  Total progressive loading time: " << duration.count() << "ms\n";
            std::cout << "  Final image size: " << (finalImage->size() / (1024 * 1024)) << " MB\n";
        }
...
void demonstrateTiledProcessing() {
    ...
    try {
        ...
        processor.setOverlapSize(16); // 16-pixel overlap for seamless processing
        processor.setMaxMemoryUsage(512 * 1024 * 1024); // 512 MB memory limit
        
        // Set up processing function (simple blur filter)
        processor.setProcessingFunction([](const std::vector<uint8_t>& input, int width, int height, int channels) -> std::vector<uint8_t> {
            ...
            for (int y = 1; y < height - 1; ++y) {
                for (int x = 1; x < width - 1; ++x) {
                    for (int c = 0; c < channels; ++c) {
                        ...
                        output[outIdx] = static_cast<uint8_t>(std::clamp(sum, 0.0f, 255.0f));
                    }
                }
            }
        ...
        auto totalDuration = duration_cast<seconds>(steady_clock::now() - start);
        
        std::cout << "  Tiled processing completed in " << totalDuration.count() << "s\n";
        
        // Verify output file
        std::ifstream outputCheck(outputFile, std::ios::binary | std::ios::ate);
        if (outputCheck) {
            size_t outputSize = outputCheck.tellg();
            std::cout << "  Output file size: " << (outputSize / (1024 * 1024)) << " MB\n";
            outputCheck.close();
        }
        
        // Get memory usage statistics
        auto memStats = processor.getMemoryStatistics();
        std::cout << "  Peak memory usage: " << (memStats.peakMemoryUsage / (1024 * 1024)) << " MB\n";
        std::cout << "  Average tile processing time: " << memStats.averageTileProcessingTime << "ms\n";
    ...
    try {
        ...
        for (const auto& [method, name] : methods) {
            ...
            try {
                ...
                bool compressed = compressor.compressFile(inputFile, compressedFile);
                
                auto compressionTime = duration_cast<milliseconds>(steady_clock::now() - start);
                
                if (compressed) {
                    // Get file sizes
                    std::ifstream input(inputFile, std::ios::binary | std::ios::ate);
                    std::ifstream output(compressedFile, std::ios::binary | std::ios::ate);
                    ...
                    double compressionSpeed = (inputSize / (1024.0 * 1024.0)) / (compressionTime.count() / 1000.0);
                    
                    std::cout << "  Compression time: " << compressionTime.count() << "ms\n";
                    std::cout << "  Original size: " << (inputSize / (1024 * 1024)) << " MB\n";
                    std::cout << "  Compressed size: " << (outputSize / (1024 * 1024)) << " MB\n";
                    std::cout << "  Compression ratio: " << std::fixed << std::setprecision(2) << compressionRatio << ":1\n";
                    std::cout << "  Compression speed: " << std::setprecision(1) << compressionSpeed << " MB/s\n";
                    ...
                    bool decompressed = decompressor.decompressFile(compressedFile, decompressedFile);
                    
                    auto decompressionTime = duration_cast<milliseconds>(steady_clock::now() - start);
                    
                    if (decompressed) {
                        double decompressionSpeed = (inputSize / (1024.0 * 1024.0)) / (decompressionTime.count() / 1000.0);
                        
                        std::cout << "  Decompression time: " << decompressionTime.count() << "ms\n";
                        std::cout << "  Decompression speed: " << decompressionSpeed << " MB/s\n";
                        
                        // Verify integrity
                        std::ifstream original(inputFile, std::ios::binary);
                        std::ifstream restored(decompressedFile, std::ios::binary);
                        ...
                        std::cout << "  Data integrity: " << (identical ? "VERIFIED" : "FAILED") << "\n";
        ...
        for (const auto& [strategy, name] : strategies) {
            ...
            try {
                ...
                for (int i = 0; i < numChunks; ++i) {
                    int y = i * chunkHeight;
                    int height = (i == numChunks - 1) ? properties.height - y : chunkHeight;
                    
                    auto chunkData = reader.readRegion(0, y, properties.width, height);
                ...
                std::cout << "  Peak memory usage: " << (peakMemory / (1024 * 1024)) << " MB\n";
                std::cout << "  Memory efficiency: " << 
                             (static_cast<double>(peakMemory) / (properties.width * properties.height * properties.channels)) << "x\n";
                ...
                std::cout << "  Cache efficiency: " << 
                             (static_cast<double>(memStats.cacheHits) / (memStats.cacheHits + memStats.cacheMisses) * 100) << "%\n";
...
int main() {
    ...
    std::cout << "This example demonstrates streaming I/O operations for large images\n";
