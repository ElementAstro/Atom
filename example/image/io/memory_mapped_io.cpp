...
void createTestImageFile(const std::string& filename, int width, int height) {
    std::cout << "Creating test image file: " << filename << " (" << width << "x" << height << ")\n";
    ...
    try {
        ...
        uint32_t header[3] = {static_cast<uint32_t>(width), static_cast<uint32_t>(height), 3};
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                imageData[index + 1] = static_cast<uint8_t>((x * 2) % 256); // G
                imageData[index + 2] = static_cast<uint8_t>((y * 2) % 256); // B
            }
        }
        ...
        std::cout << "  File size: " << ((sizeof(header) + dataSize) / (1024 * 1024)) << " MB\n";
...
void demonstrateBasicMemoryMapping() {
    ...
    try {
        ...
        std::cout << "  File size: " << (mmFile.size() / (1024 * 1024)) << " MB\n";
        std::cout << "  Mapped address: " << mmFile.data() << "\n";
        
        // Read header
        const uint32_t* header = reinterpret_cast<const uint32_t*>(mmFile.data());
        uint32_t fileWidth = header[0];
        uint32_t fileHeight = header[1];
        uint32_t fileChannels = header[2];
        
        std::cout << "  Image dimensions: " << fileWidth << "x" << fileHeight << "x" << fileChannels << "\n";
        
        // Access image data
        const uint8_t* imageData = reinterpret_cast<const uint8_t*>(mmFile.data() + sizeof(uint32_t) * 3);
        ...
        std::cout << "  Sequential access time: " << duration.count() << " μs\n";
        std::cout << "  Throughput: " << (dataSize / (1024.0 * 1024.0)) / (duration.count() / 1000000.0) << " MB/s\n";
        ...
        std::cout << "  Average time per access: " << (duration.count() / numAccesses) << " ns\n";
        ...
        std::cout << "  Region (" << regionX << "," << regionY << " " << regionWidth << "x" << regionHeight << ")\n";
...
void compareMemoryMappingVsTraditionalIO() {
    ...
    try {
        ...
        std::cout << "Comparing access methods for " << (fileSize / (1024 * 1024)) << " MB file\n";
        
        // Test parameters
        const int numTests = 1000;
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_int_distribution<size_t> dis(12, fileSize - 1000); // Skip header
        ...
        auto traditionalTime = duration_cast<microseconds>(steady_clock::now() - start);
        
        std::cout << "  Traditional I/O time: " << traditionalTime.count() << " μs\n";
        std::cout << "  Average seek time: " << (traditionalTime.count() / numTests) << " ns\n";
        ...
        auto mmapTime = duration_cast<microseconds>(steady_clock::now() - start);
        
        std::cout << "  Memory mapping time: " << mmapTime.count() << " μs\n";
        std::cout << "  Average access time: " << (mmapTime.count() / numTests) << " ns\n";
        std::cout << "  Checksum: " << mmapChecksum << "\n";
        
        // Performance comparison
        std::cout << "\nPerformance Comparison:\n";
        double speedup = static_cast<double>(traditionalTime.count()) / mmapTime.count();
        std::cout << "  Memory mapping speedup: " << std::fixed << std::setprecision(1) << speedup << "x\n";
        ...
        std::cout << "  Traditional sequential read: " << seqTime.count() << " μs\n";
        std::cout << "  Sequential throughput: " << (readSize / (1024.0 * 1024.0)) / (seqTime.count() / 1000000.0) << " MB/s\n";
        
        // Memory mapped sequential access
        start = steady_clock::now();
        
        const uint8_t* mmapData = reinterpret_cast<const uint8_t*>(mmFile.data()) + 12;
        ...
        auto mmapSeqTime = duration_cast<microseconds>(steady_clock::now() - start);
        
        std::cout << "  Memory mapped sequential: " << mmapSeqTime.count() << " μs\n";
        std::cout << "  Mmap sequential throughput: " << (readSize / (1024.0 * 1024.0)) / (mmapSeqTime.count() / 1000000.0) << " MB/s\n";
        
        std::cout << "  Sequential checksums match: " << (seqChecksum == mmapSeqChecksum ? "YES" : "NO") << "\n";
...
void demonstrateVirtualMemoryManagement() {
    ...
    try {
        ...
        std::cout << "  Max resident memory: " << (vmManager.getMaxResidentPages() * vmManager.getPageSize() / 1024) << " KB\n";
        ...
        std::cout << "  Virtual size: " << (vmFile->getVirtualSize() / (1024 * 1024)) << " MB\n";
        ...
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_int_distribution<size_t> dis(0, vmFile->getVirtualSize() - 1);
        ...
        std::cout << "  Cache hit rate: " << (static_cast<double>(finalStats.cacheHits) / numAccesses * 100) << "%\n";
        std::cout << "  Resident pages: " << finalStats.residentPages << "\n";
        std::cout << "  Memory usage: " << (finalStats.residentPages * vmManager.getPageSize() / 1024) << " KB\n";
        ...
        std::cout << "    Page faults: " << (seqStats.pageFaults - finalStats.pageFaults) << "\n";
        std::cout << "    Throughput: " << (seqSize / (1024.0 * 1024.0)) / (seqTime.count() / 1000000.0) << " MB/s\n";
        ...
        auto localityTime = duration_cast<microseconds>(steady_clock::now() - start);
        ...
        std::cout << "    Cache hit rate: " << (static_cast<double>(localityStats.cacheHits) / 10000 * 100) << "%\n";
...
void demonstrateCopyOnWrite() {
    ...
    try {
        ...
        MemoryMappedFile readOnlyView(filename, MemoryMappedFile::AccessMode::ReadOnly);
        MemoryMappedFile copyOnWriteView(filename, MemoryMappedFile::AccessMode::CopyOnWrite);
        ...
        std::cout << "  Read-only view: " << readOnlyView.data() << " (size: " << readOnlyView.size() << ")\n";
        std::cout << "  Copy-on-write view: " << copyOnWriteView.data() << " (size: " << copyOnWriteView.size() << ")\n";
        
        // Read original data
        const uint8_t* originalData = reinterpret_cast<const uint8_t*>(readOnlyView.data()) + 12;
        uint8_t* cowData = reinterpret_cast<uint8_t*>(copyOnWriteView.data()) + 12;
        ...
        auto modifyTime = duration_cast<microseconds>(steady_clock::now() - start);
        ...
        std::cout << "  Data differences: " << differences << " out of 1000 bytes\n";
        std::cout << "  Copy-on-write working: " << (differences > 0 ? "YES" : "NO") << "\n";
        ...
        std::cout << "  Memory overhead: " << (memInfo.privatePages * 4096 / 1024) << " KB\n";
        ...
        auto regularTime = duration_cast<microseconds>(steady_clock::now() - start);
        
        // COW approach
        MemoryMappedFile cowTest(filename, MemoryMappedFile::AccessMode::CopyOnWrite);
        ...
        std::cout << "  Regular allocation + copy + modify: " << regularTime.count() << " μs\n";
        std::cout << "  Copy-on-write modify: " << cowTime.count() << " μs\n";
        std::cout << "  COW performance ratio: " << (static_cast<double>(cowTime.count()) / regularTime.count()) << "\n";
...
void demonstrateCrossPlatformCompatibility() {
    ...
    try {
        ...
        MemoryMappedFile::Capabilities caps = MemoryMappedFile::getCapabilities();
        
        std::cout << "Memory mapping capabilities:\n";
        std::cout << "  Read-only mapping: " << (caps.supportsReadOnly ? "YES" : "NO") << "\n";
        std::cout << "  Read-write mapping: " << (caps.supportsReadWrite ? "YES" : "NO") << "\n";
        std::cout << "  Copy-on-write: " << (caps.supportsCopyOnWrite ? "YES" : "NO") << "\n";
        std::cout << "  Large files (>4GB): " << (caps.supportsLargeFiles ? "YES" : "NO") << "\n";
        std::cout << "  Sparse files: " << (caps.supportsSparseFiles ? "YES" : "NO") << "\n";
        std::cout << "  Memory advice: " << (caps.supportsMemoryAdvice ? "YES" : "NO") << "\n";
        
        // Test different access modes
        std::vector<std::pair<MemoryMappedFile::AccessMode, std::string>> modes = {
        ...
        for (const auto& [mode, name] : modes) {
            ...
            try {
                ...
                if (mmFile.isValid()) {
                    ...
                    std::cout << "  Size: " << (mmFile.size() / 1024) << " KB\n";
                    std::cout << "  Address: " << mmFile.data() << "\n";
                    
                    // Test basic read
                    const uint8_t* data = reinterpret_cast<const uint8_t*>(mmFile.data());
                    uint32_t checksum = 0;
                    for (size_t i = 0; i < std::min(size_t(1000), mmFile.size()); ++i) {
                        checksum += data[i];
                    }
                    ...
                    if (mode != MemoryMappedFile::AccessMode::ReadOnly) {
                        try {
                            uint8_t* writeData = reinterpret_cast<uint8_t*>(mmFile.data());
                            uint8_t originalByte = writeData[12];
                            writeData[12] = static_cast<uint8_t>((originalByte + 1) % 256);
        ...
        if (caps.supportsMemoryAdvice) {
            ...
            MemoryMappedFile mmFile(filename);
            if (mmFile.isValid()) {
                std::vector<std::pair<MemoryMappedFile::MemoryAdvice, std::string>> adviceTypes = {
                    {MemoryMappedFile::MemoryAdvice::Normal, "Normal"},
                    {MemoryMappedFile::MemoryAdvice::Sequential, "Sequential"},
                    {MemoryMappedFile::MemoryAdvice::Random, "Random"},
                    {MemoryMappedFile::MemoryAdvice::WillNeed, "Will Need"},
                    {MemoryMappedFile::MemoryAdvice::DontNeed, "Don't Need"}
                };
                ...
                for (const auto& [advice, name] : adviceTypes) {
                    try {
                        ...
                        std::cout << "  " << name << " advice: FAILED (" << e.what() << ")\n";
                    }
                }
            }
        }
