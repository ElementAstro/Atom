...
vector<cv::Mat> generateQualityTestFrames() {
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            frame2.at<uint8_t>(y, x) = static_cast<uint8_t>(max(0, min(255, value)));
        }
    }
    // Add moderate noise
    ...
    cv::circle(frame3, cv::Point(width/2, height/2), 50, cv::Scalar(200), -1);
    cv::circle(frame3, cv::Point(width/4, height/4), 30, cv::Scalar(180), -1);
    // Blur the image
...
void demonstrateBasicQualityAssessment() {
    ...
    try {
        ...
        for (size_t i = 0; i < frames.size(); ++i) {
            ...
            cout << "  Sharpness: " << fixed << setprecision(3) << metrics.sharpness << "\n";
            ...
            // Classify quality
            string qualityClass;
            if (metrics.overallScore > 0.8) qualityClass = "Excellent";
            else if (metrics.overallScore > 0.6) qualityClass = "Good";
            else if (metrics.overallScore > 0.4) qualityClass = "Fair";
            else if (metrics.overallScore > 0.2) qualityClass = "Poor";
            else qualityClass = "Very Poor";
...
void demonstrateCustomQualityParameters() {
    ...
    try {
        ...
        params.enableBrightness = false;  // Disable brightness for dark astronomical images
        ...
        for (size_t i = 0; i < frames.size(); ++i) {
            ...
            cout << "Frame " << i << ": ";
            cout << "Score=" << fixed << setprecision(3) << metrics.overallScore;
        ...
        for (size_t i = 0; i < frames.size(); ++i) {
            QualityMetrics defaultMetrics = defaultAssessor.assessFrame(frames[i]);
            QualityMetrics customMetrics = customAssessor.assessFrame(frames[i]);
            
            cout << "Frame " << i << ": ";
            cout << "Default=" << fixed << setprecision(3) << defaultMetrics.overallScore;
            cout << ", Custom=" << customMetrics.overallScore;
            cout << " (diff=" << (customMetrics.overallScore - defaultMetrics.overallScore) << ")\n";
        }
...
void demonstrateFrameSelection() {
    ...
    try {
        ...
        for (int topN : topCounts) {
            ...
            for (int i = 0; i < min(topN, static_cast<int>(rankedFrames.size())); ++i) {
...
void demonstrateStatisticalAnalysis() {
    ...
    try {
        ...
        auto calculateStats = [](const vector<double>& values) {
            if (values.empty()) return make_tuple(0.0, 0.0, 0.0, 0.0);
        ...
        auto [sharpMean, sharpStd, sharpMin, sharpMax] = calculateStats(sharpnessValues);
        ...
        cout << "SNR: mean=" << snrMean << ", std=" << snrStd 
             << ", range=[" << snrMin << ", " << snrMax << "]\n";
        
        auto [entMean, entStd, entMin, entMax] = calculateStats(entropyValues);
        cout << "Entropy: mean=" << entMean << ", std=" << entStd 
             << ", range=[" << entMin << ", " << entMax << "]\n";
        
        auto [scoreMean, scoreStd, scoreMin, scoreMax] = calculateStats(overallScores);
        ...
        for (double score : overallScores) {
            ...
            else if (score > 0.2) qualityDistribution["Poor"]++;
            else qualityDistribution["Very Poor"]++;
        }
...
void demonstratePerformanceCharacteristics() {
    ...
    try {
        ...
        for (const auto& size : testSizes) {
            cout << "Testing " << size.width << "x" << size.height << " frames:\n";
            ...
            double pixelsPerSecond = (size.width * size.height * iterations * 1000000.0) / duration.count();
            
            cout << "  Average assessment time: " << fixed << setprecision(2) << avgTime << " μs\n";
            cout << "  Processing rate: " << (pixelsPerSecond / 1000000.0) << " Mpixels/s\n";
        }
...
int main() {
    ...
    cout << "This example demonstrates SER frame quality assessment and ranking\n";
