...
int main() {
    ...
    std::cout << "- Supports various pixel formats (8-bit, 16-bit, RGB, mono)\n";
    ...
    try {
        ...
        for (int i = 0; i < std::min(frame_count, 10); ++i) {
            ...
            cv::randu(frame, cv::Scalar(100), cv::Scalar(300)); // Background noise
            
            // Add simulated astronomical object with atmospheric turbulence
            int center_x = width/2 + static_cast<int>(5 * sin(i * 0.5)); // Simulate seeing
            int center_y = height/2 + static_cast<int>(3 * cos(i * 0.3));
            cv::circle(frame, cv::Point(center_x, center_y), 20, cv::Scalar(2000), -1);
            cv::circle(frame, cv::Point(center_x, center_y), 15, cv::Scalar(4000), -1);
            cv::circle(frame, cv::Point(center_x, center_y), 10, cv::Scalar(8000), -1);
            ...
            std::cout << "   Frame " << i+1 << ": quality=" << quality << std::endl;
        }
        ...
        std::sort(indices.begin(), indices.end(), 
                  [&](size_t a, size_t b) { return frame_quality[a] > frame_quality[b]; });

        size_t best_frames_count = std::min(static_cast<size_t>(5), frames.size());
        std::cout << "\n3. Frame selection completed\n";
        std::cout << "   Best " << best_frames_count << " frames selected for stacking\n";
        ...
        for (size_t i = 1; i < best_frames_count; ++i) {
            ...
            cv::warpAffine(current_frame, aligned_frame, translation_matrix, current_frame.size());
            aligned_frames.push_back(aligned_frame);
            
            std::cout << "   Frame " << indices[i]+1 << " aligned (shift: " 
                      << shift_point.x << ", " << shift_point.y << ")\n";
        }

        // Frame stacking (averaging)
        std::cout << "\n5. Stacking frames...\n";
        cv::Mat stacked_frame = cv::Mat::zeros(reference_frame.size(), CV_32FC1);
        ...
        std::cout << "   Stacking completed using " << aligned_frames.size() << " frames\n";

        // Calculate improvement metrics
        cv::Scalar noise_before = cv::mean(frames[0]);
        cv::Scalar noise_after = cv::mean(final_image);
        double snr_improvement = sqrt(static_cast<double>(aligned_frames.size()));
        
        std::cout << "\n6. Stacking results:\n";
        std::cout << "   Signal-to-noise improvement: " << snr_improvement << "x\n";
        std::cout << "   Theoretical noise reduction: " << 1.0/snr_improvement << "x\n";
...
#else
    std::cout << "OpenCV not available. SER video processing example cannot run." << std::endl;
    ...
    std::cout << "\nSER video format processing example completed." << std::endl;
    return 0;
}
