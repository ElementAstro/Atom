...
struct ConversionTestResult {
    ...
    ConversionTestResult(const std::string& name, const std::string& src,
...
public:
     ...
     */
    QualityMetrics calculateQualityMetrics(const blob& original,
        ...
        for (size_t i = 0; i < original.size(); ++i) {
            double diff = static_cast<double>(original.data()[i]) -
                          static_cast<double>(converted.data()[i]);
            mse += diff * diff;
        }
        mse /= original.size();
        metrics.mse = mse;
     ...
     */
    ConversionTestResult testConversion(
        const std::string& testName, const blob& sourceData,
        const std::string& sourceFormat, const std::string& targetFormat,
        const std::map<std::string, std::any>& options = {}) {
        ConversionTestResult result(testName, sourceFormat, targetFormat);
        result.originalSize = sourceData.size();

        std::cout << "Testing: " << testName << " (" << sourceFormat << " -> "
                  << targetFormat << ") ... ";

        auto start = high_resolution_clock::now();

        try {
            auto convertedData = converter_.convert(sourceData, sourceFormat,
                                                    targetFormat, options);

            result.conversionTime = duration_cast<microseconds>(
                high_resolution_clock::now() - start);
            result.convertedSize = convertedData.size();
            result.compressionRatio =
                static_cast<double>(result.originalSize) / result.convertedSize;
    ...
    void printSummary() const {
        ...
        if (successful > 0) {
            ...
            std::cout << std::left << std::setw(25) << "Test" << std::setw(15)
                      << "Compression" << std::setw(12) << "Quality"
                      << std::setw(10) << "Time (μs)" << "\n";
            ...
            for (const auto& result : results_) {
                if (result.success) {
                    std::cout << std::left << std::setw(25) << result.testName
                              ...
                              << std::setprecision(2) << result.compressionRatio
                              << std::setw(12) << std::setprecision(1)
                              << result.qualityScore << std::setw(10)
                              << result.conversionTime.count() << "\n";
                }
            }
        }
...
std::map<std::string, blob> createTestImages() {
    ...
    // 4. Checkerboard pattern (specific compression characteristics)
    {
        ...
        testImages["checkerboard"] =
            blob(checkerboard.data(), checkerboard.size());
    }

    // 5. Single pixel image (edge case)
    {
        std::vector<uint8_t> singlePixel = {255, 128, 64};  // RGB
        testImages["single_pixel"] =
            blob(singlePixel.data(), singlePixel.size());
    }
...
void testBasicFormatConversions() {
    ...
    std::vector<std::pair<std::string, std::string>> formatPairs = {
        {"RAW", "JPEG"}, {"RAW", "PNG"},  {"RAW", "BMP"}, {"RAW", "TIFF"},
        {"JPEG", "PNG"}, {"PNG", "JPEG"}, {"BMP", "PNG"}, {"TIFF", "JPEG"}};

    for (const auto& [sourceFormat, targetFormat] : formatPairs) {
        for (const auto& [imageName, imageData] : testImages) {
            std::string testName =
                imageName + "_" + sourceFormat + "_to_" + targetFormat;
            tester.testConversion(testName, imageData, sourceFormat,
                                  targetFormat);
        }
    }
...
void testLossyVsLosslessConversions() {
    ...
    for (int quality : qualityLevels) {
        std::map<std::string, std::any> jpegOptions = {{"quality", quality},
                                                       {"optimize", true}};

        for (const auto& [imageName, imageData] : testImages) {
            std::string testName =
                imageName + "_JPEG_quality_" + std::to_string(quality);
            tester.testConversion(testName, imageData, "RAW", "JPEG",
                                  jpegOptions);
        }
    }
...
void testEdgeCases() {
    ...
    // Test 2: Very large dimensions (simulated)
    {
        ...
        tester.testConversion("very_large_dimensions", largeImage, "RAW",
                              "PNG");
    }
    ...
    // Test 5: Corrupted data patterns
    {
        ...
        tester.testConversion("alternating_pattern", alternatingBlob, "RAW",
                              "PNG");
    }
...
void testColorSpaceConversions() {
    ...
    for (int y = 0; y < 100; ++y) {
        for (int x = 0; x < 100; ++x) {
            int index = (y * 100 + x) * 3;
            rgbImage[index] =
                static_cast<uint8_t>(x * 255 / 100);  // R gradient
            rgbImage[index + 1] =
                static_cast<uint8_t>(y * 255 / 100);  // G gradient
    ...
    for (const auto& colorSpace : colorSpaces) {
        try {
            auto convertedData =
                colorConverter.convert(rgbBlob, "RGB", colorSpace);
            ...
            auto roundTripData =
                colorConverter.convert(convertedData, colorSpace, "RGB");
            std::string roundTripName = "RGB_to_" + colorSpace + "_to_RGB";

            // Calculate quality loss in round-trip
            auto metrics =
                tester.calculateQualityMetrics(rgbBlob, roundTripData);
            std::cout << "  Round-trip quality (PSNR): " << std::fixed
                      << std::setprecision(2) << metrics.psnr << " dB\n";
...
void testMetadataPreservation() {
    ...
    for (const auto& format : metadataFormats) {
        std::map<std::string, std::any> options = {{"preserve_metadata", true},
                                                   {"metadata", metadata}};

        std::string testName = "metadata_preservation_" + format;
        auto result =
            tester.testConversion(testName, imageBlob, "RAW", format, options);

        if (result.success) {
            // In a real implementation, you would verify that metadata was
            // preserved
            std::cout << "  " << format
                      << " metadata preservation: SIMULATED SUCCESS\n";
        }
    }
...
void testCompressionArtifacts() {
    ...
    for (int level : compressionLevels) {
        std::map<std::string, std::any> options = {{"quality", level}};

        // Test high contrast image
        std::string testName1 = "high_contrast_jpeg_q" + std::to_string(level);
        auto result1 = tester.testConversion(testName1, highContrastBlob, "RAW",
                                             "JPEG", options);

        // Test fine details image
        std::string testName2 = "fine_details_jpeg_q" + std::to_string(level);
        auto result2 = tester.testConversion(testName2, fineDetailsBlob, "RAW",
                                             "JPEG", options);

        if (result1.success && result2.success) {
            std::cout << "  Quality " << level
                      << " - High contrast PSNR: " << std::fixed
                      << std::setprecision(2) << result1.qualityScore << " dB, "
                      << "Fine details PSNR: " << result2.qualityScore
                      << " dB\n";
        }
    }
...
void testConversionPerformance() {
    ...
    for (const auto& [size, sizeName] : imageSizes) {
        ...
        for (const auto& format : formats) {
            ...
            auto result =
                tester.testConversion(testName, imageBlob, "RAW", format);

            if (result.success) {
                double pixelsPerSecond =
                    (size * size * 1e6) / result.conversionTime.count();
                std::cout << "  " << testName << ": " << std::scientific
                          << std::setprecision(2) << pixelsPerSecond
                          << " pixels/sec\n";
            }
        }
    }
...
int main() {
    ...
    std::cout << "This example demonstrates format conversion edge cases and "
