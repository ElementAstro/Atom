...
void demonstrateHeaderOperations() {
    ...
    try {
        ...
        cout << "  Image dimensions: " << header.imageWidth << "x" << header.imageHeight << "\n";
        cout << "  Pixel depth: " << header.pixelDepth << " bits\n";
        cout << "  Color format: " << header.colorID << " (";
        
        switch (static_cast<SERColorID>(header.colorID)) {
            case SERColorID::Mono: cout << "Mono"; break;
            case SERColorID::BayerRGGB: cout << "Bayer RGGB"; break;
            case SERColorID::BayerGRBG: cout << "Bayer GRBG"; break;
            case SERColorID::BayerGBRG: cout << "Bayer GBRG"; break;
            case SERColorID::BayerBGGR: cout << "Bayer BGGR"; break;
            case SERColorID::RGB: cout << "RGB"; break;
            case SERColorID::BGR: cout << "BGR"; break;
        ...
        cout << "  Date/Time: " << put_time(localtime(&time_t), "%Y-%m-%d %H:%M:%S") << "\n";
        ...
        cout << "  Total data size: " << (totalDataSize / (1024 * 1024)) << " MB\n";
...
void demonstrateSERWriting() {
    ...
    try {
        ...
        for (uint64_t i = 0; i < header.frameCount; ++i) {
            cv::Mat frame = generateTestFrame(header.imageWidth, header.imageHeight, static_cast<int>(i));
...
void demonstrateSERReading() {
    ...
    try {
        ...
        vector<uint64_t> framesToRead = {0, header.frameCount / 4, header.frameCount / 2, header.frameCount - 1};
            ...
            if (frameIndex < header.frameCount) {
                ...
                if (hasTimestamps) {
                    ...
                    if (timestamp.has_value()) {
                        ...
                        auto time_t = chrono::system_clock::to_time_t(timePoint);
                        cout << "    Timestamp: " << put_time(localtime(&time_t), "%H:%M:%S") << "\n";
                    }
                }
            }
        }
        ...
        try {
            ...
            switch (colorId) {
                case SERColorID::Mono:
                    testFrame = generateTestFrame(header.imageWidth, header.imageHeight, 0);
                    break;
                    
                case SERColorID::RGB:
                case SERColorID::BGR:
                    testFrame = cv::Mat(header.imageHeight, header.imageWidth, CV_8UC3);
                    for (int y = 0; y < header.imageHeight; ++y) {
                        for (int x = 0; x < header.imageWidth; ++x) {
                            cv::Vec3b& pixel = testFrame.at<cv::Vec3b>(y, x);
                            pixel[0] = static_cast<uint8_t>((x + y) % 256);      // B or R
                            pixel[1] = static_cast<uint8_t>((x * 2 + y) % 256);  // G
                            pixel[2] = static_cast<uint8_t>((x + y * 2) % 256);  // R or B
                        }
                    }
                    break;
                ...
                case SERColorID::BayerRGGB:
                    testFrame = generateTestFrame(header.imageWidth, header.imageHeight, 0);
                    // Bayer pattern is stored as mono but interpreted differently
                    break;
...
void demonstrateErrorHandling() {
    ...
    try {
        ...
        cout << "  Invalid header validation: " << (isValid ? "FAILED" : "PASSED") << "\n";
