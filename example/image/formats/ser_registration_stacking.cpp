...
vector<cv::Mat> generateRegistrationTestFrames() {
    ...
    for (size_t i = 0; i < transformMatrices.size(); ++i) {
        ...
        cv::warpAffine(baseFrame, transformedFrame, transformMatrices[i], cv::Size(width, height));
...
void demonstrateFrameRegistration() {
    ...
    try {
        ...
        cout << "Generated " << frames.size() << " test frames with simulated motion\n";
        ...
        for (const auto& [method, name] : methods) {
            ...
            try {
                ...
                registrar.setReferenceFrame(frames[0]); // Use first frame as reference
                ...
                for (size_t i = 1; i < frames.size(); ++i) {
                    ...
                    FrameTransformation transform = registrar.calculateTransformation(frames[i]);
                    
                    auto end = chrono::high_resolution_clock::now();
                    auto duration = chrono::duration_cast<chrono::microseconds>(end - start);
                    
                    cout << "  Frame " << i << ": ";
                    cout << "dx=" << fixed << setprecision(2) << transform.translation.x;
...
void demonstrateFrameStacking() {
    ...
    try {
        ...
        for (size_t i = 1; i < frames.size(); ++i) {
            auto [registeredFrame, transform] = registrar.registerFrame(frames[i]);
        ...
        for (const auto& [method, name] : methods) {
            ...
            try {
                ...
                if (method == StackingMethod::WeightedAverage) {
                    ...
                    stackedResult = stacker.stackFramesWithWeights(registeredFrames, weights);
                } else {
                ...
                cout << "  Result size: " << stackedResult.cols << "x" << stackedResult.rows << "\n";
                cout << "  Mean value: " << fixed << setprecision(2) << meanVal[0] << "\n";
                cout << "  Value range: [" << minVal << ", " << maxVal << "]\n";
                
                // Calculate noise reduction estimate
                double noiseReduction = sqrt(static_cast<double>(registeredFrames.size()));
                cout << "  Theoretical noise reduction: " << setprecision(1) << noiseReduction << "x\n";
...
void demonstrateAstronomicalWorkflow() {
    ...
    try {
        ...
        for (size_t i = 0; i < frames.size(); ++i) {
            ...
            frameQualities.push_back({static_cast<int>(i), metrics.overallScore});
            cout << "  Frame " << i << ": quality = " << fixed << setprecision(3) << metrics.overallScore << "\n";
        }
        ...
        cout << "  Selected " << selectedFrames.size() << " frames above quality threshold " << qualityThreshold << "\n";
        cout << "  Selected frame indices: ";
        for (int idx : selectedIndices) cout << idx << " ";
        cout << "\n";
        ...
        for (size_t i = 1; i < selectedFrames.size(); ++i) {
            auto [alignedFrame, transform] = registrar.registerFrame(selectedFrames[i]);
            alignedFrames.push_back(alignedFrame);
            transformations.push_back(transform);
            
            cout << "  Frame " << selectedIndices[i] << " aligned: ";
            cout << "offset=(" << fixed << setprecision(2) << transform.translation.x;
        ...
        stackingWeights.push_back(frameQualities[selectedIndices[0]].second); // Reference frame
        
        for (size_t i = 0; i < transformations.size(); ++i) {
            double qualityWeight = frameQualities[selectedIndices[i + 1]].second;
        ...
        auto start = chrono::high_resolution_clock::now();
        cv::Mat finalResult = stacker.stackFramesWithWeights(alignedFrames, stackingWeights);
        auto end = chrono::high_resolution_clock::now();
        auto duration = chrono::duration_cast<chrono::milliseconds>(end - start);
        ...
        cout << "  Final image size: " << finalResult.cols << "x" << finalResult.rows << "\n";
        ...
        cout << "  Weighted stacking quality: " << fixed << setprecision(3) << finalQuality.overallScore << "\n";
        cout << "  Simple average quality: " << averageQuality.overallScore << "\n";
        cout << "  Quality improvement: " << ((finalQuality.overallScore / averageQuality.overallScore - 1.0) * 100) << "%\n";
        
        double theoreticalSNRImprovement = sqrt(static_cast<double>(alignedFrames.size()));
        cout << "  Theoretical SNR improvement: " << setprecision(1) << theoreticalSNRImprovement << "x\n";
        cout << "  Actual SNR improvement: " << (finalQuality.snr / averageQuality.snr) << "x\n";
...
void demonstratePerformanceOptimization() {
    ...
    try {
        ...
        for (int frameCount : frameCounts) {
            ...
            vector<cv::Mat> testFrames(allFrames.begin(), allFrames.begin() + min(frameCount, static_cast<int>(allFrames.size())));
            ...
            for (size_t i = 1; i < testFrames.size(); ++i) {
                auto [registered, transform] = registrar.registerFrame(testFrames[i]);
                registeredFrames.push_back(registered);
            }
            
            auto regEnd = chrono::high_resolution_clock::now();
            auto regDuration = chrono::duration_cast<chrono::milliseconds>(regEnd - start);
            ...
            auto stackDuration = chrono::duration_cast<chrono::milliseconds>(stackEnd - regEnd);
            auto totalDuration = chrono::duration_cast<chrono::milliseconds>(stackEnd - start);
            ...
            cout << "  Time per frame: " << (totalDuration.count() / testFrames.size()) << "ms\n";
            
            // Memory usage estimate
            size_t frameSize = testFrames[0].total() * testFrames[0].elemSize();
            size_t totalMemory = frameSize * testFrames.size() * 2; // Original + registered
            cout << "  Memory usage: ~" << (totalMemory / (1024 * 1024)) << "MB\n";
        }
...
int main() {
    ...
    cout << "This example demonstrates SER frame registration and stacking operations\n";
    ...
    cout << "- Multiple registration algorithms (phase correlation, feature matching, etc.)\n";
    cout << "- Various stacking methods (mean, median, sigma clipping, weighted)\n";
