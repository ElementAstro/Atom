...
// Sample struct for shared memory usagestruct SensorData {
int id;
double temperature;
double humidity;
double pressure;
...
void print() const {
    ...
    std::cout << "Humidity: " << std::fixed << std::setprecision(2) << humidity
              << " %" << std::endl;
    std::cout << "Pressure: " << std::fixed << std::setprecision(2) << pressure
              << " hPa" << std::endl;
...
// Ensure the struct is trivially
// copyablestatic_assert(std::is_trivially_copyable_v<SensorData>,
              "SensorData must be trivially copyable");

              // Smaller struct for partial read/write examplesstruct
              // SensorStatus {
              bool active;
              ...
              // Ensure the struct is trivially
              // copyablestatic_assert(std::is_trivially_copyable_v<SensorStatus>,
              "SensorStatus must be trivially copyable");

              // Generate random sensor dataSensorData
              // generateRandomSensorData(int id) {
              ...
              static std::uniform_real_distribution<> pressure_dist(990.0,
                                                                    1030.0);
              ...
              std::strncpy(data.location, loc.c_str(),
                           sizeof(data.location) - 1);
              data.location[sizeof(data.location) - 1] =
              ...
              int main() {
                  ...
                  // Check if shared memory exists and remove it if it does
                  // (cleanup)
                  if (atom::connection::SharedMemory<SensorData>::exists(
                          shm_name)) {
                      std::cout << "Shared memory '" << shm_name
                                << "' already exists from a previous run.\n";
                      std::cout << "Please note: You might need to manually "
                                   "remove it using "
                                   "system commands\n";
                      std::cout << "if the previous process crashed or didn't "
                                   "clean up properly.\n";
                  ...
                  try {
                      atom::connection::SharedMemory<SensorData> sharedMemory(
                          shm_name, true, initialDataOpt);

                      std::cout << "Shared memory created successfully.\n";
                      std::cout << "Name: " << sharedMemory.getName()
                                << std::endl;
                      std::cout << "Size: " << sharedMemory.getSize()
                                << " bytes\n";
                      std::cout << "Version: " << sharedMemory.getVersion()
                                << std::endl;
                      std::cout << "Is creator: "
                                << (sharedMemory.isCreator() ? "Yes" : "No")
                                << std::endl;
                      std::cout << "Is initialized: "
                                << (sharedMemory.isInitialized() ? "Yes" : "No")
                                << std::endl;
                      ...
                      try {
                          ...
                          const atom::connection::SharedMemoryException& e) {
                          std::cout << "Expected exception caught: " << e.what()
                                    << std::endl;
                          std::cout << "Error code: " << e.getErrorCodeString()
                                    << std::endl;
                      }
                      ...
                      if (optData) {
                          ...
                          std::cout
                              << "No data available (expected after clear)\n";
                      }

                  } catch (const atom::connection::SharedMemoryException& e) {
                      std::cerr << "Error: " << e.what() << std::endl;
                      std::cerr << "Error code: " << e.getErrorCodeString()
                                << std::endl;
                      return 1;
                  }
                  ...
                  try {
                      ...
                      std::cout
                          << "Creating shared memory as 'creator' process...\n";
                      atom::connection::SharedMemory<SensorData> creator(
                          shm_name, true);
                      ...
                      std::cout << "\nOpening the same shared memory as "
                                   "'consumer' process...\n";
                      atom::connection::SharedMemory<SensorData> consumer(
                          shm_name, false);
                      ...
                      std::cout << "Version: " << consumer.getVersion()
                                << std::endl;
                      std::cout << "Is creator: "
                                << (consumer.isCreator() ? "Yes" : "No")
                                << std::endl;
                      std::cout << "Is initialized: "
                                << (consumer.isInitialized() ? "Yes" : "No")
                                << std::endl;
                  ...
                  try {
                      ...
                      std::cout
                          << "Trying to open non-existent shared memory...\n";
                  ...
                  try {
                      ...
                      try {
                          SensorData data =
                              sharedMemory.read(std::chrono::milliseconds(500));
                          auto end = std::chrono::high_resolution_clock::now();
                          std::chrono::duration<double, std::milli> elapsed =
                              end - start;

                          std::cout << "Read successful, took "
                                    << elapsed.count() << " ms\n";
                          std::cout << "Data read:\n";
                          data.print();
                      } catch (
                  ...
                  try {
                      ...
                      constexpr size_t active_offset =
                          offsetof(SensorData, active);
                      std::cout
                      ...
                      bool isActive =
                          sharedMemory.readPartial<bool>(active_offset);
                      ...
                      std::cout
                          << "Status active: " << (status.active ? "Yes" : "No")
                          << std::endl;
                      std::cout << "Status last update: " << status.last_update
                                << std::endl;

                      // Write partial status (this will write to active and
                      // timestamp fields)
                      ...
                      std::cout << "Status active: "
                                << (readStatus.active ? "Yes" : "No")
                                << std::endl;
                      std::cout
                          << "Status last update: " << readStatus.last_update
                          << std::endl;

                      // Demonstrate out-of-bounds handling
                      std::cout << "\nTrying to read beyond the bounds of "
                                   "shared memory...\n";
                      try {
                          double value = sharedMemory.readPartial<double>(
                              sizeof(SensorData) - 4);
                  ...
                  try {
                      ...
                      for (size_t i = 0;
                           i < std::min(size_t(10), binaryData.size()); ++i) {
                      ...
                      std::cout << "First few bytes: ";
                      for (size_t i = 0;
                           i < std::min(size_t(10), readData.size()); ++i) {
                          std::cout << std::setw(2) << std::setfill('0')
                                    << std::hex << static_cast<int>(readData[i])
                                    << " ";
                      }
                      std::cout << std::dec << std::endl;
                      ...
                      bool dataMatches =
                          std::equal(binaryData.begin(), binaryData.end(),
                                     readData.begin());
                      ...
                      std::vector<std::byte> smallBuffer(
                          64);  // Only read first 64 bytes
                      size_t bytesReadPartial = sharedMemory.readSpan(
                          std::span<std::byte>(smallBuffer));

                      std::cout << "\nRead " << bytesReadPartial
                                << " bytes into smaller buffer\n";
                      std::cout << "First few bytes: ";
                      for (size_t i = 0;
                           i < std::min(size_t(10), smallBuffer.size()); ++i) {
                          std::cout << std::setw(2) << std::setfill('0')
                                    << std::hex
                      ...
                      std::vector<std::byte> tooLargeBuffer(sizeof(SensorData) +
                                                            100);
                      try {
                          sharedMemory.writeSpan(
                              std::span<const std::byte>(tooLargeBuffer));
                  ...
                  try {
                      ...
                      std::cout
                          << "Creating shared memory with SensorData size...\n";
                      ...
                      std::cout << "Original shared memory size: "
                                << originalMemory.getSize() << " bytes\n";
                      ...
                      std::cout << "New size will be: " << newSize
                                << " bytes\n";

                      originalMemory.resize(newSize);

                      std::cout << "Shared memory after resize: "
                                << originalMemory.getSize() << " bytes\n";
                      ...
                      // Now we'll create a new shared memory object for the
                      // large data
                      std::cout
                      ...
                      for (size_t i = 0; i < largeData.historical_data.size();
                           ++i) {
                          largeData.historical_data[i] =
                              20.0 + 0.1 * static_cast<double>(i);
                      }
                      ...
                      std::cout << "...\n";
                      std::cout << "Notes: " << readLargeData.notes
                                << std::endl;
                  ...
                  try {
                      ...
                      std::cout << "Doing other work while reading "
                                   "asynchronously...\n";
                      for (int i = 0; i < 3; ++i) {
                          std::cout << "Work item " << (i + 1)
                                    << " completed\n";
                      ...
                      std::cout << "Doing other work while writing "
                                   "asynchronously...\n";
                      for (int i = 0; i < 3; ++i) {
                          std::cout << "Work item " << (i + 1)
                                    << " completed\n";
                  ...
                  try {
                      // Create shared memory
                      atom::connection::SharedMemory<SensorData> producer(
                          producer_name, true);

                      // Create a second shared memory object to simulate
                      // another process
                      atom::connection::SharedMemory<SensorData> consumer(
                          producer_name, false);
                      ...
                      std::size_t callbackId = consumer.registerChangeCallback(
                          [&callbackCount](const SensorData& data) {
                              std::cout << "Callback triggered! Count: "
                                        << ++callbackCount << std::endl;
                              std::cout << "Received sensor data:\n";
                              data.print();
                              std::cout << std::endl;
                          });

                      std::cout << "Registered change callback with ID: "
                                << callbackId << std::endl;
                      ...
                      std::cout << "\nCallback was triggered " << callbackCount
                                << " times\n";

                      // Unregister callback
                      std::cout << "\nUnregistering callback...\n";
                      bool unregistered =
                          consumer.unregisterChangeCallback(callbackId);
                      std::cout << "Callback unregistered: "
                                << (unregistered ? "Yes" : "No") << std::endl;

                      // Write more data to verify no more callbacks
                      std::cout << "\nWriting more data after unregistering "
                                   "callback...\n";
                      ...
                      std::thread writerThread([&producer]() {
                          std::cout << "Writer thread: waiting 1 second before "
                                       "updating data...\n";
                      ...
                      std::cout << "Main thread: waiting for data to change (2 "
                                   "second "
                                   "timeout)...\n";
                      bool changed = consumer.waitForChange(
                  ...
                  try {
                      ...
                      std::thread consumerThread([&]() {
                          try {
                              while (!stopThreads) {
                                  try {
                                      ...
                                      if (changed) {
                                          ...
                                          // Process the data (just print a
                                          // summary in this example)
                                          std::cout
                                              << "Consumer: Processing sensor #"
                                              << data.id << " from "
                                              << data.location
                                  ...
                                  } catch (const atom::connection::
                                               SharedMemoryException& e) {
                                      // Ignore timeouts, report other errors
                                      if (e.getErrorCode() !=
                                          atom::connection::
                                              SharedMemoryException::ErrorCode::
                                                  TIMEOUT) {
                                          std::cerr << "Consumer exception: "
                                                    << e.what() << std::endl;
                                      }
                                  }
                              }
                              ...
                              std::cerr
                                  << "Consumer thread exception: " << e.what()
                                  << std::endl;
                          }
                      });
                      ...
                      std::cout << "Running producer-consumer pattern for 5 "
                                   "seconds...\n";
                  ...
                  try {
                      ...
                      // Use withLock to perform an atomic read-modify-write
                      // operation
                      std::cout << "\nPerforming atomic read-modify-write "
                                   "operation...\n";
                  ...
                  std::cout
                      << "Completed all examples. Resources will be cleaned up "
                         "automatically\n";
                  std::cout << "when shared memory objects go out of scope.\n";

                  // The destructors for the SharedMemory objects will handle
                  // resource cleanup
