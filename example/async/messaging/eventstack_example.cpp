...
// 为Event类提供to_string和from_string函数，满足Serializable概念namespace std {
...
// 演示基本操作void demonstrateBasicOperations() {
std::cout << "\n===== 基本操作演示 =====\n";

atom::async::EventStack<Event> stack;

// 1. 检查初始状态
std::cout << "初始栈是否为空: " << (stack.isEmpty() ? "是" : "否") << std::endl;
...
// 演示边界情况void demonstrateBoundaryConditions() {
std::cout << "\n===== 边界情况演示 =====\n";
...
// 演示并发安全性void demonstrateConcurrency() {
std::cout << "\n===== 并发操作演示 =====\n";

atom::async::EventStack<Event> stack;

// 1. 并发推入和弹出
std::cout << "开始并发测试: 5个线程推入100个事件，5个线程弹出事件" << std::endl;
...
std::cout << "理论上的大小应该接近: " << 5 * 100 - 5 * 80 << " (推入 - 弹出)"
          << std::endl;
}

// 演示高级操作void demonstrateAdvancedOperations() {
std::cout << "\n===== 高级操作演示 =====\n";
...
stack.sortEvents([](const Event& a, const Event& b) {
...
std::cout << "是否存在包含'Event6'的事件: " << (any ? "是" : "否") << std::endl;
...
// 演示序列化和反序列化void demonstrateSerializationDeserialization() {
std::cout << "\n===== 序列化与反序列化演示 =====\n";
...
// 演示错误处理void demonstrateErrorHandling() {
std::cout << "\n===== 错误处理演示 =====\n";
