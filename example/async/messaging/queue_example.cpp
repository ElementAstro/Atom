...
// Helper function for generating random integersint getRandomInt(int min, int
// max) {
...
// Example 1: Basic usage of ThreadSafeQueuevoid basicUsageExample() {
...
// Example 2: Working with different data typesvoid dataTypesExample() {
...
// Example 3: Multi-threading with ThreadSafeQueuevoid multithreadingExample() {
...
// Example 4: Handle edge cases and boundary valuesvoid edgeCasesExample() {
...
std::cout << "Back element exists? " << (backElement.has_value() ? "Yes" : "No")
          << std::endl;

auto takeResult = emptyQueue.tryTake();
std::cout << "Can take from empty? " << (takeResult.has_value() ? "Yes" : "No")
          << std::endl;
...
std::cout << "All threads finished, total taken: " << totalTaken << std::endl;
std::cout << "Queue size after concurrent taking: " << concurrentQueue.size()
          << std::endl;
assert(concurrentQueue.empty());
}

// Example 5: Timeouts and waiting for elementsvoid timeoutExample() {
...
std::thread waitThread([&queue]() {
    ...
    if (result) {
        ...
        std::cout << "Timeout after " << elapsed.count() << "ms" << std::endl;
    }
});
...
std::thread waitUntilThread([&queue]() {
    ...
    if (result) {
        ...
        std::cout << "Timed out waiting until specific time point" << std::endl;
    }
});
...
// Example 6: Advanced features - filtering, transforming, batch processingvoid
...
// Example 7: Use every queue method oncevoid completeAPIExample() {
...
std::thread waiterThread([&queue]() {
    ...
    if (result) {
        std::cout << "Found item '" << *result << "' after " << elapsed.count()
                  << "ms" << std::endl;
    } else {
        std::cout << "Wait returned without result after " << elapsed.count()
                  << "ms" << std::endl;
    }
});
...
// Example 8: Error handling scenariosvoid errorHandlingExample() {
