...
std::thread t3([&refPromise, &accumulator]() {
    print_safe("Thread [", get_thread_id(), "] adding values to accumulator");
    for (int i = 0; i < 5; ++i) {
...
for (int i = 0; i < 3; ++i) {
    voidThreads.emplace_back([&voidPromises, i]() {
        ...
        print_safe("Thread [", get_thread_id(), "] completing void task ", i);
...
// 3. Edge cases and special scenarios examplesvoid edge_cases_examples() {
...
auto failingPromise = atom::async::makePromiseFromFunction([]() -> std::string {
    print_safe("Thread [", get_thread_id(),
...
// 6. Promise coroutine support examplesvoid coroutine_examples() {
