...
// 辅助函数：生成随机整数int getRandomInt(int min, int max) {
...
// 辅助函数：创建多个线程执行指定函数template <typename Func>
...
// 辅助函数：测量执行时间template <typename Func>
...
// 1. LockFreeStack 示例void lockFreeStackExample() {
...
// 2. LockFreeHashTable 示例void lockFreeHashTableExample() {
...
std::cout << "未知用户是否存在: " << (unknownScore.has_value() ? "是" : "否")
          << std::endl;

// 测试下标操作符 (会创建缺失的条目)
std::cout << "\n测试下标操作符:" << std::endl;
std::cout << "使用下标操作符前Dave存在: "
          << (userScores.find("Dave").has_value() ? "是" : "否") << std::endl;

userScores["Dave"] = 75;  // 创建新条目
std::cout << "使用下标操作符后Dave分数: " << userScores["Dave"] << std::endl;
...
std::cout << "擦除后Bob存在: "
          << (userScores.find("Bob").has_value() ? "是" : "否") << std::endl;
...
// 3. ThreadSafeVector 示例void threadSafeVectorExample() {
...
std::cout << "try_at(0): "
          << (elem0.has_value() ? std::to_string(*elem0) : "无值") << std::endl;

auto elem3 = safeVec.try_at(3);
std::cout << "try_at(3): "
          << (elem3.has_value() ? std::to_string(*elem3) : "无值") << std::endl;
...
// 4. LockFreeList 示例void lockFreeListExample() {
...
std::cout << "链表是否为空: " << (nameList.empty() ? "是" : "否") << std::endl;
...
atom::async::ThreadSafeVector<int> vector(OPERATIONS / 2);  // 预分配一半容量
    ...
    runWithThreads(THREADS, [&](int threadId) {
        for (int i = 0; i < OPS_PER_THREAD; ++i) {
            if (i % 4 == 0) {  // 25% 读取
                ...
                if (size > 0) {
                    vector.try_at(getRandomInt(0, static_cast<int>(size) - 1));
                }
            } else if (i % 4 == 1) {  // 25% 弹出
                vector.popBack();
            } else {  // 50% 推入
