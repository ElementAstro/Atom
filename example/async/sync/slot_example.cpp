...
// 辅助类：用于演示信号槽的对象class TestReceiver {
public:
...
// 1. 基本信号槽使用示例void basicSignalExample() {
...
// 2. 异步信号槽示例void asyncSignalExample() {
...
// 3. 自动断开连接信号示例void autoDisconnectSignalExample() {
...
auto id2 = adSignal.connect(
    std::bind(&TestReceiver::handleSignal, &receiver, std::placeholders::_1));
...
// 4. 链式信号示例void chainedSignalExample() {
...
// 为每个信号连接槽
rootSignal.connect([](int value) {
    std::cout << "根信号槽收到: " << value << std::endl;
});
...
// 5. 线程安全信号示例void threadSafeSignalExample() {
...
// 6. 有限次数信号示例void limitedSignalExample() {
...
for (int i = 1; i <= 5; ++i) {
    ...
    std::cout << "发射 #" << i << " 结果: " << (emitted ? "成功" : "已达到限制")
...
std::cout << "重置后发射结果: " << (emitted ? "成功" : "失败")
          << ", 剩余调用次数: " << limitedSignal.remainingCalls() << std::endl;

// 尝试创建无效限制的信号
try {
    atom::async::LimitedSignal<int> invalidSignal(0);  // 应抛出异常
} catch (const std::exception& e) {
    std::cout << "创建无效LimitedSignal时捕获异常: " << e.what() << std::endl;
}
}

// 7. 协程信号示例void coroutineSignalExample() {
...
// 8. 作用域信号槽示例void scopedSignalExample() {
...
auto slot1 = std::make_shared<atom::async::ScopedSignal<int>::SlotType>(
    [](int value) { std::cout << "作用域槽 1 收到: " << value << std::endl; });

auto slot2 = std::make_shared<atom::async::ScopedSignal<int>::SlotType>(
    [](int value) { std::cout << "作用域槽 2 收到: " << value << std::endl; });
...
// 9. 错误处理示例void errorHandlingExample() {
...
atom::async::AsyncSignal<int> asyncSignal;
asyncSignal.connect([](int value) {
    throw std::runtime_error("异步槽故意抛出的异常");
});
...
// 10. 多参数信号示例void multiParameterSignalExample() {
