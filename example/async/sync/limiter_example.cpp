...
// Helper struct for coroutine taskstruct task {
struct promise_type {
...
// 示例协程函数auto rate_limited_task(atom::async::RateLimiter& limiter,
                       ...
                       int main() {
                           ...
                           std::cout << "===== atom::async::limiter.hpp "
                                        "使用示例 ====="
                                     << std::endl
                                     << std::endl;
                           ...
                           LOG("1. RateLimiter 基本用法");
                           {
                               ...
                               limiter.setFunctionLimit(
                                   "test_function", 3,
                                   2s);  // 每2秒最多允许3次调用

                               LOG("尝试执行 test_function 5次 "
                                   "(限制为每2秒3次)");
                               ...
                               // 检查被拒绝的请求数量
                               auto rejected =
                                   limiter.getRejectedRequests("test_function");
                               LOG("test_function 被拒绝的请求数: " +
                           ...
                           LOG("2. RateLimiter 不同参数组合");
                           {
                               ...
                               std::vector<std::pair<
                                   ...
                                   settings = {
                                       {"batch_func1",
                                        atom::async::RateLimiter::Settings(5,
                                                                           3s)},
                                       {"batch_func2",
                                        atom::async::RateLimiter::Settings(
                                            3, 4s)}};
                               ...
                               LOG("\n批量设置了函数限制 (batch_func1, "
                                   "batch_func2)");

                               // 批量获取限流器
                               std::vector<std::string_view> func_names = {
                                   "batch_func1", "batch_func2"};
                           ...
                           {
                               atom::async::RateLimiter limiter;
                               limiter.setFunctionLimit("pausable_function", 2,
                                                        1s);
                           ...
                           LOG("5. RateLimiter 边缘情况");
                           {
                               ...
                               // 边缘情况1: 设置为0的限制
                               try {
                                   limiter.setFunctionLimit("zero_limit", 0,
                                                            1s);
                               ...
                               // 边缘情况2: 设置为负的时间窗口
                               try {
                                   limiter.setFunctionLimit("negative_window",
                                                            5, -1s);
                               } catch (const std::invalid_argument& e) {
                                   LOG("边缘情况2 - 负的时间窗口: " +
                                       std::string(e.what()));
                               ...
                               // 边缘情况3: 非常高的请求限制
                               limiter.setFunctionLimit("very_high_limit",
                                                        1000000, 1s);
                               LOG("边缘情况3 - 设置了非常高的请求限制: "
                                   "1000000/秒");

                               // 边缘情况4: 非常低的请求限制
                               limiter.setFunctionLimit("very_low_limit", 1,
                                                        300s);
                               LOG("边缘情况4 - 设置了非常低的请求限制: "
                                   "1/300秒");

                               // 边缘情况5: 对不存在的函数获取被拒绝的请求数
                               auto rejected = limiter.getRejectedRequests(
                           ...
                           //==============================================================
                           LOG("Debounce/Throttle 示例已迁移至独立文件: "
                               "example/async/lodash.cpp");
                           LOG("该文件包含: "
                               "Debounce(leading/trailing/maxWait/cancel/flush/"
                               "reset/callCount)、");
                           LOG("Throttle(leading/trailing/both/cancel/reset/"
                               "callCount)、以及工厂类用法。");
                           LOG("此 limiter.cpp 保留 RateLimiter "
                               "相关演示，避免篇幅过长。");
                           ...
                           {
                               // 获取单例实例
                               auto& limiter = atom::async::
                                   RateLimiterSingleton::instance();

                               // 设置限流参数
                               limiter.setFunctionLimit("singleton_func", 2,
                                                        1s);
