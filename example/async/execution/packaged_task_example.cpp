...
// Helper class: Print thread IDstd::string get_thread_id() {
std::stringstream ss;
ss << std::this_thread::get_id();
return ss.str();
}

// Print mutexstd::mutex print_mutex;

// Thread-safe print functiontemplate <typename... Args>
void print_safe(Args&&... args) {
    std::lock_guard<std::mutex> lock(print_mutex);
    (std::cout << ... << args) << std::endl;
}

// Print separator linevoid print_section(const std::string& title) {
std::lock_guard<std::mutex> lock(print_mutex);
std::cout << "\n========== " << title << " ==========\n" << std::endl;
}

// Simple counter to observe callback executionclass CallbackCounter {
public:
...
// 1. Basic Usage Examplesvoid basic_usage_examples() {
...
print_safe("Main thread [", get_thread_id(), "] Waiting for Task 1 result...");
...
atom::async::EnhancedPackagedTask<std::string, int, double> task2([](int a,
                                                                     double b) {
    print_safe("Thread [", get_thread_id(), "] Calculating ", a, " + ", b);
...
// 2. Different Parameter Combination Examplesvoid
...
// 3. Edge Cases and Special Situations Examplesvoid edge_cases_examples() {
...
try {
    ...
    print_safe("Error: Creation with nullptr did not throw.");  // Should not
                                                                // reach here
...
print_safe("Task 2 cancellation successful? ", was_cancelled ? "Yes" : "No");
...
// 4. Error Handling Examplesvoid error_handling_examples() {
...
// 5. Callback Function Examplesvoid callback_examples() {
...
atom::async::EnhancedPackagedTask<int> task([]() {
    print_safe("Thread [", get_thread_id(), "] Executing task with callbacks");
    std::this_thread::sleep_for(std::chrono::milliseconds(200));
    return 100;
});
...
// 6. Performance Test Examplevoid performance_examples() {
...
print_safe("Executed ", completed_count, " tasks successfully (", failed_count,
           " failed) in: ", duration.count(), " ms");
