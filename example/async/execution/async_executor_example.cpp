...
// 格式化时间为字符串的辅助函数std::string formatTime(const
...
// 打印带时间戳和线程ID的消息void log(const std::string& message) {
...
// 示例基础任务：休眠并返回一个值int basicTask(int id, int sleepMs) {
log("执行任务 #" + std::to_string(id) + "，休眠 " + std::to_string(sleepMs) +
    "ms");
...
// 计算密集型任务long long fibonacciTask(int n) {
if (n <= 1)
    return n;
...
// 1. 基本用法示例void basicUsageExample() {
log("\n=== 1. 基本用法示例 ===");
...
// 2. 优先级任务示例void priorityTasksExample() {
log("\n=== 2. 优先级任务示例 ===");
...
// 3. 延迟执行示例void deferredTasksExample() {
log("\n=== 3. 延迟执行示例 ===");
...
// 5. 错误处理示例void errorHandlingExample() {
log("\n=== 5. 错误处理示例 ===");
