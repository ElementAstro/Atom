...
// 辅助函数：格式化输出当前时间std::string getCurrentTimeStr() {
...
// 辅助函数：打印带有线程ID的消息void printMessage(const std::string& message) {
std::cout << "[" << getCurrentTimeStr() << "][线程 "
          << std::this_thread::get_id() << "] " << message << std::endl;
}

// 基本任务函数：休眠指定时间并返回一个值int basicTask(int id, int sleepMs) {
...
// 示例1：基本用法void basicUsageExample() {
std::cout << "\n=== 示例1：基本线程池用法 ===\n" << std::endl;
...
// 示例2：使用不同的线程池配置void threadPoolConfigExample() {
std::cout << "\n=== 示例2：不同线程池配置 ===\n" << std::endl;

// 高性能线程池
{
    ThreadPool::Options options = ThreadPool::Options::createHighPerformance();
...
// 自定义配置线程池
{
    ...
    options.threadPriority = ThreadPool::Options::ThreadPriority::BelowNormal;
...
// 示例3：批量任务提交和处理void batchTasksExample() {
std::cout << "\n=== 示例3：批量任务提交 ===\n" << std::endl;
...
// 示例4：Promise和Future高级用法void promiseFutureExample() {
...
try {
    ...
    printMessage("Promise任务回调2: 新结果是 " + std::to_string(result * 2));
...
// 示例5：错误处理void errorHandlingExample() {
std::cout << "\n=== 示例5：错误处理 ===\n" << std::endl;
...
// 示例7：自动增长和工作窃取void autoGrowthExample() {
std::cout << "\n=== 示例7：自动增长和工作窃取 ===\n" << std::endl;
...
// 观察线程池是否自动增长
std::this_thread::sleep_for(500ms);
printMessage("提交任务后线程池大小: " + std::to_string(pool.getThreadCount()));
...
// 示例8：全局线程池和辅助函数void globalPoolExample() {
std::cout << "\n=== 示例8：全局线程池和辅助函数 ===\n" << std::endl;
...
// 示例9：边界情况和极限测试void edgeCasesExample() {
std::cout << "\n=== 示例9：边界情况和极限测试 ===\n" << std::endl;
