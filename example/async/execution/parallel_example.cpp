...
~Timer() {
    ...
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                        end_time - start_time)
                        .count();
    std::cout << "完成 " << operation_name << "，耗时: " << duration << " ms"
              << std::endl;
}
}
;

// 辅助函数 - 生成随机数据template <typename T>
...
// 1. 基本的 for_each 并行示例void basic_parallel_for_each() {
std::cout << "\n===== 基本的并行 for_each 示例 =====\n";
...
// 2. 使用 map 函数转换数据void parallel_map_example() {
std::cout << "\n===== 并行 map 示例 =====\n";
...
// 3. 使用 reduce 函数并行求和void parallel_reduce_example() {
std::cout << "\n===== 并行 reduce 求和示例 =====\n";
...
// 4. 并行 filter 过滤数据void parallel_filter_example() {
std::cout << "\n===== 并行 filter 过滤示例 =====\n";
...
// 5. 并行排序示例void parallel_sort_example() {
std::cout << "\n===== 并行排序示例 =====\n";
...
// 验证降序排序是否正确
bool is_descending = std::is_sorted(data.begin(), data.end(), std::greater<>());
std::cout << "降序排序" << (is_descending ? "成功" : "失败") << std::endl;
}

// 6. 使用 C++20 的 std::span 和 std::ranges 的示例void cpp20_features_example()
// {
std::cout << "\n===== C++20 特性示例 =====\n";

// 创建示例数据
std::vector<float> data = generate_random_data<float>(1'000'000, 0.0f, 100.0f);
print_sample(data, "原始数据");
...
// 7. 协程任务示例 - 使用标准库替代void coroutine_task_example() {
std::cout << "\n===== 协程任务示例 =====\n";
...
// 8. SIMD 操作示例void simd_operations_example() {
std::cout << "\n===== SIMD 操作示例 =====\n";
...
// 9. 边界情况和错误处理示例void edge_cases_and_error_handling() {
std::cout << "\n===== 边界情况和错误处理示例 =====\n";
...
// 空数据集
{
    ...
    try {
        ...
        std::cout << "空数据集的 for_each 发生错误: " << e.what() << std::endl;
    }
...
// 单元素数据集
{
    ...
    try {
        ...
        std::cout << "单元素数据集的 sort 成功完成，结果: " << single_data[0]
                  << std::endl;
    } catch (const std::exception& e) {
        std::cout << "单元素数据集的 sort 发生错误: " << e.what() << std::endl;
    }
}
...
// 10. jthread 用法示例 - 使用标准库代替void jthread_example() {
