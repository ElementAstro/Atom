...
int main() {
    ...
    try {
        ...
        std::vector<std::pair<std::string, shortcut_detector::Shortcut>>
            testShortcuts = {
                {"Ctrl+C", shortcut_detector::ShortcutFactory::create(
                               'C', true, false, false, false)},
                {"Alt+Tab", shortcut_detector::ShortcutFactory::createVK(
                                VK_TAB, false, true, false, false)},
                {"Win+D", shortcut_detector::ShortcutFactory::create(
                              'D', false, false, false, true)},
                {"Ctrl+Alt+Del", shortcut_detector::ShortcutFactory::createVK(
                                     VK_DELETE, true, true, false, false)},
                {"F1", shortcut_detector::ShortcutFactory::createVK(
                           VK_F1, false, false, false, false)}};

        spdlog::info("Testing {} shortcuts", testShortcuts.size());

        for (const auto& [name, shortcut] : testShortcuts) {
            spdlog::info("Testing shortcut: {} ({})", name,
                         shortcut.toString());
            ...
            switch (result.status) {
                ...
                case shortcut_detector::ShortcutStatus::CapturedByApp:
                    std::cout << "Captured by Application: "
                              << result.capturingApplication;
                    break;
                case shortcut_detector::ShortcutStatus::CapturedBySystem:
                    std::cout << "Captured by System: "
                              << result.capturingApplication;
                    break;
