...
namespace {
    ...
    public:
        ...
        CFStringWrapper(CFStringWrapper&& other) noexcept : m_string(other.m_string) {
...
public:
    MacOSClipboard() {
        @autoreleasepool {
            ...
            if (!m_pasteboard) {
                throw ClipboardSystemException("Failed to access general pasteboard");
            }
    ...
    bool setText(std::string_view text) override {
        @autoreleasepool {
            ...
            try {
                ...
                if (!nsString) {
                    throw ClipboardFormatException("Failed to create NSString from text");
                }
                
                auto guard = make_scope_guard([nsString] { [nsString release]; });
                
                [m_pasteboard clearContents];
                BOOL success = [m_pasteboard setString:nsString forType:NSPasteboardTypeString];
    ...
    bool setData(unsigned int format, std::span<const std::byte> data) override {
        @autoreleasepool {
            if (!m_pasteboard) {
                return false;
            }
            ...
            try {
                ...
                [m_pasteboard clearContents];
                BOOL success = [m_pasteboard setData:nsData forType:pasteboardType];
    ...
    std::optional<std::vector<std::byte>> getData(unsigned int format) override {
        @autoreleasepool {
            if (!m_pasteboard) {
                return std::nullopt;
            }
    ...
    bool setImage(const cv::Mat& image) override {
        @autoreleasepool {
            ...
            try {
                ...
                auto guard = make_scope_guard([imageRep] { [imageRep release]; });
                
                std::memcpy([imageRep bitmapData], rgbImage.data, rgbImage.total() * rgbImage.elemSize());
                
                NSImage* nsImage = [[NSImage alloc] init];
                [nsImage addRepresentation:imageRep];
                
                auto imageGuard = make_scope_guard([nsImage] { [nsImage release]; });
    ...
    std::optional<cv::Mat> getImageAsMat() override {
        @autoreleasepool {
            ...
            try {
                NSArray* images = [m_pasteboard readObjectsForClasses:@[[NSImage class]] options:nil];
                ...
                if (imageData) {
                    std::memcpy(result.data, imageData, result.total() * result.elemSize());
    ...
    std::optional<cimg_library::CImg<unsigned char>> getImageAsCImg() override {
        ...
        cimg_library::CImg<unsigned char> result(mat->cols, mat->rows, 1, mat->channels());
...
unsigned int Clipboard::Impl::registerFormat(std::string_view formatName) {
    @autoreleasepool {
        NSString* nsFormatName = [[NSString alloc] 
            initWithBytes:formatName.data() 
        ...
        auto guard = make_scope_guard([nsFormatName] { [nsFormatName release]; });
