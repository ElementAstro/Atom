        ...
        switch (format) {
            ...
            case RegistryFormat::BINARY: {
                ...
                serializeNode = [&](const RegistryNode& node) {
                    // Write timestamps
                    file.write(reinterpret_cast<const char*>(&node.created),
                               sizeof(node.created));
                    file.write(
                        reinterpret_cast<const char*>(&node.lastModified),
                        sizeof(node.lastModified));

                    // Write values count
                    uint32_t valueCount =
                        static_cast<uint32_t>(node.values.size());
                    file.write(reinterpret_cast<const char*>(&valueCount),
                               sizeof(valueCount));
                    ...
                    for (const auto& [name, value] : node.values) {
                        ...
                        file.write(reinterpret_cast<const char*>(&nameLen),
                                   sizeof(nameLen));
                        file.write(name.c_str(), nameLen);

                        // Write type length and type
                        uint32_t typeLen =
                            static_cast<uint32_t>(value.type.size());
                        file.write(reinterpret_cast<const char*>(&typeLen),
                                   sizeof(typeLen));
                        file.write(value.type.c_str(), typeLen);

                        // Write data length and data
                        uint32_t dataLen =
                            static_cast<uint32_t>(value.data.size());
                        ...
                        // Write timestamp
                        file.write(
                            reinterpret_cast<const char*>(&value.lastModified),
                            sizeof(value.lastModified));
                    }

                    // Write children count
                    uint32_t childCount =
                        static_cast<uint32_t>(node.children.size());
                    file.write(reinterpret_cast<const char*>(&childCount),
                               sizeof(childCount));
                    ...
                    for (const auto& [childName, childNode] : node.children) {
                        ...
                        uint32_t childNameLen =
                            static_cast<uint32_t>(childName.size());
                        file.write(reinterpret_cast<const char*>(&childNameLen),
                                   sizeof(childNameLen));
                ...
                spdlog::debug("Registry saved in binary format to: {}",
                              actualFilePath);
                break;
            }
        ...
        switch (format) {
            ...
            case RegistryFormat::BINARY: {
                ...
                if (std::string(magic, 8) != std::string("ATOMREG\0", 8)) {
                    lastError =
                        "Invalid binary registry file: bad magic number";
                ...
                if (version != 1) {
                    lastError = "Unsupported binary registry version: " +
                                std::to_string(version);
                    spdlog::error("Unsupported binary registry version: {}",
                                  version);
                    return RegistryResult::INVALID_FORMAT;
                }
                ...
                deserializeNode = [&]() -> RegistryNode {
                    ...
                    // Read timestamps
                    file.read(reinterpret_cast<char*>(&node.created),
                              sizeof(node.created));
                    file.read(reinterpret_cast<char*>(&node.lastModified),
                              sizeof(node.lastModified));

                    // Read values count
                    uint32_t valueCount;
                    file.read(reinterpret_cast<char*>(&valueCount),
                              sizeof(valueCount));
                    ...
                    for (uint32_t i = 0; i < valueCount; ++i) {
                        ...
                        // Read type
                        uint32_t typeLen;
                        file.read(reinterpret_cast<char*>(&typeLen),
                                  sizeof(typeLen));
                        ...
                        // Read data
                        uint32_t dataLen;
                        file.read(reinterpret_cast<char*>(&dataLen),
                                  sizeof(dataLen));
                        ...
                        // Read timestamp
                        std::time_t lastModified;
                        file.read(reinterpret_cast<char*>(&lastModified),
                                  sizeof(lastModified));
                    ...
                    // Read children count
                    uint32_t childCount;
                    file.read(reinterpret_cast<char*>(&childCount),
                              sizeof(childCount));
                    ...
                    for (uint32_t i = 0; i < childCount; ++i) {
                        // Read child name
                        uint32_t childNameLen;
                        file.read(reinterpret_cast<char*>(&childNameLen),
                                  sizeof(childNameLen));
                ...
                spdlog::debug("Registry loaded from binary format: {}",
                              filePath);
                return RegistryResult::SUCCESS;
            }
