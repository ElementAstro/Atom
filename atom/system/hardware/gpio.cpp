        ...
        for (DWORD i = 0; SetupDiEnumDeviceInterfaces(deviceInfo, NULL, &guid,
            ...
            if (SetupDiGetDeviceInterfaceDetail(deviceInfo, &interfaceData,
                ...
                if (deviceHandle_ != INVALID_HANDLE_VALUE) {
                    // Verify this is a GPIO-capable device by checking
                    // attributes
                    ...
                    if (HidD_GetAttributes(deviceHandle_, &attributes)) {
                        ...
                        spdlog::debug(
                            "Found HID device - VID: 0x{:04X}, PID: 0x{:04X}, "
                            "Version: 0x{:04X}",
                            attributes.VendorID, attributes.ProductID,
                            attributes.VersionNumber);

                        // Accept any HID device as potential GPIO controller
                        // In production, you might want to filter by specific
                        // VID/PID For example: if (attributes.VendorID ==
                        // 0x1234 && attributes.ProductID == 0x5678)
                        ...
                        spdlog::debug(
                            "Device does not support HID attributes, "
                            "continuing search");
