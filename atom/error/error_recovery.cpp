...
auto CircuitBreaker::execute(Func&& func,
...
template <typename T>
ErrorRecoveryExecutor<T>& ErrorRecoveryExecutor<T>::withRetryPolicy(
...
template <typename T>
ErrorRecoveryExecutor<T>& ErrorRecoveryExecutor<T>::withCircuitBreaker(
...
template <typename T>
ErrorRecoveryExecutor<T>& ErrorRecoveryExecutor<T>::withFallback(
...
template <typename T>
ErrorRecoveryExecutor<T>& ErrorRecoveryExecutor<T>::withTimeout(
    std::chrono::milliseconds timeout) {
    timeout_ = timeout;
    return *this;
}
...
template <typename T>
template <typename Func, typename... Args>
std::future<T> ErrorRecoveryExecutor<T>::executeAsync(Func&& func,
                                                      Args&&... args) {
    return std::async(
        std::launch::async,
        [this, func = std::forward<Func>(func), args...]() mutable {
...
template <typename Func, typename... Args>
auto Bulkhead::execute(Func&& func,
