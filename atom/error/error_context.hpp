...
public:
    ...
    [[nodiscard]] auto getMessage() const -> const std::string& { return message_; }
    [[nodiscard]] auto getTimestamp() const -> std::chrono::system_clock::time_point { return timestamp_; }
    [[nodiscard]] auto getThreadId() const -> std::thread::id { return threadId_; }
    
    // Error metadata
    [[nodiscard]] auto getSeverity() const -> ErrorSeverity { return metadata_.severity; }
    [[nodiscard]] auto getCategory() const -> ErrorCategory { return metadata_.category; }
    [[nodiscard]] auto getRecoveryStrategy() const -> ErrorRecoveryStrategy { return metadata_.recovery; }
    [[nodiscard]] auto getMetadata() const -> const ErrorMetadata& { return metadata_; }
    ...
    auto setSystemInfo(const std::string& key, std::string value) -> ErrorContext&;
    [[nodiscard]] auto getSystemInfo(const std::string& key) const -> std::string;
    ...
    static auto create(int errorCode, const std::string& message = "") -> std::shared_ptr<ErrorContext>;
    static auto createWithCorrelation(int errorCode, const std::string& correlationId, 
...
/**
 * @brief Error context manager for tracking error contexts across the application
 */
...
public:
     ...
     */
    [[nodiscard]] auto getContext(const ErrorId& errorId) const -> std::shared_ptr<ErrorContext>;
    
    /**
     * @brief Get all contexts with a specific correlation ID
     */
    [[nodiscard]] auto getContextsByCorrelation(const std::string& correlationId) const 
...
#define CREATE_ERROR_CONTEXT(errorCode, message) \
    atom::error::ErrorContext::create(errorCode, message) \
        ->setSystemInfo("file", ATOM_FILE_NAME) \
...
#define SCOPED_ERROR_CONTEXT(errorCode, message) \
    atom::error::ScopedErrorContext scopedContext(CREATE_ERROR_CONTEXT(errorCode, message))
