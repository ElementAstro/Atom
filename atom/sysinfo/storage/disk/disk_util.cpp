...
std::unordered_map<
    std::string, std::pair<std::string, std::chrono::steady_clock::time_point>>
    g_fsTypeCache;
...
std::future<std::string> getFileSystemTypeAsync(const std::string& path) {
    return std::async(std::launch::async,
                      [path]() { return getFileSystemType(path); });
}

DiskPerformanceMetrics benchmarkDiskPerformance(const std::string& path,
                                                uint32_t testSizeMB) {
    ...
    try {
        ...
        // Calculate latencies
        auto writeDuration =
            std::chrono::duration_cast<std::chrono::milliseconds>(writeEnd -
                                                                  writeStart);
        auto readDuration =
            std::chrono::duration_cast<std::chrono::milliseconds>(readEnd -
                                                                  readStart);
...
std::vector<double> calculateDiskUsagePercentageBatch(
    ...
    std::transform(diskInfos.begin(), diskInfos.end(),
                   std::back_inserter(results),
                   [](const DiskInfo& info) -> double {
                       return calculateDiskUsagePercentage(info.totalSpace,
                                                           info.freeSpace);
                   });
