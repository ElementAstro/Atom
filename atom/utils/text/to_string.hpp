 ...
 * @details This concept checks if a type has key_type, mapped_type, and is a
 * range.
 */
template <typename T>
...
public:
    explicit ToStringException(std::string message)
        : message_(std::format("ToString conversion error: {}",
                               std::move(message))) {}
...
constexpr auto toString(T&& value) -> std::string {
    try {
        ...
        throw ToStringException(
            std::format("String conversion failed: {}", e.what()));
    }
}
 ...
 * @return The converted std::string.
 */
inline auto toString(const std::string& value) -> std::string { return value; }
...
constexpr auto toString(T value) -> std::string {
    try {
        ...
        throw ToStringException(
            std::format("Enum conversion failed: {}", e.what()));
    }
}
    ...
    try {
        ...
        if constexpr (MapType<T>) {
            ...
            for (const auto& [key, value] : container) {
                ...
                try {
                    result +=
                        std::format("{}: {}", toString(key), toString(value));
                } catch (const std::exception& e) {
        ...
        throw ToStringException(
            std::format("Container conversion failed: {}", e.what()));
    }
}
...
auto toString(const T& value) -> std::string {
    try {
        ...
        throw ToStringException(
            std::format("General type conversion failed: {}", e.what()));
    }
}
...
auto joinCommandLine(const Args&... args) -> std::string {
    ...
    try {
        ...
        throw ToStringException(
            std::format("Command line joining failed: {}", e.what()));
    }
}
...
auto toStringArray(const T& array,
    ...
    try {
        ...
        throw ToStringException(
            std::format("Array conversion failed: {}", e.what()));
    }
}
...
auto toStringRange(Iterator begin, Iterator end,
    ...
    try {
        ...
        throw ToStringException(
            std::format("Range conversion failed: {}", e.what()));
    }
}
...
auto toString(const std::array<T, N>& array) -> std::string {
    try {
        ...
        throw ToStringException(
            std::format("std::array conversion failed: {}", e.what()));
    }
}
...
auto tupleToStringImpl(const Tuple& tpl, std::index_sequence<I...>,
    ...
    try {
        ...
        bool first = true;
        (((!first ? result += separator : result), first = false,
          [&result, &tpl]() {
              try {
        ...
        throw ToStringException(
            std::format("Tuple conversion failed: {}", e.what()));
    }
}
...
auto toString(const std::optional<T>& opt) -> std::string {
    try {
        ...
        throw ToStringException(
            std::format("Optional conversion failed: {}", e.what()));
    }
}
