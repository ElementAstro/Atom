...
void SecureMemory::secureClear(void* ptr, size_t size) noexcept {
    ...
    if (RAND_bytes(static_cast<unsigned char*>(ptr), static_cast<int>(size)) != 1) {
...
template void SecureMemory::secureClear<uint8_t>(std::vector<uint8_t>&) noexcept;
...
Result<std::vector<uint8_t>> KeyDerivation::deriveKey(
    ...
    int result = PKCS5_PBKDF2_HMAC(
        password.data(), static_cast<int>(password.length()),
...
std::vector<uint8_t> EncryptedData::serialize() const {
    ...
    // Format: [version:1][method:1][iterations:4][salt_len:4][iv_len:4][tag_len:4][cipher_len:4]
...
Result<EncryptedData> EncryptedData::deserialize(const std::vector<uint8_t>& data) {
    ...
    result.ciphertext.assign(data.begin() + pos, data.begin() + pos + cipherLen);
...
Result<EncryptedData> Encryption::encrypt(
    ...
    SecureMemory::secureClear(const_cast<std::vector<uint8_t>&>(keyResult.value()));
...
Encryption::encryptAesGcm(
    ...
    try {
        ...
        // Encrypt
        std::vector<uint8_t> ciphertext(plaintext.length() + 16); // Extra space for padding
        int len = 0;
...
Result<std::string> Encryption::decryptAesGcm(
    ...
    try {
        ...
        return Result<std::string>(std::string(plaintext.begin(),
