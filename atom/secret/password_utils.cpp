...
Result<std::string> PasswordGenerator::generatePassword(const GenerationOptions& options) {
    ...
    if (charset.empty()) {
        return Result<std::string>("No characters available for password generation");
    }
...
Result<std::string> PasswordGenerator::generateMemorablePassword(
    ...
    static const std::vector<std::string> words = {
        ...
        "zebra", "beach", "cloud", "dream", "earth", "field", "green", "happy",
        "island", "magic", "north", "power", "quick", "smile", "trust", "world"
    };
...
std::string PasswordGenerator::buildCharacterSet(const GenerationOptions& options) {
    std::string charset;
...
std::string PasswordGenerator::validateOptions(const GenerationOptions& options) {
...
std::string PasswordGenerator::ensureMinimumRequirements(
    ...
    for (char c : password) {
        if (std::islower(c)) lowercase++;
        else if (std::isupper(c)) uppercase++;
...
PasswordValidator::AnalysisResult PasswordValidator::analyzePassword(std::string_view password) {
    ...
    for (char c : password) {
        if (std::islower(c)) result.hasLowercase = true;
        else if (std::isupper(c)) result.hasUppercase = true;
        else if (std::isdigit(c)) result.hasDigits = true;
        else result.hasSpecial = true;
    }
    ...
    if (result.hasLowercase) score += 5;
    if (result.hasUppercase) score += 5;
    if (result.hasDigits) score += 5;
    if (result.hasSpecial) score += 10;

    // Pattern penalties
    if (result.hasRepeatedChars) score -= 20;
    if (result.hasSequentialChars) score -= 15;
    if (result.isCommonPassword) score -= 30;

    // Entropy bonus
    if (result.entropy > 50) score += 15;

    result.score = std::max(0, std::min(100, score));

    // Determine strength
    if (result.score < 20) result.strength = PasswordStrength::VeryWeak;
    else if (result.score < 40) result.strength = PasswordStrength::Weak;
    else if (result.score < 60) result.strength = PasswordStrength::Medium;
    else if (result.score < 80) result.strength = PasswordStrength::Strong;
    else result.strength = PasswordStrength::VeryStrong;
...
Result<bool> PasswordValidator::validatePassword(
    ...
    if (settings.requireMixedCase && (!hasLower || !hasUpper)) {
        return Result<bool>("Password must contain both uppercase and lowercase letters");
    }
    ...
    if (settings.requireSpecialChars && !hasSpecial) {
        return Result<bool>("Password must contain at least one special character");
    }
...
bool PasswordValidator::isCommonPassword(std::string_view password) {
    ...
    return std::find(commonPasswords.begin(), commonPasswords.end(), lowerPassword)
           != commonPasswords.end();
}

double PasswordValidator::estimateCrackTime(std::string_view password, double guessesPerSecond) {
...
int PasswordValidator::calculateCharacterSetSize(std::string_view password) {
    ...
    if (hasUpper) size += 26;
    if (hasDigit) size += 10;
    if (hasSpecial) size += 32; // Approximate number of common special characters
...
const std::vector<std::string>& PasswordValidator::getCommonPasswords() {
    static const std::vector<std::string> commonPasswords = {
        ...
        "master", "princess", "login", "guest", "solo", "sunshine", "shadow",
...
bool SecureComparison::constantTimeEquals(std::string_view a, std::string_view b) noexcept {
...
bool SecureComparison::constantTimeEquals(const void* a, const void* b, size_t size) noexcept {
