...
public:
     ...
     */
    template <typename T>
    [[nodiscard]] static auto convolve(std::span<const T> image, i32 width, i32 height,
                                      std::span<const f32> kernel, i32 kernel_size) -> std::vector<T> {
        if (kernel_size % 2 == 0) {
            ...
            for (i32 x = 0; x < width; ++x) {
                ...
                for (i32 ky = -half_kernel; ky <= half_kernel; ++ky) {
                    for (i32 kx = -half_kernel; kx <= half_kernel; ++kx) {
                        ...
                        i32 kernel_idx = (ky + half_kernel) * kernel_size + (kx + half_kernel);
                        sum += static_cast<f32>(image[py * width + px]) * kernel[kernel_idx];
                    }
                }
                
                result[y * width + x] = static_cast<T>(std::clamp(sum, 
                    static_cast<f32>(std::numeric_limits<T>::min()),
     ...
     */
    template <typename T>
    [[nodiscard]] static auto gaussianBlur(std::span<const T> image, i32 width, i32 height, 
                                          f32 sigma) -> std::vector<T> {
        // Generate Gaussian kernel
        i32 kernel_size = static_cast<i32>(std::ceil(6 * sigma)) | 1; // Ensure odd size
     ...
     */
    template <typename T>
    [[nodiscard]] static auto sobelEdgeDetection(std::span<const T> image, i32 width, i32 height) -> std::vector<T> {
        // Sobel X kernel
        constexpr std::array<f32, 9> sobel_x = {
            -1, 0, 1,
            -2, 0, 2,
            -1, 0, 1
        };
        ...
        // Sobel Y kernel
        constexpr std::array<f32, 9> sobel_y = {
            -1, -2, -1,
             0,  0,  0,
             1,  2,  1
        };
        ...
        for (usize i = 0; i < image.size(); ++i) {
            f32 magnitude = std::sqrt(static_cast<f32>(grad_x[i] * grad_x[i] + grad_y[i] * grad_y[i]));
            result[i] = static_cast<T>(std::clamp(magnitude,
                static_cast<f32>(std::numeric_limits<T>::min()),
                static_cast<f32>(std::numeric_limits<T>::max())));
        }
     ...
     */
    template <typename T>
    [[nodiscard]] static auto laplacianEdgeDetection(std::span<const T> image, i32 width, i32 height) -> std::vector<T> {
        constexpr std::array<f32, 9> laplacian = {
             0, -1,  0,
            -1,  4, -1,
             0, -1,  0
        };
     ...
     */
    template <typename T>
    [[nodiscard]] static auto adjustBrightnessContrast(std::span<const T> image, 
                                                      f32 brightness, f32 contrast) -> std::vector<T> {
        std::vector<T> result(image.size());
        ...
        if constexpr (std::same_as<T, u8>) {
            ...
            for (usize i = 0; i < simd_end; i += 8) {
                ...
                __m128i bytes = _mm_loadl_epi64(reinterpret_cast<const __m128i*>(&image[i]));
                ...
                // Pack back to bytes (this is simplified - full implementation would need proper packing)
                for (i32 j = 0; j < 8; ++j) {
                    result[i + j] = static_cast<u8>(_mm256_extract_epi32(ints, j));
                }
            }
...
#endif
        {
            ...
            for (usize i = 0; i < image.size(); ++i) {
                f32 value = static_cast<f32>(image[i]) * contrast + brightness;
                result[i] = static_cast<T>(std::clamp(value,
                    static_cast<f32>(std::numeric_limits<T>::min()),
                    static_cast<f32>(std::numeric_limits<T>::max())));
            }
        }
     ...
     */
    template <typename T>
    [[nodiscard]] static auto computeHistogram(std::span<const T> image, i32 bins = 256) -> std::vector<u32> {
        ...
        f32 scale = static_cast<f32>(bins - 1) / static_cast<f32>(max_val - min_val);
     ...
     */
    template <typename T>
    [[nodiscard]] static auto histogramEqualization(std::span<const T> image) -> std::vector<T> {
