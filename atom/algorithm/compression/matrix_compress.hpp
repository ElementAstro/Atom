...
auto MatrixCompressor::downsample(const M& matrix, i32 factor) -> Matrix {
    ...
    // Use ceiling division to include edge blocks
    i32 newRows = std::max(1, (rows + factor - 1) / factor);
    i32 newCols = std::max(1, (cols + factor - 1) / factor);
        ...
        for (i32 i = 0; i < newRows; ++i) {
            for (i32 j = 0; j < newCols; ++j) {
                ...
                for (i32 di = 0; di < factor && i * factor + di < rows; ++di) {
                    for (i32 dj = 0; dj < factor && j * factor + dj < cols;
                         ++dj) {
