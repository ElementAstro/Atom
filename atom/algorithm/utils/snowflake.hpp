...
public:
    ...
    [[nodiscard]] auto nextid() -> std::array<u64, N> {
        ...
        for (usize i = 0; i < N; ++i) {
            ...
            // Increment sequence for next ID in batch
            if (i < N - 1) {
                sequence_ = (sequence_ + 1) & SEQUENCE_MASK;
                if (sequence_ == 0) {
                    u64 current_last = last_timestamp_.load();
     ...
     * Snowflake instance by verifying the datacenter ID, worker ID,
     * secret key, and timestamp.
     *
    ...
    void deserialize(const std::string &state) {
        ...
        // Note: secret_key_ is NOT restored to maintain instance uniqueness
    }
