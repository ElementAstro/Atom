...
public:
     ...
     */
    [[nodiscard]] static auto generateTimeBased(const std::array<u8, 6>& node_id) -> UUID {
        static thread_local std::random_device rd;
        static thread_local std::mt19937 gen(rd());
        static thread_local std::uniform_int_distribution<u16> clock_seq_dis(0, 0x3FFF);
     ...
     * @return Nil UUID
     */
    [[nodiscard]] static auto generateNil() -> UUID {
        return UUID{};
    }
    ...
    [[nodiscard]] auto isNil() const -> bool {
        return std::all_of(data_.begin(), data_.end(), [](u8 b) { return b == 0; });
    }
...
struct hash<atom::algorithm::UUID> {
    auto operator()(const atom::algorithm::UUID& uuid) const noexcept -> size_t {
        const auto& data = uuid.getData();
        size_t h1 = hash<uint64_t>{}(*reinterpret_cast<const uint64_t*>(data.data()));
        size_t h2 = hash<uint64_t>{}(*reinterpret_cast<const uint64_t*>(data.data() + 8));
        return h1 ^ (h2 << 1);
    }
};
} // namespace std
