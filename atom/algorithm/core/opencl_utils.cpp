...
auto Platform::getDevices(cl_platform_id platform,
                          DeviceType device_type) -> std::vector<cl_device_id> {
    cl_uint num_devices;
    cl_int err =
        clGetDeviceIDs(platform, static_cast<cl_device_type>(device_type), 0,
                       nullptr, &num_devices);
...
auto Platform::getDeviceInfo(cl_device_id device) -> DeviceInfo {
    ...
    clGetDeviceInfo(device, CL_DEVICE_VENDOR, vendor_size, vendor.data(),
                    nullptr);
    ...
    clGetDeviceInfo(device, CL_DEVICE_VERSION, version_size, version.data(),
                    nullptr);
...
auto Platform::createContext(const std::vector<cl_device_id>& devices)
    -> Context {
    cl_int err;
    cl_context context =
        clCreateContext(nullptr, static_cast<cl_uint>(devices.size()),
...
auto Platform::createCommandQueue(const Context& context,
                                  cl_device_id device) -> CommandQueue {
    cl_int err;
    cl_command_queue queue =
        clCreateCommandQueue(context.get(), device, 0, &err);
    if (err != CL_SUCCESS) {
        THROW_RUNTIME_ERROR("Failed to create OpenCL command queue: error {}",
                            err);
    }
...
auto Platform::createBuffer(const Context& context, MemoryFlags flags,
                            usize size, void* host_ptr) -> Buffer {
    cl_int err;
    cl_mem buffer = clCreateBuffer(
        context.get(), static_cast<cl_mem_flags>(flags), size, host_ptr, &err);
    if (err != CL_SUCCESS) {
...
auto Platform::buildKernel(const Context& context,
    ...
    cl_program program = clCreateProgramWithSource(
        context.get(), 1, &source_ptr, &source_size, &err);
    if (err != CL_SUCCESS) {
    ...
    // Build program
    err = clBuildProgram(
        program, static_cast<cl_uint>(devices.size()), devices.data(),
        build_options.empty() ? nullptr : build_options.c_str(), nullptr,
        nullptr);
    ...
    if (err != CL_SUCCESS) {
        THROW_RUNTIME_ERROR("Failed to create OpenCL kernel '{}': error {}",
                            kernel_name, err);
    }
