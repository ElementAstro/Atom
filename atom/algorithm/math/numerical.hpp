...
public:
     ...
     */
    [[nodiscard]] static auto newtonRaphson(const Function& f, const Function& df,
                                           T initial_guess, T tolerance = T{1e-10},
     ...
     * @return Root if found, nullopt otherwise
     */
    [[nodiscard]] static auto bisection(const Function& f, T a, T b,
                                       T tolerance = T{1e-10},
                                       usize max_iterations = 100) -> std::optional<T> {
        T fa = f(a);
        T fb = f(b);
     ...
     * @return Root if found, nullopt otherwise
     */
    [[nodiscard]] static auto secant(const Function& f, T x0, T x1,
                                    T tolerance = T{1e-10},
                                    usize max_iterations = 100) -> std::optional<T> {
        T f0 = f(x0);
        T f1 = f(x1);
     ...
     */
    [[nodiscard]] static auto centralDifference(const Function& f, T x, T h = T{1e-8}) -> T {
     ...
     * @return Approximate derivative value
     */
    [[nodiscard]] static auto forwardDifference(const Function& f, T x, T h = T{1e-8}) -> T {
        return (f(x + h) - f(x)) / h;
    }
     ...
     * @return Approximate derivative value
     */
    [[nodiscard]] static auto backwardDifference(const Function& f, T x, T h = T{1e-8}) -> T {
        return (f(x) - f(x - h)) / h;
    }
     ...
     */
    [[nodiscard]] static auto gaussianElimination(std::vector<std::vector<T>>& A,
