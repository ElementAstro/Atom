...
public:
     ...
     * @param data Input data
     * @param sample_correction Whether to use sample correction (n-1 denominator)
     * @return Sample variance
     */
    [[nodiscard]] static auto variance(std::span<const T> data, bool sample_correction = true) -> T {
        if (data.size() <= 1) {
            return T{0};
        }
     ...
     */
    [[nodiscard]] static auto standardDeviation(std::span<const T> data, bool sample_correction = true) -> T {
    ...
    [[nodiscard]] static auto kurtosis(std::span<const T> data) -> T {
        ...
        T sum_fourth = std::transform_reduce(
            data.begin(), data.end(), T{0}, std::plus<T>{},
            [mean_val, std_dev](T x) {
                T normalized = (x - mean_val) / std_dev;
        ...
        return (sum_fourth / static_cast<T>(data.size())) - T{3}; // Excess kurtosis
    }
     ...
     */
    [[nodiscard]] static auto correlation(std::span<const T> x, std::span<const T> y) -> T {
     ...
     * @return Covariance
     */
    [[nodiscard]] static auto covariance(std::span<const T> x, std::span<const T> y, 
     ...
     */
    [[nodiscard]] static auto percentile(std::vector<T> data, T percentile) -> T {
     ...
     */
    [[nodiscard]] static auto detectOutliers(std::vector<T> data, T multiplier = T{1.5}) -> std::vector<T> {
        if (data.size() < 4) {
            return {};
        }
     ...
     */
    [[nodiscard]] static auto zScores(std::span<const T> data) -> std::vector<T> {
        if (data.empty()) {
            return {};
        }
