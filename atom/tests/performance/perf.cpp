...
std::shared_ptr<spdlog::logger> Perf::logger = []() {
    try {
        ...
        auto logger = std::make_shared<spdlog::logger>("perf", console_sink);
        logger->set_level(spdlog::level::info);
        spdlog::register_logger(logger);
        return logger;
    } catch (const spdlog::spdlog_ex& ex) {
        auto fallback = spdlog::stdout_color_mt("perf_fallback");
        fallback->error("Failed to create perf logger: {}", ex.what());
        return fallback;
    }
}();
