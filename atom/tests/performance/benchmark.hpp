#ifndef ATOM_TEST_PERFORMANCE_BENCHMARK_HPP
#define ATOM_TEST_PERFORMANCE_BENCHMARK_HPP
...
private:
    ...
    void warmupRun(const SetupFunc& setupFunc, const Func& func,
        ...
        try {
            ...
            log(LogLevel::Minimal, "Exception during warmup for benchmark '" +
                                       name_ + "': " + e.what());
            // Decide if warmup failure should prevent the actual run? For now,
            // just log.
        } catch (...) {
            log(LogLevel::Minimal,
                "Unknown exception during warmup for benchmark '" + name_ +
                    "'");
        }
    }
...
// --- Static Member Implementations (if not defined inline or need more complex
// logic) ---

// Placeholder implementations for platform-specific features
inline bool Benchmark::isCpuStatsSupported() noexcept {
    // TODO: Implement platform-specific check (e.g., using cpuid, performance
    // counters)
    return false;  // Default to not supported
}

inline auto Benchmark::getMemoryUsage() noexcept -> MemoryStats {
    // TODO: Implement platform-specific memory query (e.g., /proc/self/statm on
    // Linux, GetProcessMemoryInfo on Windows)
    return {};  // Return default (zeroed) stats
}

inline auto Benchmark::getCpuStats() noexcept -> CPUStats {
    // TODO: Implement platform-specific CPU counter query (e.g., RDPMC,
    // perf_event_open on Linux, QueryPerformanceCounter on Windows - though
    // that's time, not CPU stats)
    return {};  // Return default (zeroed) stats
}
...
inline std::string Benchmark::getCurrentTimestamp() noexcept {
    try {
        ...
        // Note: std::gmtime might be preferred for timezone independence, but
        // requires careful handling of the returned struct pointer. Using
        // localtime_s or localtime_r for thread-safety if available, otherwise
        // fallback with mutex or accept potential issues. For simplicity here,
        // using std::put_time with std::localtime (potentially non-threadsafe
...
#else
        // Fallback - potentially not thread-safe, consider adding mutex if
        // needed
...
inline void Benchmark::validateInputs() const {
    ...
    if (config_.maxIterations &&
        *config_.maxIterations < static_cast<size_t>(config_.minIterations)) {
        throw std::invalid_argument(
            "Maximum iterations cannot be less than minimum iterations.");
    }
    ...
    if (config_.enableCpuStats && !isCpuStatsSupported()) {
        // Log a warning instead of throwing, allow benchmark to run without CPU
        // stats
        staticLog(LogLevel::Minimal,
                  "Warning: CPU statistics requested but not supported on this "
                  "platform for benchmark '" +
                      name_ + "'. Disabling.");
        // Ideally, modify the config_ instance, but it's const here.
        // This check should perhaps happen earlier or the config should be
        // mutable. For now, the run logic will simply not collect the stats if
        // getCpuStats returns defaults.
    }
    // Add more validation as needed (e.g., file path validity if outputFilePath
    // is set)
}
...
inline void Benchmark::staticLog(LogLevel level, const std::string& message) {
    ...
    if (level > currentGlobalLevel && level != LogLevel::Minimal)
        return;  // Minimal always logs if not Silent
...
inline void Benchmark::log(LogLevel level, const std::string& message) const {
    ...
    if (config_.customLogger) {
        ...
        // Default logger: print to cerr for Minimal, cout otherwise
        auto& stream = (level == LogLevel::Minimal) ? std::cerr : std::cout;
        stream << "[" << getCurrentTimestamp() << "] [" << suiteName_ << "/"
...
inline void Benchmark::registerGlobalLogger(
    std::function<void(const std::string&)> logger) noexcept {
...
inline auto Benchmark::getResults()
    -> const std::map<std::string, std::vector<Benchmark::Result>> {
    std::lock_guard lock(resultsMutex);
    // Return a copy to avoid external modification issues, though returning
    // const& might be more efficient if caller is trusted
    return results;
}

// --- Analysis and Reporting Method Implementations ---

inline auto Benchmark::calculateStandardDeviation(
    ...
    return std::sqrt(
        sq_sum / (values.size() - 1));  // Use sample standard deviation (N-1)
}

inline auto Benchmark::calculateAverageCpuStats(
    std::span<const CPUStats> stats) noexcept -> CPUStats {
    if (stats.empty()) {
        return {};
    }
...
inline void Benchmark::analyzeResults(
    ...
    if (durations.empty()) {
        log(LogLevel::Minimal,
            "No iterations were run for benchmark: " + name_);
        return;
    }
    ...
    if (config_.enableMemoryStats && !memoryStats.empty()) {
        ...
        result.peakMemoryUsage =
            static_cast<double>(peakUsage);  // Use the overall peak observed
    }
...
inline std::string Benchmark::Result::toString() const {
    ...
    if (avgCPUStats.has_value()) {
        ...
        if (avgCPUStats->instructionsExecuted != 0)
            ss << " Instr=" << avgCPUStats->instructionsExecuted;
        if (avgCPUStats->cyclesElapsed != 0)
            ss << ", Cycles=" << avgCPUStats->cyclesElapsed;
        if (avgCPUStats->branchMispredictions != 0)
            ss << ", BranchMispred=" << avgCPUStats->branchMispredictions;
        if (avgCPUStats->cacheMisses != 0)
            ss << ", CacheMiss=" << avgCPUStats->cacheMisses;
    ...
    for (const auto& [suiteName, suiteResults] : results) {
        if (suite.empty() || suite == suiteName) {
            ...
            for (const auto& result : suiteResults) {
                staticLog(
                    LogLevel::Normal,
                    result.toString());  // Use staticLog for console output
            }
        }
    }
...
inline void Benchmark::exportResults(const std::string& filename,
    ...
    if (!outFile) {
        throw std::runtime_error("Failed to open file for exporting results: " +
                                 filename);
    }

    // Disable exceptions for the stream after opening, handle errors manually
    // if needed
    ...
    try {
        switch (format) {
                ...
                for (const auto& [suiteName, suiteResults] : results) {
                    ...
                    for (const auto& res : suiteResults) {
                        ...
                        outFile << "      \"timestamp\": \"" << res.timestamp
                                << "\",\n";
                        outFile
                            << "      \"time_avg_us\": " << res.averageDuration
                            << ",\n";
                        outFile << "      \"time_min_us\": " << res.minDuration
                                << ",\n";
                        outFile << "      \"time_max_us\": " << res.maxDuration
                                << ",\n";
                        outFile << "      \"time_median_us\": "
                                << res.medianDuration << ",\n";
                        outFile << "      \"time_stddev_us\": "
                                << res.standardDeviation << ",\n";
                        outFile << "      \"throughput_ops_sec\": "
                                << res.throughput;  // No comma for last
                                                    // standard field
                        if (res.avgMemoryUsage)
                            outFile << ",\n      \"memory_avg_bytes\": "
                                    << *res.avgMemoryUsage;
                        if (res.peakMemoryUsage)
                            outFile << ",\n      \"memory_peak_bytes\": "
                                    << *res.peakMemoryUsage;
                        if (res.avgCPUStats) {
                            outFile << ",\n      \"cpu_avg_instructions\": "
                                    << res.avgCPUStats->instructionsExecuted;
                            outFile << ",\n      \"cpu_avg_cycles\": "
                                    << res.avgCPUStats->cyclesElapsed;
                            outFile << ",\n      "
                                       "\"cpu_avg_branch_mispredictions\": "
                                    << res.avgCPUStats->branchMispredictions;
                            outFile << ",\n      \"cpu_avg_cache_misses\": "
                                    << res.avgCPUStats->cacheMisses;
                        }
                        if (res.instructionsPerCycle)
                            outFile << ",\n      \"cpu_ipc\": "
                                    << *res.instructionsPerCycle;
                        outFile << "\n    }";
                        firstResult = false;
                    }
                    outFile << "\n  ]";
            ...
            case ExportFormat::Csv: {
                // Header row
                outFile << "Suite,Name,Iterations,Source,Timestamp,AvgTime(us),"
                           "MinTime(us),MaxTime(us),MedianTime(us),StdDevTime("
                           "us),Throughput(ops/sec)";
                // Add optional headers based on whether data exists for *any*
                // result
                bool hasMem = std::any_of(
                    results.begin(), results.end(), [](const auto& p) {
                        return std::any_of(
                            p.second.begin(), p.second.end(),
                            [](const auto& r) {
                                return r.avgMemoryUsage.has_value();
                            });
                    });
                bool hasPeakMem = std::any_of(
                    results.begin(), results.end(), [](const auto& p) {
                        return std::any_of(
                            p.second.begin(), p.second.end(),
                            [](const auto& r) {
                                return r.peakMemoryUsage.has_value();
                            });
                    });
                bool hasCpu = std::any_of(
                    results.begin(), results.end(), [](const auto& p) {
                        return std::any_of(p.second.begin(), p.second.end(),
                                           [](const auto& r) {
                                               return r.avgCPUStats.has_value();
                                           });
                    });
                bool hasIpc = std::any_of(
                    results.begin(), results.end(), [](const auto& p) {
                        return std::any_of(
                            p.second.begin(), p.second.end(),
                            [](const auto& r) {
                                return r.instructionsPerCycle.has_value();
                            });
                    });

                if (hasMem)
                    outFile << ",AvgMem(bytes)";
                if (hasPeakMem)
                    outFile << ",PeakMem(bytes)";
                if (hasCpu)
                    outFile
                        << ",AvgInstr,AvgCycles,AvgBranchMispred,AvgCacheMiss";
                if (hasIpc)
                    outFile << ",IPC";
                outFile << "\n";
                ...
                for (const auto& [suiteName, suiteResults] : results) {
                    for (const auto& res : suiteResults) {
                        outFile << "\"" << suiteName << "\",\"" << res.name
                                << "\"," << res.iterations << ",\""
                                << res.sourceLine << "\",\"" << res.timestamp
                                << "\"," << res.averageDuration << ","
                                << res.minDuration << "," << res.maxDuration
                                << "," << res.medianDuration << ","
                                << res.standardDeviation << ","
                                << res.throughput;
                        if (hasMem)
                            outFile
                                << ","
                                << (res.avgMemoryUsage
                                        ? std::to_string(*res.avgMemoryUsage)
                                        : "");
                        if (hasPeakMem)
                            outFile
                                << ","
                                << (res.peakMemoryUsage
                                        ? std::to_string(*res.peakMemoryUsage)
                                        : "");
                        if (hasCpu) {
                            if (res.avgCPUStats) {
                                outFile << ","
                                ...
                                outFile << ",,,,";  // Empty cells if this
                                                    // result lacks CPU stats
                                                    // but others have it
                            }
                        }
                        if (hasIpc)
            ...
            case ExportFormat::Markdown: {
                ...
                for (const auto& [suiteName, suiteResults] : results) {
                    ...
                    outFile << "| Benchmark | Iterations | Avg Time (us) | Min "
                               "Time (us) | Max Time (us) | Median Time (us) | "
                               "StdDev (us) | Throughput (ops/s) |";
                    if (hasMem)
                        outFile << " Avg Mem (B) |";
                    if (hasPeakMem)
                        outFile << " Peak Mem (B) |";
                    if (hasCpu)
                        outFile << " Avg Instr | Avg Cycles | Avg Branch "
                                   "Mispred | Avg Cache Miss |";
                    if (hasIpc)
                        outFile << " IPC |";
                    outFile << " Source |\n";
                    ...
                    for (const auto& res : suiteResults) {
                        ...
                        if (hasCpu) {
                            if (res.avgCPUStats) {
                                outFile << res.avgCPUStats->instructionsExecuted
                                        << " | "
                                        << res.avgCPUStats->cyclesElapsed
                                        << " | "
                                        << res.avgCPUStats->branchMispredictions
                        ...
                        if (hasIpc) {
                            outFile << std::fixed
                                    << std::setprecision(3);  // IPC precision
                            outFile << (res.instructionsPerCycle
                                            ? std::to_string(
                                                  *res.instructionsPerCycle)
                                            : "")
                                    << " |";
        ...
        throw std::runtime_error("Error writing benchmark results to file '" +
                                 filename + "': " + e.what());
    } catch (...) {
        outFile.close();
        throw std::runtime_error(
            "Unknown error writing benchmark results to file '" + filename +
            "'.");
    }
 ...
 * @param teardownFunc Function to clean up after the benchmark.
 * @param config Configuration settings for the benchmark (Benchmark::Config
 * object).
 */
#define BENCHMARK(suiteName, name, setupFunc, func, teardownFunc, config)    \
    do {                                                                     \
        try {                                                                \
            Benchmark bench(suiteName, name, config,                         \
                            std::source_location::current());                \
            bench.run(setupFunc, func, teardownFunc);                        \
        } catch (const std::exception& e) {                                  \
            Benchmark::staticLog(                                            \
                Benchmark::LogLevel::Minimal,                                \
                "Exception during benchmark setup or execution [" #suiteName \
                "/" #name "]: " +                                            \
                    std::string(e.what()));                                  \
        } catch (...) {                                                      \
            Benchmark::staticLog(Benchmark::LogLevel::Minimal,               \
                                 "Unknown exception during benchmark setup " \
                                 "or execution [" #suiteName "/" #name "]"); \
        }                                                                    \
    } while (false)
...
#define BENCHMARK_DEFAULT(suiteName, name, setupFunc, func, teardownFunc) \
    BENCHMARK(suiteName, name, setupFunc, func, teardownFunc,             \
              Benchmark::Config{})
...
#define BENCHMARK_SIMPLE(suiteName, name, func)                        \
    BENCHMARK(                                                         \
        suiteName, name,                                               \
        []() -> int { return 0; }, /* Empty setup returns dummy int */ \
...
/**
 * @brief Macro to compare multiple implementations
 * @param suiteName Name of the benchmark suite
...
/**
 * @brief Macro to run a benchmark with iterations
 * @param suiteName Name of the benchmark suite
 * @param name Name of the benchmark
...
/**
 * @brief Helper class for benchmark suites
 */
...
public:
    ...
    /**
     * @brief Run all benchmarks in the suite
     */
    void run() {
        for (auto& [benchName, func, config] : benchmarks_) {
            try {
    ...
    /**
     * @brief Run and print results
     */
    void runAndPrint() {
        run();
        Benchmark::printResults(name_);
    }
