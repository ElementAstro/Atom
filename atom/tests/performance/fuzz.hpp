#ifndef ATOM_TEST_PERFORMANCE_FUZZ_HPP
#define ATOM_TEST_PERFORMANCE_FUZZ_HPP
...
private:
    ...
    auto generateIntegersBulkSIMD(int count, IntType min,
                                  IntType max) -> std::vector<IntType>;

    template <typename RealType>
    auto generateRealsBulkSIMD(int count, RealType min,
                               RealType max) -> std::vector<RealType>;

    auto generateBooleansBulkSIMD(int count,
                                  double trueProbability) -> std::vector<bool>;
