#ifndef ATOM_TEST_CORE_TEST_RUNNER_HPP
#define ATOM_TEST_CORE_TEST_RUNNER_HPP
...
private:
    ...
    /**
     * @brief Execute a single test case with retry logic and timeout support
    ...
    void executeTestCase(const TestCase& testCase, int maxRetries) {
        ...
        for (int attempt = 0; attempt <= maxRetries && !shouldStop_.load();
            ...
            try {
                if (timeout.count() > 0) {
                    ...
                    auto result = executeWithTimeout(testCase.func, timeout);
                    passed = result.first;
                    timedOut = result.second;
                    if (timedOut) {
                        errorMessage = "Test timed out after " +
        ...
        recordTestResult(testCase.name, passed, false,
                         passed ? "" : errorMessage, duration, timedOut);
    }
     ...
     * @return Pair of (passed, timedOut)
     */
    auto executeWithTimeout(const std::function<void()>& func,
