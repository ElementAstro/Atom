#ifndef ATOM_TEST_CORE_TEST_CLI_HPP
#define ATOM_TEST_CORE_TEST_CLI_HPP
...
public:
    ...
    [[nodiscard]] auto parse(int argc, char* argv[]) -> ParseResult {
        ParseResult result;
        if (argc < 1) {
            result.errorMessage = "No arguments provided";
            return result;
        }
        programName_ = argv[0];
        ...
        return parseArgsWithResult(args);
    }
    ...
    [[nodiscard]] auto parse(std::span<const std::string_view> args)
        ...
        if (!args.empty()) {
            ...
            return parseArgsWithResult(args.subspan(1));
        }
        result.errorMessage = "No arguments provided";
        return result;
    }
...
private:
    ...
    [[nodiscard]] auto parseArgsWithResult(
        ...
        for (size_t i = 0; i < args.size(); ++i) {
                ...
                if (arg.length() > 2) {
                    ...
                    for (size_t j = 1; j < arg.length(); ++j) {
                        ...
                        if (shortIt == shortNameMap_.end()) {
                            result.errorMessage =
                                "Unknown short option component '" +
                                short_flag + "' in '" + std::string(arg) + "'";
                            return result;
                        }
                        ...
                        if (longIt == options_.end() ||
                            ...
                            if (j == arg.length() - 1 &&
                                ...
                                result.errorMessage =
                                    "Combined short option '" +
                                    std::string(arg) +
                                    "' contains non-flag or requires value "
                                    "before the end.";
                                return result;
                            }
                        }
            ...
            if (!optionPtr) {
                result.errorMessage = "Unknown option: " + std::string(arg);
                return result;
            }
        ...
        for (const auto& [name, option] : options_) {
            if (option.required && !option.isSet) {
                result.errorMessage = "Missing required option: " + name;
                return result;
            }
        }

        // Check for help and list flags
        result.helpRequested = contains("--help");
        result.listRequested = contains("--list");
        result.success = true;
        return result;
    }
};
