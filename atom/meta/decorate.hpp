 ...
 * decorator pattern - OPTIMIZED VERSION \author Max Qian <lightapt.com>
 * (Original) \date 2025-03-12 (Updated) \optimized 2025-01-22 - Performance
 * optimizations by AI Assistant \copyright Copyright (C) 2023-2024 Max Qian
 *
 * ADVANCED META UTILITIES OPTIMIZATIONS:
 * - Reduced std::function overhead with template-based decorators and type
 * erasure
 * - Optimized exception handling with fast-path checks and noexcept
 * specifications
 * - Enhanced memory management with object pooling and small object
 * optimization
 * - Improved template instantiation with better constraints and concept
 * validation
 * - Added compile-time decorator composition optimizations with perfect
 * forwarding
 * - Lock-free decorator caching with atomic operations for high-throughput
 * scenarios
 * - Advanced decorator chaining with compile-time validation and optimization
 * - Memory-efficient decorator storage with template specialization and
 * compression
 */
...
/**
 * \brief Optimized concept to check if a function is callable with specific
 * arguments and return type \tparam F Function type \tparam R Expected return
 * type \tparam Args Argument types
 */
...
template <typename F, typename... Args>
concept Cacheable =
    std::invocable<F, Args...> && (std::is_copy_constructible_v<Args> && ...) &&
