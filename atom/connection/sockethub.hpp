...
// Define concepts for client event handlers
template <typename T>
...
struct ClientInfo {
    ...
    std::chrono::steady_clock::time_point
        connectedTime;           // When client connected
    uint64_t bytesReceived = 0;  // Bytes received from client
    uint64_t bytesSent = 0;      // Bytes sent to client
};
...
public:
     ...
     * @param handler A function to handle client connect events
     * @throws std::invalid_argument If handler is invalid
     */
     ...
     * @param handler A function to handle client disconnect events
     * @throws std::invalid_argument If handler is invalid
     */
    template <ClientEventHandler H>
