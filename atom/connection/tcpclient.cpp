...
public:
    explicit Impl(const Options& options) : options_(options) {
        try {
            ...
            socket_ = socket(options.ipv6_enabled ? AF_INET6 : AF_INET, SOCK_STREAM, IPPROTO_TCP);
            if (socket_ < 0) {
            ...
            if (epoll_fd_ == -1) {
                throw createSystemError("Failed to create epoll file descriptor");
            }
#elif defined(__APPLE__)
            // Create kqueue for async I/O on macOS
            kqueue_fd_ = kqueue();
            if (kqueue_fd_ == -1) {
                throw createSystemError("Failed to create kqueue file descriptor");
            }
#endif
        } catch (const std::exception& e) {
            last_error_ = std::system_error(std::make_error_code(std::errc::io_error), e.what());
            cleanupResources();
            throw;
        }
    }
        ...
        try {
            ...
            for (struct addrinfo* rp = result; rp != nullptr; rp = rp->ai_next) {
                ...
                if (epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, socket_, &event) == -1) {
                    return type::unexpected(createSystemError("Failed to add socket to epoll"));
                }
#elif defined(__APPLE__)
                ...
                if (kevent(kqueue_fd_, &event, 1, nullptr, 0, nullptr) == -1) {
                    return type::unexpected(createSystemError("Failed to add socket to kqueue"));
                }
#endif
                ...
                if (bytes_sent < 0) {
                    ...
                    if (WSAGetLastError() == WSAEWOULDBLOCK) {
                        ...
                        if (!waitForSendReady(std::chrono::seconds(5))) {
                            auto error = createSystemError("Send operation timed out");
                            last_error_ = error;
                            return type::unexpected(error);
                        }
                        continue;  // Retry send
    ...
    void setOnDataReceivedCallback(const std::function<void(std::span<const char>)>& callback) {
        onDataReceivedCallback_ = callback;
    }

    void setOnErrorCallback(const std::function<void(const std::system_error&)>& callback) {
...
#endif
                    throw createSystemError("Receive failed in background thread");
                } else if (bytes_read == 0) {
                    // Connection closed
                    connected_ = false;
...
TcpClient::TcpClient(Options options) : impl_(std::make_unique<Impl>(options)) {}
...
type::expected<size_t, std::system_error> TcpClient::send(std::span<const char> data) {
    return impl_->send(data);
}

Task<type::expected<size_t, std::system_error>> TcpClient::send_async(std::span<const char> data) {
...
Task<type::expected<std::vector<char>, std::system_error>> TcpClient::receive_async(
    size_t max_size, std::chrono::milliseconds timeout) {
