...
struct RegistrationParameters {
    ...
    bool usePolynomialTransform =
        false;                 // Use higher-order transforms for distortion
...
struct FrameTransformation {
    ...
    cv::Mat applyToFrame(const cv::Mat& frame,
                         const cv::Size& outputSize = cv::Size()) const;
};
...
public:
    ...
    std::pair<cv::Mat, FrameTransformation> registerFrame(
    ...
    std::vector<cv::Mat> registerFrames(
