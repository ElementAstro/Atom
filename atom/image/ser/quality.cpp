...
std::vector<double> QualityAssessor::getQualityScores(
...
std::vector<size_t> QualityAssessor::sortFramesByQuality(
    const std::vector<cv::Mat>& frames) const {
...
std::vector<cv::Mat> QualityAssessor::selectBestFrames(
    const std::vector<cv::Mat>& frames, size_t count) const {
    if (frames.empty()) {
        return {};
    }
...
void QualityAssessor::addCustomMetric(const std::string& name,
    ...
    if (weight <= 0.0) {
        throw InvalidParameterException(
            "Metric weight must be greater than zero");
    }
...
double QualityAssessor::getMetricValue(const cv::Mat& frame,
...
double QualityAssessor::getCustomMetricValue(
    const cv::Mat& frame, const std::string& metricName) const {
    auto it = customMetrics.find(metricName);
    if (it == customMetrics.end()) {
        throw InvalidParameterException(
            std::format("Unknown custom metric: {}", metricName));
    }

    return it->second.first(frame);
}

std::vector<QualityAssessor::MetricDetails> QualityAssessor::getDetailedMetrics(
...
double QualityAssessor::calculateCompositeScore(const cv::Mat& frame) const {
    ...
    // Standard metrics
    const std::vector<double> values = {sharpness,  snr,      entropy,
                                        brightness, contrast, starCount};
