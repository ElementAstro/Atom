...
bool FormatDetector::checkSignature(const uint8_t* data, size_t size,
    ...
    // Note: MagicSignature::offset is unsigned; negative offsets are not
    // supported. Ensure offset is within bounds and signature fits in buffer.
    if (offset > size || (offset + signature.signature.size()) > size) {
        return false;
    }
