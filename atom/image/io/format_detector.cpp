...
void FormatDetector::initializeSignatures() {
    // JPEG signatures
    signatures_.push_back({{0xFF, 0xD8, 0xFF}, 0, ImageFormat::JPEG, DetectionConfidence::CERTAIN, "JPEG image"});
    
    // PNG signature
    signatures_.push_back({{0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A}, 0, ImageFormat::PNG, DetectionConfidence::CERTAIN, "PNG image"});
    
    // BMP signatures
    signatures_.push_back({{0x42, 0x4D}, 0, ImageFormat::BMP, DetectionConfidence::CERTAIN, "BMP image"});
    
    // TIFF signatures (little and big endian)
    signatures_.push_back({{0x49, 0x49, 0x2A, 0x00}, 0, ImageFormat::TIFF, DetectionConfidence::CERTAIN, "TIFF image (little endian)"});
    signatures_.push_back({{0x4D, 0x4D, 0x00, 0x2A}, 0, ImageFormat::TIFF, DetectionConfidence::CERTAIN, "TIFF image (big endian)"});
    
    // GIF signatures
    signatures_.push_back({{0x47, 0x49, 0x46, 0x38, 0x37, 0x61}, 0, ImageFormat::GIF, DetectionConfidence::CERTAIN, "GIF87a image"});
    signatures_.push_back({{0x47, 0x49, 0x46, 0x38, 0x39, 0x61}, 0, ImageFormat::GIF, DetectionConfidence::CERTAIN, "GIF89a image"});
    
    // WebP signature
    signatures_.push_back({{0x52, 0x49, 0x46, 0x46}, 0, ImageFormat::WEBP, DetectionConfidence::HIGH, "WebP container"});
    signatures_.push_back({{0x57, 0x45, 0x42, 0x50}, 8, ImageFormat::WEBP, DetectionConfidence::CERTAIN, "WebP image"});
    
    // AVIF signature (ftyp box with AVIF brand)
    signatures_.push_back({{0x66, 0x74, 0x79, 0x70, 0x61, 0x76, 0x69, 0x66}, 4, ImageFormat::AVIF, DetectionConfidence::CERTAIN, "AVIF image"});
    
    // HEIF signature
    signatures_.push_back({{0x66, 0x74, 0x79, 0x70, 0x68, 0x65, 0x69, 0x63}, 4, ImageFormat::HEIF, DetectionConfidence::CERTAIN, "HEIF image"});
    
    // TGA signature (no reliable magic number, check footer)
    // Note: TGA footer detection would need special handling for negative offsets
    
    // PSD signature
    signatures_.push_back({{0x38, 0x42, 0x50, 0x53}, 0, ImageFormat::PSD, DetectionConfidence::CERTAIN, "Photoshop PSD"});
    
    // HDR signature
    signatures_.push_back({{0x23, 0x3F, 0x52, 0x41, 0x44, 0x49, 0x41, 0x4E, 0x43, 0x45, 0x0A}, 0, ImageFormat::HDR, DetectionConfidence::CERTAIN, "Radiance HDR"});
    
    // EXR signature
    signatures_.push_back({{0x76, 0x2F, 0x31, 0x01}, 0, ImageFormat::EXR, DetectionConfidence::CERTAIN, "OpenEXR image"});
    
    // FITS signature
    signatures_.push_back({{0x53, 0x49, 0x4D, 0x50, 0x4C, 0x45, 0x20, 0x20, 0x3D, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x54}, 0, ImageFormat::FITS, DetectionConfidence::CERTAIN, "FITS astronomical image"});
    
    // SER signature
    signatures_.push_back({{0x4C, 0x55, 0x43, 0x41, 0x4D, 0x2D, 0x52, 0x45, 0x43, 0x4F, 0x52, 0x44, 0x45, 0x52}, 0, ImageFormat::SER, DetectionConfidence::CERTAIN, "SER video sequence"});
    
    // DICOM signature
    signatures_.push_back({{0x44, 0x49, 0x43, 0x4D}, 128, ImageFormat::DCM, DetectionConfidence::CERTAIN, "DICOM medical image"});

    // Initialize extension mappings
    extensionMap_ = {
        {"jpg", ImageFormat::JPEG}, {"jpeg", ImageFormat::JPEG}, {"jpe", ImageFormat::JPEG},
        {"png", ImageFormat::PNG},
        ...
        {"heif", ImageFormat::HEIF}, {"heic", ImageFormat::HEIF},
        {"tga", ImageFormat::TGA}, {"targa", ImageFormat::TGA},
        {"psd", ImageFormat::PSD},
        {"hdr", ImageFormat::HDR}, {"pic", ImageFormat::HDR},
        {"exr", ImageFormat::EXR},
        {"fits", ImageFormat::FITS}, {"fit", ImageFormat::FITS}, {"fts", ImageFormat::FITS},
        {"ser", ImageFormat::SER},
        {"dcm", ImageFormat::DCM}, {"dicom", ImageFormat::DCM},
        {"nii", ImageFormat::NII}, {"nii.gz", ImageFormat::NII},
        {"h5", ImageFormat::HDF5}, {"hdf5", ImageFormat::HDF5},
        {"raw", ImageFormat::RAW}, {"dng", ImageFormat::DNG},
        {"cr2", ImageFormat::CR2}, {"nef", ImageFormat::NEF},
        {"arw", ImageFormat::ARW}, {"orf", ImageFormat::ORF},
        {"rw2", ImageFormat::RW2}, {"raf", ImageFormat::RAF},
        {"pef", ImageFormat::PEF}, {"x3f", ImageFormat::X3F}
    };
    ...
    formatNameMap_ = {
        ...
        {ImageFormat::SER, "SER"},
        {ImageFormat::DCM, "DICOM"},
        {ImageFormat::RAW, "RAW"},
        {ImageFormat::DNG, "DNG"}
    };
}

FormatDetectionResult FormatDetector::detectFromFile(const std::filesystem::path& filePath) const {
    ...
    if (magicResult.confidence >= DetectionConfidence::HIGH) {
        ...
        if (magicResult.format != ImageFormat::UNKNOWN) {
            result.confidence = std::min(result.confidence, magicResult.confidence);
        }
    } else {
        result = magicResult.format != ImageFormat::UNKNOWN ? magicResult : extensionResult;
    }

    return result;
}

FormatDetectionResult FormatDetector::detectFromMemory(const void* data, size_t size) const {
    if (!data || size == 0) {
        return {};
    }
...
FormatDetectionResult FormatDetector::detectFromBlob(const blob& imageBlob) const {
    ...
    return detectFromMemory(reinterpret_cast<const uint8_t*>(&(*imageBlob.begin())), imageBlob.size());
}

FormatDetectionResult FormatDetector::detectFromExtension(const std::string& extension) const {
...
std::vector<std::string> FormatDetector::getExtensions(ImageFormat format) const {
...
bool FormatDetector::checkSignature(const uint8_t* data, size_t size, const MagicSignature& signature) const {
    ...
    return std::equal(signature.signature.begin(), signature.signature.end(), data + offset);
}
