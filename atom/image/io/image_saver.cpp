...
#define THROW_FAIL_TO_OPEN_FILE(msg, path) \
    throw std::runtime_error(std::string(msg) + std::string(path))
#define THROW_FAIL_TO_WRITE_FILE(msg, path) \
    throw std::runtime_error(std::string(msg) + std::string(path))
    ...
    try {
        ...
        if (std::filesystem::exists(filePath) && !options.overwriteExisting) {
            if (!createBackup(filePath, options.backupSuffix)) {
                result.errorMessage =
                    "Failed to create backup of existing file";
                return result;
            }
        }
        ...
        if (targetFormat == ImageFormat::UNKNOWN) {
            ...
            if (extension == ".jpg" || extension == ".jpeg")
                ...
                targetFormat = ImageFormat::TGA;
            else if (extension == ".webp")
                targetFormat = ImageFormat::WEBP;
            else if (extension == ".fits")
                targetFormat = ImageFormat::FITS;
            else {
                result.errorMessage =
        ...
        // Encode image data
        auto [encodedData, encodeResult] =
            saveFormat(processedData, targetFormat, options);
        ...
        if (options.preserveMetadata && !options.customMetadata.empty()) {
            encodedData = embedMetadata(encodedData, targetFormat,
        ...
        if (!file) {
            THROW_FAIL_TO_OPEN_FILE("Cannot open file for writing: ",
                                    filePath.string());
        }

        file.write(reinterpret_cast<const char*>(encodedData.data()),
                   encodedData.size());
        if (!file) {
            THROW_FAIL_TO_WRITE_FILE("Failed to write image data to file: ",
                                     filePath.string());
        }
        ...
        result.success = true;
        result.saveTime = std::chrono::duration_cast<std::chrono::milliseconds>(
            endTime - startTime);
        result.compressionRatio = static_cast<float>(encodedData.size()) /
                                  static_cast<float>(imageData.size());
...
std::pair<std::vector<uint8_t>, SaveResult> ImageSaver::saveToMemory(
    const blob& imageData, ImageFormat format, const SaveOptions& options,
    SaveProgressCallback progressCallback) const {
    SaveResult result;
    ...
    try {
        ...
        if (options.preserveMetadata && !options.customMetadata.empty()) {
            encodedData =
                embedMetadata(encodedData, format, options.customMetadata);
        }
...
BatchSaveResult ImageSaver::saveBatch(
    const std::vector<blob>& imageData,
    const std::vector<std::filesystem::path>& filePaths,
    const SaveOptions& options, size_t maxConcurrency,
    SaveProgressCallback progressCallback) const {
    ...
    if (imageData.size() != filePaths.size()) {
        THROW_INVALID_ARGUMENT(
            "Image data and file paths vectors must have the same size");
    }
    ...
    for (size_t i = 0; i < imageData.size(); i += maxConcurrency) {
        ...
        for (size_t j = i; j < batchEnd; ++j) {
            futures.emplace_back(std::async(
                std::launch::async,
                [this, &imageData, &filePaths, &options, &batchResult,
                 &completedCount, &progressCallback, j]() {
                    batchResult.results[j] =
                        saveToFile(imageData[j], filePaths[j], options);
                    ...
                    if (progressCallback) {
                        float progress = static_cast<float>(completed) /
                                         static_cast<float>(imageData.size());
                        progressCallback(
                            progress, "Processed " + std::to_string(completed) +
                                          "/" +
                                          std::to_string(imageData.size()));
                    }
                }));
        }
...
std::future<SaveResult> ImageSaver::saveAsync(
    const blob& imageData, const std::filesystem::path& filePath,
    const SaveOptions& options, SaveProgressCallback progressCallback) const {
    return std::async(std::launch::async, [this, imageData, filePath, options,
                                           progressCallback]() {
...
std::vector<ImageFormat> ImageSaver::getSupportedFormats() const {
    std::vector<ImageFormat> formats = {ImageFormat::JPEG, ImageFormat::PNG,
...
ImageFormat ImageSaver::getOptimalFormat(const blob& imageData,
                                         bool preferLossless) const {
    if (imageData.isEmpty()) {
...
size_t ImageSaver::estimateOutputSize(const blob& imageData, ImageFormat format,
    ...
    switch (format) {
        case ImageFormat::JPEG: {
            ...
            float compressionRatio =
                0.1f + (static_cast<float>(options.quality) / 100.0f) * 0.4f;
...
void ImageSaver::registerCustomSaver(
    ImageFormat format,
...
std::pair<std::vector<uint8_t>, SaveResult> ImageSaver::saveFormat(
    ...
    try {
        ...
        if (customIt != customSavers_.end()) {
            ...
            if (result.success) {
                // Custom savers should return data in result, but we need it as
                // vector This is a simplified approach - in practice, custom
                // savers would need to return data
...
blob ImageSaver::applyPreProcessing(const blob& imageData,
                                    const SaveOptions& options) const {
    ...
    try {
        ...
        // Additional preprocessing could be added here based on format-specific
        // options
...
std::vector<uint8_t> ImageSaver::embedMetadata(
    ...
    // Placeholder implementation - would need format-specific metadata
    // embedding
    switch (format) {
        case ImageFormat::JPEG:
...
bool ImageSaver::createBackup(const std::filesystem::path& filePath,
...
bool quickSaveImage(const blob& imageData,
                    const std::filesystem::path& filePath, int quality) {
    SaveOptions options;
...
size_t quickSaveBatch(const std::vector<blob>& imageData,
                      const std::vector<std::filesystem::path>& filePaths,
                      int quality, size_t maxConcurrency) {
    SaveOptions options;
    options.quality = quality;

    auto saver = createImageSaver();
    auto result =
        saver->saveBatch(imageData, filePaths, options, maxConcurrency);
