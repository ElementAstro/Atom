...
blob ImageTransform::resize(const blob& input, int newWidth, int newHeight,
                           InterpolationMethod method, bool preserveAspect) const {
    ...
    auto mapFunction = [inputWidth, inputHeight, newWidth, newHeight](const Point2D& output) -> Point2D {
    ...
    return applyTransformation(input, Point2D(newWidth, newHeight), mapFunction, method);
}
...
blob ImageTransform::rotate(const blob& input, double angle,
    ...
    auto mapFunction = [cosA, sinA, rotCenter, outputSize, inputWidth, inputHeight, expandCanvas]
                      (const Point2D& output) -> Point2D {
        double x = output.x;
        double y = output.y;
    ...
    return applyTransformation(input, outputSize, mapFunction, method, borderMode);
}

blob ImageTransform::affineTransform(const blob& input,
    ...
    invMatrix[0][2] = (matrix[0][1] * matrix[1][2] - matrix[1][1] * matrix[0][2]) * invDet;
    ...
    auto mapFunction = [invMatrix](const Point2D& output) -> Point2D {
        double inputX = invMatrix[0][0] * output.x + invMatrix[0][1] * output.y + invMatrix[0][2];
        double inputY = invMatrix[1][0] * output.x + invMatrix[1][1] * output.y + invMatrix[1][2];
        return Point2D(inputX, inputY);
    };
...
blob ImageTransform::correctDistortion(const blob& input,
    ...
    auto mapFunction = [k1, k2, k3, p1, p2, distCenter, width, height](const Point2D& output) -> Point2D {
        ...
        // Denormalize
        return Point2D(correctedX * width + distCenter.x, correctedY * height + distCenter.y);
    };

    return applyTransformation(input, Point2D(width, height), mapFunction);
}

blob ImageTransform::applyTransformation(const blob& input,
                                        const Point2D& outputSize,
    ...
    // Sequential transformation (replace parallel for now to avoid counting_iterator issues)
...
std::vector<uint8_t> ImageTransform::interpolatePixel(const std::vector<std::byte>& input,
                                                     double x, double y,
...
TransformMatrix ImageTransform::createRotationMatrix(double angle, const Point2D& center) {
    double radians = angle * M_PI / 180.0;
    double cosA = std::cos(radians);
    double sinA = std::sin(radians);
...
TransformMatrix ImageTransform::createScalingMatrix(double sx, double sy, const Point2D& center) {
    return {{{{sx, 0, center.x * (1 - sx)}},
             {{0, sy, center.y * (1 - sy)}},
             {{0, 0, 1}}}};
}

Point2D ImageTransform::transformPoint(const Point2D& point, const TransformMatrix& matrix) {
