...
blob AdvancedFormatProcessor::loadRAW(const std::string& filename,
...
std::pair<blob, DICOMMetadata> AdvancedFormatProcessor::loadDICOM(
    const std::string& filename, int seriesIndex, int frameIndex) const {
    DICOMMetadata metadata;
...
bool AdvancedFormatProcessor::saveAnimation(
    ...
    AdvancedFormat format, int loopCount) const {
    if (frames.empty()) {
        return false;
    }
