...
struct QualityParameters {
    ...
    std::vector<double> metricWeights = {1.0, 1.0, 1.0,
                                         0.5, 1.0, 0.5};  // For composite
    bool normalizeMetrics = true;  // Normalize metrics to 0-1 range
    std::string roiSelector =
        "centered";         // Region of interest selection method
...
public:
    ...
    std::vector<double> getQualityScores(
        const std::vector<cv::Mat>& frames) const;

    // Sort frames by quality (returns indices of frames in descending order)
    std::vector<size_t> sortFramesByQuality(
        const std::vector<cv::Mat>& frames) const;

    // Select best N frames
    std::vector<cv::Mat> selectBestFrames(const std::vector<cv::Mat>& frames,
                                          size_t count) const;
    ...
    double getCustomMetricValue(const cv::Mat& frame,
...
private:
    ...
    std::unordered_map<std::string, std::pair<QualityMetricFunction, double>>
        customMetrics;
