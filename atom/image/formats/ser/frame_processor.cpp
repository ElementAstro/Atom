    ...
    for (size_t i = 0; i < frames.size(); ++i) {
        ...
        if (progress) {
            ...
            progress(progressValue,
                     std::format("{}: Processing frame {}/{}", getName(), i + 1,
                                 frames.size()));
        }
    }
...
void CustomizableProcessor::setParameters(
...
std::unordered_map<std::string, double> CustomizableProcessor::getParameters()
    const {
...
void BaseCustomizableProcessor::setParameter(const std::string& name,
                                             double value) {
    if (!hasParameter(name)) {
        throw InvalidParameterException(
            std::format("Unknown parameter: {}", name));
    }
...
void BaseCustomizableProcessor::registerParameter(const std::string& name,
                                                  double initialValue) {
...
std::vector<cv::Mat> ProcessingPipeline::process(
    const std::vector<cv::Mat>& frames, const ProgressCallback& progress) {
    ...
    for (size_t i = 0; i < processors.size(); ++i) {
        ...
        if (progress) {
            progress(static_cast<double>(i) / processors.size(),
                     std::format("Running processor {}/{}: {}", i + 1,
                                 processors.size(), processor->getName()));
        }
...
void ProcessingPipeline::addProcessor(
    std::shared_ptr<FrameProcessor> processor) {
    if (!processor) {
        throw InvalidParameterException(
            "Cannot add null processor to pipeline");
    }
...
void ProcessingPipeline::removeProcessor(size_t index) {
    if (index >= processors.size()) {
        throw InvalidParameterException::outOfRange("index", index, 0,
                                                    processors.size() - 1);
    }
    processors.erase(processors.begin() + index);
}

std::vector<std::shared_ptr<FrameProcessor>> ProcessingPipeline::getProcessors()
    const {
    return processors;
}

void ProcessingPipeline::clear() { processors.clear(); }
