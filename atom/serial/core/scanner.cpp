...
constexpr size_t INITIAL_PORT_VECTOR_SIZE =
    32;  // Increased for better pre-allocation
constexpr size_t EXPECTED_PORT_INFO_STRING_SIZE =
    128;  // For string pre-allocation
...
SerialPortScanner::check_ports_availability(
    const std::vector<std::string>& port_names) {
    if (port_names.empty()) {
        return ErrorInfo("Port names list cannot be empty", 0,
...
#ifdef _WIN32
            HANDLE handle = CreateFileA(
                port_name.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr,
...
void SerialPortScanner::optimize_cache() {
    ...
    if (port_cache_.size() > config_.max_cache_size) {
        ...
        const size_t to_remove =
            port_cache_.size() - (config_.max_cache_size * 3 / 4);
