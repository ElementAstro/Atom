...
#ifndef ATOM_ASYNC_CORE_ASYNC_HPP
...
// Implementation of AsyncWorker<void> specialization methods
template <typename Func, typename... Args>
    requires InvocableWithArgs<Func, Args...> &&
             std::is_same_v<std::invoke_result_t<Func, Args...>, void>
void AsyncWorker<void>::startAsync(Func&& func, Args&&... args) {
    ...
    try {
        auto wrapped_func = [this, f = std::decay_t<Func>(func),
                             ... args =
...
inline void AsyncWorker<void>::waitForCompletion() {
    ...
    if (timeout_ != std::chrono::seconds(0)) {
        ...
        while (!isDone()) {
            ...
            if (currentTime - startTime >= timeout_) {
                THROW_TIMEOUT_EXCEPTION(
                    "Timeout occurred while waiting for task completion");
            }
        }
    } else {
        while (!isDone()) {
...
inline auto AsyncWorker<void>::isDone() const noexcept -> bool {
    State current_state = state_.load(std::memory_order_acquire);
    return current_state == State::COMPLETED ||
           current_state == State::FAILED || current_state == State::CANCELLED;
}
...
#endif  // ATOM_ASYNC_CORE_ASYNC_HPP