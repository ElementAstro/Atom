...
protected:
    void SetUp() override {
        ...
        ON_CALL(*mockProcessManager,
                createProcess(::testing::_, ::testing::_, ::testing::_))
            .WillByDefault(::testing::Return(true));
        ON_CALL(*mockProcessManager,
                terminateProcess(::testing::_, ::testing::_))
            .WillByDefault(::testing::Return(true));
        ON_CALL(*mockProcessManager,
                terminateProcessByName(::testing::_, ::testing::_))
            .WillByDefault(::testing::Return(true));
        ...
        ON_CALL(*mockProcessManager,
                runScript(::testing::_, ::testing::_, ::testing::_))
            .WillByDefault(::testing::Return(true));
    ...
    void TearDown() override { mockProcessManager.reset(); }
...
TEST_F(ProcessManagerTest, CreateProcessSuccess) {
    EXPECT_CALL(*mockProcessManager,
                createProcess("echo 'Hello World'", "test_echo", false))
        .WillOnce(::testing::Return(true));

    bool result = mockProcessManager->createProcess("echo 'Hello World'",
                                                    "test_echo", false);
    EXPECT_TRUE(result);
}
...
TEST_F(ProcessManagerTest, CreateProcessFailure) {
    EXPECT_CALL(*mockProcessManager,
                createProcess("invalid_command", "test_invalid", false))
        .WillOnce(::testing::Return(false));

    bool result = mockProcessManager->createProcess("invalid_command",
                                                    "test_invalid", false);
    EXPECT_FALSE(result);
}
...
TEST_F(ProcessManagerTest, CreateBackgroundProcess) {
    EXPECT_CALL(*mockProcessManager,
                createProcess("long_running_task", "bg_task", true))
        .WillOnce(::testing::Return(true));

    bool result =
        mockProcessManager->createProcess("long_running_task", "bg_task", true);
    EXPECT_TRUE(result);
}
...
TEST_F(ProcessManagerTest, TerminateProcessByName) {
    ...
    bool result =
        mockProcessManager->terminateProcessByName("test_process", 15);
    EXPECT_TRUE(result);
}
...
TEST_F(ProcessManagerTest, WaitForCompletion) {
    EXPECT_CALL(*mockProcessManager, waitForCompletion()).Times(1);
...
TEST_F(ProcessManagerErrorTest, InvalidProcessCreation) {
    ...
    // Invalid command
    EXPECT_CALL(*mockProcessManager,
                createProcess("nonexistent_command_xyz", "invalid", false))
        .WillOnce(::testing::Return(false));
    EXPECT_FALSE(mockProcessManager->createProcess("nonexistent_command_xyz",
                                                   "invalid", false));
}
...
TEST_F(ProcessManagerErrorTest, InvalidProcessTermination) {
    ...
    EXPECT_CALL(*mockProcessManager,
                terminateProcessByName("nonexistent_process", 15))
        .WillOnce(::testing::Return(false));
    EXPECT_FALSE(
        mockProcessManager->terminateProcessByName("nonexistent_process", 15));
}
...
TEST_F(ProcessManagerErrorTest, InvalidScriptExecution) {
    ...
    EXPECT_CALL(*mockProcessManager,
                runScript(invalidScript, "invalid_script", false))
        .WillOnce(::testing::Return(false));
    EXPECT_FALSE(
        mockProcessManager->runScript(invalidScript, "invalid_script", false));
}
...
TEST_F(ProcessManagerIntegrationTest, DISABLED_ActualProcessCreation) {
    ...
    bool created =
        processManager->createProcess(testCommand, "integration_test", false);
    EXPECT_TRUE(created);
