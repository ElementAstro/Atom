...
protected:
    ...
    bool compareArrays(const T* a, const T* b, size_t size, T tolerance = T(1e-6)) {
...
TYPED_TEST_P(SIMDWrapperTest, LoadStoreOperations) {
    ...
    EXPECT_TRUE(this->compareArrays(this->alignedData1, this->alignedResult, N));
    
    // Test unaligned load/store
    Vec unalignedVec = Vec::loadu(this->testData1.data());
    unalignedVec.storeu(this->alignedResult);
    EXPECT_TRUE(this->compareArrays(this->testData1.data(), this->alignedResult, N));
}
...
TYPED_TEST_P(SIMDWrapperTest, ArithmeticOperations) {
    ...
    if constexpr (std::is_floating_point_v<T>) {
        ...
        for (size_t i = 0; i < N; ++i) {
            EXPECT_NEAR(divResult[i], this->testData1[i] / this->testData2[i], T(1e-6));
        }
    }
}
...
TYPED_TEST_P(SIMDWrapperTest, MinMaxOperations) {
    ...
    for (size_t i = 0; i < N; ++i) {
        EXPECT_EQ(minResult[i], std::min(this->testData1[i], this->testData2[i]));
    }
    
    // Test max
    auto maxResult = Traits::max(this->testData1, this->testData2);
    for (size_t i = 0; i < N; ++i) {
        EXPECT_EQ(maxResult[i], std::max(this->testData1[i], this->testData2[i]));
    }
    
    // Test horizontal operations
    T expectedSum = std::accumulate(this->testData1.begin(), this->testData1.end(), T(0));
    T actualSum = Traits::horizontal_sum(this->testData1);
    EXPECT_EQ(actualSum, expectedSum);
    
    T expectedMax = *std::max_element(this->testData1.begin(), this->testData1.end());
    T actualMax = Traits::horizontal_max(this->testData1);
    EXPECT_EQ(actualMax, expectedMax);
    
    T expectedMin = *std::min_element(this->testData1.begin(), this->testData1.end());
...
TYPED_TEST_P(SIMDWrapperTest, FusedOperations) {
    ...
    auto fmaddResult = Traits::fmadd(this->testData1, this->testData2, this->testData3);
    for (size_t i = 0; i < N; ++i) {
        T expected = this->testData1[i] * this->testData2[i] + this->testData3[i];
        if constexpr (std::is_floating_point_v<T>) {
    ...
    auto fmsubResult = Traits::fmsub(this->testData1, this->testData2, this->testData3);
    for (size_t i = 0; i < N; ++i) {
        T expected = this->testData1[i] * this->testData2[i] - this->testData3[i];
        if constexpr (std::is_floating_point_v<T>) {
...
TYPED_TEST_P(SIMDWrapperTest, PerformanceTest) {
    ...
    EXPECT_LT(duration.count(), 100000) << "SIMD operations took too long: " << duration.count() << "μs";
}

// Register the test suite for different types and sizes
REGISTER_TYPED_TEST_SUITE_P(SIMDWrapperTest,
    BasicConstruction,
    LoadStoreOperations,
