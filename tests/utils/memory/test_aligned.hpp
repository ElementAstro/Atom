...
TEST_F(AlignedStorageTest, ValidAlignedStorage) {
    ...
    EXPECT_NO_THROW(ValidateAlignedStorage<8, 4, 8, 4> validator1;
                    (void)validator1;  // Suppress unused variable warning
    );

    // Test case where storage is larger than implementation
    EXPECT_NO_THROW(ValidateAlignedStorage<8, 4, 16, 4> validator2;
                    (void)validator2;);

    // Test case where storage alignment is multiple of implementation alignment
    EXPECT_NO_THROW(ValidateAlignedStorage<8, 4, 8, 8> validator3;
                    (void)validator3;);

    // Test case with larger alignment multiple
    EXPECT_NO_THROW(ValidateAlignedStorage<8, 2, 8, 8> validator4;
                    (void)validator4;);
}
...
TEST_F(AlignedStorageTest, CommonDataTypes) {
    ...
    EXPECT_NO_THROW(ValidateAlignedStorage<sizeof(int), alignof(int),
                                           sizeof(int), alignof(int)>
                        intValidator;
                    (void)intValidator;);

    // Test with double (typically 8 bytes, 8-byte aligned)
    EXPECT_NO_THROW(ValidateAlignedStorage<sizeof(double), alignof(double),
                                           sizeof(double), alignof(double)>
                        doubleValidator;
                    (void)doubleValidator;);

    // Test with char (1 byte, 1-byte aligned)
    EXPECT_NO_THROW(ValidateAlignedStorage<sizeof(char), alignof(char),
                                           sizeof(char), alignof(char)>
                        charValidator;
                    (void)charValidator;);

    // Test with pointer (typically 8 bytes on 64-bit, pointer-aligned)
    EXPECT_NO_THROW(ValidateAlignedStorage<sizeof(void*), alignof(void*),
...
TEST_F(AlignedStorageTest, OversizedStorage) {
    ...
    EXPECT_NO_THROW(ValidateAlignedStorage<4, 4, 64, 4> oversizedValidator1;
                    (void)oversizedValidator1;);

    // Storage with much larger alignment
    EXPECT_NO_THROW(ValidateAlignedStorage<8, 4, 8, 32> oversizedValidator2;
                    (void)oversizedValidator2;);

    // Both size and alignment oversized
    EXPECT_NO_THROW(ValidateAlignedStorage<8, 4, 64, 32> oversizedValidator3;
...
TEST_F(AlignedStorageTest, MinimumValues) {
    ...
    EXPECT_NO_THROW(ValidateAlignedStorage<1, 1, 1, 1> minValidator;
                    (void)minValidator;);

    // Implementation size 1, storage larger
    EXPECT_NO_THROW(ValidateAlignedStorage<1, 1, 8, 1> minValidator2;
                    (void)minValidator2;);

    // Implementation alignment 1, storage alignment larger
    EXPECT_NO_THROW(ValidateAlignedStorage<8, 1, 8, 8> minValidator3;
                    (void)minValidator3;);
}
...
TEST_F(AlignedStorageTest, PowerOfTwoAlignments) {
    ...
    EXPECT_NO_THROW([]() {
        ValidateAlignedStorage<16, 2, 16, 4>
            alignMultiple1;  // 4 is multiple of 2
        ValidateAlignedStorage<16, 4, 16, 8>
            alignMultiple2;  // 8 is multiple of 4
        ValidateAlignedStorage<16, 8, 16, 16>
            alignMultiple3;  // 16 is multiple of 8
        (void)alignMultiple1;
        (void)alignMultiple2;
        (void)alignMultiple3;
    }());
...
TEST_F(AlignedStorageTest, RealisticScenarios) {
    ...
    EXPECT_NO_THROW(ValidateAlignedStorage<16, 4, 16, 16>
                        simdValidator;  // 16-byte aligned for SSE
                    (void)simdValidator;);

    // Scenario: Cache-line aligned storage
    EXPECT_NO_THROW(ValidateAlignedStorage<32, 8, 64, 64>
                        cacheLineValidator;  // 64-byte cache line
...
TEST_F(AlignedStorageTest, BoostSpecificFeatures) {
    ...
    EXPECT_NO_THROW(ValidateAlignedStorage<8, 4, 8, 4> boostValidator;
                    (void)boostValidator;);
    ...
    SUCCEED()
        << "Boost-specific aligned storage validation compiled successfully";
}
#endif
...
TEST_F(AlignedStorageTest, CompileTimeValidation) {
    ...
    constexpr bool test2 =
        std::is_class_v<ValidateAlignedStorage<16, 8, 32, 16>>;
    constexpr bool test3 =
        std::is_class_v<ValidateAlignedStorage<1, 1, 1024, 1>>;
    ...
    EXPECT_EQ(sizeof(ValidateAlignedStorage<8, 4, 8, 4>),
              1);  // Empty class size
}
...
TEST_F(AlignedStorageTest, DocumentationExamples) {
    ...
    EXPECT_NO_THROW(ValidateAlignedStorage<16, 4, 16, 16>
                        simdExample;  // 4 floats, 16-byte aligned
                    (void)simdExample;);

    // Example 3: Cache-friendly storage
    EXPECT_NO_THROW(ValidateAlignedStorage<32, 8, 64, 64>
                        cacheExample;  // Cache line aligned
                    (void)cacheExample;);
}
