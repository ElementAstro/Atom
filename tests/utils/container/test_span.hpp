...
TEST_F(SpanTest, SortSpanStrings) {
    std::vector<std::string> unsortedStrings = {"zebra", "apple", "banana",
                                                "cherry"};
...
TEST_F(SpanTest, FilterSpan) {
    // Filter even numbers
    auto evenNumbers =
        filterSpan(constIntSpan, [](int x) { return x % 2 == 0; });
    std::vector<int> expectedEven = {2, 4, 6, 8, 10};
    EXPECT_EQ(evenNumbers, expectedEven);

    // Filter numbers greater than 5
    auto greaterThanFive =
        filterSpan(constIntSpan, [](int x) { return x > 5; });
...
TEST_F(SpanTest, FilterSpanStrings) {
    ...
    auto longStrings = filterSpan(
        constStringSpan, [](const std::string& s) { return s.length() > 5; });
...
TEST_F(SpanTest, CountIfSpan) {
    // Count even numbers
    size_t evenCount =
        countIfSpan(constIntSpan, [](int x) { return x % 2 == 0; });
    EXPECT_EQ(evenCount, 5);  // 2, 4, 6, 8, 10

    // Count odd numbers
    size_t oddCount =
        countIfSpan(constIntSpan, [](int x) { return x % 2 == 1; });
    EXPECT_EQ(oddCount, 5);  // 1, 3, 5, 7, 9

    // Count numbers greater than 5
    size_t greaterCount =
        countIfSpan(constIntSpan, [](int x) { return x > 5; });
...
TEST_F(SpanTest, LargeSpanPerformance) {
    ...
    size_t countResult =
        countIfSpan(largeSpan, [](int x) { return x % 2 == 0; });
    ...
    EXPECT_LT(duration.count(), 1000)
        << "Large span operations took too long: " << duration.count() << "ms";
}
