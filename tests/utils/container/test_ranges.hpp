...
TEST_F(RangesTest, FilterAndTransform) {
    ...
    std::vector<int> expected = {4, 16, 36, 64,
                                 100};  // 2^2, 4^2, 6^2, 8^2, 10^2
...
TEST_F(RangesTest, FilterAndTransformStrings) {
    // Filter strings longer than 5 characters and convert to uppercase first
    // letter
    auto result = filterAndTransform(
        strings, [](const std::string& s) { return s.length() > 5; },
        [](const std::string& s) {
            std::string upper = s;
            if (!upper.empty())
...
TEST_F(RangesTest, FilterAndTransformEmpty) {
    auto result = filterAndTransform(
        emptyNumbers, [](int x) { return x > 0; }, [](int x) { return x * 2; });
...
TEST_F(RangesTest, GroupAndAggregateSum) {
    ...
    auto result = groupAndAggregate(
        numbers, [](int x) { return x % 2; },  // 0 for even, 1 for odd
        [](int x) { return x; });

    std::map<int, int> expected = {
        {0, 30}, {1, 25}};  // even: 2+4+6+8+10=30, odd: 1+3+5+7+9=25
    EXPECT_EQ(result, expected);
}
...
TEST_F(RangesTest, DropFirst) {
    ...
    // If not implemented, this test should be removed or the function should be
    // implemented
...
TEST_F(RangesTest, LargeDatasetPerformance) {
    ...
    auto result = filterAndTransform(
        largeNumbers, [](int x) { return x % 100 == 0; },
        [](int x) { return x / 100; });
    ...
    EXPECT_LT(duration.count(), 1000)
        << "Large dataset processing took too long: " << duration.count()
        << "ms";

    // Verify result correctness
    EXPECT_EQ(actual.size(),
              1000);  // Numbers 100, 200, ..., 100000 divided by 100
...
TEST_F(RangesTest, EdgeCases) {
    ...
    auto result2 = filterAndTransform(
        single, [](int x) { return x > 0; }, [](int x) { return x * 2; });
    std::vector<int> actual(result2.begin(), result2.end());
    EXPECT_EQ(actual, std::vector<int>{84});

    // Test with all elements filtered out
    auto result3 = filterAndTransform(
        numbers, [](int x) { return x > 100; }, [](int x) { return x; });
...
TEST_F(RangesTest, ConstCorrectness) {
    ...
    auto filtered = filterAndTransform(
        constNumbers, [](int x) { return x % 2 == 0; },
        [](int x) { return x * x; });
...
TEST_F(RangesTest, ExceptionSafety) {
    ...
    EXPECT_NO_THROW({
        auto result = filterAndTransform(
            testData, [](int x) { return x > 0; }, [](int x) { return x * x; });
        std::vector<int> actual(result.begin(), result.end());
    });
