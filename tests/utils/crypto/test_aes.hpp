...
TEST_F(AESTest, EncryptionWithEmptyInput) {
    std::vector<unsigned char> iv, tag;
    EXPECT_THROW(encryptAES("", key_, iv, tag), atom::error::InvalidArgument);
}

TEST_F(AESTest, EncryptionWithInvalidKey) {
    std::vector<unsigned char> iv, tag;
    EXPECT_THROW(encryptAES(testData_, "", iv, tag),
...
TEST_F(AESTest, DecryptionWithInvalidTag) {
    ...
    // Corrupt tag
    tag[0] ^= 0xFF;
    EXPECT_THROW(decryptAES(ciphertext, key_, iv, tag),
...
TEST_F(AESTest, CompressionWithEmptyInput) {
    EXPECT_THROW(compress(""), atom::error::InvalidArgument);
}
...
TEST_F(AESTest, ErrorHandlingComprehensive) {
    ...
    EXPECT_THROW(encryptAES(testData_, "short", iv, tag),
                 atom::error::InvalidArgument);
    EXPECT_THROW(
        encryptAES(testData_, "toolongkey123456789012345678901234567890", iv,
                   tag),
        atom::error::InvalidArgument);
    ...
    EXPECT_THROW(decryptAES(ciphertext, wrongKey, iv, tag),
                 atom::error::RuntimeError);
    ...
    if (!corruptedCiphertext.empty()) {
        ...
        EXPECT_THROW(decryptAES(corruptedCiphertext, key_, iv, tag),
                     atom::error::RuntimeError);
    }
    ...
    if (!wrongIV.empty()) {
        wrongIV[0] ^= 0xFF;
        EXPECT_THROW(decryptAES(ciphertext, key_, wrongIV, tag),
                     atom::error::RuntimeError);
    }
}
...
TEST_F(AESTest, CompressionEdgeCases) {
    ...
    EXPECT_GE(compressed.size(),
              randomData.size() * 0.8);  // Allow some compression
    ...
    EXPECT_LT(compressedPattern.size(),
              repetitiveData.size() / 4);  // Should compress very well
}
...
TEST_F(AESTest, PerformanceTest) {
    ...
    for (size_t size : dataSizes) {
        ...
        EXPECT_LT(duration.count(),
                  1000 * (size / 1024));  // Roughly 1 second per MB
...
TEST_F(AESTest, ThreadSafety) {
    ...
    for (int t = 0; t < numThreads; ++t) {
        futures.push_back(
            std::async(std::launch::async, [this, operationsPerThread, t]() {
                for (int i = 0; i < operationsPerThread; ++i) {
                    try {
                        ...
                        std::string ciphertext =
                            encryptAES(data, key_, iv, tag);
                        std::string decrypted =
                            decryptAES(ciphertext, key_, iv, tag);
