...
TEST_F(LocaleTest, GetSystemLanguageInfo) {
    ...
    // Language code should follow ISO standards (2-3 characters, possibly with region)
    // Examples: "en", "en_US", "en-US", "zh_CN", etc.
    std::regex langCodePattern(R"([a-z]{2,3}([_-][A-Z]{2})?([_-][a-zA-Z0-9]+)?)");
    ...
    EXPECT_LT(info.currencySymbol.length(), 10); // Currency symbols are typically short
    ...
    EXPECT_TRUE(info.decimalSymbol == "." || info.decimalSymbol == "," ||
               info.decimalSymbol == "٫" || info.decimalSymbol == "·"); // Common decimal separators
    ...
    std::vector<std::string> commonEncodings = {
        "UTF-8", "utf-8", "UTF-16", "utf-16", "ASCII", "ascii",
        "ISO-8859", "Windows-1252", "CP1252", "ANSI"
    };
...
TEST_F(LocaleTest, CurrencySymbolValidation) {
    ...
    EXPECT_LE(info.currencySymbol.length(), 5); // Most currency symbols are 1-3 characters
