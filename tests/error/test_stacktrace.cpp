...
Description: Unit tests for enhanced stacktrace functionality with external
library support
...
TEST_F(EnhancedStackTraceTest, AvailableBackends) {
    ...
    EXPECT_TRUE(std::find(backends.begin(), backends.end(), "builtin") !=
                backends.end());

    // Check for external libraries if compiled with them
#ifdef ATOM_USE_CPPTRACE
    EXPECT_TRUE(std::find(backends.begin(), backends.end(), "cpptrace") !=
                backends.end());
#endif

#ifdef ATOM_USE_BACKWARD_CPP
    EXPECT_TRUE(std::find(backends.begin(), backends.end(), "backward") !=
                backends.end());
#endif

#ifdef ATOM_USE_BOOST_STACKTRACE
    EXPECT_TRUE(std::find(backends.begin(), backends.end(), "boost") !=
                backends.end());
#endif
}
...
TEST_F(EnhancedStackTraceTest, BasicCapture) { deepFunction1(); }
...
TEST_F(EnhancedStackTraceTest, Prettification) {
    std::string input =
        "std::__1::vector<int, std::allocator<int>> class MyClass::method()";
