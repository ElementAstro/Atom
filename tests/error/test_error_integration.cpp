...
TEST_F(ErrorIntegrationTest, CompleteErrorFlow) {
    ...
    GlobalErrorHandler::getInstance().setGlobalHandler(
        [&capturedErrors, &capturedErrorsMutex](std::shared_ptr<ErrorContext> context) {
    ...
    std::lock_guard<std::mutex> lock(capturedErrorsMutex);
    EXPECT_GE(capturedErrors.size(), 2); // At least direct report and correlated report
...
TEST_F(ErrorIntegrationTest, ConcurrentErrorReporting) {
    ...
    reporter.addHandler("counter", [&totalProcessed](std::shared_ptr<ErrorContext> context) {
        totalProcessed++;
    });
    ...
    for (int t = 0; t < numThreads; ++t) {
        threads.emplace_back([&totalReported, errorsPerThread, t]() {
            for (int i = 0; i < errorsPerThread; ++i) {
                std::string message = "Thread " + std::to_string(t) + " Error " + std::to_string(i);
    ...
    EXPECT_GE(totalProcessed.load(), numThreads * errorsPerThread * 0.9); // Allow for some processing delay
...
TEST_F(ErrorIntegrationTest, ErrorRecoveryWithRetry) {
    ...
    auto unreliableFunction = [&attemptCount]() -> int {
        ...
        if (attempt < 2) {
            throw std::runtime_error("Simulated failure " + std::to_string(attempt));
        }
        return 42;
    };
    ...
    executor.withRetryPolicy(RecoveryStrategyFactory::createFixedRetry(3, std::chrono::milliseconds(10)));
...
TEST_F(ErrorIntegrationTest, ErrorRecoveryWithFallback) {
    ...
    executor.withFallback(RecoveryStrategyFactory::createDefaultFallback<std::string>("fallback_value"));
...
TEST_F(ErrorIntegrationTest, ErrorRecoveryWithCircuitBreaker) {
    ...
    ErrorRecoveryExecutor<int> executor;
    auto circuitBreaker = RecoveryStrategyFactory::createCircuitBreaker(2, std::chrono::milliseconds(100));
...
TEST_F(ErrorIntegrationTest, ErrorFormattingIntegration) {
    ...
    EXPECT_TRUE(plainOutput.find("Integration test error") != std::string::npos);
    ...
    EXPECT_TRUE(htmlOutput.find("<div class=\"error-report") != std::string::npos);
}
...
TEST_F(ErrorIntegrationTest, ErrorReportingPerformance) {
    ...
    reporter.addHandler("perf_counter", [&processedCount](std::shared_ptr<ErrorContext> context) {
        processedCount++;
    });
    ...
    auto reportDuration = std::chrono::duration_cast<std::chrono::milliseconds>(reportTime - startTime);
    auto totalDuration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
    
    // Performance expectations (adjust based on system capabilities)
    EXPECT_LT(reportDuration.count(), 1000); // Reporting should be fast (< 1 second)
    EXPECT_LT(totalDuration.count(), 5000);  // Total processing should be reasonable (< 5 seconds)
    
    std::cout << "Reported " << numErrors << " errors in " << reportDuration.count() << "ms" << std::endl;
    std::cout << "Processed " << processedCount.load() << " errors in " << totalDuration.count() << "ms" << std::endl;
}
...
TEST_F(ErrorIntegrationTest, ErrorFormattingPerformance) {
    ...
    auto context = ErrorContext::create(100, "Performance test error with long message and detailed context information");
    ...
    context->setStackTrace("Stack trace line 1\nStack trace line 2\nStack trace line 3");
    ...
    // Performance expectation: should be able to format at least 1000 errors per second
    double formatsPerSecond = (numFormats * 1000000.0) / duration.count();
    EXPECT_GT(formatsPerSecond, 1000.0);
    
    std::cout << "Formatted " << numFormats << " errors in " << duration.count() << " microseconds" << std::endl;
    std::cout << "Performance: " << formatsPerSecond << " formats per second" << std::endl;
}
...
TEST_F(ErrorIntegrationTest, ErrorContextMemoryManagement) {
    ...
    for (int i = 0; i < numContexts; ++i) {
        auto context = ErrorContext::create(100 + i, "Memory test error " + std::to_string(i));
