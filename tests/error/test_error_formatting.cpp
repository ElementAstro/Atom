...
TEST_F(ErrorFormattingTest, PlainTextFormatter) {
    ...
    EXPECT_TRUE(formatted.find("Test file not found error") != std::string::npos);
    EXPECT_TRUE(formatted.find("Severity: ERROR") != std::string::npos);
    EXPECT_TRUE(formatted.find("Category: IO") != std::string::npos);
    EXPECT_TRUE(formatted.find("Correlation ID: test-correlation-123") != std::string::npos);
...
TEST_F(ErrorFormattingTest, JsonFormatter) {
    ...
    EXPECT_TRUE(formatted.find("\"message\": \"Test file not found error\"") != std::string::npos);
    EXPECT_TRUE(formatted.find("\"severity\": \"ERROR\"") != std::string::npos);
    EXPECT_TRUE(formatted.find("\"category\": \"IO\"") != std::string::npos);
    EXPECT_TRUE(formatted.find("\"correlationId\": \"test-correlation-123\"") != std::string::npos);
    EXPECT_TRUE(formatted.find("\"tags\": [\"test\", \"file-system\"]") != std::string::npos);
}
...
TEST_F(ErrorFormattingTest, JsonFormatterMultiple) {
    ...
    std::vector<std::shared_ptr<ErrorContext>> contexts = {testContext_, context2};
...
TEST_F(ErrorFormattingTest, ColoredFormatter) {
    ...
    EXPECT_TRUE(formatted.find("\033[") != std::string::npos); // ANSI escape sequence
    EXPECT_TRUE(formatted.find("ERROR REPORT") != std::string::npos);
    EXPECT_TRUE(formatted.find("Test file not found error") != std::string::npos);
}
...
TEST_F(ErrorFormattingTest, HtmlFormatter) {
    ...
    EXPECT_TRUE(formatted.find("<div class=\"error-report") != std::string::npos);
    EXPECT_TRUE(formatted.find("Error Report") != std::string::npos);
    EXPECT_TRUE(formatted.find("Test file not found error") != std::string::npos);
...
TEST_F(ErrorFormattingTest, HtmlFormatterMultiple) {
    ...
    EXPECT_TRUE(formatted.find("Test file not found error") != std::string::npos);
    EXPECT_TRUE(formatted.find("Second error") != std::string::npos);
    EXPECT_TRUE(formatted.find("<div class=\"error-reports\">") != std::string::npos);
...
TEST_F(ErrorFormattingTest, CustomFormatter) {
    ...
    EXPECT_TRUE(std::find(available.begin(), available.end(), "custom") != available.end());
}
...
TEST_F(ErrorFormattingTest, TemplateFormatter) {
    std::string templateStr = "Error {error_code}: {message} (Severity: {severity})";
