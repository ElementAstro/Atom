...
class TestValidator:
    ...
    def validate_file_structure(self, file_path: Path) -> List[str]:
        ...
        if (
            "#ifndef" not in content
            or "#define" not in content
            or "#endif" not in content
        ):
    ...
    def count_tests(self, file_path: Path) -> Tuple[int, int]:
        ...
        # Count test classes
        test_classes = len(
            re.findall(r"class\s+\w+Test\s*:\s*public\s+::testing::Test", content)
        )
    ...
    def validate_test_naming(self, file_path: Path) -> List[str]:
        ...
        for class_name, test_name in tests:
            ...
            if len(test_name) < 5:
                issues.append(
                    f"Test name '{test_name}' is too short, should be more descriptive"
                )
    ...
    def check_cmake_integration(self) -> List[str]:
        ...
        for test_file in self.test_files:
            if test_file.name not in cmake_content:
                issues.append(
                    f"Test file {test_file.name} not listed in CMakeLists.txt"
                )
    ...
    def generate_report(self) -> None:
        ...
        if self.stats["files_with_issues"] == 0:
            print(
                "  - Test suite looks good! Consider running actual tests to verify functionality"
            )
        print()

        # Overall assessment
        success_rate = (
            (self.stats["total_files"] - self.stats["files_with_issues"])
