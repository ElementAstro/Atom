...
public:
    ...
    void readData(std::ifstream& file, int64_t dataSize) override {
        ...
        if (file.fail() && !file.eof()) {
            throw FITSDataException(FITSDataErrorCode::DataReadError, "Failed to read data");
        }
    }
    ...
    void writeData(std::ofstream& file) const override {
        ...
        if (file.fail()) {
            throw FITSDataException(FITSDataErrorCode::DataWriteError, "Failed to write data");
        }
    }
...
TEST_F(FitsDataTest, ErrorCodeFunctionality) {
    ...
    EXPECT_EQ(errorCode.value(), static_cast<int>(FITSDataErrorCode::InvalidDataSize));
