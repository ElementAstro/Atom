...
protected:
    ...
    void createTestFitsFiles() {
        ...
        test_fits_file = FitsTestDataGenerator::createTempFitsFile(32, 32, 1, 16);
        fileManager->registerTempFile(test_fits_file);

        // Create a multi-channel FITS file
        multi_channel_fits_file = FitsTestDataGenerator::createTempFitsFile(64, 64, 3, 16);
...
TEST_F(FitsUtilsTest, OpenCVProcessing) {
    ...
    // Test edge detection
    std::map<std::string, double> params = {{"threshold1", 100.0}, {"threshold2", 200.0}};
...
TEST_F(FitsUtilsTest, MultiChannelOperations) {
    ...
    std::vector<FitsImage> channelImages = {
        image.extractChannel(0),
...
TEST_F(FitsUtilsTest, DISABLED_PerformanceTest) {
    ...
    std::cout << "Large image processing time: " << duration.count() << " ms" << std::endl;
