...
public:
     ...
     */
    static std::vector<std::byte> generateGradientImage(int width, int height, int channels = 3) {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                for (int c = 0; c < channels; ++c) {
                    ...
                    switch (c) {
                        case 0: // Red channel
                            value = static_cast<uint8_t>((x * 255) / (width - 1));
                            break;
                        case 1: // Green channel
                            value = static_cast<uint8_t>((y * 255) / (height - 1));
                            break;
                        case 2: // Blue channel
                            value = static_cast<uint8_t>(((x + y) * 255) / (width + height - 2));
                            break;
                        default: // Additional channels
                            value = static_cast<uint8_t>((x * y * 255) / ((width - 1) * (height - 1)));
                            break;
                    }
     ...
     * @return Vector of test image data
     */
    static std::vector<std::byte> generateCheckerboard(int width, int height, int channels = 1, int blockSize = 8) {
        std::vector<std::byte> data;
        data.reserve(width * height * channels);
     ...
     * @return Vector of test image data
     */
    static std::vector<std::byte> generateRandomNoise(int width, int height, int channels = 1, uint32_t seed = 42) {
        std::vector<std::byte> data;
        data.reserve(width * height * channels);
     ...
     * @return Vector of test image data
     */
    static std::vector<std::byte> generateSolidColor(int width, int height, int channels, const std::vector<uint8_t>& color) {
        std::vector<std::byte> data;
        data.reserve(width * height * channels);
     ...
     * @return Vector of test image data
     */
    static std::vector<std::byte> generateCircularPattern(int width, int height, int channels = 1) {
        std::vector<std::byte> data;
        data.reserve(width * height * channels);
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                uint8_t value = static_cast<uint8_t>((1.0 - std::min(distance / maxRadius, 1.0)) * 255);
...
public:
     ...
     */
    static std::string createTempFitsFile(int width = 10, int height = 10, int channels = 1, int bitpix = 32) {
        namespace fs = std::filesystem;
        
        std::string tempFilePath = (fs::temp_directory_path() /
                                   fs::path("test_fits_temp_" + std::to_string(std::random_device{}()) + ".fits")).string();

        std::ofstream outFile(tempFilePath, std::ios::binary);

        // Write FITS header
        writeHeaderCard(outFile, "SIMPLE", "T", "Standard FITS format");
        writeHeaderCard(outFile, "BITPIX", std::to_string(bitpix), "Bits per pixel");
        writeHeaderCard(outFile, "NAXIS", std::to_string(channels > 1 ? 3 : 2), "Number of axes");
        writeHeaderCard(outFile, "NAXIS1", std::to_string(width), "Width");
        writeHeaderCard(outFile, "NAXIS2", std::to_string(height), "Height");
        if (channels > 1) {
            writeHeaderCard(outFile, "NAXIS3", std::to_string(channels), "Channels");
        }
...
public:
     ...
     */
    static void assertFloatNear(double expected, double actual, double tolerance = 1e-6) {
     ...
     * @param channels Number of channels
     */
    static void assertValidImageDimensions(int width, int height, int channels = 1) {
    ...
    static void assertFileExists(const std::string& filePath) {
        EXPECT_TRUE(std::filesystem::exists(filePath)) << "File should exist: " << filePath;
    }
    ...
    static void assertFileNotExists(const std::string& filePath) {
        EXPECT_FALSE(std::filesystem::exists(filePath)) << "File should not exist: " << filePath;
    }
};
