...
protected:
    ...
    void createTestImages() {
        ...
        auto checkerboardData =
            TestDataGenerator::generateCheckerboard(256, 256, 3, 8);
        checkerboard_image =
            blob(checkerboardData.data(), checkerboardData.size());

        auto circleData =
            TestDataGenerator::generateCircularPattern(256, 256, 3);
        circle_image = blob(circleData.data(), circleData.size());

        auto solidData =
            TestDataGenerator::generateSolidColor(256, 256, 3, {128, 128, 128});
        solid_image = blob(solidData.data(), solidData.size());

        auto noisyData =
            TestDataGenerator::generateRandomNoise(256, 256, 3, 12345);
    ...
    blob gradient_image, checkerboard_image, circle_image, solid_image,
        noisy_image;
};
...
TEST_F(ComputerVisionTest, FeatureDetectionORB) {
    ...
    auto keypoints =
        cv->detectFeatures(checkerboard_image, FeatureDetectorType::ORB, 100);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    auto keypoints =
        cv->detectFeatures(checkerboard_image, FeatureDetectorType::SIFT, 50);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    auto keypoints =
        cv->detectFeatures(checkerboard_image, FeatureDetectorType::AKAZE, 100);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    auto keypoints =
        cv->detectFeatures(checkerboard_image, FeatureDetectorType::BRISK, 100);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    auto keypoints =
        cv->detectFeatures(checkerboard_image, FeatureDetectorType::FAST, 100);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    auto keypoints = cv->detectFeatures(checkerboard_image,
                                        FeatureDetectorType::HARRIS, 50, 0.01);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    auto keypoints =
        cv->detectFeatures(checkerboard_image, FeatureDetectorType::GFTT, 100);
...
TEST_F(ComputerVisionTest, ObjectDetectionBasic) {
    ...
    try {
        auto detections = cv->detectObjects(
            circle_image, ObjectDetectionModel::YOLO_V5, 0.5, 0.4);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    try {
        auto faces = cv->detectFaces(circle_image, FaceModel::DNN_FACE, 30, 1.1,
                                     true, false);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    try {
        auto segments = cv->segmentImage(checkerboard_image,
                                         SegmentationMethod::SLIC, 100, 10.0);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    try {
        auto flow = cv->estimateOpticalFlow(checkerboard_image, gradient_image,
                                            "lucas_kanade");
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    auto quality = cv->analyzeQuality(
        noisy_image, {"sharpness", "noise", "exposure", "contrast"});
...
TEST_F(ComputerVisionTest, AnomalyDetection) {
    ...
    try {
        ...
        auto [score, regions] =
            cv->detectAnomalies(noisy_image, references, "statistical", 0.5);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    try {
        std::vector<blob> database = {gradient_image, checkerboard_image,
                                      circle_image, solid_image};
        auto similar =
            cv->findSimilarImages(checkerboard_image, database, 3, "cosine");
...
TEST_F(ComputerVisionTest, TextRecognition) {
    ...
    try {
        auto textResults =
            cv->recognizeText(solid_image, "eng", "tesseract", true);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    try {
        std::vector<blob> frames = {checkerboard_image, gradient_image,
                                    circle_image};
        Detection initialDet(0, "object", 0.9, 50, 50, 100, 100);
        std::vector<Detection> initialDetections = {initialDet};

        auto trackingResults =
            cv->trackObjects(frames, initialDetections, "kcf");
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    auto smallData =
        TestDataGenerator::generateSolidColor(8, 8, 3, {128, 128, 128});
    blob small_image(smallData.data(), smallData.size());

    auto keypoints =
        cv->detectFeatures(small_image, FeatureDetectorType::ORB, 10);
    EXPECT_GE(keypoints.size(), 0);
#else
    GTEST_SKIP() << "OpenCV not available";
#endif
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    auto keypoints =
        cv->detectFeatures(checkerboard_image, FeatureDetectorType::ORB, 10000);
    EXPECT_GE(keypoints.size(), 0);
...
#ifdef ATOM_IMAGE_HAS_OPENCV
    auto keypoints = cv->detectFeatures(checkerboard_image,
                                        FeatureDetectorType::HARRIS, 100, 0.0);
    EXPECT_GE(keypoints.size(), 0);
#else
    GTEST_SKIP() << "OpenCV not available";
#endif
...
TEST_F(ComputerVisionTest, DISABLED_PerformanceFeatureDetection) {
    ...
    auto keypoints =
        cv->detectFeatures(large_image, FeatureDetectorType::ORB, 1000);
    ...
    EXPECT_GT(keypoints.size(), 0);
    std::cout << "Feature detection took " << duration.count() << " ms"
              << std::endl;
#else
    GTEST_SKIP() << "OpenCV not available";
#endif
