...
protected:
    ...
    void createTestImages() {
        ...
        auto lowContrastData =
            TestDataGenerator::generateGradientImage(32, 32, 1);
        ...
        low_contrast_image =
            blob(lowContrastData.data(), lowContrastData.size());
        ...
        for (auto& pixel : brightData) {
            uint8_t value = static_cast<uint8_t>(pixel);
            pixel = static_cast<std::byte>(
                std::min(255, static_cast<int>(value) + 128));
        }
        ...
        for (auto& pixel : hdrData) {
            ...
            uint8_t value = static_cast<uint8_t>(pixel);
            pixel = static_cast<std::byte>(
                std::min(255, static_cast<int>(value) * 2));
        }
    ...
    blob low_contrast_image, dark_image, bright_image, noisy_image, color_image,
        hdr_image;
};
...
TEST_F(EnhancementTest, CLAHEHistogramEqualization) {
    ...
    auto result = enhancement->equalizeHistogram(
        low_contrast_image, HistogramMethod::CLAHE, params);
...
TEST_F(EnhancementTest, ReinhardToneMapping) {
    ...
    auto result = enhancement->toneMapping(
...
TEST_F(EnhancementTest, ToneMappingOperators) {
    std::vector<ToneMappingOperator> operators = {
        ToneMappingOperator::REINHARD, ToneMappingOperator::DRAGO,
        ToneMappingOperator::MANTIUK,  ToneMappingOperator::FATTAL,
        ToneMappingOperator::DURAND,   ToneMappingOperator::GAMMA,
        ToneMappingOperator::LINEAR,   ToneMappingOperator::LOGARITHMIC};
...
TEST_F(EnhancementTest, GammaCorrectionColorSpaces) {
    std::vector<ColorSpace> colorSpaces = {ColorSpace::RGB, ColorSpace::HSV,
                                           ColorSpace::HSL, ColorSpace::LAB,
                                           ColorSpace::YUV, ColorSpace::GRAY};

    for (const auto& colorSpace : colorSpaces) {
        auto result =
            enhancement->gammaCorrection(color_image, 2.2, colorSpace);
...
TEST_F(EnhancementTest, ImageSharpening) {
    std::vector<std::string> sharpenMethods = {"unsharp_mask", "high_pass",
                                               "clarity"};

    for (const auto& method : sharpenMethods) {
        auto result =
            enhancement->sharpen(low_contrast_image, 1.0, 1.0, 0.0, method);
...
TEST_F(EnhancementTest, NoiseReduction) {
    std::vector<std::string> denoiseMethods = {"bilateral", "nlm", "bm3d",
                                               "dct"};
...
TEST_F(EnhancementTest, WhiteBalanceCorrection) {
    ...
    auto result = enhancement->colorCorrection(
        color_image, ColorCorrectionMethod::WHITE_BALANCE, params);
...
TEST_F(EnhancementTest, SaturationVibranceAdjustment) {
    ...
    for (const auto& [saturation, vibrance] : values) {
        auto result =
            enhancement->vibranceSaturation(color_image, vibrance, saturation);
...
TEST_F(EnhancementTest, ClarityEnhancement) {
    ...
    auto result =
        enhancement->clarity(low_contrast_image, clarityAmount, radius);
...
TEST_F(EnhancementTest, DehazeOperation) {
    ...
    auto result =
        enhancement->dehaze(low_contrast_image, strength, preserveColors);
...
TEST_F(EnhancementTest, HighlightsShadowsAdjustment) {
    ...
    auto result =
        enhancement->shadowHighlight(bright_image, shadows, highlights, radius);
...
TEST_F(EnhancementTest, ColorGrading) {
    ...
    auto result = enhancement->colorCorrection(
        color_image, ColorCorrectionMethod::COLOR_GRADING, params);
...
TEST_F(EnhancementTest, AutoLevels) {
    auto result = enhancement->colorCorrection(
        low_contrast_image, ColorCorrectionMethod::AUTO_LEVELS);
...
TEST_F(EnhancementTest, AutoColorCorrection) {
    auto result = enhancement->colorCorrection(
        color_image, ColorCorrectionMethod::AUTO_COLOR);
...
TEST_F(EnhancementTest, ParameterValidation) {
    ...
    EXPECT_THROW(enhancement->gammaCorrection(dark_image, -1.0),
                 std::invalid_argument);
    EXPECT_THROW(enhancement->gammaCorrection(dark_image, 0.0),
                 std::invalid_argument);

    // Test invalid brightness values
    EXPECT_THROW(enhancement->adjustBrightnessContrast(dark_image, -200.0),
                 std::invalid_argument);
    EXPECT_THROW(enhancement->adjustBrightnessContrast(dark_image, 200.0),
                 std::invalid_argument);

    // Test invalid contrast values
    EXPECT_THROW(enhancement->adjustBrightnessContrast(dark_image, 0.0, -1.0),
                 std::invalid_argument);
}
...
TEST_F(EnhancementTest, EmptyImageHandling) {
    ...
    EXPECT_THROW(enhancement->equalizeHistogram(emptyImage),
                 std::invalid_argument);
    EXPECT_THROW(enhancement->adjustBrightnessContrast(emptyImage, 0.0, 1.0),
                 std::invalid_argument);
    EXPECT_THROW(enhancement->gammaCorrection(emptyImage, 2.2),
                 std::invalid_argument);
}
...
TEST_F(EnhancementTest, ImagePropertiesPreservation) {
    ...
    auto enhanced =
        enhancement->adjustBrightnessContrast(color_image, 10.0, 1.1);
...
TEST_F(EnhancementTest, ExtremeParameterValues) {
    ...
    auto extremeBright =
        enhancement->adjustBrightnessContrast(dark_image, 99.0, 1.0);
    EXPECT_GT(extremeBright.size(), 0);

    auto extremeContrast =
        enhancement->adjustBrightnessContrast(low_contrast_image, 0.0, 2.9);
