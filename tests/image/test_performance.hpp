...
protected:
    ...
    void createTestData() {
        ...
        auto medium_data = TestDataGenerator::generateGradientImage(256, 256, 3);
        medium_image = blob(medium_data.data(), medium_data.size(), 256, 256, 3);
        
        // Large image (1024x1024) - only for stress tests
        auto large_data = TestDataGenerator::generateGradientImage(1024, 1024, 3);
...
TEST_F(PerformanceTest, BlobCreationPerformance) {
    ...
    auto medium_time = measureExecutionTime([]() {
        for (int i = 0; i < 1000; ++i) {
            auto data = TestDataGenerator::generateSolidColor(256, 256, 3, {128});
            blob test_blob(data.data(), data.size(), 256, 256, 3);
            (void)test_blob;
        }
    });
    
    std::cout << "Small blob creation (" << iterations << " iterations): " 
              << small_time << " ms" << std::endl;
    std::cout << "Medium blob creation (" << iterations << " iterations): " 
              << medium_time << " ms" << std::endl;
...
TEST_F(PerformanceTest, BlobCopyPerformance) {
    ...
    std::cout << "Large blob copy (" << iterations << " iterations): " 
              << copy_time << " ms" << std::endl;
...
TEST_F(PerformanceTest, BlobSlicePerformance) {
    ...
    std::cout << "Blob slice operations (" << iterations << " iterations): " 
              << slice_time << " ms" << std::endl;
...
TEST_F(PerformanceTest, ImageProcessingPerformance) {
    ...
    auto rotation_time = measureExecutionTime([this, iterations]() {
        for (int i = 0; i < iterations; ++i) {
            auto result = processor->rotate(medium_image, i * 7.2); // Different angles
            (void)result;
        }
    });
    ...
    std::cout << "Resize operations (" << iterations << " iterations): " 
              << resize_time << " ms" << std::endl;
    std::cout << "Rotation operations (" << iterations << " iterations): " 
              << rotation_time << " ms" << std::endl;
    std::cout << "Filter operations (" << iterations << " iterations): " 
              << filter_time << " ms" << std::endl;
...
TEST_F(PerformanceTest, BatchProcessingPerformance) {
    ...
    std::cout << "Batch processing (" << image_batch.size() << " images): " 
...
TEST_F(PerformanceTest, DISABLED_MemoryStressTest) {
    ...
    auto allocation_time = measureExecutionTime([&large_blobs, large_size, iterations]() {
        for (int i = 0; i < iterations; ++i) {
            auto data = TestDataGenerator::generateGradientImage(large_size, large_size, 3);
            large_blobs.emplace_back(data.data(), data.size(), large_size, large_size, 3);
        }
    });
    
    std::cout << "Large memory allocation (" << iterations << " x " 
              << large_size << "x" << large_size << " images): " 
              << allocation_time << " ms" << std::endl;
    
    // Test should complete without memory errors
    EXPECT_EQ(large_blobs.size(), iterations);
    EXPECT_LT(allocation_time, 60000.0); // Should complete in less than 1 minute
}
...
TEST_F(PerformanceTest, ConcurrentAccessPerformance) {
        ...
        for (int t = 0; t < num_threads; ++t) {
            threads.emplace_back([this, operations_per_thread, &completed_operations]() {
                for (int i = 0; i < operations_per_thread; ++i) {
                    ...
                    if (size > 0) {
                        auto slice = medium_image.slice(0, std::min(size, size_t(1000)));
                        (void)slice;
    ...
    std::cout << "Concurrent access (" << num_threads << " threads, " 
              ...
              << concurrent_time << " ms" << std::endl;
    
    EXPECT_EQ(completed_operations.load(), num_threads * operations_per_thread);
    EXPECT_LT(concurrent_time, 10000.0); // Should complete in less than 10 seconds
}
...
TEST_F(PerformanceTest, FITSFileIOPerformance) {
    ...
    std::cout << "FITS write (" << width << "x" << height << "): "
              << write_time << " ms" << std::endl;
    std::cout << "FITS read (" << width << "x" << height << "): "
...
TEST_F(PerformanceTest, FragmentedMemoryPerformance) {
    ...
    auto fragmentation_time = measureExecutionTime([&]() {
        for (int i = 0; i < num_allocations; ++i) {
            ...
            int size = 64 + (i % 128);
            auto data = TestDataGenerator::generateSolidColor(size, size, 1, {static_cast<uint8_t>(i % 256)});
    ...
    std::cout << "Fragmented memory allocation (" << num_allocations << " blobs): "
              << fragmentation_time << " ms" << std::endl;
    std::cout << "Fragmented memory access (" << num_allocations << " blobs): "
              << access_time << " ms" << std::endl;

    EXPECT_EQ(blobs.size(), num_allocations);
    EXPECT_LT(fragmentation_time, 15000.0); // Should complete in less than 15 seconds
...
TEST_F(PerformanceTest, CachePerformanceTest) {
    ...
    std::cout << "Repeated slice operations (" << iterations << " iterations): "
              << repeated_slice_time << " ms" << std::endl;
    std::cout << "Varied slice operations (" << iterations << " iterations): "
              << varied_slice_time << " ms" << std::endl;
...
TEST_F(PerformanceTest, ScalabilityTest) {
    ...
    for (int size : sizes) {
        ...
        std::cout << "Size " << size << "x" << size << ": " << time << " ms" << std::endl;
    }
...
TEST_F(PerformanceTest, ResourceCleanupStressTest) {
    ...
    std::cout << "Resource cleanup stress test (" << stress_iterations << " iterations): "
              << cleanup_time << " ms" << std::endl;

    // Should complete without memory leaks or excessive time
    EXPECT_LT(cleanup_time, 30000.0); // Should complete in less than 30 seconds
}
...
TEST_F(PerformanceTest, DISABLED_MemoryPressureTest) {
    ...
    auto pressure_time = measureExecutionTime([&]() {
        for (int i = 0; i < num_images; ++i) {
            auto data = TestDataGenerator::generateGradientImage(pressure_size, pressure_size, 3);
            pressure_blobs.emplace_back(data.data(), data.size(), pressure_size, pressure_size, 3);
    ...
    std::cout << "Memory pressure test (" << num_images << " x "
              << pressure_size << "x" << pressure_size << " images): "
              << pressure_time << " ms" << std::endl;

    EXPECT_EQ(pressure_blobs.size(), num_images);
    EXPECT_LT(pressure_time, 120000.0); // Should complete in less than 2 minutes
}
