...
TEST_F(PerformanceTest, FITSFileIOPerformance) {
    ...
    file_manager.registerTempFile(test_fits_file);

    // Test FITS write performance
    auto write_time = measureExecutionTime([&]() {
        try {
            FITSFile fits_file;
            // Create a simple FITS file with default HDU
            fits_file.writeFITS(test_fits_file);
        } catch (...) {
    ...
    auto read_time = measureExecutionTime([&]() {
        try {
            FITSFile fits_file;
            fits_file.readFITS(test_fits_file, false, false);
            auto hdu_count = fits_file.getHDUCount();
            (void)hdu_count;
        } catch (...) {
            // Handle gracefully if FITS not available
        }
    ...
    std::cout << "FITS write: " << write_time << " ms" << std::endl;
    std::cout << "FITS read: " << read_time << " ms" << std::endl;
