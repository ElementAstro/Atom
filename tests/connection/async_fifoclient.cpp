...
std::string generate_unique_fifo_path() {
    ...
    // These tests won't create the server side, so paths are for client side
    // tests.
...
void create_fifo(const std::string& path) {
    if (mkfifo(path.c_str(), 0666) == -1) {
        if (errno != EEXIST) {
            FAIL() << "Failed to create FIFO " << path << ": "
                   << strerror(errno);
        }
    }
}
...
void remove_fifo(const std::string& path) {
    if (unlink(path.c_str()) == -1) {
        // Ignore ENOENT (file not found) as it might have been removed by a
        // test
        if (errno != ENOENT) {
            // Log error but don't fail test teardown
            std::cerr << "Failed to remove FIFO " << path << ": "
...
void write_to_fifo(const std::string& path, const std::string& data) {
    ...
    while (total_written < data.size()) {
        ssize_t written = ::write(fd, data.c_str() + total_written,
        ...
        if (written == 0) {
            // Should not happen with blocking write unless pipe is closed by
            // reader
            ::close(fd);
...
std::string read_from_fifo(const std::string& path, size_t expected_size) {
    ...
    while (total_read < expected_size) {
        ssize_t bytes_read =
            ::read(fd, buffer.data() + total_read, expected_size - total_read);
        if (bytes_read == -1) {
            if (errno == EINTR)
                continue;
            ::close(fd);
        ...
        if (bytes_read == 0) {
            ...
            ::close(fd);
            ADD_FAILURE()
                << "EOF encountered before reading expected size from FIFO";
            return "";
        }
...
TEST_F(FifoClientTest, DefaultConstructor) {
    atom::connection::AsyncFifoClient client;
...
TEST_F(FifoClientTest, PathConstructor) {
    ...
    atom::connection::AsyncFifoClient client(fifo_path_);
...
#ifndef _WIN32
    std::string bad_path =
        "/sys/test_fifo_bad";  // System directory, should fail mkfifo/open
#else
    // On Windows, CreateFileA with OPEN_EXISTING will fail if pipe server isn't
    // running. This test verifies the exception is thrown.
...
TEST_F(FifoClientTest, GetPath) {
    ...
    client.close();
    EXPECT_EQ(client.getPath(),
              fifo_path_);  // Path should be retained after close
}
...
TEST_F(FifoClientTest, WriteSyncTimeout) {
    ...
    // Client writes with a short timeout. No reader is present, so it should
    // time out.
    ...
    EXPECT_GE(std::chrono::duration_cast<std::chrono::milliseconds>(end_time -
                                                                    start_time)
                  .count(),
              40);  // Allow some jitter
    EXPECT_LE(std::chrono::duration_cast<std::chrono::milliseconds>(end_time -
                                                                    start_time)
                  .count(),
              200);  // Upper bound
...
TEST_F(FifoClientTest, ReadSyncTimeout) {
    ...
    // Client reads with a short timeout. No writer is present, so it should
    // time out.
    auto start_time = std::chrono::steady_clock::now();
...
TEST_F(FifoClientTest, MoveConstructor) {
    atom::connection::AsyncFifoClient original_client;
    original_client.open(fifo_path_);
    EXPECT_TRUE(original_client.isOpen());
    EXPECT_EQ(original_client.getPath(), fifo_path_);

    atom::connection::AsyncFifoClient moved_client = std::move(original_client);

    EXPECT_TRUE(moved_client.isOpen());
    EXPECT_EQ(moved_client.getPath(), fifo_path_);

    // Original client should be in a valid but unspecified state (likely
    // closed/invalid) The unique_ptr pimpl_ will be null in the original
    // object. Calling methods on the moved-from object is undefined behavior,
    // but checking isOpen() might be safe if the implementation handles null
    // pimpl_. Let's just check the moved_client state.
...
TEST_F(FifoClientTest, MoveAssignment) {
    atom::connection::AsyncFifoClient client1;
    client1.open(fifo_path_);
    EXPECT_TRUE(client1.isOpen());

    // Create a second client, potentially with a different path if needed, but
    // here just default
    atom::connection::AsyncFifoClient client2;
    // client2 is not open
    ...
    // EXPECT_EQ(client1.getPath(), ""); // This might depend on Impl move
    // semantics

    // Test moving an open client
    atom::connection::AsyncFifoClient client3;
...
#endif
    atom::connection::AsyncFifoClient client4;
...
TEST_F(FifoClientTest, WriteToClosed) {
    atom::connection::AsyncFifoClient client;  // Starts closed
...
TEST_F(FifoClientTest, WriteWithTimeout) {
    ...
    EXPECT_LE(duration.count(),
              200);  // Should not take much longer than timeout
...
TEST_F(FifoClientTest, ConcurrentWrites) {
    ...
    for (int i = 0; i < num_writes; ++i) {
        ...
        futures.push_back(
            client.write(test_data[i], std::chrono::milliseconds(500)));
    }

    // Start a reader in a separate thread to consume the data
    std::thread reader_thread([&]() {
        std::this_thread::sleep_for(
            std::chrono::milliseconds(50));  // Let writes start
#ifndef _WIN32
        for (int i = 0; i < num_writes; ++i) {
...
TEST_F(FifoClientTest, ConcurrentReads) {
    ...
    std::thread writer_thread([&]() {
        std::this_thread::sleep_for(
            std::chrono::milliseconds(50));  // Let reads start
#ifndef _WIN32
        for (int i = 0; i < num_reads; ++i) {
            std::string data =
                "Concurrent read data " + std::to_string(i) + "\n";
...
#ifndef _WIN32
        std::string data =
            read_from_fifo(fifo_path_, 15);  // "Successful write\n"
        EXPECT_EQ(data, "Successful write\n");
#endif
    });

    auto success_future =
        client.write("Successful write\n", std::chrono::milliseconds(200));
