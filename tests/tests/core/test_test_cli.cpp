...
TEST_F(CLIParserTest, NoArguments) {
    ...
    auto result = parser.parse(static_cast<int>(args.size()),
                               const_cast<char**>(args.data()), config);
...
TEST_F(CLIParserTest, QuietFlag) {
    auto args = createArgs({"test_program", "--quiet"});
    TestRunnerConfig config;
    CLIParser parser;
...
TEST_F(CLIParserTest, OutputFormatOption) {
    auto args = createArgs({"test_program", "--output=json"});
    TestRunnerConfig config;
    CLIParser parser;
