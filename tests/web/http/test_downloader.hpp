// filepath: tests/web/test_downloader.hpp
#ifndef TEST_DOWNLOADER_HPP
#define TEST_DOWNLOADER_HPP
...
protected:
    ...
    void waitForDownloads(DownloadManager& dm, int maxWaitSeconds = 30) {
        ...
        while (dm.getActiveTaskCount() > 0) {
            ...
            if (std::chrono::duration_cast<std::chrono::seconds>(now - start).count() > maxWaitSeconds) {
                break;
            }
        }
    }
    ...
    const std::string TEST_SMALL_FILE_URL = "https://httpbin.org/bytes/1024";  // 1KB
    const std::string TEST_MEDIUM_FILE_URL = "https://httpbin.org/bytes/10240"; // 10KB
    const std::string TEST_LARGE_FILE_URL = "https://httpbin.org/bytes/102400"; // 100KB
    const std::string TEST_INVALID_URL = "https://invalid.nonexistent.domain/file.txt";
...
TEST_F(DownloadManagerTest, PauseResumeTask) {
    ...
    dm.addTask(TEST_LARGE_FILE_URL, outputFile.string());  // Larger file for testing
...
TEST_F(DownloadManagerTest, SetThreadCount) {
    ...
    // Add multiple tasks
    for (int i = 0; i < 5; ++i) {
        fs::path outputFile = downloadDir / ("thread_test_" + std::to_string(i) + ".bin");
        dm.addTask(TEST_SMALL_FILE_URL, outputFile.string());
    }
...
TEST_F(DownloadManagerTest, SpeedLimit) {
    ...
    EXPECT_GT(duration.count(), 5);  // Should take at least 5 seconds for 10KB at 1KB/s
}
