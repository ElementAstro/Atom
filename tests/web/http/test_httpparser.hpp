...
TEST_F(HttpParserMalformedTest, MalformedHeaders) {
    std::vector<std::string> malformedHeaders = {
        ...
        "Header: Value\r\n With Continuation\r\n",             // Line continuation with space
        "Header: Value\r\n\tWith Tab Continuation\r\n",        // Line continuation with tab
        "Header: Value\nNext-Header: Value\n",                 // LF instead of CRLF
        "Header: Value\r\r\nNext-Header: Value\r\n",           // Extra CR
        "Header: Value\r\nNext-Header: Value\r\r\n",           // Extra CR at end
    };
    ...
    for (const auto& headerLine : malformedHeaders) {
        ...
        // Some malformed headers might be parsed with warnings, others might fail
...
TEST_F(HttpParserLargeDataTest, VeryLargeHeaders) {
    ...
    for (size_t size : sizes) {
        ...
        }) << "Parser should handle large header of size: " << size;
    }
}
...
TEST_F(HttpParserLargeDataTest, ManyHeaders) {
    ...
    EXPECT_NO_THROW({
        ...
        if (parseResult) {
            auto allHeaders = parser->getAllHeaders();
            EXPECT_GT(allHeaders.size(), 500);  // Should have parsed many headers
        }
    });
}
...
TEST_F(HttpParserLargeDataTest, VeryLargeBodies) {
    ...
    for (size_t size : sizes) {
        ...
        }) << "Parser should handle large body of size: " << size;
    }
}
...
TEST_F(HttpParserLargeDataTest, LargeUrlPaths) {
    ...
    for (size_t size : pathSizes) {
        ...
        std::string request = "GET " + largePath + " HTTP/1.1\r\n"
                             "Host: example.com\r\n"
                             "\r\n";
        ...
        }) << "Parser should handle large path of size: " << size;
    }
}
...
TEST_F(HttpParserMethodsTest, AllStandardHttpMethods) {
    ...
    for (const auto& [methodStr, expectedMethod] : methods) {
        std::string request = methodStr + " /path HTTP/1.1\r\n"
                             "Host: example.com\r\n"
                             "\r\n";
        ...
        if (expectedMethod != HttpMethod::UNKNOWN) {
            EXPECT_EQ(HttpHeaderParser::methodToString(expectedMethod), methodStr);
        }
    }
}

TEST_F(HttpParserMethodsTest, CustomAndUnknownMethods) {
    std::vector<std::string> customMethods = {
        "CUSTOM", "WEBDAV", "PROPFIND", "PROPPATCH", "MKCOL", "COPY", "MOVE", "LOCK", "UNLOCK"
    };
    ...
    for (const auto& methodStr : customMethods) {
        ...
        EXPECT_EQ(HttpHeaderParser::stringToMethod(methodStr), HttpMethod::UNKNOWN);
    }
}
...
TEST_F(HttpParserMethodsTest, MethodsWithBodies) {
    ...
    for (const auto& method : methodsWithBodies) {
        std::string request = method + " /api/data HTTP/1.1\r\n"
                             "Host: example.com\r\n"
                             "Content-Type: application/json\r\n"
...
TEST_F(HttpParserEncodingTest, UrlEncodingEdgeCases) {
    std::vector<std::pair<std::string, std::string>> testCases = {
        ...
        {"key=value&other=data", "key%3Dvalue%26other%3Ddata"}, // Query-like string
    };
    ...
    for (const auto& [input, expected] : testCases) {
        ...
        std::string decoded = HttpHeaderParser::urlDecode(encoded);
        EXPECT_EQ(decoded, input) << "Round-trip encoding/decoding failed for: " << input;
    }
}
...
TEST_F(HttpParserEncodingTest, UnicodeInHeaders) {
    std::vector<std::string> unicodeStrings = {
        ...
        "Здравствуй",              // Cyrillic
        "🚀🌟💻",                  // Emojis
        "Iñtërnâtiônàlizætiøn",    // Mixed accented characters
        "Ελληνικά",                // Greek
        "日本語",                   // Japanese
        "한국어",                   // Korean
        "עברית",                   // Hebrew
    ...
    for (const auto& unicodeStr : unicodeStrings) {
        ...
        ASSERT_TRUE(retrievedValue.has_value()) << "Failed to retrieve unicode header: " << unicodeStr;
        EXPECT_EQ(*retrievedValue, unicodeStr) << "Unicode string corrupted: " << unicodeStr;
    }
}
...
TEST_F(HttpParserEncodingTest, BinaryDataInBodies) {
    ...
    std::vector<std::string> binaryData = {
        ...
        std::string{'\r', '\n', '\r', '\n'},                   // CRLF sequences
        std::string("Mixed") + '\x00' + "binary" + '\xFF' + '\xFE' + "data" + '\x01' + '\x02', // Mixed text and binary
    };
...
TEST_F(HttpParserPerformanceTest, ParsingPerformance) {
    ...
    for (size_t bodySize : bodySizes) {
        ...
        EXPECT_TRUE(parseResult) << "Should parse request with body size: " << bodySize;
        EXPECT_LT(duration.count(), 1000) << "Parsing should be fast for body size: " << bodySize;
...
TEST_F(HttpParserPerformanceTest, RepeatedParsing) {
    ...
    EXPECT_LT(duration.count(), 5000); // Should complete 1000 parses in less than 5 seconds
}
...
TEST_F(HttpParserPerformanceTest, BuildingPerformance) {
    ...
    EXPECT_LT(duration.count(), 2000); // Should complete 1000 builds in less than 2 seconds
}
    ...
    for (int i = 0; i < numThreads; ++i) {
        threads.emplace_back([&, i]() {
            try {
                ...
                for (int j = 0; j < 100; ++j) {
                    std::string request = "GET /path" + std::to_string(i) + "/" + std::to_string(j) + " HTTP/1.1\r\n"
                                         "Host: example.com\r\n"
                                         "User-Agent: Thread-" + std::to_string(i) + "\r\n"
                                         "\r\n";

                    if (parser.parseRequest(request)) {
                        std::string path = parser.getPath();
                        std::string expectedPath = "/path" + std::to_string(i) + "/" + std::to_string(j);
...
TEST_F(HttpParserAdvancedMalformedTest, MalformedCookieStrings) {
    std::vector<std::string> malformedCookies = {
        ...
        std::string("name=value; comment=") + std::string(1000, 'x'), // Very long comment
    };
    ...
    for (const auto& cookieStr : malformedCookies) {
        ...
        }) << "Parser should not crash on malformed cookie: " << cookieStr;
    }
}
...
TEST_F(HttpParserAdvancedMalformedTest, InvalidUrlParameterFormats) {
    std::vector<std::string> malformedUrls = {
        ...
        "https://example.com/path?param=val%1",      // Incomplete hex in URL encoding
        "https://example.com/path?param=val%00",     // Null byte encoded
        "https://example.com/path?param=val%FF",     // High byte encoded
        "https://example.com/path?" + std::string(10000, 'x'), // Very long query string
        "https://example.com/path?param=" + std::string(5000, 'v'), // Very long parameter value
    };
    ...
    for (const auto& url : malformedUrls) {
        ...
        }) << "Parser should not crash on malformed URL: " << url;
    }
}
...
TEST_F(HttpParserSecurityTest, OversizedHeadersAndValues) {
    ...
    for (size_t size : sizes) {
        ...
        }) << "Parser should handle large header name of size: " << size;

        // Large header value
        std::string largeHeaderValue(size, 'V');
        std::string request2 = "GET /path HTTP/1.1\r\n"
                              "Host: example.com\r\n"
        ...
        EXPECT_NO_THROW({
            ...
            // Should handle gracefully, may succeed or fail
        }) << "Parser should handle large header value of size: " << size;
    }
}
...
TEST_F(HttpParserEncodingAdvancedTest, MixedEncodingInHeadersAndBody) {
    ...
    std::vector<std::pair<std::string, std::string>> encodingTests = {
        ...
        {"Content-Type: text/plain; charset=utf-8\r\nX-Custom: café", "Body with UTF-8: 你好世界"},

        // ASCII headers with UTF-8 body
        {"Content-Type: text/plain\r\nX-Custom: test", "UTF-8 body: 🚀🌟💻"},

        // Headers with various Unicode characters
        {"Content-Type: text/plain\r\nX-Unicode: Iñtërnâtiônàlizætiøn", "Regular ASCII body"},

        // Mixed scripts in headers
        {"Content-Type: text/plain\r\nX-Mixed: English中文العربية", "Mixed body content"},
    ...
    for (const auto& [headerPart, bodyPart] : encodingTests) {
        std::string request = "POST /test HTTP/1.1\r\n"
                             "Host: example.com\r\n" +
                             headerPart + "\r\n"
                             "Content-Length: " + std::to_string(bodyPart.length()) + "\r\n"
                             "\r\n" + bodyPart;
...
TEST_F(HttpParserEncodingAdvancedTest, ByteOrderMarkHandling) {
    ...
    std::vector<std::string> bomTests = {
        ...
        std::string("\xEF\xBB\xBF") + "GET /path HTTP/1.1\r\nHost: example.com\r\n\r\n",

        // UTF-8 BOM in header value
        "GET /path HTTP/1.1\r\n"
        "Host: example.com\r\n"
        "X-BOM: " + std::string("\xEF\xBB\xBF") + "value\r\n"
        "\r\n",
        ...
        // UTF-16 BE BOM
        std::string("\xFE\xFF") + "GET /path HTTP/1.1\r\nHost: example.com\r\n\r\n",

        // UTF-16 LE BOM
        std::string("\xFF\xFE") + "GET /path HTTP/1.1\r\nHost: example.com\r\n\r\n",
    };
...
TEST_F(HttpParserContentLengthTest, ContentLengthMismatchScenarios) {
    ...
    for (const auto& [headerPart, bodyPart] : mismatchTests) {
        ...
        EXPECT_NO_THROW({
            ...
            (void)result; // Suppress unused variable warning
            // Should handle gracefully, behavior may vary
        }) << "Parser should handle Content-Length mismatch: " << headerPart;
    }
}
...
TEST_F(HttpParserContentLengthTest, InvalidContentLengthValues) {
    ...
    for (const auto& contentLength : invalidContentLengths) {
        ...
        EXPECT_NO_THROW({
            ...
            (void)result; // Suppress unused variable warning
            // Should handle gracefully, may succeed or fail
        }) << "Parser should handle invalid Content-Length: " << contentLength;
    }
}
...
TEST_F(HttpParserContentLengthTest, MultipleContentLengthHeaders) {
    ...
    for (const auto& request : multipleContentLengthTests) {
        ...
        }) << "Parser should handle multiple Content-Length headers consistently";
    }
}
...
TEST_F(HttpParserStateManagementTest, ParserReuseAcrossMultipleRequests) {
    ...
    for (size_t i = 0; i < requests.size(); ++i) {
        ...
        if (i > 0) {
            ...
            EXPECT_FALSE(parser->hasHeader("User-Agent") && parser->hasHeader("Content-Type"))
...
TEST_F(HttpParserStateManagementTest, StatePersistenceBetweenOperations) {
    ...
    auto params = parser->parseUrlParameters("https://example.com/path?param=value");
    (void)encoded; (void)decoded; (void)cookies; (void)params; // Suppress warnings
...
TEST_F(HttpParserStateManagementTest, MemoryManagementUnderStress) {
    ...
    for (int i = 0; i < 1000; ++i) {
        std::string request = "POST /test" + std::to_string(i) + " HTTP/1.1\r\n"
                             "Host: example.com\r\n"
                             "Content-Type: application/json\r\n"
                             "X-Request-ID: " + std::to_string(i) + "\r\n"
                             "Content-Length: " + std::to_string(10 + i % 100) + "\r\n"
                             "\r\n" +
    ...
    std::string finalRequest = "GET /final HTTP/1.1\r\nHost: example.com\r\n\r\n";
...
TEST_F(HttpParserStateManagementTest, ConcurrentStyleOperations) {
    ...
    for (int threadId = 0; threadId < 10; ++threadId) {
        // Create separate parser for each "thread" to avoid actual concurrency issues
        auto threadParser = std::make_unique<HttpHeaderParser>();

        for (int i = 0; i < 100; ++i) {
            try {
                std::string request = "GET /thread" + std::to_string(threadId) +
                                     "/request" + std::to_string(i) + " HTTP/1.1\r\n"
                                     "Host: example.com\r\n"
                                     "X-Thread-ID: " + std::to_string(threadId) + "\r\n"
                                     "X-Request-ID: " + std::to_string(i) + "\r\n"
                                     "\r\n";
                ...
                auto encoded = threadParser->urlEncode("test data " + std::to_string(i));
                auto params = threadParser->parseUrlParameters("https://example.com/test?id=" + std::to_string(i));
                (void)encoded; (void)params; // Suppress warnings
...
TEST_F(HttpParserAdvancedCookieTest, ComplexCookieAttributeCombinations) {
    std::vector<std::string> complexCookies = {
        ...
        "conflictCookie=value; expires=Wed, 21 Oct 2025 07:28:00 GMT; max-age=7200",

        // Invalid but parseable attributes
        "testCookie=value; secure=false; httponly=true; samesite=invalid",

        // Case variations
        "caseCookie=value; SECURE; HttpOnly; SameSite=Lax; PATH=/test; DOMAIN=example.com",

        // Quoted values
        "quotedCookie=\"quoted value\"; path=\"/quoted/path\"; domain=\".quoted.com\"",

        // Special characters in values
        "specialCookie=value!@#$%^&*(); path=/special; comment=\"Special cookie with symbols\"",
        ...
        "cookie1=value1; path=/; cookie2=value2; domain=.example.com; cookie3=value3; secure",
    };

    for (const auto& cookieStr : complexCookies) {
        EXPECT_NO_THROW({
            auto cookies = parser->parseCookies(cookieStr);
            EXPECT_GT(cookies.size(), 0) << "Should parse at least one cookie from: " << cookieStr;
        }) << "Parser should handle complex cookie: " << cookieStr;
    }
}
...
TEST_F(HttpParserAdvancedCookieTest, CookieExpirationDateParsing) {
    ...
    for (const auto& [expiresStr, shouldBeValid] : expirationTests) {
        ...
        }) << "Parser should handle expiration date: " << expiresStr;
    }
}
    ...
    for (const auto& [request, expectedMethod] : caseSensitivityTests) {
        ...
        if (parseResult) {
            ...
            EXPECT_EQ(expectedMethod, HttpMethod::UNKNOWN)
                << "Failed parsing should only occur for UNKNOWN methods: " << request;
        }
    }
}
...
TEST_F(HttpParserAdvancedMethodTest, WebDAVMethods) {
    ...
    for (const auto& method : webdavMethods) {
        std::string request = method + " /webdav/resource HTTP/1.1\r\n"
                             "Host: example.com\r\n"
                             "Depth: 1\r\n"
                             "\r\n";
        ...
        // WebDAV methods should be parsed as UNKNOWN since they're not in the standard enum
        EXPECT_EQ(parser->getMethod(), HttpMethod::UNKNOWN)
...
TEST_F(HttpParserAdvancedVersionTest, VersionWithExtraWhitespace) {
    ...
    for (const auto& request : whitespaceTests) {
        EXPECT_NO_THROW({
            ...
            (void)result; // Suppress unused variable warning
            // Should handle gracefully, behavior may vary
        }) << "Parser should handle whitespace variations: " << request;
    }
}
...
TEST_F(HttpParserAdvancedHeaderTest, HeaderFoldingAndContinuation) {
    // Note: Header folding is deprecated in HTTP/1.1 but may still be encountered
    ...
    for (const auto& request : foldingTests) {
        ...
        }) << "Parser should handle header folding: " << request;
    }
}
...
TEST_F(HttpParserAdvancedHeaderTest, HeadersWithSpecialCharacters) {
    ...
    for (const auto& [headerName, headerValue] : specialCharTests) {
        ...
        }) << "Parser should handle special characters in header: " << headerName;
    }
}
