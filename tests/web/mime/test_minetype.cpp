// Test file for atom/web/mime/minetype module
...
TEST_F(MimeTypesTest, FilenamesWithSpecialCharacters) {
    ...
    for (const auto& filename : specialFilenames) {
        ...
        EXPECT_EQ(*type, "text/plain")
            << "Wrong type for filename: " << filename;
    }
}
...
TEST_F(MimeTypesTest, CaseInsensitiveExtensions) {
    ...
    std::vector<std::string> caseVariations = {"file.txt", "file.TXT",
                                               "file.Txt", "file.tXt"};
...
TEST_F(MimeTypesTest, InvalidMimeTypeFormats) {
    ...
    EXPECT_THROW(mime->addType("invalid-mime-type", ".test"),
                 MimeTypeException);
    EXPECT_THROW(mime->addType("invalid/mime/type/too/many/slashes", ".test"),
                 MimeTypeException);
...
TEST_F(MimeTypesTest, InvalidExtensionFormats) {
    ...
    EXPECT_THROW(mime->addType("application/test", "no-dot"),
                 MimeTypeException);
}
...
TEST_F(MimeTypesTest, CacheEffectiveness) {
    ...
    auto uncachedDuration =
        std::chrono::duration_cast<std::chrono::microseconds>(end - start);

    // Cached lookups should be faster (though this is not guaranteed in all
    // cases) This is more of a performance observation than a strict
    // requirement
...
TEST_F(MimeTypesTest, ConfigurationPersistence) {
    ...
    EXPECT_EQ(retrievedConfig.enableDeepScanning,
              originalConfig.enableDeepScanning);
