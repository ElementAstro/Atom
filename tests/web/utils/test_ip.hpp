// filepath: tests/web/utils/test_ip.hpp
#ifndef TEST_IP_HPP
#define TEST_IP_HPP
...
protected:
    ...
    struct sockaddr_in createIPv4SockAddr(const std::string& ip, uint16_t port = 80) {
    ...
    struct sockaddr_in6 createIPv6SockAddr(const std::string& ip, uint16_t port = 80) {
...
TEST_F(IPUtilsTest, IsValidIPv6EdgeCases) {
    ...
    // Test maximum length
    std::string maxLengthIPv6 = "2001:0db8:85a3:0000:0000:8a2e:0370:7334";  // 39 characters
...
TEST_F(IPUtilsTest, IsValidIPv4SpecialAddresses) {
    ...
    for (const auto& ip : specialAddresses) {
        EXPECT_TRUE(isValidIPv4(ip)) << "Special IPv4 address should be valid: " << ip;
    }
}
...
TEST_F(IPUtilsTest, IsValidIPv6SpecialAddresses) {
    ...
    for (const auto& ip : specialAddresses) {
        EXPECT_TRUE(isValidIPv6(ip)) << "Special IPv6 address should be valid: " << ip;
    }
}
...
TEST_F(IPUtilsTest, IpToStringVariousIPv4Addresses) {
    ...
    for (const auto& expectedIP : testAddresses) {
        ...
        EXPECT_STREQ(buffer.data(), expectedIP.c_str()) << "Mismatch for: " << expectedIP;
    }
}
...
TEST_F(IPUtilsTest, CrossPlatformConsistency) {
    ...
    std::vector<std::string> testCases = {
        "127.0.0.1", "::1", "192.168.1.1", "2001:db8::1"
    };
    ...
    for (const auto& testCase : testCases) {
        ...
        EXPECT_NE(ipv4Result, ipv6Result) << "Address format ambiguity: " << testCase;
    }
}
