...
def apply_template(module_name, template_type="auto", dry_run=False):
    ...
    if template_type == "auto":
        ...
        print(
            f"Detected test type for {module_name}: {test_type} ({cpp_count} .cpp, {hpp_count} .hpp)"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Apply standardized CMakeLists.txt templates to test modules"
    )
    parser.add_argument(
        "module", nargs="?", help="Module name to process (or 'all' for all modules)"
    )
    parser.add_argument(
        "--type",
        choices=["auto", "cpp", "header_only"],
        default="auto",
    ...
    if args.list:
        ...
        for module in modules:
            ...
            print(
                f"{module['name']:<15} {cmake_status:<8} {module['test_type']:<12} {files_info:<15} {status}"
            )
